...
INSERT INTO venda_has_produto2 (idVendaProdutoFK, idRelacionado, selecionado, entregou, recebeu, status, statusOriginal, idCompra, idNFeSaida, idNFeFutura, fornecedor, idVenda, idLoja, 
            ...
            idVenda, idLoja, idProduto, produto, obs, lote, prcUnitario, descUnitario, caixas, quant, un, quantCaixa, codComercial, formComercial, parcial, desconto, parcialDesc, descGlobal, total, 
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_compras_financeiro` AS select group_concat(distinct `pf`.`status` separator ', ') AS `Status`,group_concat(distinct `pf`.`statusFinanceiro` separator ', ') AS `Status Financeiro`,group_concat(distinct `pf`.`ordemCompra` separator ', ') AS `OC`,`pf`.`idCompra` AS `Compra`,group_concat(distinct `pf`.`fornecedor` separator ', ') AS `Fornecedor`,count(0) AS `Produtos`,sum(`pf`.`preco`) AS `Total`,group_concat(distinct `pf`.`idVenda` separator ', ') AS `Código` from `staccato`.`pedido_fornecedor_has_produto` `pf` where (`pf`.`ordemCompra` is not null) group by `pf`.`ordemCompra`;
