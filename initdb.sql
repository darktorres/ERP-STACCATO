...
CREATE TABLE IF NOT EXISTS `mydb`.`orcamento` (
  ...
  `idOrcamentoBase` VARCHAR(11) GENERATED ALWAYS AS (left(`idOrcamento`,11)) STORED,
  `idLoja` INT(10) UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `mydb`.`venda` (
  ...
  `idVendaBase` VARCHAR(11) GENERATED ALWAYS AS (left(`idVenda`,11)) STORED,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_venda` (`idLoja` INT, `idUsuario` INT, `statusFinanceiro` INT, `dataFinanceiro` INT, `Status` INT, `Dias restantes` INT, `Código` INT, `Vendedor` INT, `Consultor` INT, `Cliente` INT, `Profissional` INT, `Data` INT, `Total R$` INT, `Data Followup` INT, `Observação` INT);
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_orcamento` AS select `u`.`idLoja` AS `idLoja`,`u`.`idUsuario` AS `idUsuario`,`o`.`idUsuarioConsultor` AS `idUsuarioConsultor`,`o`.`status` AS `Status`,(`o`.`validade` - (to_days(now()) - to_days(`o`.`data`))) AS `Dias restantes`,`o`.`idOrcamento` AS `Código`,`u`.`nome` AS `Vendedor`,`u2`.`nome` AS `Consultor`,`c`.`nome_razao` AS `Cliente`,`p`.`nome_razao` AS `Profissional`,`c`.`tel` AS `Telefone`,`c`.`telCel` AS `Celular`,if(isnull(`p`.`telCel`),if(isnull(`p`.`tel`),`p`.`telCom`,`p`.`tel`),`p`.`telCel`) AS `Tel Prof`,`o`.`data` AS `Data`,`o`.`total` AS `Total`,`ohf`.`dataFollowup` AS `Data Followup`,`ohf`.`dataProxFollowup` AS `Data Próx Followup`,`ohf`.`observacao` AS `Observação` from ((((((`mydb`.`orcamento` `o` left join `mydb`.`cliente` `c` on((`o`.`idCliente` = `c`.`idCliente`))) left join `mydb`.`usuario` `u` on((`o`.`idUsuario` = `u`.`idUsuario`))) left join `mydb`.`profissional` `p` on((`o`.`idProfissional` = `p`.`idProfissional`))) left join `mydb`.`usuario` `u2` on((`o`.`idUsuarioConsultor` = `u2`.`idUsuario`))) left join (select max(`tmp`.`idFollowup`) AS `idFollowup`,`tmp`.`idOrcamentoBase` AS `idOrcamentoBase` from `mydb`.`orcamento_has_followup` `tmp` group by `tmp`.`idOrcamentoBase`) `ohf2` on((`o`.`idOrcamentoBase` = `ohf2`.`idOrcamentoBase`))) left join `mydb`.`orcamento_has_followup` `ohf` on((`ohf2`.`idFollowup` = `ohf`.`idFollowup`)));
