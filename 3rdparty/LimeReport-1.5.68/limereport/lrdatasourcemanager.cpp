...
int DataNode::row() {
  if (m_parent) { return m_parent->m_childs.indexOf(const_cast<DataNode *>(this)); }
  return 0;
}
...
void DataSourceModel::updateModel() {
  ...
  foreach (QString name, m_dataManager->userVariableNames()) { vars->addChild(name, DataNode::Variable, QIcon(":/report/images/value")); }
}

DataSourceManager::DataSourceManager(QObject *parent) : QObject(parent), m_lastError(""), m_designTime(false), m_needUpdate(false), m_dbCredentialsProvider(0), m_hasChanges(false) {
...
void DataSourceManager::setDefaultDatabasePath(const QString &defaultDatabasePath) { m_defaultDatabasePath = defaultDatabasePath; }
...
bool DataSourceManager::addModel(const QString &name, QAbstractItemModel *model, bool owned) {
  if (m_datasources.contains(name.toLower())) removeDatasource(name.toLower());
...
void DataSourceManager::registerDbCredentialsProvider(IDbCredentialsProvider *provider) { m_dbCredentialsProvider = provider; }
...
QSharedPointer<QAbstractItemModel> DataSourceManager::previewSQL(const QString &connectionName, const QString &sqlText, QString masterDatasource) {
  ...
  if (db.isValid() && db.isOpen()) {
    ...
    foreach (QString param, aliasesToParam.keys()) {
      ...
      if (value.isValid() || m_designTime) query.bindValue(':' + param, value);
    }
      ...
      if (!rx.cap(1).isEmpty()) {
        ...
        if (!varName.isEmpty()) {
          if (!varParam.isEmpty() && varParam.compare("nobind") == 0) {
            ...
            query.replace(pos, rx.cap(0).length(), QString(tr("Unknown parameter \"%1\" for variable \"%2\" found!").arg(varName).arg(varParam)));
          }
        } else {
...
bool DataSourceManager::isQuery(const QString &dataSourceName) { return (queryByName(dataSourceName)); }
...
bool DataSourceManager::checkConnectionDesc(ConnectionDesc *connection) {
  ...
  if (connection->isInternal()) QSqlDatabase::removeDatabase(connection->name());
  return false;
}
...
void DataSourceManager::putQueryDesc(QueryDesc *queryDesc) {
  if (!containsDatasource(queryDesc->queryName())) {
    ...
    connect(queryDesc, SIGNAL(queryTextChanged(QString, QString)), this, SLOT(slotQueryTextChanged(QString, QString)));
  } else
...
void DataSourceManager::putSubQueryDesc(SubQueryDesc *subQueryDesc) {
  if (!containsDatasource(subQueryDesc->queryName())) {
    ...
    connect(subQueryDesc, SIGNAL(queryTextChanged(QString, QString)), this, SLOT(slotQueryTextChanged(QString, QString)));
  } else
    throw ReportError(tr("Datasource with name \"%1\" already exists!").arg(subQueryDesc->queryName()));
}
...
void DataSourceManager::putCSVDesc(CSVDesc *csvDesc) {
  if (!containsDatasource(csvDesc->name())) {
    ...
    connect(csvDesc, SIGNAL(cvsTextChanged(QString, QString)), this, SLOT(slotCSVTextChanged(QString, QString)));
  } else
...
bool DataSourceManager::initAndOpenDB(QSqlDatabase &db, ConnectionDesc &connectionDesc) {
  ...
  if (connectionDesc.port() != "") db.setPort(replaceVariables(connectionDesc.port()).toInt());

  if (!connectionDesc.keepDBCredentials() && m_dbCredentialsProvider) {
    if (!m_dbCredentialsProvider->getUserName(connectionDesc.name()).isEmpty()) db.setUserName(m_dbCredentialsProvider->getUserName(connectionDesc.name()));
    if (!m_dbCredentialsProvider->getPassword(connectionDesc.name()).isEmpty()) db.setPassword(m_dbCredentialsProvider->getPassword(connectionDesc.name()));
  }

  QString dbName = replaceVariables(connectionDesc.databaseName());
  if (connectionDesc.driver().compare("QSQLITE") == 0) {
    if (!defaultDatabasePath().isEmpty()) { dbName = !QFileInfo(dbName).exists() ? defaultDatabasePath() + QFileInfo(dbName).fileName() : dbName; }
...
void DataSourceManager::setReportSettings(ReportSettings *reportSettings) { m_reportSettings = reportSettings; }
...
bool DataSourceManager::connectConnection(ConnectionDesc *connectionDesc) {
  ...
  if (!QSqlDatabase::contains(connectionDesc->name())) {
    ...
    if (!connected) {
      if (!dbError.trimmed().isEmpty()) setLastError(dbError);
    ...
    if (!connectionDesc->isEqual(db) && connectionDesc->isInternal()) {
      ...
      if (!connected && connectionDesc->isInternal()) connected = initAndOpenDB(db, *connectionDesc);
    }
  }
  ...
  if (!connected) {
    ...
    foreach (QString datasourceName, dataSourceNames()) {
      if (isProxy(datasourceName)) {
        ...
        if (ph) { ph->invalidate(designTime() ? IDataSource::DESIGN_MODE : IDataSource::RENDER_MODE); }
      }
    }
    if (designTime()) emit datasourcesChanged();
  }
...
QList<QString> DataSourceManager::childDatasources(const QString &parentDatasourceName) {
  ...
  foreach (QString datasourceName, dataSourceNames()) {
    if (isSubQuery(datasourceName)) {
      ...
      if (sh && sh->masterDatasource().compare(parentDatasourceName, Qt::CaseInsensitive) == 0) { result.append(datasourceName); }
    }
  }
  return result;
}
...
bool DataSourceManager::containsDatasource(const QString &dataSourceName) { return m_datasources.contains(dataSourceName.toLower()); }

bool DataSourceManager::isSubQuery(const QString &dataSourceName) { return subQueryIndexByName(dataSourceName) != -1; }

bool DataSourceManager::isProxy(const QString &dataSourceName) { return proxyIndexByName(dataSourceName) != -1; }

bool DataSourceManager::isCSV(const QString &datasourceName) { return csvIndexByName(datasourceName) != -1; }

bool DataSourceManager::isConnection(const QString &connectionName) { return connectionIndexByName(connectionName) != -1; }
...
QStringList DataSourceManager::dataSourceNames() {
  QStringList result;
  for (int i = 0; i < m_datasources.keys().count(); i++) { result.append(m_datasources.keys().at(i)); }
  result.sort();
  return result;
}
...
QStringList DataSourceManager::dataSourceNames(const QString &connectionName) {
  ...
  foreach (QueryDesc *query, m_queries) {
    if (query->connectionName().compare(connectionName, Qt::CaseInsensitive) == 0) { result.append(query->queryName()); }
  }
...
QStringList DataSourceManager::fieldNames(const QString &datasourceName) {
  ...
  if (ds && !ds->isInvalid()) {
    for (int i = 0; i < ds->columnCount(); i++) { result.append(ds->columnNameByIndex(i)); }
    result.sort();
  }
  return result;
}
...
int DataSourceManager::elementsCount(const QString &collectionName) {
  ...
  if (collectionName == "variables") { return m_reportVariables.variablesCount(); }
  if (collectionName == "csvs") { return m_csvs.count(); }
  return 0;
}

QObject *DataSourceManager::elementAt(const QString &collectionName, int index) {
  if (collectionName == "connections") { return m_connections.at(index); }
  if (collectionName == "queries") { return m_queries.at(index); }
  if (collectionName == "subqueries") { return m_subqueries.at(index); }
  if (collectionName == "subproxies") { return m_proxies.at(index); }
  if (collectionName == "variables") { return m_reportVariables.variableAt(index); }
  if (collectionName == "csvs") { return m_csvs.at(index); }
  return 0;
}
...
void DataSourceManager::collectionLoadFinished(const QString &collectionName) {
  ...
  if (collectionName.compare("queries", Qt::CaseInsensitive) == 0) {
    ...
    while (it.hasNext()) {
      ...
      if (!m_datasources.contains(it.value()->queryName().toLower())) {
        connect(it.value(), SIGNAL(queryTextChanged(QString, QString)), this, SLOT(slotQueryTextChanged(QString, QString)));
  ...
  if (collectionName.compare("subqueries", Qt::CaseInsensitive) == 0) {
    ...
    while (it.hasNext()) {
      ...
      if (!m_datasources.contains(it.value()->queryName().toLower())) {
        connect(it.value(), SIGNAL(queryTextChanged(QString, QString)), this, SLOT(slotQueryTextChanged(QString, QString)));
        putHolder(it.value()->queryName(), new SubQueryHolder(it.value()->queryText(), it.value()->connectionName(), it.value()->master(), this));
      } else {
        delete it.value();
        it.remove();
      }
  ...
  if (collectionName.compare("csvs", Qt::CaseInsensitive) == 0) {
    ...
    while (it.hasNext()) {
      it.next();
      if (!m_datasources.contains(it.value()->name().toLower())) {
        connect(it.value(), SIGNAL(cvsTextChanged(QString, QString)), this, SLOT(slotCSVTextChanged(QString, QString)));
...
void DataSourceManager::changeVariable(const QString &name, const QVariant &value) {
  if (m_userVariables.containsVariable(name)) { m_userVariables.changeVariable(name, value); }
  if (m_reportVariables.containsVariable(name)) { m_reportVariables.changeVariable(name, value); }
}

void DataSourceManager::setSystemVariable(const QString &name, const QVariant &value, RenderPass pass) { addVariable(name, value, VarDesc::System, pass); }

void DataSourceManager::setLastError(const QString &value) {
  m_lastError = value;
  if (!value.isEmpty() && !m_errorsList.contains(value)) { m_errorsList.append(value); }
}

void DataSourceManager::invalidateLinkedDatasources(QString datasourceName) {
  foreach (QString name, dataSourceNames()) {
    if (isSubQuery(name)) {
      if (subQueryByName(name)->master().compare(datasourceName) == 0) dataSourceHolder(name)->invalidate(designTime() ? IDataSource::DESIGN_MODE : IDataSource::RENDER_MODE);
    }
    if (isProxy(name)) {
...
void DataSourceManager::slotQueryTextChanged(const QString &queryName, const QString &queryText) {
  ...
  if (holder) { holder->setQueryText(queryText); }
  m_varToDataSource.clear();
}
...
void DataSourceManager::slotVariableHasBeenAdded(const QString &variableName) {
  ...
  if (variableType(variableName) == VarDesc::Report) m_hasChanges = true;
}
...
void DataSourceManager::slotCSVTextChanged(const QString &csvName, const QString &csvText) {
  ...
  if (holder) { holder->setCSVText(csvText); }
}
...
void DataSourceManager::clear(ClearMethod method) {
  ...
  QList<ConnectionDesc *>::iterator cit = m_connections.begin();
  while (cit != m_connections.end()) {
    if ((*cit)->isInternal()) QSqlDatabase::removeDatabase((*cit)->name());
    delete (*cit);
    cit = m_connections.erase(cit);
  }
...
GroupFunction *DataSourceManager::addGroupFunction(const QString &name, const QString &expression, const QString &band, const QString &dataBand) {
  ...
  if (gf) { m_groupFunctions.insert(band, gf); }
  return gf;
}
...
QString DataSourceManager::extractDataSource(const QString &fieldName) { return fieldName.left(fieldName.indexOf('.')); }

QString DataSourceManager::extractFieldName(const QString &fieldName) { return fieldName.right((fieldName.length() - fieldName.indexOf('.')) - 1); }

bool DataSourceManager::containsField(const QString &fieldName) {
  IDataSource *ds = dataSource(extractDataSource(fieldName));
  if (ds) { return ds->columnIndexByName(extractFieldName(fieldName)) != -1; }
  return false;
}
...
QVariant DataSourceManager::fieldDataByRowIndex(const QString &fieldName, int rowIndex) {
  if (containsField(fieldName)) {
    IDataSource *ds = dataSource(extractDataSource(fieldName));
    if (ds) { return ds->dataByRowIndex(extractFieldName(fieldName), rowIndex); }
  }
  return QVariant();
}
...
QVariant DataSourceManager::fieldDataByKey(const QString &datasourceName, const QString &valueFieldName, const QString &keyFieldName, QVariant keyValue) {
  IDataSource *ds = dataSource(datasourceName);
  if (ds) { return ds->dataByKeyField(valueFieldName, keyFieldName, keyValue); }
  return QVariant();
}
...
QVariant DataSourceManager::variable(const QString &variableName) {
  if (m_userVariables.containsVariable(variableName)) return m_userVariables.variable(variableName);
  return m_reportVariables.variable(variableName);
}

RenderPass DataSourceManager::variablePass(const QString &name) {
  if (m_userVariables.containsVariable(name)) return m_userVariables.variablePass(name);
  return m_reportVariables.variablePass(name);
}

bool DataSourceManager::variableIsSystem(const QString &name) {
  if (m_reportVariables.containsVariable(name)) return (m_reportVariables.variableType(name) == VarDesc::System);
  return false;
}

bool DataSourceManager::variableIsMandatory(const QString &name) {
  if (m_reportVariables.containsVariable(name)) return m_reportVariables.variableByName(name)->isMandatory();
  return false;
}

void DataSourceManager::setVarableMandatory(const QString &name, bool value) {
  if (m_reportVariables.containsVariable(name)) m_reportVariables.variableByName(name)->setMandatory(value);
}
...
QStringList DataSourceManager::variableNamesByRenderPass(RenderPass pass) {
  ...
  foreach (QString variableName, m_reportVariables.variableNames()) {
    if (m_reportVariables.variablePass(variableName) == pass) { result.append(variableName); }
  }
  return result;
}
...
VarDesc::VarType DataSourceManager::variableType(const QString &name) {
  if (m_reportVariables.containsVariable(name)) return m_reportVariables.variableType(name);
  return VarDesc::User;
}

VariableDataType DataSourceManager::variableDataType(const QString &name) {
  if (m_reportVariables.containsVariable(name)) return m_reportVariables.variableByName(name)->dataType();
  return Enums::Undefined;
}

void DataSourceManager::setVariableDataType(const QString &name, VariableDataType value) {
  if (m_reportVariables.containsVariable(name)) m_reportVariables.variableByName(name)->setDataType(value);
}
