...
TextAlignmentEditorWidget::TextAlignmentEditorWidget(const QString &title, QWidget *parent) : ItemEditorWidget(title, parent), m_textAttibutesIsChanging(false), m_flag(0) { initEditor(); }
...
void TextAlignmentEditorWidget::slotPropertyChanged(const QString &objectName, const QString &property, const QVariant &oldValue, const QVariant &newValue) {
  ...
  if (item() && (item()->objectName() == objectName) && (property == "alignment")) { updateValues(Qt::Alignment(item()->property("alignment").value<int>())); }
}
...
void TextAlignmentEditorWidgetForPage::initEditor() {
  ...
  connect(m_page, SIGNAL(itemPropertyChanged(QString, QString, QVariant, QVariant)), this, SLOT(slotPropertyChanged(QString, QString, QVariant, QVariant)));
}
...
void TextAlignmentEditorWidgetForDesigner::initEditor() {
  connect(m_reportEditor, SIGNAL(itemPropertyChanged(QString, QString, QVariant, QVariant)), this, SLOT(slotPropertyChanged(QString, QString, QVariant, QVariant)));
}
