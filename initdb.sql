...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_compras_financeiro` AS select group_concat(distinct `p`.`status` separator ', ') AS `Status`,group_concat(distinct `p`.`statusFinanceiro` separator ', ') AS `Status Financeiro`,group_concat(distinct `p`.`ordemCompra` separator ', ') AS `OC`,`p`.`idCompra` AS `Compra`,group_concat(distinct `p`.`fornecedor` separator ', ') AS `Fornecedor`,count(`p`.`idProduto`) AS `Produtos`,sum(`p`.`preco`) AS `Total`,group_concat(distinct `p`.`idVenda` separator ', ') AS `Código` from `mydb`.`pedido_fornecedor_has_produto` `p` where (`p`.`idCompra` is not null) group by `p`.`idCompra`;
...
BEGIN
    ...
    CALL update_venda_status(NEW.idVenda);
END$$
