...
ScriptEngineNode::ScriptEngineNode(const QString &name, const QString &description, ScriptEngineNode::NodeType type, ScriptEngineNode *parent, const QIcon &icon)
...
ScriptEngineModel::ScriptEngineModel(ScriptEngineManager *scriptManager) : m_rootNode(new ScriptEngineNode()) { setScriptEngineManager(scriptManager); }
...
QModelIndex ScriptEngineModel::index(int row, int column, const QModelIndex &parent) const {
  if (!m_rootNode) return QModelIndex();
...
void ScriptEngineManager::deleteFunction(const QString &functionsName) { m_functions.remove(functionsName); }
...
bool ScriptEngineManager::addFunction(const JSFunctionDesc &functionDescriber) {
  ...
  if (functionManager.toQObject() == functionDescriber.manager()) {
    ...
    if (!checkWrapper.isError()) {
      ...
      if (m_model) m_model->updateModel();
      return true;
    } else {
...
Q_DECL_DEPRECATED
#endif
bool ScriptEngineManager::addFunction(const QString &name, QScriptEngine::FunctionSignature function, const QString &category, const QString &description) {
  ...
  if (dataManager && m_dataManager != dataManager) {
    ...
    if (m_dataManager) {
      foreach (QString func, m_dataManager->groupFunctionNames()) {
        JSFunctionDesc describer(func, tr("GROUP FUNCTIONS"), func + "(\"" + tr("FieldName") + "\",\"" + tr("BandName") + "\")", LimeReport::Const::FUNCTION_MANAGER_NAME, m_functionManager,
...
QString ScriptEngineManager::expandScripts(QString context, QVariant &varValue, QObject *reportItem) {
  ...
  if (context.contains(rx)) {
    ...
    if (ScriptEngineManager::instance().dataManager() != dataManager()) ScriptEngineManager::instance().setDataManager(dataManager());
    ...
    if (scriptExtractor.parse()) { context = replaceScripts(context, varValue, reportItem, se, scriptExtractor.scriptTree()); }
  }
  return context;
}
...
QString ScriptEngineManager::replaceScripts(QString context, QVariant &varValue, QObject *reportItem, ScriptEngineType *se, ScriptNode *scriptTree) {
  foreach (ScriptNode *item, scriptTree->children()) {
    ...
    if (item->children().size() > 0) scriptBody = replaceScripts(scriptBody, varValue, reportItem, se, item);
...
int ScriptEngineManager::findPageIndexByBookmark(const QString &uniqKey) {
  for (int i = 0; i < m_context->reportPages()->size(); ++i) {
    if (m_context->reportPages()->at(i)->bookmarks().contains(uniqKey)) return i + 1;
    foreach (BandDesignIntf *band, m_context->reportPages()->at(i)->bands()) {
      if (band->bookmarks().contains(uniqKey)) return i + 1;
    }
  }
  return -1;
}
...
void ScriptEngineManager::clearTableOfContents() {
  if (m_context) {
    if (m_context->tableOfContents()) m_context->tableOfContents()->clear();
  }
}
...
bool ScriptEngineManager::createNumberFomatFunction() {
  ...
  fd.setDescription("numberFormat(\"" + tr("Value") + "\",\"" + tr("Format") + "\",\"" + tr("Precision") + "\",\"" + tr("Locale") + "\")");
...
bool ScriptEngineManager::createGetFieldByKeyFunction() {
  ...
  fd.setDescription("getFieldByKeyField(\"" + tr("Datasource") + "\", \"" + tr("ValueField") + "\",\"" + tr("KeyField") + "\", \"" + tr("KeyFieldValue") + "\")");
...
#ifdef USE_QTSCRIPTENGINE
  m_scriptEngine->setDefaultPrototype(qMetaTypeId<QComboBox *>(), m_scriptEngine->newQObject(new ComboBoxPrototype()));
#endif
  createLineFunction();
  ...
  while (curPos < m_context.length()) {
    ...
    case OpenBracketFound:
      if (m_context[curPos] == '}') {
        ...
        if (m_context[curPos] == '{') extractBracket(curPos, scriptNode);
      }
      [[fallthrough]];
    case None:
      if (m_context[curPos] == '$') {
        int startPos = curPos;
        if (isStartScriptLexem(curPos)) extractScript(curPos, substring(m_context, startPos, curPos), scriptNode->createChildNode());
...
bool ScriptExtractor::isStartScriptLexem(int &curPos) { return isStartLexem(curPos, Const::SCRIPT_SIGN); }

bool ScriptExtractor::isStartFieldLexem(int &curPos) { return isStartLexem(curPos, Const::FIELD_SIGN); }

bool ScriptExtractor::isStartVariableLexem(int &curPos) { return isStartLexem(curPos, Const::VARIABLE_SIGN); }

QString ScriptExtractor::substring(const QString &value, int start, int end) { return value.mid(start, end - start); }
...
void ScriptEngineContext::setTableOfContents(TableOfContents *tableOfContents) { m_tableOfContents = tableOfContents; }

PageItemDesignIntf *ScriptEngineContext::getCurrentPage() const { return m_currentPage; }
...
BandDesignIntf *ScriptEngineContext::getCurrentBand() const { return m_currentBand; }

void ScriptEngineContext::setCurrentBand(BandDesignIntf *currentBand) { m_currentBand = currentBand; }
...
#endif
    foreach (BaseDesignIntf *child, item->childBaseItems()) { baseDesignIntfToScript(pageName, child); }
  }
}
...
#ifdef USE_QJSENGINE
  if (res.isError()) {
    QMessageBox::critical(0, tr("Error"), QString("Line %1: %2 ").arg(res.property("lineNumber").toString()).arg(res.toString()));
    return false;
  }
#else
  if (engine->hasUncaughtException()) {
    QMessageBox::critical(0, tr("Error"), QString("Line %1: %2 ").arg(engine->uncaughtExceptionLineNumber()).arg(engine->uncaughtException().toString()));
    return false;
  }
#endif
  return true;
...
void JSFunctionDesc::setScriptWrapper(const QString &scriptWrapper) { m_scriptWrapper = scriptWrapper; }
...
QVariant ScriptFunctionsManager::calcGroupFunction(const QString &name, const QString &expressionID, const QString &bandName) { return calcGroupFunction(name, expressionID, bandName, 0); }
...
QVariant ScriptFunctionsManager::numberFormat(QVariant value, const char &format, int precision, const QString &locale) {
  return (locale.isEmpty()) ? QString::number(value.toDouble(), format, precision) : QLocale(locale).toString(value.toDouble(), format, precision);
}

QVariant ScriptFunctionsManager::dateFormat(QVariant value, const QString &format, const QString &locale) {
  return (locale.isEmpty()) ? QLocale().toString(value.toDate(), format) : QLocale(locale).toString(value.toDate(), format);
}

QVariant ScriptFunctionsManager::timeFormat(QVariant value, const QString &format) { return QLocale().toString(value.toTime(), format); }

QVariant ScriptFunctionsManager::dateTimeFormat(QVariant value, const QString &format, const QString &locale) {
  return (locale.isEmpty()) ? QLocale().toString(value.toDateTime(), format) : QLocale(locale).toString(value.toDateTime(), format);
}
...
void ScriptFunctionsManager::addBookmark(const QString &uniqKey, const QString &content) { scriptEngineManager()->addBookMark(uniqKey, content); }

int ScriptFunctionsManager::findPageIndexByBookmark(const QString &uniqKey) { return scriptEngineManager()->findPageIndexByBookmark(uniqKey); }

void ScriptFunctionsManager::addTableOfContentsItem(const QString &uniqKey, const QString &content, int indent) { scriptEngineManager()->addTableOfContentsItem(uniqKey, content, indent); }

void ScriptFunctionsManager::clearTableOfContents() { scriptEngineManager()->clearTableOfContents(); }
...
QJSValue ScriptFunctionsManager::createWrapper(QJSValue item) {
  ...
  if (object) {
    ...
    if (wrapper) { return m_scriptEngineManager->scriptEngine()->newQObject(wrapper->createWrapper(item.toQObject())); }
  }
  return QJSValue();
}
...
QFont ScriptFunctionsManager::font(QVariantMap params) {
  if (!params.contains("family")) {
    ...
    if (params.contains("pointSize")) result.setPointSize(params.value("pointSize").toInt());
    if (params.contains("bold")) result.setBold(params.value("bold").toBool());
    if (params.contains("italic")) result.setItalic(params.value("italic").toBool());
    if (params.contains("underline")) result.setUnderline(params.value("underline").toBool());
    return result;
  }
}

ScriptEngineManager *ScriptFunctionsManager::scriptEngineManager() const { return m_scriptEngineManager; }

void ScriptFunctionsManager::setScriptEngineManager(ScriptEngineManager *scriptEngineManager) { m_scriptEngineManager = scriptEngineManager; }
...
void TableOfContents::slotOneSlotDS(CallbackInfo info, QVariant &data) {
  ...
  case LimeReport::CallbackInfo::RowCount: data = m_tableOfContents.count(); break;
  case LimeReport::CallbackInfo::ColumnCount: data = columns.size(); break;
  ...
  case LimeReport::CallbackInfo::ColumnData:
    if (info.index < m_tableOfContents.count()) {
      ...
      if (info.columnName.compare("Content", Qt::CaseInsensitive) == 0) data = item->content.rightJustified(item->indent + item->content.size());
      if (info.columnName.compare("Content Key", Qt::CaseInsensitive) == 0) data = item->uniqKey;
      if (info.columnName.compare("Page number", Qt::CaseInsensitive) == 0) data = QString::number(item->pageNumber);
    }
    break;
  default: break;
  }
...
QObject *ComboBoxWrapperCreator::createWrapper(QObject *item) {
  ...
  if (comboBox) { return new ComboBoxWrapper(comboBox); }
  return 0;
}
...
bool DatasourceFunctions::next(const QString &datasourceName) {
  if (m_dataManager && m_dataManager->dataSource(datasourceName)) return m_dataManager->dataSource(datasourceName)->next();
  return false;
}

bool DatasourceFunctions::prior(const QString &datasourceName) {
  if (m_dataManager && m_dataManager->dataSource(datasourceName)) return m_dataManager->dataSource(datasourceName)->prior();
  return false;
}

bool DatasourceFunctions::isEOF(const QString &datasourceName) {
  if (m_dataManager && m_dataManager->dataSource(datasourceName)) return m_dataManager->dataSource(datasourceName)->eof();
  return true;
}
...
QObject *DatasourceFunctions::createTableBuilder(QObject *horizontalLayout) {
  ...
  if (l) return new TableBuilder(l, m_dataManager);
  return 0;
}

TableBuilder::TableBuilder(HorizontalLayout *layout, DataSourceManager *dataManager) : m_horizontalLayout(layout), m_baseLayout(0), m_dataManager(dataManager) {
  if (m_horizontalLayout) m_patternLayout = dynamic_cast<HorizontalLayout *>(m_horizontalLayout->cloneItem(m_horizontalLayout->itemMode()));
}
...
QObject *TableBuilder::currentRow() {
  checkBaseLayout();
  if (m_baseLayout && m_baseLayout->childrenCount() > 0) return m_baseLayout->at(m_baseLayout->childrenCount() - 1);
  return 0;
}
...
void TableBuilder::fillInRowData(QObject *row) {
  ...
  if (layout) {
    for (int i = 0; i < layout->childrenCount(); ++i) {
      ...
      if (item && dm) item->updateItemSize(dm);
    }
  }
}
...
void ComboBoxPrototype::addItem(const QString &text) {
  ...
  if (comboBox) { comboBox->addItem(text); }
}

void ComboBoxPrototype::addItems(const QStringList &texts) {
  QComboBox *comboBox = qscriptvalue_cast<QComboBox *>(thisObject());
  if (comboBox) { comboBox->addItems(texts); }
}
#endif
