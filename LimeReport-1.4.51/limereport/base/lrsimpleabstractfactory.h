...
class SimpleAbstractFactory : public Singleton<SimpleAbstractFactory<AbstractProduct, IdentifierType, ProductCreator>> {
private:
...
public:
  bool registerCreator(const IdentifierType &id, ProductCreator creator) { return (m_factoryMap.insert(id, creator).value() == creator); }
...
public:
  ...
  bool unregisterCreator(const IdentifierType &id) { return SimpleFactory::unregisterCreator(id) && (m_attribsMap.remove(id) == 1); }
