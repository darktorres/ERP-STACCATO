...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`DEXCO_INVOICE_INVOICEITEM` (`codi_ven` INT, `codi_rev` INT, `codi_fab` INT, `modl_ven` INT, `nume_ven` INT, `seri_ven` INT, `fina_ven` INT, `data_ven` INT, `oper_ven` INT, `cfop_ven` INT, `desc_opr` INT, `stat_ven` INT, `sina_opr` INT, `codi_cli` INT, `codi_pro` INT, `desc_pro` INT, `ncmp_pro` INT, `unid_pro` INT, `barr_pro` INT, `cind_pro` INT, `lote_ivn` INT, `pbru_ivn` INT, `pliq_ivn` INT, `qtde_ivn` INT, `codi_ved` INT, `nome_ved` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`DEXCO_PRODUCTSTOCK`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`DEXCO_PRODUCTSTOCK` (`codi_rev` INT, `codi_fab` INT, `codi_pro` INT, `desc_pro` INT, `ncmp_pro` INT, `unid_pro` INT, `barr_pro` INT, `cind_pro` INT, `date_pro` INT, `lote_pro` INT, `dven_pro` INT, `qtde_pro` INT, `qtdi_pro` INT, `qtdt_pro` INT, `pest_pro` INT);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`DEXCO_INVOICE_INVOICEITEM` AS select cast(`vp2`.`idVenda` as char(65535) charset utf8mb4) AS `codi_ven`,cast(`l`.`sigla` as char(20) charset utf8mb4) AS `codi_rev`,cast(`p`.`idFornecedor` as char(65535) charset utf8mb4) AS `codi_fab`,cast('55' as char(2) charset utf8mb4) AS `modl_ven`,cast(`n`.`numeroNFe` as char(20) charset utf8mb4) AS `nume_ven`,cast(substr(`n`.`chaveAcesso`,23,3) as char(20) charset utf8mb4) AS `seri_ven`,cast(extractvalue(`n`.`xml`,'//nfeProc//NFe//infNFe//ide//finNFe') as char(1) charset utf8mb4) AS `fina_ven`,cast(extractvalue(`n`.`xml`,'//nfeProc//NFe//infNFe//ide//dhEmi') as date) AS `data_ven`,cast(`vp2`.`idVendaProduto2` as char(20) charset utf8mb4) AS `oper_ven`,cast(`ehc`.`cfop` as char(4) charset utf8mb4) AS `cfop_ven`,cast(extractvalue(`n`.`xml`,'//nfeProc//NFe//infNFe//ide//natOp') as char(60) charset utf8mb4) AS `desc_opr`,cast(if((`vp2`.`status` = 'CANCELADO'),'C','A') as char(1) charset utf8mb4) AS `stat_ven`,cast(if((`n`.`tipo` = 'ENTRADA'),'E','S') as char(1) charset utf8mb4) AS `sina_opr`,cast(`c`.`idCliente` as char(20) charset utf8mb4) AS `codi_cli`,cast(`p`.`idProduto` as char(65535) charset utf8mb4) AS `codi_pro`,cast(`p`.`descricao` as char(60) charset utf8mb4) AS `desc_pro`,cast(`p`.`ncm` as char(8) charset utf8mb4) AS `ncmp_pro`,cast(`p`.`un` as char(10) charset utf8mb4) AS `unid_pro`,cast(`p`.`codBarras` as char(14) charset utf8mb4) AS `barr_pro`,cast(`p`.`codComercial` as char(15) charset utf8mb4) AS `cind_pro`,cast(`e`.`lote` as char(20) charset utf8mb4) AS `lote_ivn`,cast(`ehc`.`valor` as decimal(18,4)) AS `pbru_ivn`,cast(`ehc`.`valor` as decimal(18,4)) AS `pliq_ivn`,cast(`ehc`.`quantTrib` as decimal(18,3)) AS `qtde_ivn`,cast(`u`.`idUsuario` as char(20) charset utf8mb4) AS `codi_ved`,cast(`u`.`nome` as char(60) charset utf8mb4) AS `nome_ved` from ((((((((`staccato`.`venda_has_produto2` `vp2` left join `staccato`.`produto` `p` on((`p`.`idProduto` = `vp2`.`idProduto`))) left join `staccato`.`loja` `l` on((`vp2`.`idLoja` = `l`.`idLoja`))) left join `staccato`.`nfe` `n` on((`vp2`.`idNFeSaida` = `n`.`idNFe`))) left join `staccato`.`estoque_has_consumo` `ehc` on((`vp2`.`idVendaProduto2` = `ehc`.`idVendaProduto2`))) left join `staccato`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) left join `staccato`.`venda` `v` on((`vp2`.`idVenda` = `v`.`idVenda`))) left join `staccato`.`cliente` `c` on((`v`.`idCliente` = `c`.`idCliente`))) left join `staccato`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`))) where (`p`.`idFornecedor` in (1,2,6,8,331,343,346)) group by `vp2`.`idVendaProduto2`;
