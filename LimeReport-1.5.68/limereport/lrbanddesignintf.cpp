...
BandMarker::BandMarker(BandDesignIntf *band, QGraphicsItem *parent) : QGraphicsItem(parent), m_rect(0, 0, 30, 30), m_band(band) { setAcceptHoverEvents(true); }
...
void BandMarker::paint(QPainter *painter, const QStyleOptionGraphicsItem * /**option*/, QWidget * /*widget*/) {
  ...
  painter->fillRect(QRectF(boundingRect().bottomLeft().x(), boundingRect().bottomLeft().y() - 4, boundingRect().width(), 4), Qt::lightGray);
...
void BandMarker::mousePressEvent(QGraphicsSceneMouseEvent *event) {
  if (event->button() == Qt::LeftButton) {
    if (!(event->modifiers() & Qt::ControlModifier)) m_band->scene()->clearSelection();
...
void BandMarker::contextMenuEvent(QGraphicsSceneContextMenuEvent *event) { m_band->contextMenuEvent(event); }
...
void BandMarker::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if (hasCursor()) {
    ...
    if (!m_band->isFixedPos()) m_band->setItemPos(QPointF(m_band->pos().x(), m_band->pos().y() + delta));
  }
}
...
BandDesignIntf::BandDesignIntf(BandsType bandType, const QString &xmlTypeName, QObject *owner, QGraphicsItem *parent)
    : ItemsContainerDesignInft(xmlTypeName, owner, parent), m_bandType(bandType), m_bandIndex(static_cast<int>(bandType)), m_dataSourceName(""), m_autoHeight(true), m_keepBottomSpace(false),
      m_keepTopSpace(true), m_parentBand(0), m_parentBandName(""), m_bandMarker(0), m_tryToKeepTogether(false), m_splitable(false), m_keepFooterTogether(false), m_maxScalePercent(0),
      m_sliceLastRow(false), m_printIfEmpty(false), m_columnsCount(1), m_columnIndex(0), m_columnsFillDirection(Horizontal), m_reprintOnEachPage(false), m_startNewPage(false),
      m_startFromNewPage(false), m_printAlways(false), m_repeatOnEachRow(false), m_useAlternateBackgroundColor(false), m_bottomSpace(0), m_shiftItems(0) {
  ...
  connect(this, SIGNAL(propertyObjectNameChanged(QString, QString)), this, SLOT(slotPropertyObjectNameChanged(const QString &, const QString &)));
}
...
QRectF BandDesignIntf::boundingRect() const { return ItemsContainerDesignInft::boundingRect().adjusted(0, -4, 0, 4); }
...
void BandDesignIntf::changeBandIndex(int value, bool firstTime) {
  ...
  if (firstTime && bandHeader()) value += 1;
...
void BandDesignIntf::setKeepBottomSpace(bool value) {
  if (m_keepBottomSpace != value) {
    ...
    if (!isLoading()) notify("keepBottomSpace", !value, value);
  }
}
...
void BandDesignIntf::removeChildBand(BandDesignIntf *band) { m_childBands.removeAt(m_childBands.indexOf(band)); }

void BandDesignIntf::setParentBand(BandDesignIntf *band) {
  m_parentBand = band;
  if (band) {
    if (parentBandName().compare(band->objectName(), Qt::CaseInsensitive) != 0) setParentBandName(band->objectName());
...
void BandDesignIntf::setParentBandName(const QString &parentBandName) {
  ...
  if (itemMode() & DesignMode && !m_parentBandName.isEmpty()) {
    if ((parentBand() == 0) || (parentBand()->objectName() != parentBandName)) setParentBand(findParentBand());
  }
}
...
int BandDesignIntf::maxChildIndex(QSet<BandDesignIntf::BandsType> ignoredBands) const {
  int curIndex = bandIndex();
  foreach (BandDesignIntf *childBand, childBands()) {
    if (!ignoredBands.contains(childBand->bandType())) { curIndex = std::max(curIndex, childBand->maxChildIndex(ignoredBands)); }
  }
  return curIndex;
}

int BandDesignIntf::rootIndex(BandDesignIntf *parentBand) { return rootBand(parentBand)->bandIndex(); }

BandDesignIntf *BandDesignIntf::rootBand(BandDesignIntf *parentBand) {
  BandDesignIntf *currentBand = this;
  while (currentBand->parentBand() && currentBand->parentBand() != parentBand) currentBand = currentBand->parentBand();
  return currentBand;
}
...
int BandDesignIntf::minChildIndex(QSet<BandDesignIntf::BandsType> ignoredBands) {
  int curIndex = bandIndex();
  foreach (BandDesignIntf *childBand, childBands()) {
    if (!ignoredBands.contains(childBand->bandType()) && childBand->bandIndex() < bandIndex()) { curIndex = std::min(curIndex, childBand->minChildIndex(ignoredBands)); }
  }
  return curIndex;
}
...
void BandDesignIntf::setTryToKeepTogether(bool value) {
  if (m_tryToKeepTogether != value) {
    ...
    if (!isLoading()) notify("keepSubdetailTogether", !value, value);
  }
}
...
void BandDesignIntf::setColumnsFillDirection(BandDesignIntf::BandColumnsLayoutType value) {
  if (m_columnsFillDirection != value) {
    ...
    if (!isLoading()) notify("columnsFillDirection", oldValue, value);
  }
}

void BandDesignIntf::moveItemsDown(qreal startPos, qreal offset) {
  foreach (QGraphicsItem *item, childItems()) {
    if (item->pos().y() >= startPos) item->setPos(item->x(), item->y() + offset);
  }
}
...
void BandDesignIntf::preparePopUpMenu(QMenu &menu) {
  ...
  foreach (QAction *action, menu.actions()) { action->setEnabled(!disabledActions.contains(action->text())); }
...
void BandDesignIntf::processPopUpAction(QAction *action) {
  if (action->text().compare(tr("Auto height")) == 0) { setProperty("autoHeight", action->isChecked()); }

  if (action->text().compare(tr("Splittable")) == 0) { setProperty("splittable", action->isChecked()); }

  if (action->text().compare(tr("Keep top space")) == 0) { setProperty("keepTopSpace", action->isChecked()); }

  if (action->text().compare(tr("Keep bottom space")) == 0) { setProperty("keepBottomSpace", action->isChecked()); }
  if (action->text().compare(tr("Keep top space")) == 0) { setProperty("keepTopSpace", action->isChecked()); }

  if (action->text().compare(tr("Print if empty")) == 0) { setProperty("printIfEmpty", action->isChecked()); }
...
void BandDesignIntf::parentObjectLoadFinished() {
  if (!parentBandName().isEmpty()) setParentBand(findParentBand());
}
...
void BandDesignIntf::emitBandReRendered(BandDesignIntf *oldBand, BandDesignIntf *newBand) { emit bandReRendered(oldBand, newBand); }

void BandDesignIntf::setSplittable(bool value) {
  if (m_splitable != value) {
    bool oldValue = m_splitable;
    m_splitable = value;
    if (!isLoading()) notify("splittable", oldValue, value);
  }
}

bool bandIndexLessThen(const BandDesignIntf *b1, const BandDesignIntf *b2) { return b1->bandIndex() < b2->bandIndex(); }
...
BandDesignIntf *BandDesignIntf::findParentBand() {
  if (parent() && (!dynamic_cast<BaseDesignIntf *>(parent())->isLoading())) {
    foreach (QObject *item, parent()->children()) {
      ...
      if (band && (band->objectName().compare(parentBandName(), Qt::CaseInsensitive) == 0)) return band;
    }
  }
  return 0;
}
...
void BandDesignIntf::geometryChangedEvent(QRectF, QRectF) {
  if (((itemMode() & DesignMode) || (itemMode() & EditMode)) && parentItem()) { updateBandMarkerGeometry(); }
  foreach (BaseDesignIntf *item, childBaseItems()) {
...
QVariant BandDesignIntf::itemChange(QGraphicsItem::GraphicsItemChange change, const QVariant &value) {
  ...
  if (change == ItemSelectedChange) {
    if (m_bandMarker) {
      m_bandMarker->update(0, 0, m_bandMarker->boundingRect().width(), m_bandMarker->boundingRect().width());
      m_bandNameLabel->updateLabel(bandTitle());
...
void BandDesignIntf::childBandDeleted(QObject *band) { m_childBands.removeAt(m_childBands.indexOf(reinterpret_cast<BandDesignIntf *>(band))); }
...
void BandDesignIntf::setUseAlternateBackgroundColor(bool useAlternateBackgroundColor) {
  if (m_useAlternateBackgroundColor != useAlternateBackgroundColor) {
    ...
    if (!isLoading()) notify("useAlternateBackgroundColor", oldValue, useAlternateBackgroundColor);
  }
}
...
void BandDesignIntf::setAlternateBackgroundColor(const QColor &alternateBackgroundColor) {
  if (m_alternateBackgroundColor != alternateBackgroundColor) {
    ...
    if (!isLoading()) notify("alternateBackgroundColor", oldValue, alternateBackgroundColor);
  }
}

qreal BandDesignIntf::bottomSpace() const { return height() - findMaxBottom(); }

void BandDesignIntf::slotPropertyObjectNameChanged(const QString &, const QString &newName) {
  update();
  if (m_bandNameLabel) m_bandNameLabel->updateLabel(newName);
}
...
void BandDesignIntf::setKeepTopSpace(bool value) {
  if (m_keepTopSpace != value) {
    ...
    if (!isLoading()) notify("keepTopSpace", !value, value);
  }
}

int BandDesignIntf::bootomSpace() const { return m_bottomSpace; }

void BandDesignIntf::setBootomSpace(int bootomSpace) { m_bottomSpace = bootomSpace; }
...
void BandDesignIntf::setRepeatOnEachRow(bool repeatOnEachRow) {
  if (m_repeatOnEachRow != repeatOnEachRow) {
    ...
    if (!isLoading()) notify("repeatOnEachRow", !m_repeatOnEachRow, m_repeatOnEachRow);
  }
}
...
void BandDesignIntf::setPrintAlways(bool printAlways) {
  if (m_printAlways != printAlways) {
    ...
    if (!isLoading()) notify("printAlways", !m_printAlways, m_printAlways);
  }
}
...
void BandDesignIntf::setStartFromNewPage(bool startWithNewPage) {
  if (m_startFromNewPage != startWithNewPage) {
    ...
    if (!isLoading()) notify("startFromNewPage", !startWithNewPage, startWithNewPage);
  }
}
...
void BandDesignIntf::setStartNewPage(bool startNewPage) {
  if (m_startNewPage != startNewPage) {
    ...
    if (!isLoading()) notify("startNewPage", !startNewPage, startNewPage);
  }
}
...
void BandDesignIntf::setAutoHeight(bool value) {
  if (m_autoHeight != value) {
    m_autoHeight = value;
    if (!isLoading()) notify("autoHeight", !value, value);
  }
}
...
void BandDesignIntf::setReprintOnEachPage(bool reprintOnEachPage) {
  if (m_reprintOnEachPage != reprintOnEachPage) {
    ...
    if (!isLoading()) notify("reprintOnEachPage", !m_reprintOnEachPage, m_reprintOnEachPage);
  }
}
...
void BandDesignIntf::setPrintIfEmpty(bool printIfEmpty) {
  if (m_printIfEmpty != printIfEmpty) {
    ...
    if (!isLoading()) notify("printIfEmpty", !m_printIfEmpty, m_printIfEmpty);
  }
}
...
void BandDesignIntf::setSliceLastRow(bool sliceLastRow) {
  if (m_sliceLastRow != sliceLastRow) {
    ...
    if (!isLoading()) notify("sliceLastRow", !sliceLastRow, sliceLastRow);
  }
}

int BandDesignIntf::maxScalePercent() const { return m_maxScalePercent; }

void BandDesignIntf::setMaxScalePercent(int maxScalePercent) { m_maxScalePercent = maxScalePercent; }
...
void BandDesignIntf::setKeepFooterTogether(bool value) {
  if (m_keepFooterTogether != value) {
    ...
    if (!isLoading()) notify("keepFooterTogether", oldValue, value);
  }
}
...
void BandDesignIntf::updateItemSize(DataSourceManager *dataManager, RenderPass pass, int maxHeight) {
  ...
  if (borderLines() != 0) { spaceBorder += borderLineSize() + 2; }
  ...
  if (autoHeight()) {
    if (!keepTopSpace()) {
      ...
      foreach (BaseDesignIntf *item, childBaseItems()) { item->setY(item->y() - minTop); }
    }
...
DataBandDesignIntf::DataBandDesignIntf(BandDesignIntf::BandsType bandType, QString xmlTypeName, QObject *owner, QGraphicsItem *parent) : BandDesignIntf(bandType, xmlTypeName, owner, parent) {}

BandNameLabel::BandNameLabel(BandDesignIntf *band, QGraphicsItem *parent) : QGraphicsItem(parent), m_rect(5, 5, 30, 30), m_band(band) { setAcceptHoverEvents(true); }
...
void BandNameLabel::updateLabel(const QString &bandName) {
  ...
  QFontMetrics fontMetrics(font);
  prepareGeometryChange();
  m_rect = QRectF(m_band->pos().x() + 10, m_band->pos().y() - (fontMetrics.height() + 10), fontMetrics.horizontalAdvance(bandName) + 20, fontMetrics.height() + 10);
  update();
}
