...
ReportEnginePrivate::ReportEnginePrivate(QObject *parent)
    : QObject(parent), m_preparedPagesManager(new PreparedPages(&m_preparedPages)), m_fileName(""), m_settings(0), m_ownedSettings(false), m_printer(new QPrinter(QPrinter::HighResolution)),
      m_printerSelected(false), m_showProgressDialog(true), m_reportName(""), m_activePreview(0), m_previewWindowIcon(":/report/images/logo32"), m_previewWindowTitle(tr("Preview")),
      m_reportRendering(false), m_resultIsEditable(true), m_passPhrase("HjccbzHjlbyfCkjy"), m_fileWatcher(new QFileSystemWatcher(this)), m_reportLanguage(QLocale::AnyLanguage),
      m_previewLayoutDirection(Qt::LayoutDirectionAuto), m_designerFactory(0), m_previewScaleType(FitWidth), m_previewScalePercent(0), m_startTOCPage(0), m_previewPageBackgroundColor(Qt::gray),
      m_saveToFileVisible(true), m_printToPdfVisible(true), m_printVisible(true) {
#ifdef HAVE_STATIC_BUILD
  initResources();
  initReportItems();
#ifdef HAVE_REPORT_DESIGNER
  ...
  connect(tableOfContents, SIGNAL(getCallbackData(LimeReport::CallbackInfo, QVariant &)), m_scriptEngineContext->tableOfContents(), SLOT(slotOneSlotDS(LimeReport::CallbackInfo, QVariant &)));
...
ReportEnginePrivate::~ReportEnginePrivate() {
  if (m_designerWindow) { m_designerWindow->close(); }
  if (m_activePreview) { m_activePreview->close(); }
...
PageDesignIntf *ReportEnginePrivate::createPage(const QString &pageName, bool preview) {
  ...
  if (!preview) page->setReportEditor(this);
...
void ReportEnginePrivate::collectionLoadFinished(const QString &) {
  foreach (PageDesignIntf *page, m_pages) {
    ...
    page->setReportSettings(&m_reportSettings);
    page->setSceneRect(-Const::SCENE_MARGIN, -Const::SCENE_MARGIN, page->pageItem()->width() + Const::SCENE_MARGIN * 2, page->pageItem()->boundingRect().height() + Const::SCENE_MARGIN * 2);
  }
...
void ReportEnginePrivate::showError(QString message) { QMessageBox::critical(0, tr("Error"), message); }
...
void ReportEnginePrivate::updateTranslations() {
  foreach (ReportTranslation *translation, m_translations.values()) {
    ...
    foreach (PageDesignIntf *page, m_pages) { translation->updatePageTranslation(page); }
...
void ReportEnginePrivate::slotPreviewWindowDestroyed(QObject *window) {
  if (m_activePreview == window) { m_activePreview = 0; }
}
...
bool ReportEnginePrivate::printPages(ReportPages pages, QPrinter *printer) {
  ...
  if (printer && printer->isValid()) {
    try {
      if (pages.count() > 0) { internalPrintPages(pages, *printer); }
...
void ReportEnginePrivate::internalPrintPages(ReportPages pages, QPrinter &printer) {
  ...
  int pageCount = (printer.printRange() == QPrinter::AllPages) ? pages.size() : printer.toPage() - printer.fromPage();

  emit printingStarted(pageCount);
  foreach (PageItemDesignIntf::Ptr page, pages) {
    if (!m_cancelPrinting &&
        ((printer.printRange() == QPrinter::AllPages) || ((printer.printRange() == QPrinter::PageRange) && (currenPage >= printer.fromPage()) && (currenPage <= printer.toPage())))) {
...
void ReportEnginePrivate::printPages(ReportPages pages, QMap<QString, QPrinter *> printers, bool printToAllPrinters) {
  ...
  QMap<QString, QSharedPointer<PrintProcessor>> printProcessors;
  for (int i = 0; i < printers.keys().count(); ++i) { printProcessors.insert(printers.keys()[i], QSharedPointer<PrintProcessor>(new PrintProcessor(printers[printers.keys()[i]]))); }
...
QStringList ReportEnginePrivate::aviableReportTranslations() {
  QStringList result;
  foreach (QLocale::Language language, aviableLanguages()) { result << QLocale::languageToString(language); }
  return result;
}
...
void ReportEnginePrivate::setReportTranslation(const QString &languageName) {
  foreach (QLocale::Language language, aviableLanguages()) {
    if (QLocale::languageToString(language).compare(languageName) == 0) { setReportLanguage(language); }
  }
}
...
bool ReportEnginePrivate::printReport(QMap<QString, QPrinter *> printers, bool printToAllPrinters) {
  try {
    ...
    ReportPages pages = renderToPages();
    dataManager()->setDesignTime(designTime);
    if (pages.count() > 0) { printPages(pages, printers, printToAllPrinters); }
...
bool ReportEnginePrivate::printToPDF(const QString &fileName) { return exportReport("PDF", fileName); }
...
bool ReportEnginePrivate::exportReport(QString exporterName, const QString &fileName, const QMap<QString, QVariant> &params) {
  ...
  if (ExportersFactory::instance().map().contains(exporterName)) {
    ...
    if (!fn.isEmpty()) {
      QFileInfo fi(fn);
      if (fi.suffix().isEmpty()) fn += QString(".%1").arg(e->exporterFileExt());
...
bool ReportEnginePrivate::showPreviewWindow(ReportPages pages, PreviewHints hints, QPrinter *printer) {
  ...
  if (pages.count() > 0) {
    ...
    connect(w, SIGNAL(onSave(bool &, LimeReport::IPreparedPages *)), this, SIGNAL(onSavePreview(bool &, LimeReport::IPreparedPages *)));
    w->exec();
    return true;
  }
  return false;
...
ReportDesignWindowInterface *ReportEnginePrivate::getDesignerWindow() {
  ...
  if (m_designerWindow) { m_datasources->updateDatasourceModel(); }
...
PreviewReportWidget *ReportEnginePrivate::createPreviewWidget(QWidget *parent) {
  ...
  try {
    ...
    ReportPages pages = renderToPages();
    dataManager()->setDesignTime(true);
    if (pages.count() > 0) widget->d_ptr->setPages(pages);
  } catch (ReportError &exception) {
    saveError(exception.what());
    showError(exception.what());
  }
...
void ReportEnginePrivate::setCurrentReportsDir(const QString &dirName) {
  if (QDir(dirName).exists()) m_reportsDir = dirName;
}
...
bool ReportEnginePrivate::slotLoadFromFile(const QString &fileName) {
  ...
  if (!QFile::exists(fileName)) {
    if (hasActivePreview()) {
      QMessageBox::information(NULL, tr("Report File Change"), tr("The report file \"%1\" has changed names or been deleted.\n\nThis preview is no longer valid.").arg(fileName));
...
void ReportEnginePrivate::cancelRender() {
  if (m_reportRender) m_reportRender->cancelRender();
...
bool ReportEnginePrivate::saveToFile(const QString &fileName) {
  ...
  if (saved) {
    foreach (PageDesignIntf *page, m_pages) { page->setToSaved(); }
  }
  dropChanges();
...
bool ReportEnginePrivate::isNeedToSave() {
  ...
  if (dataManager()->hasChanges()) { return true; }
  if (scriptContext()->hasChanges()) return true;
  return false;
}
...
void ReportEnginePrivate::addWatermark(const WatermarkSetting &watermarkSetting) { m_watermarks.append(watermarkSetting); }
...
PageItemDesignIntf *ReportEnginePrivate::getPageByName(const QString &pageName) {
  foreach (PageItemDesignIntf *page, m_renderingPages) {
    if (page->objectName().compare(pageName, Qt::CaseInsensitive) == 0) return page;
  }
  return 0;
}
...
bool ReportEnginePrivate::showPreparedPages(PreviewHints hints) { return showPreparedPages(0, hints); }

bool ReportEnginePrivate::showPreparedPages(QPrinter *defaultPrinter, PreviewHints hints) { return showPreviewWindow(m_preparedPages, hints, defaultPrinter); }
...
Qt::LayoutDirection ReportEnginePrivate::previewLayoutDirection() { return m_previewLayoutDirection; }

void ReportEnginePrivate::setPreviewLayoutDirection(const Qt::LayoutDirection &layoutDirection) { m_previewLayoutDirection = layoutDirection; }
...
void ReportEnginePrivate::reorderPages(const QList<PageDesignIntf *> &reorderedPages) {
  m_pages.clear();
  foreach (PageDesignIntf *page, reorderedPages) { m_pages.append(page); }
}

void ReportEnginePrivate::clearSelection() {
  foreach (PageDesignIntf *page, m_pages) {
    foreach (QGraphicsItem *item, page->selectedItems()) { item->setSelected(false); }
  }
}
...
bool ReportEnginePrivate::removeTranslationLanguage(QLocale::Language language) { return m_translations.remove(language) != 0; }
      ...
      foreach (ItemTranslation *itemTranslation, pageTranslation->itemsTranslation) {
        ...
        if (item) {
          foreach (PropertyTranslation *propertyTranslation, itemTranslation->propertyesTranslation) {
            if (propertyTranslation->checked) item->setProperty(propertyTranslation->propertyName.toLatin1(), propertyTranslation->value);
          }
        }
      }
    }
...
ReportTranslation *ReportEnginePrivate::reportTranslation(QLocale::Language language) { return m_translations.value(language); }
...
bool ReportEnginePrivate::suppressFieldAndVarError() const { return m_reportSettings.suppressAbsentFieldsAndVarsWarnings(); }

void ReportEnginePrivate::setSuppressFieldAndVarError(bool suppressFieldAndVarError) { m_reportSettings.setSuppressAbsentFieldsAndVarsWarnings(suppressFieldAndVarError); }
...
void ReportEnginePrivate::setPreviewWindowTitle(const QString &previewWindowTitle) { m_previewWindowTitle = previewWindowTitle; }

QColor ReportEnginePrivate::previewWindowPageBackground() { return m_previewPageBackgroundColor; }

void ReportEnginePrivate::setPreviewWindowPageBackground(QColor color) { m_previewPageBackgroundColor = color; }

QIcon ReportEnginePrivate::previewWindowIcon() const { return m_previewWindowIcon; }

void ReportEnginePrivate::setPreviewWindowIcon(const QIcon &previewWindowIcon) { m_previewWindowIcon = previewWindowIcon; }
  ...
  for (int index = 0; index < pageCount(); ++index) {
    ...
    if (page != 0) {
      foreach (BaseDesignIntf *item, page->pageItem()->childBaseItems()) {
        ...
        if (proxyItem) { proxyItem->setExternalPainter(this); }
      }
    }
  }
}

void ReportEnginePrivate::paintByExternalPainter(const QString &objectName, QPainter *painter, const QStyleOptionGraphicsItem *options) { emit externalPaint(objectName, painter, options); }
...
ReportPages ReportEnginePrivate::renderToPages() {
  ...
  updateTranslations();
  connect(m_reportRender.data(), SIGNAL(pageRendered(int)), this, SIGNAL(renderPageFinished(int)));
  ...
  if (m_pages.count()) {
    ...
    foreach (PageDesignIntf *page, m_pages) {
      ...
      if (!m_watermarks.isEmpty()) {
        foreach (WatermarkSetting watermarkSetting, m_watermarks) { watermarks.append(createWatermark(page, watermarkSetting)); }
      }
      ...
      for (int i = 0; i < m_renderingPages.count(); ++i) {
        ...
        if (page->isTOC()) {
          ...
          if (i < m_renderingPages.count()) {
            ...
            if (m_renderingPages.count() > (pageAfterTOCIndex)) secondPage = m_renderingPages.at(pageAfterTOCIndex);
            ReportPages pages = m_reportRender->renderTOC(page, true, secondPage && secondPage->resetPageNumber());
            for (int j = 0; j < pages.count(); ++j) { result.insert(startTOCPage + j, pages.at(j)); }
...
ReportEngine::ReportEngine(QObject *parent) : QObject(parent), d_ptr(new ReportEnginePrivate()), m_showDesignerModal(true) {
  Q_D(ReportEngine);
  d->q_ptr = this;
  ...
  connect(d, SIGNAL(getAvailableDesignerLanguages(QList<QLocale::Language> *)), this, SIGNAL(getAvailableDesignerLanguages(QList<QLocale::Language> *)));
  connect(d, SIGNAL(currentDefaultDesignerLanguageChanged(QLocale::Language)), this, SIGNAL(currentDefaultDesignerLanguageChanged(QLocale::Language)));
  connect(d, SIGNAL(getCurrentDefaultDesignerLanguage()), this, SIGNAL(getCurrentDefaultDesignerLanguage()));

  connect(d, SIGNAL(externalPaint(const QString &, QPainter *, const QStyleOptionGraphicsItem *)), this, SIGNAL(externalPaint(const QString &, QPainter *, const QStyleOptionGraphicsItem *)));
  connect(d, SIGNAL(onSavePreview(bool &, LimeReport::IPreparedPages *)), this, SIGNAL(onSavePreview(bool &, LimeReport::IPreparedPages *)));
}
...
void ReportEngine::previewReport(PreviewHints hints) {
  Q_D(ReportEngine);
  if (m_settings) d->setSettings(m_settings);
...
ReportEngine::ReportEngine(ReportEnginePrivate &dd, QObject *parent) : QObject(parent), d_ptr(&dd), m_showDesignerModal(true) {
  Q_D(ReportEngine);
  d->q_ptr = this;
  connect(d, SIGNAL(renderStarted()), this, SIGNAL(renderStarted()));
  connect(d, SIGNAL(renderPageFinished(int)), this, SIGNAL(renderPageFinished(int)));
...
void ReportEngine::setShowDesignerModal(bool showDesignerModal) { m_showDesignerModal = showDesignerModal; }
...
PrintProcessor::PrintProcessor(QPrinter *printer) : m_printer(printer), m_painter(0), m_firstPage(true) { m_renderPage.setItemMode(PrintMode); }
...
qreal WatermarkHelper::valueToPixels(qreal value) {
  ...
  case LimeReport::ItemGeometry::Millimeters: return value * Const::mmFACTOR;
  case LimeReport::ItemGeometry::Pixels: return value;
  }
  return -1;
}
