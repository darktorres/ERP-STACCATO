...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_calendario_produto` AS select `vhp`.`idEvento` AS `idEvento`,`vp2`.`idVenda` AS `idVenda`,`vp2`.`idVendaProduto2` AS `idVendaProduto2`,`vp2`.`fornecedor` AS `fornecedor`,`vp2`.`produto` AS `produto`,`vp2`.`formComercial` AS `formComercial`,`vp2`.`codComercial` AS `codComercial`,`vhp`.`caixas` AS `caixas`,`vhp`.`quant` AS `quant`,`vp2`.`un` AS `un`,`vhp`.`kg` AS `kg`,`e`.`idEstoque` AS `idEstoque`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco`,if(isnull(`pf1`.`idVenda`),'✓','') AS `isEstoque` from (((((`staccato`.`veiculo_has_produto` `vhp` left join `staccato`.`venda_has_produto2` `vp2` on((`vhp`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) left join `staccato`.`estoque_has_consumo` `ehc` on((`vp2`.`idVendaProduto2` = `ehc`.`idVendaProduto2`))) left join `staccato`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) left join `staccato`.`estoque_has_compra` `ecomp` on((`e`.`idEstoque` = `ecomp`.`idEstoque`))) left join `staccato`.`pedido_fornecedor_has_produto` `pf1` on((`ecomp`.`idPedido1` = `pf1`.`idPedido1`))) where (`vhp`.`status` <> 'CANCELADO');
