...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`ViewDespesasExcel` AS select date_format(`cp`.`dataEmissao`,'%Y/%m') AS `mes`,`cp`.`status` AS `status`,sum(if((`cp`.`status` = 'pago'),`cp`.`valorReal`,(`cp`.`valor` + 0E0))) AS `valor`,`staccato`.`loja`.`descricao` AS `centroCusto`,`cp`.`grupo` AS `grupo` from (`staccato`.`conta_a_pagar_has_pagamento` `cp` left join `staccato`.`loja` on((`staccato`.`loja`.`idLoja` = `cp`.`centroCusto`))) where ((`cp`.`desativado` = 0) and (`cp`.`dataEmissao` > '2021-12-31') and (`cp`.`status` <> 'cancelado') and (`cp`.`status` <> 'substituido')) group by date_format(`cp`.`dataEmissao`,'%Y/%m'),`cp`.`status`,`cp`.`centroCusto`,`cp`.`grupo`;
