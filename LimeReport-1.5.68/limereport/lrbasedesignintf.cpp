...
void BaseDesignIntf::setWidth(qreal width) {
  ...
  if (!m_changingItemAlign) updateItemAlign();
}
...
void BaseDesignIntf::setHeight(qreal height) { setGeometry(QRectF(rect().x(), rect().y(), rect().width(), height)); }
...
QSizeF BaseDesignIntf::sizeMM() const { return QSizeF(width() / Const::mmFACTOR, height() / Const::mmFACTOR); }
...
QPointF BaseDesignIntf::posMM() const { return QPointF(pos().x() / Const::mmFACTOR, pos().y() / Const::mmFACTOR); }
...
void BaseDesignIntf::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if (m_resizeDirectionFlags & ResizeLeft) {
    if ((event->scenePos().x()) <= (mapToScene(0, 0).x() + (width() - Const::MINIMUM_ITEM_WIDTH)) && (width() + (event->lastScenePos().x() - event->scenePos().x()) > Const::MINIMUM_ITEM_WIDTH)) {
      qreal posRightCorner = mapToScene(0, 0).x() + width();
      qreal posLeftCorner = div(mapToParent(event->pos()).x(), hStep).quot * hStep;
      if (posLeftCorner < 0) posLeftCorner = 0;
  ...
  if (m_resizeDirectionFlags & ResizeRight) {
    if ((event->scenePos().x() >= (mapToScene(0, 0).x() + Const::MINIMUM_ITEM_WIDTH)) || (event->scenePos().x() >= (mapToScene(0, 0).x() + width()))) {
      setWidth(div(event->scenePos().x() - mapToScene(0, 0).x(), hStep).quot * hStep);
    }
  }

  if (m_resizeDirectionFlags & ResizeTop) {
    if ((event->scenePos().y()) <= (mapToScene(0, 0).y() + (height() - Const::MINIMUM_ITEM_HEIGHT)) && (height() + (event->lastScenePos().y() - event->scenePos().y()) > Const::MINIMUM_ITEM_HEIGHT)) {
      qreal posBottomCorner = mapToScene(0, 0).y() + height();
      qreal posTopCorner = div(mapToParent(event->pos()).y(), vStep).quot * vStep;
      if (posTopCorner < 0) posTopCorner = 0;
  ...
  if (m_resizeDirectionFlags & ResizeBottom) {
    if ((event->scenePos().y() > (mapToScene(0, 0).y() + height())) || (event->scenePos().y() > (mapToScene(0, 0).y() + Const::MINIMUM_ITEM_HEIGHT))) {
  ...
  if ((m_resizeDirectionFlags == Fixed) && !m_fixedPos) {
    ...
    if (page()) {
      if (!isBand() && page()->selectedItems().count() > 1) moveSelectedItems(tmpPos - pos());
      if (page()->selectedItems().count() == 1 && (page()->magneticMovement())) page()->itemMoved(this);
    }
  }
}
...
QPointF BaseDesignIntf::modifyPosForAlignedItem(const QPointF &pos) {
  ...
  if (parent) {
    ...
    case RightItemAlign: result.setX(parent->width() - (rightBorder + width())); break;
    case CenterItemAlign: result.setX((avaibleSpace - width()) / 2 + leftBorder); break;
    case ParentWidthItemAlign: result.setX(leftBorder);
...
void BaseDesignIntf::updateItemAlign() {
  ...
  if (parent) {
    ...
    if (m_itemAlign == ParentWidthItemAlign) setWidth(aviableSpace);
  }
...
void BaseDesignIntf::updatePossibleDirectionFlags() {
  ...
  case ParentWidthItemAlign: setPossibleResizeDirectionFlags(ResizeBottom | ResizeTop);
...
void BaseDesignIntf::setFillTransparentInDesignMode(bool fillTransparentInDesignMode) { m_fillTransparentInDesignMode = fillTransparentInDesignMode; }

void BaseDesignIntf::emitPosChanged(QPointF oldPos, QPointF newPos) { emit posChanged(this, oldPos, newPos); }
...
QString BaseDesignIntf::patternName() const { return (m_patternName.isEmpty()) ? objectName() : m_patternName; }
...
void BaseDesignIntf::emitObjectNamePropertyChanged(const QString &oldName, const QString &newName) { emit propertyObjectNameChanged(oldName, newName); }

int BaseDesignIntf::borderLineSize() const { return m_borderLineSize; }
...
void BaseDesignIntf::setGeometry(QRectF rect) {
  ...
  if (!isLoading()) prepareGeometryChange();
  m_rect = rect;
...
void BaseDesignIntf::setGeometryProperty(QRect rect) {
  if (rect != m_itemGeometry) {
    ...
    if ((rect.x() != geometry().x()) || (rect.y() != geometry().y())) setPos(rect.x(), rect.y());
    if (rect.width() != geometry().width()) setWidth(rect.width());
    if (rect.height() != geometry().height()) setHeight(rect.height());
...
void BaseDesignIntf::setPossibleResizeDirectionFlags(int directionsFlags) { m_possibleResizeDirectionFlags = directionsFlags; }
...
void BaseDesignIntf::drawMarker(QPainter *painter, QColor color) const {
  ...
  painter->drawRect(QRectF(rect().left() - markerSize, rect().bottom() - rect().height() / 2 - markerSize, markerSize * 2, markerSize * 2));
  painter->drawRect(QRectF(rect().right() - markerSize, rect().bottom() - rect().height() / 2 - markerSize, markerSize * 2, markerSize * 2));
  painter->drawRect(QRectF(rect().left() + rect().width() / 2 - markerSize, rect().top() - markerSize, markerSize * 2, markerSize * 2));
  painter->drawRect(QRectF(rect().left() + rect().width() / 2 - markerSize, rect().bottom() - markerSize, markerSize * 2, markerSize * 2));
  ...
  foreach (QGraphicsItem *item, selectedItems) {
    if (item != this) {
      ...
      if (selectedItem && !selectedItem->isBand()) {
        if (!selectedItem->m_fixedPos) selectedItem->setItemPos(selectedItem->pos() - delta);
      }
    }
  }
}
...
void BaseDesignIntf::contextMenuEvent(QGraphicsSceneContextMenuEvent *event) {
  ...
  if (page->selectedItems().count() > 1) { createHLayout = menu.addAction(QIcon(":/report/images/hlayout"), tr("Create Horizontal Layout")); }
  QAction *createVLayout = 0;
  if (page->selectedItems().count() > 1) { createVLayout = menu.addAction(QIcon(":/report/images/vlayout"), tr("Create Vertical Layout")); }
  menu.addSeparator();
  ...
  if (a) {
    ...
    if (a == copyAction) page->copy();
    if (a == pasteAction) page->paste();
    if (a == bringToTopAction) page->bringToFront();
    if (a == sendToBackAction) page->sendToBack();
    if (a == noBordersAction) page->setBorders(BaseDesignIntf::NoLine);
    if (a == allBordersAction) page->setBorders(BaseDesignIntf::AllLines);
    if (a == createHLayout) page->addHLayout();
    if (a == createVLayout) page->addVLayout();
...
void BaseDesignIntf::setPossibleMoveFlags(int directionsFlags) { m_possibleMoveDirectionFlags = directionsFlags; }
...
void BaseDesignIntf::invalidateRect(const QRectF &rect) {
  if (scene()) scene()->update(mapToScene(rect).boundingRect());
}
...
QObject *BaseDesignIntf::createElement(const QString & /*collectionName*/, const QString &elementType) {
  ...
  try {
    if (LimeReport::DesignElementsFactory::instance().objectCreator(elementType)) {
      ...
      if (page()) connect(obj, SIGNAL(propertyChanged(QString, QVariant, QVariant)), page(), SLOT(slotItemPropertyChanged(QString, QVariant, QVariant)));
    }
  } catch (ReportError &error) { qDebug() << error.what(); }
  return obj;
}

int BaseDesignIntf::elementsCount(const QString & /*collectionName*/) { return QObject::children().count(); }

QObject *BaseDesignIntf::elementAt(const QString & /*collectionName*/, int index) { return QObject::children().at(index); }
  ...
  if (collectionName.compare("children", Qt::CaseInsensitive) == 0) {
    if (page()) {
      ...
      foreach (QObject *obj, QObject::children()) {
        ...
        if (item && page()) { page()->registerItem(item); }
      }
    }
  }
...
BaseDesignIntf *BaseDesignIntf::cloneItemWOChild(ItemMode mode, QObject *owner, QGraphicsItem *parent) {
  ...
  for (int i = 0; i < clone->metaObject()->propertyCount(); i++) {
    if (clone->metaObject()->property(i).isWritable()) clone->setProperty(clone->metaObject()->property(i).name(), property(clone->metaObject()->property(i).name()));
  }
  ...
  for (int i = 0; i < metaObject()->propertyCount(); i++) {
    if (strcmp(metaObject()->property(i).name(), "objectName") != 0)
      if (source->property(metaObject()->property(i).name()).isValid()) {
        if (metaObject()->property(i).isWritable()) setProperty(metaObject()->property(i).name(), source->property(metaObject()->property(i).name()));
      }
  }
  objectLoadFinished();
}
...
void BaseDesignIntf::notify(const QVector<QString> &propertyNames) {
  if (!isLoading()) emit propertyesChanged(propertyNames);
}

QMap<QString, QString> BaseDesignIntf::getStringForTranslation() { return QMap<QString, QString>(); }
...
void BookmarkContainerDesignIntf::copyBookmarks(BookmarkContainerDesignIntf *source) {
  foreach (QString key, source->bookmarks()) { addBookmark(key, source->getBookMark(key)); }
}
...
SelectionMarker::SelectionMarker(QGraphicsItem *parent, BaseDesignIntf *owner) : Marker(parent, owner) { setAcceptHoverEvents(true); }

QColor SelectionMarker::color() const { return owner()->isGeometryLocked() ? Qt::darkGray : Marker::color(); }
...
void BaseDesignIntf::processPopUpAction(QAction *action) {
  if (page()) {
    if (action->text().compare(tr("Lock item geometry")) == 0) { page()->setPropertyToSelectedItems("geometryLocked", action->isChecked()); }
  }
}
