...
DELIMITER $$
USE `mydb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_produto_estoque_quant`()
BEGIN

update produto, view_estoque set produto.estoqueRestante = view_estoque.restante where produto.idEstoque = view_estoque.idEstoque;
update produto set produto.descontinuado = true where produto.estoque = true and estoqueRestante = 0;
