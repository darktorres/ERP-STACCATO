...
void calcPhys() {
  ...
  for (int i = 0; i < OBJECTS; ++i) intarray[i] = multModel(double(i)) + double(i / 3) - double((OBJECTS - i) / 2);
  calcIntersect();
  delete[] intarray;
}
...
void calcBrain() {
  ...
  double *intarray = new double[OBJECTS];
  for (int i = 0; i < OBJECTS; ++i) intarray[i] = calcSubbrain(i) + double(i * 180 / 3);
  delete[] intarray;
  // std::this_thread::sleep_for(std::chrono::milliseconds(3));
}
...
void calculatePhysics() {
  ...
  for (int i = 0; i < OBJECTS; ++i) intarray[i] = calcPhysicForObject(i);
  delete[] intarray;
  // std::this_thread::sleep_for(std::chrono::milliseconds(8));
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 2 && argv[2]) { MODELLING_STEPS = std::atoi(argv[2]); }
  if (argc > 3 && argv[3]) { RENDER_STEPS = std::atoi(argv[3]); }
  if (argc > 4 && argv[4]) { RESOURCE_LOADING_COUNT = std::atoi(argv[4]); }
