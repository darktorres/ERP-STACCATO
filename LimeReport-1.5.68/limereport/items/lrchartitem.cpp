...
LimeReport::BaseDesignIntf *createChartItem(QObject *owner, LimeReport::BaseDesignIntf *parent) { return new LimeReport::ChartItem(owner, parent); }
bool registred = LimeReport::DesignElementsFactory::instance().registerCreator(xmlTag, LimeReport::ItemAttribs(QObject::tr("Chart Item"), "Item"), createChartItem);
} // namespace
...
QColor color_map[39] = {QColor(51, 102, 204),  QColor(220, 57, 18),   QColor(225, 153, 0),   QColor(16, 150, 24), QColor(153, 0, 153),   QColor(0, 153, 198),  QColor(221, 68, 119),
                        QColor(255, 0, 0),     QColor(0, 0, 139),     QColor(0, 205, 0),     QColor(233, 30, 99), QColor(255, 255, 0),   QColor(244, 67, 54),  QColor(156, 39, 176),
                        QColor(103, 58, 183),  QColor(63, 81, 181),   QColor(33, 153, 243),  QColor(0, 150, 136), QColor(78, 175, 80),   QColor(139, 195, 74), QColor(205, 228, 57),
                        QColor(0, 139, 0),     QColor(0, 0, 255),     QColor(255, 235, 59),  QColor(255, 193, 7), QColor(255, 152, 0),   QColor(255, 87, 34),  QColor(121, 85, 72),
                        QColor(158, 158, 158), QColor(96, 125, 139),  QColor(241, 153, 185), QColor(64, 64, 64),  QColor(188, 229, 218), QColor(139, 0, 0),    QColor(139, 139, 0),
                        QColor(171, 130, 255), QColor(139, 123, 139), QColor(255, 0, 255),   QColor(139, 69, 19)};
...
SeriesItem *SeriesItem::clone() {
  ...
  for (int i = 0; i < this->metaObject()->propertyCount(); ++i) { result->setProperty(this->metaObject()->property(i).name(), property(this->metaObject()->property(i).name())); }
  return result;
}
...
void SeriesItem::fillSeriesData(IDataSource *dataSource) {
  ...
  if (dataSource) {
    ...
    while (!dataSource->eof()) {
      if (!m_labelsColumn.isEmpty()) m_data.labels().append(dataSource->data(m_labelsColumn).toString());
...
ChartItem::ChartItem(QObject *owner, QGraphicsItem *parent)
    : ItemDesignIntf(xmlTag, owner, parent), m_legendBorder(true), m_legendAlign(LegendAlignCenter), m_titleAlign(TitleAlignCenter), m_chartType(Pie), m_labelsField(""), m_isEmpty(true),
      m_showLegend(true) {
  m_labels << "First"
           << "Second"
           << "Thrid";
...
ChartItem::~ChartItem() {
  foreach (SeriesItem *series, m_series) { delete series; }
  m_series.clear();
  delete m_chart;
}
...
void ChartItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget) {
  ...
  qreal titleOffset = !m_title.isEmpty() ? (((painter->fontMetrics().height() + borderMargin * 2) < maxTitleHeight) ? (painter->fontMetrics().height() + borderMargin * 2) : (maxTitleHeight)) : 0;

  QRectF titleRect = QRectF(borderMargin, borderMargin, rect().width() - borderMargin * 2, titleOffset);
  QRectF legendRect = QRectF(0, 0, 0, 0);
  if (m_showLegend) legendRect = m_chart->calcChartLegendRect(painter->font(), rect(), false, borderMargin, titleOffset);
  QRectF diagramRect = rect().adjusted(borderMargin, titleOffset + borderMargin, -(legendRect.width() + borderMargin * 2), -borderMargin);

  paintChartTitle(painter, titleRect);
  if (m_showLegend) m_chart->paintChartLegend(painter, legendRect);
...
BaseDesignIntf *ChartItem::createSameTypeItem(QObject *owner, QGraphicsItem *parent) {
  ...
  foreach (SeriesItem *series, m_series) { result->m_series.append(series->clone()); }
  return result;
}
...
int ChartItem::elementsCount(const QString &collectionName) {
  if (collectionName.compare("series") == 0) return m_series.count();
  return 0;
}

QObject *ChartItem::elementAt(const QString &collectionName, int index) {
  if (collectionName.compare("series") == 0) return m_series.at(index);
  return 0;
}
...
void ChartItem::setChartType(const ChartType &chartType) {
  if (m_chartType != chartType) {
    ...
    delete m_chart;
    switch (m_chartType) {
    case Pie: m_chart = new PieChart(this); break;
    case VerticalBar: m_chart = new VerticalBarChart(this); break;
    case HorizontalBar: m_chart = new HorizontalBarChart(this); break;
    case Lines: m_chart = new LinesChart(this);
    }
...
void ChartItem::paintChartTitle(QPainter *painter, QRectF titleRect) {
  ...
  while ((fm.height() > titleRect.height() || fm.horizontalAdvance(m_title) > titleRect.width()) && tmpFont.pixelSize() > 1) {
  ...
  switch (m_titleAlign) {
  case TitleAlignLeft: align = Qt::AlignLeft; break;
  case TitleAlignCenter: align = Qt::AlignCenter; break;
  case TitleAlignRight: align = Qt::AlignRight; break;
  }
...
AbstractChart::AbstractChart(ChartItem *chartItem) : m_chartItem(chartItem) { m_designLabels << QObject::tr("First") << QObject::tr("Second") << QObject::tr("Thrid"); }
...
QRectF AbstractChart::calcChartLegendRect(const QFont &font, const QRectF &parentRect, bool takeAllRect, qreal borderMargin, qreal titleOffset) {
  ...
  qreal rightOffset = !takeAllRect ? ((legendSize.width() > parentRect.width() / 2 - borderMargin) ? (parentRect.width() / 2) : (parentRect.width() - legendSize.width())) : 0;

  QRectF legendRect = parentRect.adjusted(rightOffset, (legendSize.height() > (parentRect.height() - titleOffset)) ? (titleOffset) : (legendTopMargin), -borderMargin,
...
void AbstractChart::prepareLegendToPaint(QRectF &legendRect, QPainter *painter) {
  ...
  if ((legendSize.height() > legendRect.height() || legendSize.width() > legendRect.width())) {
    while ((legendSize.height() > legendRect.height() || legendSize.width() > legendRect.width()) && tmpFont.pixelSize() > 1) {
      tmpFont.setPixelSize(tmpFont.pixelSize() - 1);
      painter->setFont(tmpFont);
...
QSizeF AbstractSeriesChart::calcChartLegendSize(const QFont &font) {
  ...
  if (!m_chartItem->series().isEmpty()) {
    foreach (SeriesItem *series, m_chartItem->series()) {
      cw += fm.height();
      if (maxWidth < fm.horizontalAdvance(series->name())) maxWidth = fm.horizontalAdvance(series->name()) + 10;
    }
  } else {
    foreach (QString label, m_designLabels) {
      cw += fm.height();
      if (maxWidth < fm.horizontalAdvance(label)) maxWidth = fm.horizontalAdvance(label) + 10;
    }
  }
  cw += fm.height();
...
bool AbstractSeriesChart::verticalLabels(QPainter *painter, QRectF labelsRect) {
  ...
  foreach (QString label, m_chartItem->labels()) {
    if (fm.horizontalAdvance(label) > hStep) { return true; }
  }
  return false;
}
...
void AbstractSeriesChart::paintHorizontalLabels(QPainter *painter, QRectF labelsRect) {
  ...
  if (!m_chartItem->labels().isEmpty()) {
    if (verticalLabels(painter, labelsRect)) {
      ...
      foreach (QString label, m_chartItem->labels()) {
        painter->drawText(QRectF(QPoint(0, 0), QSize(labelsRect.height() - 4, hStep)), Qt::AlignVCenter | Qt::AlignRight, label);
      ...
      foreach (QString label, m_chartItem->labels()) {
        painter->drawText(QRectF(QPoint(0, 4), QSize(hStep, labelsRect.height() - 4)), Qt::AlignHCenter | Qt::AlignTop, label);
...
void AbstractSeriesChart::paintVerticalLabels(QPainter *painter, QRectF labelsRect) {
  ...
  painter->save();
  painter->setFont(adaptLabelsFont(labelsRect.adjusted(0, 0, -hPadding(m_chartItem->rect()), 0), painter->font()));
  ...
  if (!m_chartItem->labels().isEmpty()) {
    foreach (QString label, m_chartItem->labels()) {
      painter->drawText(QRectF(QPoint(0, vStep * curLabel), QSize(labelsRect.width() - hPadding(m_chartItem->rect()), vStep)), Qt::AlignVCenter | Qt::AlignRight | Qt::TextWordWrap, label);
...
void AbstractSeriesChart::paintHorizontalGrid(QPainter *painter, QRectF gridRect) {
  ...
  for (int i = 0; i < 5; i++) {
    painter->drawText(QRectF(gridRect.left() + 4 + hStep * i, gridRect.bottom() - painter->fontMetrics().height(), hStep, painter->fontMetrics().height()),
                      QString::number(minValue() + i * delta / 4));
    painter->drawLine(gridRect.left() + hStep * i, gridRect.bottom(), gridRect.left() + hStep * i, gridRect.top());
  }
  painter->restore();
}
...
void AbstractSeriesChart::paintVerticalGrid(QPainter *painter, QRectF gridRect) {
  ...
  for (int i = 0; i < 5; i++) {
    painter->drawText(QRectF(gridRect.bottomLeft() - QPointF(0, vStep * i + painter->fontMetrics().height()), QSizeF(valuesHMargin(painter), painter->fontMetrics().height())),
                      QString::number(minValue() + i * delta / 4));
    painter->drawLine(gridRect.bottomLeft() - QPointF(-valuesHMargin(painter), vStep * i), gridRect.bottomRight() - QPointF(0, vStep * i));
  }
...
qreal AbstractSeriesChart::valuesVMargin(QPainter *painter) { return painter->fontMetrics().height(); }
...
void AbstractBarChart::paintChartLegend(QPainter *painter, QRectF legendRect) {
  ...
  painter->setRenderHint(QPainter::Antialiasing, false);
  if (m_chartItem->drawLegendBorder()) painter->drawRect(legendRect);
  painter->setRenderHint(QPainter::Antialiasing, true);
  ...
  if (!m_chartItem->series().isEmpty()) {
    ...
    foreach (SeriesItem *series, m_chartItem->series()) {
      ...
      painter->drawEllipse(indicatorsRect.adjusted(0, cw + indicatorSize / 2, -(indicatorsRect.width() - indicatorSize), -(indicatorsRect.height() - (cw + indicatorSize + indicatorSize / 2))));
...
QRectF AbstractBarChart::verticalLabelsRect(QPainter *painter, QRectF labelsRect) {
  ...
  foreach (QString label, m_chartItem->labels()) {
    if (painter->fontMetrics().horizontalAdvance(label) > maxWidth) maxWidth = painter->fontMetrics().horizontalAdvance(label);
  }
