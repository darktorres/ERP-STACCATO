...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_galpao` AS select `ehc`.`idConsumo` AS `idEstoque_idConsumo`,`ehc`.`descricao` AS `descricao`,`e`.`status` AS `status`,`vp2`.`idVenda` AS `idVenda`,`ehc`.`codComercial` AS `codComercial`,`n`.`numeroNFe` AS `numeroNFe`,`n`.`idNFe` AS `idNFe`,`ehc`.`caixas` AS `caixas`,`ehc`.`quant` AS `quant`,`ehc`.`idBloco` AS `idBloco`,`ehc`.`idVendaProduto2` AS `idVendaProduto2`,`e`.`lote` AS `lote`,'CLIENTE' AS `tipo` from (((`staccato`.`estoque_has_consumo` `ehc` left join `staccato`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) left join `staccato`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `staccato`.`venda_has_produto2` `vp2` on((`ehc`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) where (`vp2`.`status` in ('ESTOQUE','EM COLETA','EM RECEBIMENTO','ENTREGA AGEND.','EM ENTREGA')) union select `e`.`idEstoque` AS `idEstoque_idConsumo`,`e`.`descricao` AS `descricao`,`e`.`status` AS `status`,NULL AS `idVenda`,`e`.`codComercial` AS `codComercial`,`n`.`numeroNFe` AS `numeroNFe`,`n`.`idNFe` AS `idNFe`,(`e`.`restante` / `p`.`quantCaixa`) AS `caixas`,`e`.`restante` AS `quant`,`e`.`idBloco` AS `idBloco`,NULL AS `idVendaProduto2`,`e`.`lote` AS `lote`,'EST. LOJA' AS `tipo` from ((`staccato`.`estoque` `e` left join `staccato`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `staccato`.`produto` `p` on((`e`.`idProduto` = `p`.`idProduto`))) where (`e`.`restante` > 0);
