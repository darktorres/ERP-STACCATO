...
public:
  ...
  void printTree() {
    for (auto &row : m_rows) { std::cout << row.name << " " << row.info << std::endl; }
  }
};
...
int main(int argc, char *argv[]) {
  ...
  uint32_t version = 0;
  auto blocks_counter = fillTreesFromFile(filename.c_str(), serialized_blocks, serialized_descriptors, descriptors, blocks, threaded_trees, descriptorsNumberInFile, version, true, errorMessage);
  if (blocks_counter == 0) std::cout << "Can not read blocks from file " << filename.c_str() << "\nReason: " << errorMessage.str();
