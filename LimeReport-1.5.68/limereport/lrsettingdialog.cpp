...
SettingDialog::SettingDialog(QWidget *parent) : QDialog(parent), ui(new Ui::SettingDialog), m_settings(0) {
  ui->setupUi(this);
...
bool SettingDialog::suppressAbsentFieldsAndVarsWarnings() { return ui->cbSuppressWarnings->isChecked(); }

QLocale::Language SettingDialog::designerLanguage() {
  foreach (QLocale::Language language, m_aviableLanguages) {
    if (ui->designerLanguage->currentText().compare(QLocale::languageToString(language)) == 0) return language;
  }
...
void SettingDialog::setSuppressAbsentFieldsAndVarsWarnings(bool value) { ui->cbSuppressWarnings->setChecked(value); }
...
void SettingDialog::setDesignerLanguages(QList<QLocale::Language> languages, QLocale::Language currentLanguage) {
  ...
  foreach (QLocale::Language language, languages) {
    if (language != currentLanguage) ui->designerLanguage->addItem(QLocale::languageToString(language));
  }
#ifdef HAVE_QT4
