...
QObjectPropertyModel::QObjectPropertyModel(QObject *parent /*=0*/)
    : QAbstractItemModel(parent), m_rootNode(0), m_object(0), m_dataChanging(false), m_subclassesAsLevel(true), m_validator(0), m_translateProperties(true) {}
...
void QObjectPropertyModel::slotPropertyObjectNameChanged(const QString &oldName, const QString &newName) {
  Q_UNUSED(oldName)
  Q_UNUSED(newName)
  if (m_object) updateProperty("objectName");
}
...
void QObjectPropertyModel::updateProperty(const QString &propertyName) {
  if (!m_dataChanging && m_rootNode) {
    ...
    if (propItem) propItem->updatePropertyValue();
  }
}
...
QVariant QObjectPropertyModel::data(const QModelIndex &index, int role) const {
  ...
  case Qt::DecorationRole:
    ...
    if (index.column() == 1) {
      ...
      return QIcon();
  case Qt::UserRole: return QVariant::fromValue(node);
...
ObjectPropItem *QObjectPropertyModel::createPropertyItem(QMetaProperty prop, QObject *object, ObjectPropItem::ObjectsList *objects, ObjectPropItem *parent) {
  ...
  if (creator) {
    ...
    propertyItem = new ObjectPropItem(0, 0, QString(prop.name()), QString(tr(prop.name())), object->property(prop.name()), parent);
  }
  return propertyItem;
}
...
void QObjectPropertyModel::setTranslateProperties(bool translateProperties) { m_translateProperties = translateProperties; }
...
void QObjectPropertyModel::itemDataChanged(const QModelIndex &index) { emit dataChanged(index, index); }
