...
void SQLEditDialog::setDataSources(LimeReport::DataSourceManager *dataSources, QString datasourceName) {
  ...
  if (!datasourceName.isEmpty()) {
    ...
    if (dataSources->isQuery(datasourceName)) { initQueryMode(); }
...
void SQLEditDialog::setDefaultConnection(QString defaultConnection) { m_defaultConnection = ConnectionDesc::connectionNameForUser(defaultConnection); }
...
void SQLEditDialog::slotPreviewData() {
  ...
  m_previewModel = m_datasources->previewSQL(ConnectionDesc::connectionNameForReport(ui->cbbConnection->currentText()), ui->sqlText->toPlainText(), ui->leMaster->text());
  if (m_previewModel) {
    ...
    if (ui->gbDataPreview->isVisible()) hidePreview();
