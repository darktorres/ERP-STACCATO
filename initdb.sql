...
CREATE TABLE IF NOT EXISTS `mydb`.`fornecedor` (
  ...
  `tipo` INT(10) UNSIGNED NULL DEFAULT NULL,
  `representacao` TINYINT(1) NOT NULL DEFAULT '0',
  ...
  INDEX `fk_Fornecedor_Profissional1_idx` (`tipo` ASC),
...
CREATE TABLE IF NOT EXISTS `mydb`.`estoque_has_compra` (
  ...
  `idCompra` INT(11) UNSIGNED UNSIGNED NOT NULL,
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `lastUpdated` TIMESTAMP NULL DEFAULT NULL,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_orcamento` (`idLoja` INT, `idUsuario` INT, `Status` INT, `Dias restantes` INT, `Código` INT, `Vendedor` INT, `Indicou` INT, `Cliente` INT, `Profissional` INT, `Telefone` INT, `Celular` INT, `Tel Prof` INT, `Data` INT, `Total` INT, `Data Followup` INT, `Data Próx Followup` INT, `Observação` INT);
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_compras_financeiro` AS select group_concat(distinct `p`.`status` separator ',') AS `Status`,group_concat(distinct `p`.`statusFinanceiro` separator ',') AS `Status Financeiro`,`p`.`ordemCompra` AS `OC`,`p`.`idCompra` AS `Compra`,group_concat(distinct `p`.`idVenda` separator ', ') AS `Código`,group_concat(distinct `p`.`fornecedor` separator ',') AS `Fornecedor`,count(`p`.`idProduto`) AS `Produtos`,sum(`p`.`preco`) AS `Total` from `mydb`.`pedido_fornecedor_has_produto` `p` where (`p`.`idCompra` is not null) group by `p`.`idCompra`;
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_nfe_entrada` AS select `n`.`idNFe` AS `idNFe`,`n`.`cnpjDest` AS `CNPJ Dest`,group_concat(distinct `pf`.`fornecedor` separator ', ') AS `Fornecedor`,`n`.`numeroNFe` AS `NFe`,`n`.`status` AS `Status`,group_concat(distinct `e`.`recebidoPor` separator ',') AS `Recebido Por`,group_concat(distinct `pf`.`dataRealReceb` separator ',') AS `Data Receb`,`n`.`gare` AS `GARE`,`n`.`gareData` AS `GARE Pago Em`,group_concat(distinct `pf`.`ordemCompra` order by `pf`.`ordemCompra` ASC separator ', ') AS `OC`,group_concat(distinct `pf`.`idVenda` separator ', ') AS `Venda` from ((((`mydb`.`nfe` `n` left join `mydb`.`estoque_has_nfe` `ehn` on((`n`.`idNFe` = `ehn`.`idNFe`))) left join `mydb`.`estoque` `e` on((`ehn`.`idEstoque` = `e`.`idEstoque`))) left join `mydb`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `mydb`.`pedido_fornecedor_has_produto` `pf` on(((`ehc`.`idCompra` = `pf`.`idCompra`) and (`e`.`codComercial` = `pf`.`codComercial`)))) where (`n`.`tipo` = 'ENTRADA') group by `n`.`idNFe`;
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_orcamento` AS select `u`.`idLoja` AS `idLoja`,`u`.`idUsuario` AS `idUsuario`,`o`.`status` AS `Status`,(`o`.`validade` - (to_days(now()) - to_days(`o`.`data`))) AS `Dias restantes`,`o`.`idOrcamento` AS `Código`,`u`.`nome` AS `Vendedor`,`u2`.`nome` AS `Indicou`,`c`.`nome_razao` AS `Cliente`,`p`.`nome_razao` AS `Profissional`,`c`.`tel` AS `Telefone`,`c`.`telCel` AS `Celular`,if(isnull(`p`.`telCel`),if(isnull(`p`.`tel`),`p`.`telCom`,`p`.`tel`),`p`.`telCel`) AS `Tel Prof`,`o`.`data` AS `Data`,`o`.`total` AS `Total`,`ohf`.`dataFollowup` AS `Data Followup`,`ohf`.`dataProxFollowup` AS `Data Próx Followup`,`ohf`.`observacao` AS `Observação` from ((((((`mydb`.`orcamento` `o` left join `mydb`.`cliente` `c` on((`o`.`idCliente` = `c`.`idCliente`))) left join `mydb`.`usuario` `u` on((`o`.`idUsuario` = `u`.`idUsuario`))) left join `mydb`.`profissional` `p` on((`o`.`idProfissional` = `p`.`idProfissional`))) left join `mydb`.`usuario` `u2` on((`o`.`idUsuarioConsultor` = `u2`.`idUsuario`))) left join (select max(`mydb`.`orcamento_has_followup`.`idFollowup`) AS `idFollowup`,left(`mydb`.`orcamento_has_followup`.`idOrcamento`,12) AS `idOrcamento` from `mydb`.`orcamento_has_followup` group by left(`mydb`.`orcamento_has_followup`.`idOrcamento`,12)) `ohf2` on((left(`o`.`idOrcamento`,12) = left(`ohf2`.`idOrcamento`,12)))) left join `mydb`.`orcamento_has_followup` `ohf` on((`ohf2`.`idFollowup` = `ohf`.`idFollowup`)));
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_ordemcompra_nfe` AS select distinct `pf`.`ordemCompra` AS `ordemCompra`,`n`.`numeroNFe` AS `numeroNFe`,`n`.`idNFe` AS `idNFe` from (((`mydb`.`nfe` `n` left join `mydb`.`estoque_has_nfe` `ehn` on((`n`.`idNFe` = `ehn`.`idNFe`))) left join `mydb`.`estoque_has_compra` `ehc` on((`ehc`.`idEstoque` = `ehn`.`idEstoque`))) left join `mydb`.`pedido_fornecedor_has_produto` `pf` on((`pf`.`idCompra` = `ehc`.`idCompra`))) where (`pf`.`ordemCompra` is not null);
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_venda` AS select `u`.`idLoja` AS `idLoja`,`u`.`idUsuario` AS `idUsuario`,`v`.`statusFinanceiro` AS `statusFinanceiro`,`v`.`dataFinanceiro` AS `dataFinanceiro`,`v`.`status` AS `Status`,((to_days(`v`.`data`) + `v`.`prazoEntrega`) - to_days(now())) AS `Dias restantes`,`v`.`idVenda` AS `Código`,`u`.`nome` AS `Vendedor`,`u2`.`nome` AS `Indicou`,`c`.`nome_razao` AS `Cliente`,`p`.`nome_razao` AS `Profissional`,`v`.`data` AS `Data`,`v`.`total` AS `Total R$`,`vhf`.`dataFollowup` AS `Data Followup`,`vhf`.`observacao` AS `Observação` from (((((((`mydb`.`venda` `v` left join `mydb`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`))) left join `mydb`.`cliente_has_endereco` `e` on((`v`.`idEnderecoEntrega` = `e`.`idEndereco`))) left join `mydb`.`profissional` `p` on((`v`.`idProfissional` = `p`.`idProfissional`))) left join `mydb`.`cliente` `c` on((`v`.`idCliente` = `c`.`idCliente`))) left join `mydb`.`usuario` `u2` on((`v`.`idUsuarioConsultor` = `u2`.`idUsuario`))) left join (select max(`mydb`.`venda_has_followup`.`idFollowup`) AS `idFollowup`,left(`mydb`.`venda_has_followup`.`idVenda`,12) AS `idVenda` from `mydb`.`venda_has_followup` group by left(`mydb`.`venda_has_followup`.`idVenda`,12)) `vhf2` on((left(`vhf2`.`idVenda`,11) = left(`v`.`idVenda`,11)))) left join `mydb`.`venda_has_followup` `vhf` on((`vhf2`.`idFollowup` = `vhf`.`idFollowup`)));
