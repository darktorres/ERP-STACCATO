...
public:
  ...
  // ACCESSORS
  const decSingle *data() const { return &m_data; }

  // MODIFIERS
  decSingle *data() { return &m_data; }
  ...
  uint32_t radix() const { return decSingleRadix(&m_data); }

  const char *version() const { return decSingleVersion(); }
