...
ReportDesignWindow::ReportDesignWindow(ReportEnginePrivateInterface *report, QWidget *parent, QSettings *settings)
    : ReportDesignWindowInterface(parent), m_textAttibutesIsChanging(false), m_settings(settings), m_ownedSettings(false), m_progressDialog(0), m_showProgressDialog(true),
      m_editorTabType(ReportDesignWidget::Page), m_reportItemIsLocked(false) {
  ...
  if (!report->reportName().isEmpty()) windowTitle = report->reportName() + " - " + windowTitle;
...
void ReportDesignWindow::createActions() {
  ...
  connect(m_unlockSelectedItemsAction, SIGNAL(triggered()), this, SLOT(slotUnlockSelectedItems()));
...
void ReportDesignWindow::createToolBars() {
  ...
  m_pageTools << m_mainToolBar << m_reportToolBar << m_fontEditorBar << m_textAlignmentEditorBar << m_itemsAlignmentEditorBar << m_itemsBordersEditorBar;
}
...
void ReportDesignWindow::setDocWidgetsVisibility(bool visible) {
  if (!m_hideLeftPanel->isChecked()) hideDockWidgets(Qt::LeftDockWidgetArea, !visible);
  if (!m_hideRightPanel->isChecked()) hideDockWidgets(Qt::RightDockWidgetArea, !visible);
}
...
void ReportDesignWindow::restoreSetting() {
  ...
  if (v.isValid()) { m_editorsStates[ReportDesignWidget::Dialog] = v.toByteArray(); }
  v = settings()->value("ScriptEditorsState");
  if (v.isValid()) { m_editorsStates[ReportDesignWidget::Script] = v.toByteArray(); }

  v = settings()->value("TranslationEditorsState");
  if (v.isValid()) { m_editorsStates[ReportDesignWidget::Translations] = v.toByteArray(); }
  ...
  if (v.isValid()) { m_objectInspector->setTranslateProperties(v.toBool()); }
  ...
  for (int i = 0; i < filesCount; i++) {
    if (keys.contains("fileName" + QString::number(i))) {
      m_recentFiles[settings()->value("fileName" + QString::number(i)).toString()] = settings()->value("fileData" + QString::number(i)).toDateTime();
...
void ReportDesignWindow::setSettings(QSettings *value) {
  if (m_ownedSettings) delete m_settings;
  m_settings = value;
...
void ReportDesignWindow::slotItemSelected(LimeReport::BaseDesignIntf *item) {
  if (m_objectInspector->object() != item) {
    ...
    if (m_objectInspector->subclassesAsLevel()) m_objectInspector->expandToDepth(0);
...
void ReportDesignWindow::slotItemPropertyChanged(const QString &objectName, const QString &propertyName, const QVariant &oldValue, const QVariant &newValue) {
  ...
  if (m_objectInspector->object() && (m_objectInspector->object()->objectName() == objectName)) { m_objectInspector->updateProperty(propertyName); }
}
...
void ReportDesignWindow::slotItemInserted(PageDesignIntf *, QPointF, const QString &ItemType) {
  if (!m_reportItemIsLocked) {
    m_editModeAction->setChecked(true);
    if (m_actionMap.value(ItemType)) m_actionMap.value(ItemType)->setCheckable(false);
  } else {
...
void ReportDesignWindow::slotLoadReport() {
  ...
  QString fileName = QFileDialog::getOpenFileName(this, tr("Report file name"), m_reportDesignWidget->report()->currentReportsDir(), "Report files(*.lrxml);; All files(*)");
  if (!fileName.isEmpty()) {
...
void ReportDesignWindow::slotEditLayoutMode() { m_reportDesignWidget->editLayoutMode(m_editLayoutMode->isChecked()); }
...
void ReportDesignWindow::showDefaultToolBars() {
  foreach (QToolBar *tb, m_pageTools) { tb->setVisible(m_editorTabType != ReportDesignWidget::Dialog); }
...
void ReportDesignWindow::showDefaultEditors() {
  foreach (QDockWidget *w, m_pageEditors) { w->setVisible(m_editorTabType != ReportDesignWidget::Dialog); }
#ifdef HAVE_QTDESIGNER_INTEGRATION
  foreach (QDockWidget *w, m_dialogEditors) { w->setVisible(m_editorTabType == ReportDesignWidget::Dialog); }
  for (int i = 0; i < m_docksToTabify.size() - 1; ++i) { tabifyDockWidget(m_docksToTabify.at(i), m_docksToTabify.at(i + 1)); }
...
void ReportDesignWindow::renderPageFinished(int renderedPageCount) { m_progressLabel->setText(tr("Rendered %1 pages").arg(renderedPageCount)); }
...
void ReportDesignWindow::hideDockWidgets(Qt::DockWidgetArea area, bool value) {
  ...
  case Qt::LeftDockWidgetArea:
    if (value) m_leftDocVisibleState.clear();
    currentDocState = &m_leftDocVisibleState;
    break;
  case Qt::RightDockWidgetArea:
    if (value) m_rightDocVisibleState.clear();
...
void ReportDesignWindow::slotHideLeftPanel(bool value) { hideDockWidgets(Qt::LeftDockWidgetArea, !value); }

void ReportDesignWindow::slotHideRightPanel(bool value) { hideDockWidgets(Qt::RightDockWidgetArea, !value); }
...
void ReportDesignWindow::slotPageAdded(PageDesignIntf *) { m_deletePageAction->setEnabled(m_reportDesignWidget->report()->pageCount() > 1); }

void ReportDesignWindow::slotPageDeleted() { m_deletePageAction->setEnabled(m_reportDesignWidget->report()->pageCount() > 1); }

void ReportDesignWindow::slotFilterTextChanged(const QString &filter) { m_filterModel->setFilterRegExp(QRegExp(filter, Qt::CaseInsensitive, QRegExp::FixedString)); }

#ifdef HAVE_QTDESIGNER_INTEGRATION
void ReportDesignWindow::slotDeleteDialog() {
  if (m_editorTabType == ReportDesignWidget::Dialog) { m_reportDesignWidget->report()->scriptContext()->deleteDialog(m_reportDesignWidget->activeDialogName()); }
}
...
void ReportDesignWindow::slotCancelRendering(bool) { m_reportDesignWidget->report()->cancelRender(); }
