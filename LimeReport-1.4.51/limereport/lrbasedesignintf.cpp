...
QRectF BaseDesignIntf::geometry() const { return QRectF(pos().x(), pos().y(), width(), height()); }
...
qreal BaseDesignIntf::getItemWidth() { return width() / mmFactor(); }

qreal BaseDesignIntf::getItemHeight() { return height() / mmFactor(); }

qreal BaseDesignIntf::getItemPosX() { return x() / mmFactor(); }

qreal BaseDesignIntf::getItemPosY() { return y() / mmFactor(); }
...
QSizeF BaseDesignIntf::sizeMM() const { return QSizeF(width() / m_mmFactor, height() / m_mmFactor); }

qreal BaseDesignIntf::widthMM() const { return width() / m_mmFactor; }

qreal BaseDesignIntf::heightMM() const { return height() / m_mmFactor; }

void BaseDesignIntf::setMMFactor(qreal mmFactor) { m_mmFactor = mmFactor; }

qreal BaseDesignIntf::mmFactor() const { return m_mmFactor; }

QPointF BaseDesignIntf::posMM() const { return QPointF(pos().x() / m_mmFactor, pos().y() / m_mmFactor); }
...
void BaseDesignIntf::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if ((m_resizeDirectionFlags == Fixed) && !m_fixedPos) {
    ...
    if (!isBand() && scene()->selectedItems().count() > 1) moveSelectedItems(tmpPos - pos());
    if (scene()->selectedItems().count() == 1 && (page()->magneticMovement())) page()->itemMoved(this);
  }
}
...
void BaseDesignIntf::setZValueProperty(qreal value) {
  if (not qFuzzyCompare(zValue(), value)) {
...
void BaseDesignIntf::setGeometryProperty(QRectF rect) {
  if (rect != geometry()) {
    ...
    if (not qFuzzyCompare(rect.x(), geometry().x()) or (not qFuzzyCompare(rect.y(), geometry().y()))) setPos(rect.x(), rect.y());
    if (not qFuzzyCompare(rect.width(), geometry().width())) setWidth(rect.width());
    if (not qFuzzyCompare(rect.height(), geometry().height())) setHeight(rect.height());
...
void BaseDesignIntf::contextMenuEvent(QGraphicsSceneContextMenuEvent *event) {
  ...
  if (a) {
    ...
    if (a == brinToTopAction) page->bringToFront();
...
QRectF Marker::rect() const { return m_rect; }

QColor Marker::color() const { return m_color; }

BaseDesignIntf *Marker::object() const { return m_object; }
