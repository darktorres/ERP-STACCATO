...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_compras` (`OC` INT, `Compra` INT, `Fornecedor` INT, `Vendedor` INT, `Data` INT, `Produtos` INT, `Total` INT, `dataPrevConf` INT, `Venda` INT);
...
VIEW `mydb`.`view_calendario_entrega` AS
    ...
    WHERE
        ((`vhp`.`status` = 'ENTREGA AGEND.')
            OR (`vhp`.`status` = 'EM ENTREGA'))
    GROUP BY CAST(`vhp`.`data` AS DATE) , `thv`.`idVeiculo`
    ORDER BY `vhp`.`data`;
