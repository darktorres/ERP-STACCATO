...
LimeReport::BaseDesignIntf *createBarcodeItem(QObject *owner, LimeReport::BaseDesignIntf *parent) { return new LimeReport::BarcodeItem(owner, parent); }
...
BarcodeItem::BarcodeItem(QObject *owner, QGraphicsItem *parent)
    : ContentItemDesignIntf(xmlTag, owner, parent), m_designTestValue("1"), m_barcodeType(CODE128), m_foregroundColor(Qt::black), m_backgroundColor(Qt::white), m_whitespace(10), m_angle(Angle0),
      m_barcodeWidth(0), m_securityLevel(0), m_pdf417CodeWords(928), m_inputMode(UNICODE_INPUT_MODE), m_hideText(false), m_option3(0), m_hideIfEmpty(false) {}

BarcodeItem::~BarcodeItem() {}

BaseDesignIntf *BarcodeItem::createSameTypeItem(QObject *owner, QGraphicsItem *parent) { return new BarcodeItem(owner, parent); }
  ...
  if (content().isEmpty()) {
    if (!m_datasource.isEmpty() && !m_field.isEmpty()) {
      ...
      if (ds) {
        ...
        if (data.isValid()) {
          switch (pass) {
          case FirstPass: expandContent(data.toString(), dataManager, pass); break;
          default:;
          }
        }
      }
    ...
    switch (pass) {
    case FirstPass: expandContent(content(), dataManager, pass); break;
    default:;
    }
  }
...
bool BarcodeItem::isNeedUpdateSize(RenderPass pass) const { return (pass == FirstPass) ? true : false; }
