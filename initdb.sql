...
CREATE TABLE IF NOT EXISTS `staccato`.`produto` (
  ...
  FULLTEXT INDEX `index7` (`fornecedor`) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `staccato`.`estoque_has_consumo` (
  ...
  INDEX `estoqueconsumo_idVendaProduto_idx` (`idVendaProduto2` ASC) VISIBLE,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_galpao` (`id` INT, `descricao` INT, `status` INT, `caixas` INT, `quant` INT, `bloco` INT, `idVendaProduto2` INT, `tipo` INT);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_fornecedor_logistica_agendar_coleta` AS select `subquery`.`fornecedor` AS `Fornecedor`,sum((`subquery`.`idVenda` is null)) AS `Estoque`,sum((`subquery`.`idVenda` is not null)) AS `Venda` from (select group_concat(distinct `pf`.`fornecedor` separator ',') AS `fornecedor`,`e`.`status` AS `status`,group_concat(distinct `pf`.`idVenda` separator ',') AS `idVenda` from ((`staccato`.`estoque` `e` left join `staccato`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `staccato`.`pedido_fornecedor_has_produto2` `pf` on((`ehc`.`idPedido2` = `pf`.`idPedido2`))) where ((`e`.`status` = 'EM COLETA') and (`pf`.`dataPrevColeta` is null) and (`pf`.`idPedido2` is not null)) group by `e`.`idEstoque`) `subquery` group by `subquery`.`fornecedor`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_galpao` AS select `ehc`.`idConsumo` AS `id`,`ehc`.`descricao` AS `descricao`,`vp2`.`status` AS `status`,`ehc`.`caixas` AS `caixas`,(`ehc`.`quant` * -(1)) AS `quant`,`ehc`.`bloco` AS `bloco`,`ehc`.`idVendaProduto2` AS `idVendaProduto2`,'CONSUMO' AS `tipo` from ((`staccato`.`estoque_has_consumo` `ehc` left join `staccato`.`venda_has_produto2` `vp2` on((`ehc`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) left join `staccato`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) where (`vp2`.`status` in ('ESTOQUE','ENTREGA AGEND.','EM ENTREGA')) union select `staccato`.`view_estoque_contabil`.`idEstoque` AS `idEstoque`,`staccato`.`view_estoque_contabil`.`descricao` AS `descricao`,`staccato`.`view_estoque_contabil`.`status` AS `status`,`staccato`.`view_estoque_contabil`.`Caixas contabil` AS `Caixas contabil`,`staccato`.`view_estoque_contabil`.`contabil` AS `contabil`,`staccato`.`view_estoque_contabil`.`bloco` AS `bloco`,NULL AS `idVendaProduto2`,'ESTOQUE' AS `ESTOQUE` from `staccato`.`view_estoque_contabil` where (`staccato`.`view_estoque_contabil`.`contabil` > 0);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_orcamento` AS select `o`.`idLoja` AS `idLoja`,`o`.`idUsuario` AS `idUsuario`,`o`.`idUsuarioConsultor` AS `idUsuarioConsultor`,`o`.`status` AS `Status`,(`o`.`validade` - (to_days(now()) - to_days(`o`.`data`))) AS `Dias restantes`,`o`.`idOrcamento` AS `Código`,`u`.`nome` AS `Vendedor`,`u2`.`nome` AS `Consultor`,`c`.`nome_razao` AS `Cliente`,`p`.`nome_razao` AS `Profissional`,`c`.`tel` AS `Telefone`,`c`.`telCel` AS `Celular`,if((`p`.`telCel` is null),if((`p`.`tel` is null),`p`.`telCom`,`p`.`tel`),`p`.`telCel`) AS `Tel Prof`,`o`.`data` AS `Data`,`o`.`total` AS `Total`,`ohf`.`dataFollowup` AS `Data Followup`,`ohf`.`dataProxFollowup` AS `Data Próx Followup`,`ohf`.`observacao` AS `Observação`,`ohf`.`semaforo` AS `semaforo` from ((((((`staccato`.`orcamento` `o` left join `staccato`.`cliente` `c` on((`o`.`idCliente` = `c`.`idCliente`))) left join `staccato`.`usuario` `u` on((`o`.`idUsuario` = `u`.`idUsuario`))) left join `staccato`.`profissional` `p` on((`o`.`idProfissional` = `p`.`idProfissional`))) left join `staccato`.`usuario` `u2` on((`o`.`idUsuarioConsultor` = `u2`.`idUsuario`))) left join (select max(`tmp`.`idFollowup`) AS `idFollowup`,`tmp`.`idOrcamentoBase` AS `idOrcamentoBase` from `staccato`.`orcamento_has_followup` `tmp` group by `tmp`.`idOrcamentoBase`) `ohf2` on((`o`.`idOrcamentoBase` = `ohf2`.`idOrcamentoBase`))) left join `staccato`.`orcamento_has_followup` `ohf` on((`ohf2`.`idFollowup` = `ohf`.`idFollowup`)));
