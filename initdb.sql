...
CREATE TABLE IF NOT EXISTS `staccato`.`estoque` (
  ...
  `restante` DECIMAL(15,4) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `staccato`.`estoque_has_consumo` (
  ...
  `idPedido2` INT(11) NULL DEFAULT NULL,
  `status` VARCHAR(45) NULL DEFAULT 'TEMP',
  `local` VARCHAR(100) NULL DEFAULT 'TEMP',
  `idProduto` INT(11) UNSIGNED NULL DEFAULT NULL,
  `fornecedor` VARCHAR(100) NULL DEFAULT NULL,
...
-- -----------------------------------------------------
-- Table `staccato`.`forma_pagamento_has_loja`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`forma_pagamento_has_loja` (
...
-- -----------------------------------------------------
-- Table `staccato`.`gare_aliquotas`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`gare_aliquotas` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `ncm` VARCHAR(45) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `staccato`.`nfe` (
  ...
  `idVenda` VARCHAR(30) NULL DEFAULT NULL,
  `ordemCompra` VARCHAR(45) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `staccato`.`venda_has_produto2` (
  ...
  INDEX `fk_vendaproduto2_nfe_idx` (`idNFeSaida` ASC, `idNFeFutura` ASC),
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_a_pagar_vencer_base` (`Data Pagamento` INT, `CONFERIDO` INT, `PENDENTE` INT, `Total` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_a_pagar_vencidos_base`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_a_pagar_vencidos_base` (`Data Pagamento` INT, `CONFERIDO` INT, `PENDENTE` INT, `Total` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_consistencia_compra` (`descricao` INT, `prcUnitario` INT, `preco` INT, `prcUnitario * quant` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_entrega_produtos` (`idVendaProduto2` INT, `idProduto` INT, `dataPrevEnt` INT, `dataRealEnt` INT, `status` INT, `fornecedor` INT, `idVenda` INT, `produto` INT, `caixas` INT, `quant` INT, `un` INT, `unCaixa` INT, `codComercial` INT, `formComercial` INT, `idConsumo` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_estoque_consumo`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_estoque_consumo` (`Venda` INT, `idEstoque` INT, `statusProduto` INT, `status` INT, `local` INT, `fornecedor` INT, `descricao` INT, `quant` INT, `quantUpd` INT, `un` INT, `caixas` INT, `codComercial` INT, `dataRealEnt` INT, `created` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_estoque_contabil`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_estoque_contabil` (`cnpjDest` INT, `status` INT, `idEstoque` INT, `fornecedor` INT, `descricao` INT, `contabil` INT, `restante` INT, `unEst` INT, `Caixas` INT, `lote` INT, `local` INT, `bloco` INT, `codComercial` INT, `nfe` INT, `dataPrevColeta` INT, `dataRealColeta` INT, `dataPrevReceb` INT, `dataRealReceb` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_fluxo_caixa_realizado` (`Compra/Venda` INT, `dataRealizado` INT, `R$` INT, `Tipo` INT, `contaDestino` INT, `idConta` INT, `Status` INT, `ENT/SAI` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_nfe_entrada` (`idNFe` INT, `CNPJ Dest` INT, `Fornecedor` INT, `NFe` INT, `Status` INT, `Recebido Por` INT, `Data Receb` INT, `GARE` INT, `GARE Pago Em` INT, `OC` INT, `Venda` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_ordemcompra` (`idVenda` INT, `idVendaProduto1` INT, `status` INT, `fornecedor` INT, `produto` INT, `codComercial` INT, `quant` INT, `un` INT, `caixas` INT, `formComercial` INT, `obs` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_pagamento_loja` (`idLoja` INT, `idPagamento` INT, `pagamento` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_produto_estoque` (`idVendaProduto2` INT, `idProduto` INT, `fornecedor` INT, `produto` INT, `descUnitario` INT, `caixas` INT, `quant` INT, `un` INT, `unCaixa` INT, `codComercial` INT, `formComercial` INT, `total` INT, `codBarras` INT, `ncm` INT, `cfop` INT, `unTrib` INT, `numeroPedido` INT, `itemPedido` INT, `tipoICMS` INT, `orig` INT, `cstICMS` INT, `modBC` INT, `vBC` INT, `pICMS` INT, `vICMS` INT, `modBCST` INT, `pMVAST` INT, `vBCST` INT, `pICMSST` INT, `vICMSST` INT, `cEnq` INT, `cstIPI` INT, `cstPIS` INT, `vBCPIS` INT, `pPIS` INT, `vPIS` INT, `cstCOFINS` INT, `vBCCOFINS` INT, `pCOFINS` INT, `vCOFINS` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_produto_pendente`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_produto_pendente` (`status` INT, `idVendaProduto2` INT, `idVendaProdutoFK` INT, `fornecedor` INT, `idVenda` INT, `idProduto` INT, `produto` INT, `obs` INT, `colecao` INT, `formComercial` INT, `quant` INT, `un` INT, `Un2` INT, `Caixas` INT, `kgcx` INT, `codComercial` INT, `codBarras` INT, `custo` INT, `idCompra` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_recebimento` (`prazoEntrega` INT, `dataPrevReceb` INT, `idEstoque` INT, `lote` INT, `local` INT, `bloco` INT, `fornecedor` INT, `numeroNFe` INT, `idVenda` INT, `ordemCompra` INT, `produto` INT, `codComercial` INT, `quant` INT, `un` INT, `caixas` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_relatorio`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_relatorio` (`Loja` INT, `Vendedor` INT, `idUsuario` INT, `idVenda` INT, `Mês` INT, `Faturamento` INT, `Comissão` INT, `%` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_relatorio_vendedor` (`Loja` INT, `Vendedor` INT, `idUsuario` INT, `Faturamento` INT, `Comissão` INT, `%` INT, `Mês` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_venda` (`idLoja` INT, `idUsuario` INT, `idUsuarioConsultor` INT, `statusFinanceiro` INT, `dataFinanceiro` INT, `Status` INT, `Dias restantes` INT, `Código` INT, `OC Rep` INT, `Vendedor` INT, `Consultor` INT, `Cliente` INT, `Profissional` INT, `Data` INT, `Total R$` INT, `Data Followup` INT, `Observação` INT);
...
INSERT INTO venda_has_produto2 (idVendaProdutoFK, idRelacionado, selecionado, entregou, recebeu, status, statusOriginal, idCompra, idNFeSaida, idNFeFutura, fornecedor, idVenda, idLoja, 
            ...
            mostrarDesconto, 0, 0, reposicaoEntrega, reposicaoReceb, dataPrevCompra, dataRealCompra, dataPrevConf, dataRealConf, dataPrevFat, dataRealFat, dataPrevColeta, 
...
-- -----------------------------------------------------
-- procedure copy_pedido_fornecedor_has_produto
-- -----------------------------------------------------

DELIMITER $$
USE `staccato`$$
CREATE DEFINER=`root`@`%` PROCEDURE `copy_pedido_fornecedor_has_produto`(currentIdPedido INT(11))
BEGIN
INSERT INTO pedido_fornecedor_has_produto2 (idPedidoFK, selecionado, aliquotaSt, st, status, statusFinanceiro, ordemCompra, ordemRepresentacao, idVenda, idVendaProduto2, 
                         ...
                         dataRealEnt) select idPedido1, selecionado, aliquotaSt, st, status, statusFinanceiro, ordemCompra, ordemRepresentacao, idVenda, (select idVendaProduto2 from venda_has_produto2 where idVendaProdutoFK IN (select idVendaProduto1 from pedido_fornecedor_has_produto where idPedido1 = currentIdPedido)), idCompra, fornecedor, 
...
BEGIN
	INSERT INTO comissao (idVendaProduto1, `Loja`, `Vendedor`, `idUsuario`, `idVenda`, `Mês`, `Faturamento`, `Comissão`, `%`) SELECT 
    vp.idVendaProduto1,
    `l`.`descricao` AS `Loja`,
    `u`.`nome` AS `Vendedor`,
...
SET 
    descontinuado = TRUE
WHERE
    descontinuado = FALSE AND estoque = FALSE AND validade < CURDATE();
...
BEGIN
	...
	UPDATE produto p 
	SET estoqueRestante = @restante, descontinuado = (@restante <= 0)
	WHERE p.idEstoque = currentId;
END$$
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`ViewDespesas` AS select date_format(`develop`.`conta_a_pagar_has_pagamento`.`dataEmissao`,'%Y/%m') AS `mes`,`develop`.`loja`.`nomeFantasia` AS `nomeFantasia`,'Venda' AS `representacao`,'' AS `sem custo`,(sum(if((`develop`.`conta_a_pagar_has_pagamento`.`status` = 'PAGO'),`develop`.`conta_a_pagar_has_pagamento`.`valorReal`,`develop`.`conta_a_pagar_has_pagamento`.`valor`)) + 0E0) AS `DespTotal`,sum(if((`develop`.`conta_a_pagar_has_pagamento`.`grupo` like 'RH%'),if((`develop`.`conta_a_pagar_has_pagamento`.`status` = 'PAGO'),`develop`.`conta_a_pagar_has_pagamento`.`valorReal`,`develop`.`conta_a_pagar_has_pagamento`.`valor`),0E0)) AS `DespRH`,sum(if((`develop`.`conta_a_pagar_has_pagamento`.`grupo` = 'RT\'s'),if((`develop`.`conta_a_pagar_has_pagamento`.`status` = 'PAGO'),`develop`.`conta_a_pagar_has_pagamento`.`valorReal`,`develop`.`conta_a_pagar_has_pagamento`.`valor`),0E0)) AS `DespRT`,sum(if((`develop`.`conta_a_pagar_has_pagamento`.`grupo` = 'Impostos - ICMS;ST;ISS'),if((`develop`.`conta_a_pagar_has_pagamento`.`status` = 'PAGO'),`develop`.`conta_a_pagar_has_pagamento`.`valorReal`,`develop`.`conta_a_pagar_has_pagamento`.`valor`),0E0)) AS `DespST`,sum(if((`develop`.`conta_a_pagar_has_pagamento`.`grupo` = 'Impostos - Pis;Cofins'),if((`develop`.`conta_a_pagar_has_pagamento`.`status` = 'PAGO'),`develop`.`conta_a_pagar_has_pagamento`.`valorReal`,`develop`.`conta_a_pagar_has_pagamento`.`valor`),0E0)) AS `DespPis`,sum(if((`develop`.`conta_a_pagar_has_pagamento`.`grupo` = 'Impostos - IR;CSLL'),if((`develop`.`conta_a_pagar_has_pagamento`.`status` = 'PAGO'),`develop`.`conta_a_pagar_has_pagamento`.`valorReal`,`develop`.`conta_a_pagar_has_pagamento`.`valor`),0E0)) AS `DespIR`,sum(if((`develop`.`conta_a_pagar_has_pagamento`.`grupo` like 'Logística%'),if((`develop`.`conta_a_pagar_has_pagamento`.`status` = 'PAGO'),`develop`.`conta_a_pagar_has_pagamento`.`valorReal`,`develop`.`conta_a_pagar_has_pagamento`.`valor`),0E0)) AS `DespLog`,sum(if((`develop`.`conta_a_pagar_has_pagamento`.`grupo` = 'Produtos - Venda'),if((`develop`.`conta_a_pagar_has_pagamento`.`status` = 'PAGO'),`develop`.`conta_a_pagar_has_pagamento`.`valorReal`,`develop`.`conta_a_pagar_has_pagamento`.`valor`),0E0)) AS `DespCMV`,sum(if((`develop`.`conta_a_pagar_has_pagamento`.`grupo` = 'Aluguel'),if((`develop`.`conta_a_pagar_has_pagamento`.`status` = 'PAGO'),`develop`.`conta_a_pagar_has_pagamento`.`valorReal`,`develop`.`conta_a_pagar_has_pagamento`.`valor`),0E0)) AS `DespALU`,sum(if(((not((`develop`.`conta_a_pagar_has_pagamento`.`grupo` like 'Impostos - %'))) and (not((`develop`.`conta_a_pagar_has_pagamento`.`grupo` like 'Logística%'))) and (not((`develop`.`conta_a_pagar_has_pagamento`.`grupo` like 'RH%'))) and (`develop`.`conta_a_pagar_has_pagamento`.`grupo` <> 'RT\'s') and (`develop`.`conta_a_pagar_has_pagamento`.`grupo` <> 'Produtos - Venda') and (`develop`.`conta_a_pagar_has_pagamento`.`grupo` <> 'Aluguel')),if((`develop`.`conta_a_pagar_has_pagamento`.`status` = 'PAGO'),`develop`.`conta_a_pagar_has_pagamento`.`valorReal`,`develop`.`conta_a_pagar_has_pagamento`.`valor`),0E0)) AS `DespOutros` from (`develop`.`conta_a_pagar_has_pagamento` left join `develop`.`loja` on((`develop`.`loja`.`idLoja` = `develop`.`conta_a_pagar_has_pagamento`.`centroCusto`))) where ((`develop`.`conta_a_pagar_has_pagamento`.`grupo` <> 'Transferência') and (`develop`.`conta_a_pagar_has_pagamento`.`status` <> 'CANCELADO') and (`develop`.`conta_a_pagar_has_pagamento`.`desativado` = FALSE) and (`develop`.`conta_a_pagar_has_pagamento`.`status` <> 'SUBSTITUIDO')) group by `mes`,`develop`.`loja`.`nomeFantasia`,`representacao`,`sem custo`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`ViewDespesasExcel` AS select date_format(`cp`.`dataEmissao`,'%Y/%m') AS `mes`,`cp`.`status` AS `status`,sum(if((`cp`.`status` = 'pago'),`cp`.`valorReal`,(`cp`.`valor` + 0E0))) AS `valor`,`develop`.`loja`.`descricao` AS `centroCusto`,`cp`.`grupo` AS `grupo` from (`develop`.`conta_a_pagar_has_pagamento` `cp` left join `develop`.`loja` on((`develop`.`loja`.`idLoja` = `cp`.`centroCusto`))) where ((`cp`.`desativado` = 0) and (`cp`.`dataEmissao` > '2016-12-31') and (`cp`.`status` <> 'cancelado') and (`cp`.`status` <> 'substituido')) group by date_format(`cp`.`dataEmissao`,'%Y/%m'),`cp`.`status`,`cp`.`centroCusto`,`cp`.`grupo`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`aaa` AS select `n`.`idNFe` AS `idNFe`,`n`.`numeroNFe` AS `numeroNFe`,sum(if((`e`.`tipoICMS` = 'ICMS00'),`e`.`valor`,0)) AS `valorSujeitoSt` from (`develop`.`estoque` `e` left join `develop`.`nfe` `n` on((`n`.`idNFe` = `e`.`idNFe`))) where (`e`.`idNFe` is not null) group by `n`.`idNFe` having (`valorSujeitoSt` > 0);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`bbb` AS select `aaa`.`idNFe` AS `idNFe`,(`cp`.`valorReal` / `aaa`.`valorSujeitoSt`) AS `percst` from (`develop`.`aaa` left join `develop`.`conta_a_pagar_has_pagamento` `cp` on((`cp`.`nfe` = `aaa`.`numeroNFe`))) having (`percst` > 0);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`viewEstoqueConsumo` AS select `ehc`.`idVendaProduto2` AS `idVendaProduto2`,group_concat(distinct `ehc`.`tipoICMS` separator ',') AS `tipoICMS`,group_concat(distinct `ehc`.`vICMS` separator ',') AS `vICMS`,sum(`ehc`.`vICMSST`) AS `vICMSST`,sum(`ehc`.`quant`) AS `quant`,sum(`ehc`.`valor`) AS `valor` from `develop`.`estoque_has_consumo` `ehc` where (`ehc`.`idVendaProduto2` is not null) group by `ehc`.`idVendaProduto2`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`viewEstoqueConsumo2` AS select `ehc`.`idVendaProduto2` AS `idVendaProduto2`,`e`.`tipoICMS` AS `tipoICMS`,(`e`.`vICMS` * ((`ehc`.`quant` * -(1)) / `e`.`quant`)) AS `vICMS`,(`e`.`vICMSST` * ((`ehc`.`quant` * -(1)) / `e`.`quant`)) AS `vICMSST`,((`ehc`.`quant` * -(1)) / `e`.`quant`) AS `quant`,(`e`.`valor` * ((`ehc`.`quant` * -(1)) / `e`.`quant`)) AS `valor` from (`develop`.`estoque_has_consumo` `ehc` left join `develop`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) where (`ehc`.`idVendaProduto2` is not null) group by `ehc`.`idVendaProduto2`,`ehc`.`idEstoque`,`ehc`.`quant`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`viewFreteExcel` AS select date_format(`v`.`data`,'%Y/%m') AS `mes`,`v`.`status` AS `status`,sum(`v`.`frete`) AS `frete`,`develop`.`loja`.`descricao` AS `Loja` from (`develop`.`venda` `v` left join `develop`.`loja` on((`develop`.`loja`.`idLoja` = `v`.`idLoja`))) where ((`v`.`status` <> 'cancelado') and (`v`.`data` > '2016-12-31')) group by date_format(`v`.`data`,'%Y/%m'),`v`.`status`,`develop`.`loja`.`descricao`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`viewPedidoFornecedor` AS select `pf2`.`aliquotaSt` AS `aliquotaSt`,`pf2`.`st` AS `st`,`pf2`.`status` AS `status`,`pf2`.`idVendaProduto2` AS `idVendaProduto2`,`pf2`.`quant` AS `quant`,`pf2`.`preco` AS `preco` from `develop`.`pedido_fornecedor_has_produto2` `pf2` where (`pf2`.`status` <> 'cancelado');
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`viewRelatoriosGestao` AS select date_format(`develop`.`venda`.`data`,'%Y/%m') AS `Mes`,`develop`.`usuario`.`user` AS `Vendedor`,`develop`.`profissional`.`nome_razao` AS `Profissional`,`develop`.`loja`.`descricao` AS `Loja`,`develop`.`venda_has_produto`.`fornecedor` AS `fornecedor`,(sum(`develop`.`venda_has_produto`.`total`) + 0) AS `Total`,`develop`.`venda_has_produto`.`status` AS `status` from ((((`develop`.`venda_has_produto` left join `develop`.`venda` on((`develop`.`venda_has_produto`.`idVenda` = `develop`.`venda`.`idVenda`))) left join `develop`.`profissional` on((`develop`.`venda`.`idProfissional` = `develop`.`profissional`.`idProfissional`))) left join `develop`.`usuario` on((`develop`.`venda`.`idUsuario` = `develop`.`usuario`.`idUsuario`))) left join `develop`.`loja` on((`develop`.`venda_has_produto`.`idLoja` = `develop`.`loja`.`idLoja`))) group by date_format(`develop`.`venda`.`data`,'%Y/%m'),`develop`.`usuario`.`user`,`develop`.`loja`.`descricao`,`develop`.`profissional`.`nome_razao`,`develop`.`venda_has_produto`.`fornecedor`,`develop`.`venda_has_produto`.`status` having (`develop`.`venda_has_produto`.`status` <> 'cancelado');
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`viewResultadoCD` AS select `v`.`mes` AS `mes`,`v`.`frete` AS `frete`,`c`.`VALOR` AS `valor`,`c`.`DespLogistica` AS `DespLogistica`,`c`.`DespRH` AS `DespRH`,`c`.`DespOutras` AS `DespOutras` from (((select date_format(`develop`.`venda`.`data`,'%Y/%m') AS `mes`,sum(`develop`.`venda`.`frete`) AS `frete` from `develop`.`venda` where (`develop`.`venda`.`status` <> 'cancelado') group by date_format(`develop`.`venda`.`data`,'%Y/%m'))) `v` left join (select date_format(`develop`.`conta_a_pagar_has_pagamento`.`dataEmissao`,'%Y/%m') AS `mes`,sum(if((`develop`.`conta_a_pagar_has_pagamento`.`status` = 'PAGO'),`develop`.`conta_a_pagar_has_pagamento`.`valorReal`,`develop`.`conta_a_pagar_has_pagamento`.`valor`)) AS `VALOR`,sum(if((left(`develop`.`conta_a_pagar_has_pagamento`.`grupo`,9) = 'Logística'),`develop`.`conta_a_pagar_has_pagamento`.`valor`,0)) AS `DespLogistica`,sum(if((left(`develop`.`conta_a_pagar_has_pagamento`.`grupo`,2) = 'RH'),`develop`.`conta_a_pagar_has_pagamento`.`valor`,0)) AS `DespRH`,sum(if(((left(`develop`.`conta_a_pagar_has_pagamento`.`grupo`,9) = 'Logística') or if((left(`develop`.`conta_a_pagar_has_pagamento`.`grupo`,2) = 'RH'),`develop`.`conta_a_pagar_has_pagamento`.`valor`,0)),0,`develop`.`conta_a_pagar_has_pagamento`.`valor`)) AS `DespOutras` from `develop`.`conta_a_pagar_has_pagamento` where ((`develop`.`conta_a_pagar_has_pagamento`.`centroCusto` = 6) and (`develop`.`conta_a_pagar_has_pagamento`.`status` <> 'CANCELADO') and (`develop`.`conta_a_pagar_has_pagamento`.`status` <> 'SUBSTITUIDO') and (`develop`.`conta_a_pagar_has_pagamento`.`desativado` = FALSE)) group by date_format(`develop`.`conta_a_pagar_has_pagamento`.`dataEmissao`,'%Y/%m')) `c` on((`v`.`mes` = `c`.`mes`))) where (`v`.`mes` > '2017');
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`viewSimuladorExcel` AS select `ohp`.`idOrcamentoProduto` AS `idOrcamentoProduto`,`ohp`.`idOrcamento` AS `idOrcamento`,`ohp`.`fornecedor` AS `fornecedor`,`ohp`.`produto` AS `produto`,`ohp`.`codComercial` AS `codComercial`,`ohp`.`prcUnitario` AS `prcUnitario`,`ohp`.`quant` AS `quant`,`ohp`.`desconto` AS `desconto`,`ohp`.`descGlobal` AS `descGlobal`,`ohp`.`total` AS `total`,`o`.`frete` AS `frete`,`o`.`total` AS `totalgeral`,`p`.`custo` AS `custo`,`p`.`st` AS `st`,`pr`.`nome_razao` AS `profissional`,`pr`.`comissao` AS `comissao`,`c`.`nome_razao` AS `cliente` from ((((`develop`.`orcamento_has_produto` `ohp` left join `develop`.`orcamento` `o` on((`ohp`.`idOrcamento` = `o`.`idOrcamento`))) left join `develop`.`produto` `p` on((`p`.`idProduto` = `ohp`.`idProduto`))) left join `develop`.`profissional` `pr` on((`pr`.`idProfissional` = `o`.`idProfissional`))) left join `develop`.`cliente` `c` on((`c`.`idCliente` = `o`.`idCliente`))) where ((`o`.`status` = 'ativo') or (`o`.`status` = 'expirado'));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`viewTaxaCartaoExcel` AS select date_format(`v`.`dataEmissao`,'%Y/%m') AS `mes`,sum((if((`v`.`status` = 'RECEBIDO'),`v`.`valorReal`,`v`.`valor`) * -(1))) AS `valor`,`develop`.`loja`.`descricao` AS `Loja` from (`develop`.`conta_a_receber_has_pagamento` `v` left join `develop`.`loja` on((`develop`.`loja`.`idLoja` = `v`.`centroCusto`))) where ((`v`.`status` <> 'cancelado') and (`v`.`status` <> 'cancelado') and (`v`.`desativado` = 0) and (`v`.`taxa` = 1) and (`v`.`dataEmissao` > '2016-12-31')) group by date_format(`v`.`dataEmissao`,'%Y/%m'),`develop`.`loja`.`descricao`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_a_pagar_vencidos_base` AS select `cr`.`dataPagamento` AS `Data Pagamento`,sum(if(((`cr`.`status` = 'CONFERIDO') and (`cr`.`desativado` = FALSE)),`cr`.`valor`,0)) AS `CONFERIDO`,sum(if(((`cr`.`status` = 'PENDENTE') and (`cr`.`desativado` = FALSE)),`cr`.`valor`,0)) AS `PENDENTE`,sum(`cr`.`valor`) AS `Total` from `develop`.`conta_a_pagar_has_pagamento` `cr` where ((`cr`.`desativado` = FALSE) and (`cr`.`dataPagamento` < curdate()) and ((`cr`.`status` = 'PENDENTE') or (`cr`.`status` = 'CONFERIDO'))) group by `cr`.`dataPagamento`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_a_receber_vencidos_base` AS select `cr`.`dataPagamento` AS `Data Pagamento`,`cr`.`status` AS `Status`,sum(if(((`cr`.`tipo` like '%cartão%') and (`cr`.`desativado` = FALSE)),`cr`.`valor`,0)) AS `Cartão`,sum(if(((`cr`.`tipo` like '%cheque%') and (`cr`.`desativado` = FALSE)),`cr`.`valor`,0)) AS `Cheque`,sum(if(((`cr`.`tipo` like '%boleto%') and (`cr`.`desativado` = FALSE)),`cr`.`valor`,0)) AS `Boleto`,sum(if(((not((`cr`.`tipo` like '%cartão%'))) and (not((`cr`.`tipo` like '%cheque%'))) and (not((`cr`.`tipo` like '%boleto%'))) and (`cr`.`desativado` = FALSE)),`cr`.`valor`,0)) AS `Outros`,sum(`cr`.`valor`) AS `Total` from `develop`.`conta_a_receber_has_pagamento` `cr` where (`cr`.`desativado` = FALSE) group by `cr`.`dataPagamento`,`cr`.`representacao`,`cr`.`status` having ((`cr`.`dataPagamento` < curdate()) and (`cr`.`representacao` = 0) and ((`cr`.`status` = 'PENDENTE') or (`cr`.`status` = 'CONFERIDO')));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_agendar_coleta` AS select cast(min((`v`.`data` + interval `v`.`prazoEntrega` day)) as date) AS `prazoEntrega`,group_concat(distinct `pf2`.`dataRealFat` separator ',') AS `dataRealFat`,`e`.`idEstoque` AS `idEstoque`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco`,group_concat(distinct `pf2`.`fornecedor` separator ',') AS `fornecedor`,if(((upper(`e`.`un`) = 'M²') or (upper(`e`.`un`) = 'M2') or (upper(`e`.`un`) = 'ML')),`p`.`m2cx`,`p`.`pccx`) AS `unCaixa`,`p`.`formComercial` AS `formComercial`,`p`.`idProduto` AS `idProduto`,`n`.`numeroNFe` AS `numeroNFe`,`n`.`idNFe` AS `idNFe`,group_concat(distinct `pf2`.`idVenda` separator ',') AS `idVenda`,group_concat(distinct `pf2`.`ordemCompra` separator ',') AS `ordemCompra`,`e`.`descricao` AS `produto`,`e`.`codComercial` AS `codComercial`,abs(`e`.`quant`) AS `quant`,`e`.`un` AS `un`,`e`.`caixas` AS `caixas`,group_concat(distinct `pf2`.`kgcx` separator ',') AS `kgcx`,`n`.`transportadora` AS `Transportadora`,(`e`.`caixas` * max(`pf2`.`kgcx`)) AS `Peso` from (((((`develop`.`estoque` `e` left join `develop`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `develop`.`pedido_fornecedor_has_produto2` `pf2` on((`ehc`.`idPedido2` = `pf2`.`idPedido2`))) left join `develop`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `develop`.`venda` `v` on((`v`.`idVenda` = `pf2`.`idVenda`))) left join `develop`.`produto` `p` on((`e`.`idProduto` = `p`.`idProduto`))) where ((`e`.`status` = 'EM COLETA') and isnull(`pf2`.`dataPrevColeta`) and (`pf2`.`idPedido2` is not null)) group by `e`.`idEstoque` desc order by `prazoEntrega`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_agendar_entrega` AS select `vp2`.`idVendaProduto2` AS `idVendaProduto2`,`vp2`.`idProduto` AS `idProduto`,`vp2`.`dataPrevEnt` AS `dataPrevEnt`,`vp2`.`dataRealEnt` AS `dataRealEnt`,`vp2`.`status` AS `status`,`vp2`.`fornecedor` AS `fornecedor`,`vp2`.`idVenda` AS `idVenda`,`vp2`.`idNFeSaida` AS `idNFeSaida`,`vp2`.`idNFeFutura` AS `idNFeFutura`,`vp2`.`produto` AS `produto`,group_concat(distinct `e`.`idEstoque` separator ', ') AS `idEstoque`,group_concat(distinct `e`.`lote` separator ', ') AS `lote`,group_concat(distinct `e`.`local` separator ', ') AS `local`,group_concat(distinct `e`.`bloco` separator ', ') AS `bloco`,`vp2`.`caixas` AS `caixas`,`vp2`.`quant` AS `quant`,`vp2`.`un` AS `un`,`vp2`.`unCaixa` AS `unCaixa`,`vp2`.`codComercial` AS `codComercial`,`vp2`.`formComercial` AS `formComercial`,group_concat(`ehc`.`idConsumo` separator ',') AS `idConsumo` from ((`develop`.`venda_has_produto2` `vp2` left join `develop`.`estoque_has_consumo` `ehc` on((`vp2`.`idVendaProduto2` = `ehc`.`idVendaProduto2`))) left join `develop`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) group by `vp2`.`idVendaProduto2`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_busca_veiculo` AS select `t`.`razaoSocial` AS `razaoSocial`,`thv`.`idVeiculo` AS `idVeiculo`,`thv`.`modelo` AS `modelo`,`thv`.`capacidade` AS `capacidade`,`thv`.`placa` AS `placa`,`thv`.`desativado` AS `desativado` from (`develop`.`transportadora` `t` left join `develop`.`transportadora_has_veiculo` `thv` on((`t`.`idTransportadora` = `thv`.`idTransportadora`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_calendario` AS select `vp`.`idVenda` AS `idVenda`,`vp`.`idEvento` AS `carga`,sum(`vp`.`caixas`) AS `caixas`,sum(`vp`.`kg`) AS `kg`,`vp`.`data` AS `data`,`t`.`razaoSocial` AS `razaoSocial`,`thv`.`modelo` AS `modelo`,`vp`.`status` AS `status`,`che`.`logradouro` AS `logradouro`,`che`.`numero` AS `numero`,`che`.`bairro` AS `bairro`,`che`.`cidade` AS `cidade`,`che`.`uf` AS `uf`,group_concat(concat(left(`vp`.`produto`,25),' - ',`vp`.`caixas`,' cx.') separator '/') AS `produtos` from ((((`develop`.`veiculo_has_produto` `vp` left join `develop`.`transportadora_has_veiculo` `thv` on((`vp`.`idVeiculo` = `thv`.`idVeiculo`))) left join `develop`.`transportadora` `t` on((`t`.`idTransportadora` = `thv`.`idTransportadora`))) left join `develop`.`venda` `v` on((`v`.`idVenda` = `vp`.`idVenda`))) left join `develop`.`cliente_has_endereco` `che` on((`che`.`idEndereco` = `v`.`idEnderecoEntrega`))) where (`vp`.`status` <> 'ESTOQUE') group by `vp`.`idVeiculo`,`vp`.`data`,`vp`.`idVenda`,`vp`.`status`,`vp`.`idEvento` order by `vp`.`data`,`vp`.`idVenda`,`t`.`razaoSocial`,`thv`.`modelo`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_calendario2` AS select `x`.`razaoSocial` AS `razaoSocial`,group_concat(distinct `x`.`status` separator ',') AS `status`,sum(`x`.`caixas`) AS `caixas`,sum(`x`.`kg`) AS `kg`,`x`.`idVeiculo` AS `idVeiculo`,`x`.`modelo` AS `modelo`,`x`.`data` AS `data`,group_concat(distinct `x`.`idVenda` separator '|') AS `idVenda`,group_concat(distinct `x`.`endereco` separator '|') AS `endereco` from (select `t`.`razaoSocial` AS `razaoSocial`,group_concat(distinct `vhp`.`status` separator ',') AS `status`,sum(`vhp`.`caixas`) AS `caixas`,sum(`vhp`.`kg`) AS `kg`,group_concat(distinct `vhp`.`idEvento` separator ',') AS `idEvento`,`vhp`.`idVenda` AS `idVenda`,`thv`.`modelo` AS `modelo`,`vhp`.`idVeiculo` AS `idVeiculo`,`vhp`.`data` AS `data`,replace(concat(`che`.`logradouro`,',',`che`.`numero`,',',`che`.`cidade`,',',`che`.`uf`),' ','+') AS `endereco` from ((((`develop`.`veiculo_has_produto` `vhp` left join `develop`.`transportadora_has_veiculo` `thv` on((`vhp`.`idVeiculo` = `thv`.`idVeiculo`))) left join `develop`.`transportadora` `t` on((`thv`.`idTransportadora` = `t`.`idTransportadora`))) left join `develop`.`venda` `v` on((`vhp`.`idVenda` = `v`.`idVenda`))) left join `develop`.`cliente_has_endereco` `che` on((`che`.`idEndereco` = `v`.`idEnderecoEntrega`))) where ((`vhp`.`idVeiculo` <> 11) and (`che`.`idEndereco` <> 1)) group by `vhp`.`idVenda`,`vhp`.`data`,`vhp`.`idVeiculo`) `x` group by `x`.`idVeiculo`,`x`.`data` order by `x`.`data`,`x`.`idVeiculo`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_calendario_carga` AS select `vhp`.`idVeiculo` AS `idVeiculo`,`vhp`.`idEvento` AS `idEvento`,`vhp`.`data` AS `dataPrevEnt`,`n`.`idNFe` AS `idNFe`,`n`.`status` AS `NFe Status`,`n`.`numeroNFe` AS `numeroNFe`,any_value((select `develop`.`nfe`.`numeroNFe` from `develop`.`nfe` where (`develop`.`nfe`.`idNFe` = `vp2`.`idNFeFutura`))) AS `NFe Futura`,`vhp`.`idVenda` AS `idVenda`,`vhp`.`status` AS `status`,count(0) AS `produtos`,sum(`vhp`.`kg`) AS `kg` from ((`develop`.`veiculo_has_produto` `vhp` left join `develop`.`venda_has_produto2` `vp2` on((`vhp`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) left join `develop`.`nfe` `n` on((`vp2`.`idNFeSaida` = `n`.`idNFe`))) where (`vhp`.`status` in ('ENTREGA AGEND.','EM ENTREGA')) group by `vhp`.`idVeiculo`,`vhp`.`data`,`n`.`idNFe`,`vhp`.`idVenda`,`vhp`.`idEvento`,`vhp`.`status`,`n`.`numeroNFe`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_calendario_entrega` AS select `vhp`.`idVeiculo` AS `idVeiculo`,cast(`vhp`.`data` as date) AS `data`,`thv`.`modelo` AS `modelo`,sum(`vhp`.`kg`) AS `kg`,`t`.`razaoSocial` AS `razaoSocial`,group_concat(distinct `vhp`.`idVenda` separator ', ') AS `idVenda` from ((`develop`.`veiculo_has_produto` `vhp` left join `develop`.`transportadora_has_veiculo` `thv` on((`vhp`.`idVeiculo` = `thv`.`idVeiculo`))) left join `develop`.`transportadora` `t` on((`t`.`idTransportadora` = `thv`.`idTransportadora`))) where (`vhp`.`status` in ('ENTREGA AGEND.','EM ENTREGA')) group by cast(`vhp`.`data` as date),`vhp`.`idVeiculo` order by cast(`vhp`.`data` as date);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_calendario_produto` AS select `vhp`.`idEvento` AS `idEvento`,`vp2`.`idVenda` AS `idVenda`,`vp2`.`idVendaProduto2` AS `idVendaProduto2`,`vp2`.`fornecedor` AS `fornecedor`,`vp2`.`produto` AS `produto`,`vp2`.`formComercial` AS `formComercial`,`vp2`.`codComercial` AS `codComercial`,`ehc`.`caixas` AS `caixas`,(`ehc`.`quant` * -(1)) AS `quant`,`vp2`.`un` AS `un`,(((`ehc`.`quant` * -(1)) / `vp2`.`quant`) * `vhp`.`kg`) AS `kg`,`e`.`idEstoque` AS `idEstoque`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco` from (((`develop`.`veiculo_has_produto` `vhp` left join `develop`.`venda_has_produto2` `vp2` on((`vhp`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) left join `develop`.`estoque_has_consumo` `ehc` on((`vp2`.`idVendaProduto2` = `ehc`.`idVendaProduto2`))) left join `develop`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) where (`ehc`.`status` not in ('CANCELADO','DEVOLVIDO'));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_caminhao` AS select group_concat(distinct `vhp`.`idVeiculo` separator ',') AS `idVeiculo`,`t`.`razaoSocial` AS `Transportadora`,`thv`.`modelo` AS `Veículo`,`thv`.`placa` AS `Placa` from ((`develop`.`veiculo_has_produto` `vhp` left join `develop`.`transportadora_has_veiculo` `thv` on((`vhp`.`idVeiculo` = `thv`.`idVeiculo`))) left join `develop`.`transportadora` `t` on((`thv`.`idTransportadora` = `t`.`idTransportadora`))) group by `thv`.`idVeiculo`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_caminhao_resumo` AS select group_concat(distinct `vhp`.`idVeiculo` separator ',') AS `idVeiculo`,`vhp`.`data` AS `data`,coalesce(sum(`vhp`.`kg`),0) AS `Kg` from `develop`.`veiculo_has_produto` `vhp` group by `vhp`.`data`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_coleta` AS select cast(min((`v`.`data` + interval `v`.`prazoEntrega` day)) as date) AS `prazoEntrega`,group_concat(distinct `pf2`.`dataPrevColeta` separator ',') AS `dataPrevColeta`,`e`.`idEstoque` AS `idEstoque`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco`,group_concat(distinct `pf2`.`fornecedor` separator ',') AS `fornecedor`,group_concat(distinct `n`.`numeroNFe` separator ',') AS `numeroNFe`,group_concat(distinct `pf2`.`idVenda` separator ', ') AS `idVenda`,group_concat(distinct `pf2`.`ordemCompra` separator ', ') AS `ordemCompra`,`e`.`descricao` AS `produto`,`e`.`codComercial` AS `codComercial`,abs(`e`.`quant`) AS `quant`,`e`.`un` AS `un`,`e`.`caixas` AS `caixas`,group_concat(distinct `pf2`.`kgcx` separator ',') AS `kgcx`,group_concat(distinct `n`.`transportadora` separator ',') AS `Transportadora` from ((((`develop`.`estoque` `e` left join `develop`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `develop`.`pedido_fornecedor_has_produto2` `pf2` on((`ehc`.`idPedido2` = `pf2`.`idPedido2`))) left join `develop`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `develop`.`venda` `v` on((`v`.`idVenda` = `pf2`.`idVenda`))) where ((`e`.`status` = 'EM COLETA') and (`pf2`.`dataPrevColeta` is not null) and (`pf2`.`idPedido2` is not null)) group by `e`.`idEstoque` desc order by `prazoEntrega`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_compras` AS select any_value(`pf2`.`ordemCompra`) AS `OC`,`pf2`.`idCompra` AS `Compra`,any_value(`pf2`.`fornecedor`) AS `Fornecedor`,min(`v`.`data`) AS `Data Venda`,count(`pf2`.`idProduto`) AS `Produtos`,sum(`pf2`.`preco`) AS `Total`,any_value(`pf2`.`dataPrevConf`) AS `dataPrevConf`,group_concat(distinct substring_index(`u`.`nome`,' ',1) separator ', ') AS `Vendedor`,group_concat(distinct `pf2`.`idVenda` separator ', ') AS `Venda` from ((`develop`.`pedido_fornecedor_has_produto2` `pf2` left join `develop`.`venda` `v` on((`pf2`.`idVenda` = `v`.`idVenda`))) left join `develop`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`))) where (`pf2`.`status` = 'EM COMPRA') group by `pf2`.`idCompra`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_compras_financeiro` AS select group_concat(distinct `pf`.`status` separator ', ') AS `Status`,group_concat(distinct `pf`.`statusFinanceiro` separator ', ') AS `Status Financeiro`,group_concat(distinct `pf`.`ordemCompra` separator ', ') AS `OC`,`pf`.`idCompra` AS `Compra`,group_concat(distinct `pf`.`fornecedor` separator ', ') AS `Fornecedor`,count(0) AS `Produtos`,sum(`pf`.`preco`) AS `Total`,group_concat(distinct `pf`.`idVenda` separator ', ') AS `Código` from `develop`.`pedido_fornecedor_has_produto` `pf` where (`pf`.`idCompra` is not null) group by `pf`.`idCompra`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_consistencia_compra` AS select `develop`.`pedido_fornecedor_has_produto`.`descricao` AS `descricao`,`develop`.`pedido_fornecedor_has_produto`.`prcUnitario` AS `prcUnitario`,`develop`.`pedido_fornecedor_has_produto`.`preco` AS `preco`,(`develop`.`pedido_fornecedor_has_produto`.`prcUnitario` * `develop`.`pedido_fornecedor_has_produto`.`quant`) AS `prcUnitario * quant` from `develop`.`pedido_fornecedor_has_produto` where (((`develop`.`pedido_fornecedor_has_produto`.`prcUnitario` * `develop`.`pedido_fornecedor_has_produto`.`quant`) / `develop`.`pedido_fornecedor_has_produto`.`preco`) > 1.01);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_conta_pagar` AS select `cp`.`idPagamento` AS `idPagamento`,`cp`.`idLoja` AS `idLoja`,`cp`.`contraParte` AS `contraparte`,`cp`.`dataPagamento` AS `dataPagamento`,`cp`.`dataEmissao` AS `dataEmissao`,`cp`.`valor` AS `valor`,`cp`.`status` AS `status`,group_concat(distinct `pf`.`ordemCompra` separator ',') AS `ordemCompra`,group_concat(distinct `pf`.`idVenda` separator ', ') AS `idVenda`,group_concat(distinct `n`.`numeroNFe` separator ', ') AS `numeroNFe`,`cp`.`tipo` AS `tipo`,`cp`.`parcela` AS `parcela`,`cp`.`observacao` AS `observacao`,group_concat(distinct `pf`.`statusFinanceiro` separator ',') AS `statusFinanceiro` from ((((`develop`.`conta_a_pagar_has_pagamento` `cp` left join `develop`.`pedido_fornecedor_has_produto2` `pf` on((`cp`.`idCompra` = `pf`.`idCompra`))) left join `develop`.`estoque_has_compra` `ehc` on((`ehc`.`idPedido2` = `pf`.`idPedido2`))) left join `develop`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) left join `develop`.`nfe` `n` on((`n`.`idNFe` = `e`.`idNFe`))) where (`cp`.`desativado` = FALSE) group by `cp`.`idPagamento` order by `cp`.`dataPagamento`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_conta_receber` AS select `cr`.`idPagamento` AS `idPagamento`,`cr`.`idLoja` AS `idLoja`,`cr`.`representacao` AS `representacao`,`cr`.`contraParte` AS `contraparte`,`cr`.`dataPagamento` AS `dataPagamento`,`cr`.`dataEmissao` AS `dataEmissao`,`cr`.`idVenda` AS `idVenda`,`cr`.`valor` AS `valor`,`cr`.`tipo` AS `tipo`,`cr`.`parcela` AS `parcela`,`cr`.`observacao` AS `observacao`,`cr`.`status` AS `status`,`v`.`statusFinanceiro` AS `statusFinanceiro` from (`develop`.`conta_a_receber_has_pagamento` `cr` left join `develop`.`venda` `v` on((`cr`.`idVenda` = `v`.`idVenda`))) where ((`cr`.`desativado` = FALSE) and (`cr`.`representacao` = 0)) group by `cr`.`idPagamento` order by `cr`.`dataPagamento`,`cr`.`idVenda`,`cr`.`tipo`,`cr`.`parcela` desc;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_custo_estoque` AS select `ehc`.`idVendaProduto2` AS `idVendaProduto2`,`pf2`.`ordemCompra` AS `ordemCompra`,`pf2`.`idVenda` AS `idVenda`,`ehc`.`descricao` AS `descricao`,`ehc`.`valor` AS `R$ Consumo`,`pf2`.`preco` AS `R$ Compra`,`ehc`.`quant` AS `quant est`,`pf2`.`quant` AS `quant compra`,(`ehc`.`valor` / `pf2`.`preco`) AS `diferenca` from (`develop`.`estoque_has_consumo` `ehc` left join `develop`.`pedido_fornecedor_has_produto2` `pf2` on((`ehc`.`idVendaProduto2` = `pf2`.`idVendaProduto2`))) where ((`pf2`.`idVendaProduto2` is not null) and ((`ehc`.`valor` / `pf2`.`preco`) > 1.01));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_entrega` AS select cast(min((`v`.`data` + interval `v`.`prazoEntrega` day)) as date) AS `prazoEntrega`,`vp`.`idVenda` AS `idVenda`,group_concat(distinct `vp`.`recebeu` separator ',') AS `Recebeu`,group_concat(distinct `vp`.`entregou` separator ',') AS `Entregou`,sum((`vp`.`status` = 'ESTOQUE')) AS `Estoque`,sum((`vp`.`status` = 'ENTREGUE')) AS `Entregue`,sum(((`vp`.`status` <> 'ESTOQUE') and (`vp`.`status` <> 'ENTREGUE'))) AS `Outros`,`che`.`bairro` AS `Bairro`,`che`.`logradouro` AS `Logradouro`,`che`.`numero` AS `Número`,`che`.`cidade` AS `Cidade` from ((`develop`.`venda_has_produto` `vp` left join `develop`.`venda` `v` on((`vp`.`idVenda` = `v`.`idVenda`))) left join `develop`.`cliente_has_endereco` `che` on((`v`.`idEnderecoEntrega` = `che`.`idEndereco`))) where (not((`vp`.`idVenda` like '%D'))) group by `vp`.`idVenda`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_entrega_produtos` AS select `vp2`.`idVendaProduto2` AS `idVendaProduto2`,`vp2`.`idProduto` AS `idProduto`,`vp2`.`dataPrevEnt` AS `dataPrevEnt`,`vp2`.`dataRealEnt` AS `dataRealEnt`,`vp2`.`status` AS `status`,`vp2`.`fornecedor` AS `fornecedor`,`vp2`.`idVenda` AS `idVenda`,`vp2`.`produto` AS `produto`,`vp2`.`caixas` AS `caixas`,`vp2`.`quant` AS `quant`,`vp2`.`un` AS `un`,`vp2`.`unCaixa` AS `unCaixa`,`vp2`.`codComercial` AS `codComercial`,`vp2`.`formComercial` AS `formComercial`,group_concat(distinct `ehc`.`idConsumo` separator ',') AS `idConsumo` from (`develop`.`venda_has_produto2` `vp2` left join `develop`.`estoque_has_consumo` `ehc` on((`vp2`.`idVendaProduto2` = `ehc`.`idVendaProduto2`))) group by `vp2`.`idVendaProduto2`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_estoque_consumo` AS select `vp2`.`idVenda` AS `Venda`,`ehc`.`idEstoque` AS `idEstoque`,`vp2`.`status` AS `statusProduto`,`ehc`.`status` AS `status`,`ehc`.`local` AS `local`,`ehc`.`fornecedor` AS `fornecedor`,`ehc`.`descricao` AS `descricao`,`ehc`.`quant` AS `quant`,`ehc`.`quantUpd` AS `quantUpd`,`ehc`.`un` AS `un`,`ehc`.`caixas` AS `caixas`,`ehc`.`codComercial` AS `codComercial`,`vp2`.`dataRealEnt` AS `dataRealEnt`,`ehc`.`created` AS `created` from (`develop`.`estoque_has_consumo` `ehc` left join `develop`.`venda_has_produto2` `vp2` on((`ehc`.`idVendaProduto2` = `vp2`.`idVendaProduto2`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_estoque_contabil` AS select `n`.`cnpjDest` AS `cnpjDest`,`e`.`status` AS `status`,`e`.`idEstoque` AS `idEstoque`,`p`.`fornecedor` AS `fornecedor`,`e`.`descricao` AS `descricao`,((`e`.`quant` + coalesce(`e2`.`consumoEst`,0)) + `e`.`ajuste`) AS `contabil`,`e`.`restante` AS `restante`,`e`.`un` AS `unEst`,if(((`p`.`un` = 'M²') or (`p`.`un` = 'M2') or (`p`.`un` = 'ML')),(`e`.`restante` / `p`.`m2cx`),(`e`.`restante` / `p`.`pccx`)) AS `Caixas`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco`,`e`.`codComercial` AS `codComercial`,any_value(`n`.`numeroNFe`) AS `nfe`,any_value(`pf`.`dataPrevColeta`) AS `dataPrevColeta`,any_value(`pf`.`dataRealColeta`) AS `dataRealColeta`,any_value(`pf`.`dataPrevReceb`) AS `dataPrevReceb`,any_value(`pf`.`dataRealReceb`) AS `dataRealReceb` from (((((`develop`.`estoque` `e` left join (select `consumo`.`idEstoque` AS `idEstoque`,sum(`consumo`.`quant`) AS `consumoEst` from (`develop`.`estoque_has_consumo` `consumo` left join `develop`.`venda_has_produto2` `vp` on((`consumo`.`idVendaProduto2` = `vp`.`idVendaProduto2`))) where ((`vp`.`dataRealEnt` < now()) and (`consumo`.`status` <> 'CANCELADO')) group by `consumo`.`idEstoque`) `e2` on((`e`.`idEstoque` = `e2`.`idEstoque`))) left join `develop`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `develop`.`pedido_fornecedor_has_produto2` `pf` on((`pf`.`idPedido2` = `ehc`.`idPedido2`))) left join `develop`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `develop`.`produto` `p` on((`e`.`idProduto` = `p`.`idProduto`))) where (`e`.`status` not in ('CANCELADO','QUEBRADO')) group by `e`.`idEstoque` having (`contabil` > 0);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_estoque_disponivel` AS select `n`.`cnpjDest` AS `cnpjDest`,`e`.`status` AS `status`,`e`.`idEstoque` AS `idEstoque`,`p`.`fornecedor` AS `fornecedor`,`e`.`descricao` AS `descricao`,`e`.`restante` AS `restante`,`e`.`un` AS `unEst`,if(((`p`.`un` = 'M²') or (`p`.`un` = 'M2') or (`p`.`un` = 'ML')),(`e`.`restante` / `p`.`m2cx`),(`e`.`restante` / `p`.`pccx`)) AS `Caixas`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco`,`e`.`codComercial` AS `codComercial`,any_value(`n`.`numeroNFe`) AS `nfe`,any_value(`pf`.`dataPrevColeta`) AS `dataPrevColeta`,any_value(`pf`.`dataRealColeta`) AS `dataRealColeta`,any_value(`pf`.`dataPrevReceb`) AS `dataPrevReceb`,any_value(`pf`.`dataRealReceb`) AS `dataRealReceb` from ((((`develop`.`estoque` `e` left join `develop`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `develop`.`pedido_fornecedor_has_produto2` `pf` on((`pf`.`idPedido2` = `ehc`.`idPedido2`))) left join `develop`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `develop`.`produto` `p` on((`e`.`idProduto` = `p`.`idProduto`))) where (`e`.`status` not in ('CANCELADO','QUEBRADO')) group by `e`.`idEstoque` having (`e`.`restante` > 0);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_estoque_zerado` AS select `n`.`cnpjDest` AS `cnpjDest`,`e`.`status` AS `status`,`e`.`idEstoque` AS `idEstoque`,`p`.`fornecedor` AS `fornecedor`,`e`.`descricao` AS `descricao`,`e`.`restante` AS `restante`,`e`.`un` AS `unEst`,if(((`p`.`un` = 'M²') or (`p`.`un` = 'M2') or (`p`.`un` = 'ML')),(`e`.`restante` / `p`.`m2cx`),(`e`.`restante` / `p`.`pccx`)) AS `Caixas`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco`,`e`.`codComercial` AS `codComercial`,any_value(`n`.`numeroNFe`) AS `nfe`,any_value(`pf`.`dataPrevColeta`) AS `dataPrevColeta`,any_value(`pf`.`dataRealColeta`) AS `dataRealColeta`,any_value(`pf`.`dataPrevReceb`) AS `dataPrevReceb`,any_value(`pf`.`dataRealReceb`) AS `dataRealReceb` from ((((`develop`.`estoque` `e` left join `develop`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `develop`.`pedido_fornecedor_has_produto2` `pf` on((`pf`.`idPedido2` = `ehc`.`idPedido2`))) left join `develop`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `develop`.`produto` `p` on((`e`.`idProduto` = `p`.`idProduto`))) where (`e`.`status` not in ('CANCELADO','QUEBRADO')) group by `e`.`idEstoque` having (`e`.`restante` = 0);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_fluxo_caixa_pendente` AS select `cp`.`idCompra` AS `Compra/Venda`,`cp`.`dataPagamento` AS `dataPagamento`,`cp`.`dataRealizado` AS `dataRealizado`,(`cp`.`valor` * -(1)) AS `R$`,`cp`.`tipo` AS `Tipo`,(select concat(`develop`.`loja_has_conta`.`banco`,' - ',`develop`.`loja_has_conta`.`agencia`,' - ',`develop`.`loja_has_conta`.`conta`) from `develop`.`loja_has_conta` where (`develop`.`loja_has_conta`.`idConta` = `cp`.`contaDestino`)) AS `contaDestino`,`cp`.`contaDestino` AS `idConta`,`cp`.`status` AS `Status`,'SAIDA' AS `ENT/SAI` from `develop`.`conta_a_pagar_has_pagamento` `cp` where ((`cp`.`status` in ('PENDENTE','CONFERIDO')) and (`cp`.`desativado` = FALSE)) union all select `cr`.`idVenda` AS `idVenda`,`cr`.`dataPagamento` AS `dataPagamento`,`cr`.`dataRealizado` AS `dataRealizado`,`cr`.`valor` AS `valor`,`cr`.`tipo` AS `tipo`,(select concat(`develop`.`loja_has_conta`.`banco`,' - ',`develop`.`loja_has_conta`.`agencia`,' - ',`develop`.`loja_has_conta`.`conta`) from `develop`.`loja_has_conta` where (`develop`.`loja_has_conta`.`idConta` = `cr`.`contaDestino`)) AS `Name_exp_15`,`cr`.`contaDestino` AS `contaDestino`,`cr`.`status` AS `status`,'ENTRADA' AS `ENTRADA` from `develop`.`conta_a_receber_has_pagamento` `cr` where ((`cr`.`status` in ('PENDENTE','CONFERIDO')) and (`cr`.`desativado` = FALSE) and (`cr`.`representacao` = FALSE)) order by `dataPagamento`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_fluxo_caixa_realizado` AS select `cp`.`idCompra` AS `Compra/Venda`,`cp`.`dataRealizado` AS `dataRealizado`,(`cp`.`valorReal` * -(1)) AS `R$`,`cp`.`tipo` AS `Tipo`,(select concat(`develop`.`loja_has_conta`.`banco`,' - ',`develop`.`loja_has_conta`.`agencia`,' - ',`develop`.`loja_has_conta`.`conta`) from `develop`.`loja_has_conta` where (`develop`.`loja_has_conta`.`idConta` = `cp`.`contaDestino`)) AS `contaDestino`,`cp`.`contaDestino` AS `idConta`,`cp`.`status` AS `Status`,'SAIDA' AS `ENT/SAI` from `develop`.`conta_a_pagar_has_pagamento` `cp` where ((`cp`.`status` = 'PAGO') and (`cp`.`desativado` = FALSE)) union all select `cr`.`idVenda` AS `idVenda`,`cr`.`dataRealizado` AS `dataRealizado`,`cr`.`valorReal` AS `valorReal`,`cr`.`tipo` AS `tipo`,(select concat(`develop`.`loja_has_conta`.`banco`,' - ',`develop`.`loja_has_conta`.`agencia`,' - ',`develop`.`loja_has_conta`.`conta`) from `develop`.`loja_has_conta` where (`develop`.`loja_has_conta`.`idConta` = `cr`.`contaDestino`)) AS `Name_exp_15`,`cr`.`contaDestino` AS `contaDestino`,`cr`.`status` AS `status`,'ENTRADA' AS `ENTRADA` from `develop`.`conta_a_receber_has_pagamento` `cr` where ((`cr`.`status` = 'RECEBIDO') and (`cr`.`desativado` = FALSE)) order by `dataRealizado`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_fluxo_resumo_realizado` AS select `v`.`dataRealizado` AS `dataRealizado`,`v`.`contaDestino` AS `contaDestino`,`v`.`idConta` AS `idConta`,coalesce(sum(if((`v`.`ENT/SAI` = 'SAIDA'),`v`.`R$`,0)),0) AS `SAIDA`,coalesce(sum(if((`v`.`ENT/SAI` = 'ENTRADA'),`v`.`R$`,0)),0) AS `ENTRADA`,coalesce(sum(`v`.`R$`),0) AS `R$` from `develop`.`view_fluxo_caixa_realizado` `v` where (`v`.`Status` in ('PAGO','RECEBIDO')) group by `v`.`dataRealizado`,`v`.`idConta`,`v`.`contaDestino` order by `v`.`dataRealizado`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_followup_orcamento` AS select `ohf`.`idOrcamento` AS `idOrcamento`,`u`.`nome` AS `nome`,`ohf`.`observacao` AS `observacao`,`ohf`.`dataFollowup` AS `dataFollowup`,`ohf`.`dataProxFollowup` AS `dataProxFollowup`,`ohf`.`semaforo` AS `semaforo` from (`develop`.`orcamento_has_followup` `ohf` left join `develop`.`usuario` `u` on((`ohf`.`idUsuario` = `u`.`idUsuario`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_followup_venda` AS select `vhf`.`idVenda` AS `idVenda`,`u`.`nome` AS `nome`,`vhf`.`tipoOperacao` AS `Tipo`,`vhf`.`observacao` AS `observacao`,`vhf`.`dataFollowup` AS `dataFollowup` from (`develop`.`venda_has_followup` `vhf` left join `develop`.`usuario` `u` on((`vhf`.`idUsuario` = `u`.`idUsuario`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_fornecedor_compra_confirmar` AS select sum((case when (`pf2`.`status` = 'EM COMPRA') then 1 else 0 end)) AS `CONF`,`pf2`.`fornecedor` AS `Fornecedor` from `develop`.`pedido_fornecedor_has_produto2` `pf2` where (`pf2`.`status` = 'EM COMPRA') group by `pf2`.`fornecedor`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_fornecedor_compra_faturar` AS select sum((case when (`pf2`.`status` = 'EM FATURAMENTO') then 1 else 0 end)) AS `FAT`,`pf2`.`fornecedor` AS `Fornecedor` from `develop`.`pedido_fornecedor_has_produto2` `pf2` where (`pf2`.`status` = 'EM FATURAMENTO') group by `pf2`.`fornecedor`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_fornecedor_compra_gerar` AS select sum((case when (`pf`.`status` = 'PENDENTE') then 1 else 0 end)) AS `PEND`,`pf`.`fornecedor` AS `Fornecedor` from `develop`.`pedido_fornecedor_has_produto` `pf` where (`pf`.`status` = 'PENDENTE') group by `pf`.`fornecedor`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_fornecedor_logistica_agendar_coleta` AS select `subquery`.`fornecedor` AS `Fornecedor`,sum(isnull(`subquery`.`idVenda`)) AS `Estoque`,sum((`subquery`.`idVenda` is not null)) AS `Venda` from (select group_concat(distinct `pf`.`fornecedor` separator ',') AS `fornecedor`,`e`.`status` AS `status`,group_concat(distinct `pf`.`idVenda` separator ',') AS `idVenda` from ((`develop`.`estoque` `e` left join `develop`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `develop`.`pedido_fornecedor_has_produto2` `pf` on((`ehc`.`idPedido2` = `pf`.`idPedido2`))) where ((`e`.`status` = 'EM COLETA') and isnull(`pf`.`dataPrevColeta`) and (`pf`.`idPedido2` is not null)) group by `e`.`idEstoque`) `subquery` group by `subquery`.`fornecedor`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_fornecedor_logistica_coleta` AS select `subquery`.`fornecedor` AS `Fornecedor`,sum((`subquery`.`status` = 'EM COLETA')) AS `COLET` from (select group_concat(distinct `pf`.`fornecedor` separator ',') AS `fornecedor`,`e`.`status` AS `status` from ((`develop`.`estoque` `e` left join `develop`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `develop`.`pedido_fornecedor_has_produto2` `pf` on((`ehc`.`idPedido2` = `pf`.`idPedido2`))) where ((`e`.`status` = 'EM COLETA') and (`pf`.`dataPrevColeta` is not null) and (`pf`.`idPedido2` is not null)) group by `e`.`idEstoque`) `subquery` group by `subquery`.`fornecedor`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_fornecedor_logistica_recebimento` AS select `subquery`.`fornecedor` AS `Fornecedor`,sum((`subquery`.`status` = 'EM RECEBIMENTO')) AS `RECEB` from (select group_concat(distinct `pf`.`fornecedor` separator ',') AS `fornecedor`,`e`.`status` AS `status` from ((`develop`.`estoque` `e` left join `develop`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `develop`.`pedido_fornecedor_has_produto2` `pf` on((`ehc`.`idPedido2` = `pf`.`idPedido2`))) where ((`e`.`status` = 'EM RECEBIMENTO') and (`pf`.`dataPrevReceb` is not null) and (`pf`.`idPedido2` is not null)) group by `e`.`idEstoque`) `subquery` group by `subquery`.`fornecedor`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_fornecedor_logistica_representacao` AS select `pf`.`fornecedor` AS `Fornecedor`,count(0) AS `Produtos` from (`develop`.`pedido_fornecedor_has_produto` `pf` left join `develop`.`venda` `v` on((`pf`.`idVenda` = `v`.`idVenda`))) where ((`v`.`representacao` = 1) and (`pf`.`status` = 'EM ENTREGA')) group by `pf`.`fornecedor`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_logistica_representacao` AS select cast(min((`v`.`data` + interval `v`.`prazoEntrega` day)) as date) AS `prazoEntrega`,`pf2`.`idPedido2` AS `idPedido2`,`pf2`.`fornecedor` AS `fornecedor`,`pf2`.`idVenda` AS `idVenda`,`pf2`.`idVendaProduto2` AS `idVendaProduto2`,`c`.`nome_razao` AS `cliente`,`pf2`.`ordemCompra` AS `ordemCompra`,`pf2`.`descricao` AS `descricao`,`pf2`.`codComercial` AS `codComercial`,`pf2`.`quant` AS `quant`,`pf2`.`un` AS `un`,`pf2`.`caixas` AS `caixas`,`pf2`.`kgcx` AS `kgcx` from ((`develop`.`pedido_fornecedor_has_produto2` `pf2` left join `develop`.`venda` `v` on((`pf2`.`idVenda` = `v`.`idVenda`))) left join `develop`.`cliente` `c` on((`v`.`idCliente` = `c`.`idCliente`))) where ((`v`.`representacao` = 1) and (`pf2`.`status` = 'EM ENTREGA')) group by `pf2`.`idPedido2` order by `prazoEntrega`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_nfe_entrada` AS select `n`.`idNFe` AS `idNFe`,`n`.`cnpjDest` AS `CNPJ Dest`,group_concat(distinct `pf`.`fornecedor` separator ', ') AS `Fornecedor`,`n`.`numeroNFe` AS `NFe`,`n`.`status` AS `Status`,group_concat(distinct `e`.`recebidoPor` separator ',') AS `Recebido Por`,group_concat(distinct `pf`.`dataRealReceb` separator ',') AS `Data Receb`,`n`.`gare` AS `GARE`,`n`.`gareData` AS `GARE Pago Em`,group_concat(distinct `pf`.`ordemCompra` order by `pf`.`ordemCompra` ASC separator ', ') AS `OC`,group_concat(distinct `pf`.`idVenda` separator ', ') AS `Venda` from (((`develop`.`nfe` `n` left join `develop`.`estoque` `e` on((`n`.`idNFe` = `e`.`idNFe`))) left join `develop`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `develop`.`pedido_fornecedor_has_produto2` `pf` on((`ehc`.`idPedido2` = `pf`.`idPedido2`))) where (`n`.`tipo` = 'ENTRADA') group by `n`.`idNFe`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_nfe_saida` AS select `n`.`idNFe` AS `idNFe`,`n`.`chaveAcesso` AS `chaveAcesso`,`n`.`cnpjOrig` AS `Emitente`,`n`.`numeroNFe` AS `NFe`,`n`.`status` AS `Status`,group_concat(distinct `v`.`idVenda` separator ',') AS `Venda`,if((group_concat(distinct `c`.`pfpj` separator ',') = 'PF'),group_concat(distinct `c`.`cpf` separator ','),group_concat(distinct `c`.`cnpj` separator ',')) AS `CPF/CNPJ`,group_concat(distinct `c`.`nome_razao` separator ',') AS `Cliente`,`n`.`valor` AS `valor`,`n`.`created` AS `Criado em` from (((`develop`.`nfe` `n` left join `develop`.`venda_has_produto` `vp` on((`n`.`idNFe` = `vp`.`idNFeSaida`))) left join `develop`.`venda` `v` on((`vp`.`idVenda` = `v`.`idVenda`))) left join `develop`.`cliente` `c` on((`c`.`idCliente` = `v`.`idCliente`))) where (`n`.`tipo` = 'SAíDA') group by `n`.`idNFe` order by `n`.`numeroNFe`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_orcamento` AS select `o`.`idLoja` AS `idLoja`,`o`.`idUsuario` AS `idUsuario`,`o`.`idUsuarioConsultor` AS `idUsuarioConsultor`,`o`.`status` AS `Status`,(`o`.`validade` - (to_days(now()) - to_days(`o`.`data`))) AS `Dias restantes`,`o`.`idOrcamento` AS `Código`,`u`.`nome` AS `Vendedor`,`u2`.`nome` AS `Consultor`,`c`.`nome_razao` AS `Cliente`,`p`.`nome_razao` AS `Profissional`,`c`.`tel` AS `Telefone`,`c`.`telCel` AS `Celular`,if(isnull(`p`.`telCel`),if(isnull(`p`.`tel`),`p`.`telCom`,`p`.`tel`),`p`.`telCel`) AS `Tel Prof`,`o`.`data` AS `Data`,`o`.`total` AS `Total`,`ohf`.`dataFollowup` AS `Data Followup`,`ohf`.`dataProxFollowup` AS `Data Próx Followup`,`ohf`.`observacao` AS `Observação`,`ohf`.`semaforo` AS `semaforo` from ((((((`develop`.`orcamento` `o` left join `develop`.`cliente` `c` on((`o`.`idCliente` = `c`.`idCliente`))) left join `develop`.`usuario` `u` on((`o`.`idUsuario` = `u`.`idUsuario`))) left join `develop`.`profissional` `p` on((`o`.`idProfissional` = `p`.`idProfissional`))) left join `develop`.`usuario` `u2` on((`o`.`idUsuarioConsultor` = `u2`.`idUsuario`))) left join (select max(`tmp`.`idFollowup`) AS `idFollowup`,`tmp`.`idOrcamentoBase` AS `idOrcamentoBase` from `develop`.`orcamento_has_followup` `tmp` group by `tmp`.`idOrcamentoBase`) `ohf2` on((`o`.`idOrcamentoBase` = `ohf2`.`idOrcamentoBase`))) left join `develop`.`orcamento_has_followup` `ohf` on((`ohf2`.`idFollowup` = `ohf`.`idFollowup`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_ordemcompra` AS select `vp`.`idVenda` AS `idVenda`,`vp`.`idVendaProduto1` AS `idVendaProduto1`,`vp`.`status` AS `status`,`vp`.`fornecedor` AS `fornecedor`,`vp`.`produto` AS `produto`,`vp`.`codComercial` AS `codComercial`,`vp`.`quant` AS `quant`,`vp`.`un` AS `un`,`vp`.`caixas` AS `caixas`,`vp`.`formComercial` AS `formComercial`,`vp`.`obs` AS `obs` from `develop`.`venda_has_produto` `vp`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_ordemcompra_nfe` AS select `pf2`.`idCompra` AS `idCompra`,`pf2`.`ordemCompra` AS `ordemCompra`,`n`.`numeroNFe` AS `numeroNFe`,`n`.`idNFe` AS `idNFe` from (((`develop`.`pedido_fornecedor_has_produto2` `pf2` left join `develop`.`estoque_has_compra` `ehc` on((`pf2`.`idPedido2` = `ehc`.`idPedido2`))) left join `develop`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) left join `develop`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) where (`n`.`idNFe` is not null) group by `pf2`.`ordemCompra`,`n`.`idNFe`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_ordemcompra_resumo` AS select `v`.`idVenda` AS `Venda`,`v`.`status` AS `Status`,`pf`.`ordemCompra` AS `OC` from (`develop`.`venda` `v` left join `develop`.`pedido_fornecedor_has_produto` `pf` on((`v`.`idVenda` = `pf`.`idVenda`))) where ((`v`.`devolucao` = FALSE) and (`v`.`representacao` = FALSE));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_pagamento_loja` AS select `fpl`.`idLoja` AS `idLoja`,`fpl`.`idPagamento` AS `idPagamento`,`fp`.`pagamento` AS `pagamento` from (`develop`.`forma_pagamento` `fp` left join `develop`.`forma_pagamento_has_loja` `fpl` on((`fp`.`idPagamento` = `fpl`.`idPagamento`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_preparar_entrega` AS select `vhp`.`idEvento` AS `idEvento`,`vp2`.`idVenda` AS `idVenda`,count(`vp2`.`idVenda`) AS `produtos`,group_concat(distinct `vp2`.`idNFeSaida` separator ',') AS `idNFeSaida` from (`develop`.`veiculo_has_produto` `vhp` left join `develop`.`venda_has_produto2` `vp2` on((`vhp`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) group by `vp2`.`idVenda`,`vhp`.`idEvento`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_produto_pendente` AS select `vp2`.`status` AS `status`,`vp2`.`idVendaProduto2` AS `idVendaProduto2`,`vp2`.`idVendaProdutoFK` AS `idVendaProdutoFK`,`vp2`.`fornecedor` AS `fornecedor`,`vp2`.`idVenda` AS `idVenda`,`vp2`.`idProduto` AS `idProduto`,`vp2`.`produto` AS `produto`,`vp2`.`obs` AS `obs`,`p`.`colecao` AS `colecao`,`vp2`.`formComercial` AS `formComercial`,`vp2`.`quant` AS `quant`,`vp2`.`un` AS `un`,`p`.`un2` AS `Un2`,`vp2`.`caixas` AS `Caixas`,`p`.`kgcx` AS `kgcx`,`vp2`.`codComercial` AS `codComercial`,`p`.`codBarras` AS `codBarras`,`p`.`custo` AS `custo`,`vp2`.`idCompra` AS `idCompra` from (`develop`.`venda_has_produto2` `vp2` left join `develop`.`produto` `p` on((`vp2`.`idProduto` = `p`.`idProduto`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_recebimento` AS select cast(min((`v`.`data` + interval `v`.`prazoEntrega` day)) as date) AS `prazoEntrega`,group_concat(distinct `pf`.`dataPrevReceb` separator ',') AS `dataPrevReceb`,`e`.`idEstoque` AS `idEstoque`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco`,group_concat(distinct `pf`.`fornecedor` separator ',') AS `fornecedor`,group_concat(distinct `n`.`numeroNFe` separator ',') AS `numeroNFe`,group_concat(distinct `pf`.`idVenda` separator ', ') AS `idVenda`,group_concat(distinct `pf`.`ordemCompra` separator ', ') AS `ordemCompra`,`e`.`descricao` AS `produto`,`e`.`codComercial` AS `codComercial`,abs(`e`.`quant`) AS `quant`,`e`.`un` AS `un`,`e`.`caixas` AS `caixas` from ((((`develop`.`estoque` `e` left join `develop`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `develop`.`pedido_fornecedor_has_produto2` `pf` on((`ehc`.`idPedido2` = `pf`.`idPedido2`))) left join `develop`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `develop`.`venda` `v` on((`v`.`idVenda` = `pf`.`idVenda`))) where ((`e`.`status` = 'EM RECEBIMENTO') and (`pf`.`dataPrevReceb` is not null) and (`pf`.`idPedido2` is not null)) group by `e`.`idEstoque` desc order by `prazoEntrega`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_relatorio` AS select `c`.`Loja` AS `Loja`,`c`.`Vendedor` AS `Vendedor`,group_concat(distinct `c`.`idUsuario` separator ',') AS `idUsuario`,`c`.`idVenda` AS `idVenda`,group_concat(distinct `c`.`Mês` separator ',') AS `Mês`,sum(`c`.`Faturamento`) AS `Faturamento`,sum(`c`.`Comissão`) AS `Comissão`,`c`.`%` AS `%` from `develop`.`comissao` `c` group by `c`.`Loja`,`c`.`Vendedor`,`c`.`idVenda`,`c`.`%` order by `c`.`Loja`,`c`.`Vendedor`,`c`.`idVenda`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_relatorio_loja` AS select `v`.`Loja` AS `Loja`,sum(`v`.`Faturamento`) AS `Faturamento`,sum(`v`.`Comissão`) AS `Comissão`,((sum(`v`.`Comissão`) / sum(`v`.`Faturamento`)) * 100) AS `%`,`v`.`Mês` AS `Mês`,(group_concat(distinct `x`.`custo` separator ',') + 0.0) AS `Reposição` from ((((`develop`.`view_relatorio` `v` left join `develop`.`usuario` `u` on((`v`.`Vendedor` = `u`.`nome`))) left join `develop`.`venda` `v2` on((`v`.`idVenda` = `v2`.`idVenda`))) left join `develop`.`loja` `l` on((`v2`.`idLoja` = `l`.`idLoja`))) left join (select `l`.`descricao` AS `loja`,date_format(`v`.`data`,'%Y-%m') AS `data`,sum((`vp`.`quant` * `p`.`custo`)) AS `custo` from (((`develop`.`venda_has_produto` `vp` left join `develop`.`produto` `p` on((`vp`.`idProduto` = `p`.`idProduto`))) left join `develop`.`venda` `v` on((`vp`.`idVenda` = `v`.`idVenda`))) left join `develop`.`loja` `l` on((`v`.`idLoja` = `l`.`idLoja`))) where (`vp`.`reposicaoEntrega` = TRUE) group by `l`.`descricao`,date_format(`v`.`data`,'%Y-%m') order by `l`.`descricao`,date_format(`v`.`data`,'%Y-%m')) `x` on(((`x`.`loja` = `v`.`Loja`) and (convert(`x`.`data` using utf8) = `v`.`Mês`)))) group by `v`.`Loja`,`v`.`Mês`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_relatorio_nfe` AS select extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/ide/cNF') AS `NFe`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/ide/serie') AS `Serie`,date_format(extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/ide/dhEmi'),'%d/%m/%Y') AS `DataEmi`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vProd') AS `produtos`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vST') AS `icmsst`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vFrete') AS `frete`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vSeg') AS `seguro`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vIPI') AS `ipi`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vOutro') AS `outrasdesp`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vDesc') AS `desconto`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vNF') AS `totalnfe`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vBC') AS `bcicms`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vICMS') AS `icms`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vCOFINS') AS `cofins`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vII') AS `impimp`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/total/ICMSTot/vPIS') AS `pis`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/NFe/infNFe/ide/cNF') AS `tipo`,`develop`.`nfe`.`tipo` AS `e/s`,extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/protNFe/infProt/nProt') AS `nprotocolo`,date_format(extractvalue(`develop`.`nfe`.`xml`,'/nfeProc/protNFe/infProt/dhRecbto'),'%d/%m/%Y') AS `dataautor`,`develop`.`nfe`.`created` AS `Criado em`,`develop`.`nfe`.`status` AS `status` from `develop`.`nfe` where ((`develop`.`nfe`.`tipo` = 'SAIDA') and (`develop`.`nfe`.`status` = 'AUTORIZADO'));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_relatorio_pagar` AS select `cp`.`dataEmissao` AS `dataEmissao`,`cp`.`dataRealizado` AS `dataRealizado`,`cp`.`valorReal` AS `valorReal`,concat(`lhc`.`banco`,' - ',`lhc`.`agencia`,' - ',`lhc`.`conta`) AS `Conta`,`cp`.`observacao` AS `observacao`,`cp`.`contraParte` AS `contraParte`,`cp`.`grupo` AS `grupo`,`cp`.`subGrupo` AS `subGrupo` from ((`develop`.`conta_a_pagar_has_pagamento` `cp` left join `develop`.`nfe` `n` on((`cp`.`nfe` = `n`.`idNFe`))) left join `develop`.`loja_has_conta` `lhc` on((`cp`.`contaDestino` = `lhc`.`idConta`))) where ((`cp`.`dataRealizado` is not null) and (`cp`.`valorReal` is not null) and (`cp`.`status` = 'PAGO') and (`cp`.`desativado` = FALSE)) order by `cp`.`dataRealizado`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_relatorio_receber` AS select `cr`.`dataEmissao` AS `dataEmissao`,`cr`.`dataRealizado` AS `dataRealizado`,`cr`.`valorReal` AS `valorReal`,concat(`lhc`.`banco`,' - ',`lhc`.`agencia`,' - ',`lhc`.`conta`) AS `Conta`,`cr`.`observacao` AS `observacao`,`cr`.`contraParte` AS `contraParte`,`cr`.`grupo` AS `grupo`,`cr`.`subGrupo` AS `subGrupo` from (`develop`.`conta_a_receber_has_pagamento` `cr` left join `develop`.`loja_has_conta` `lhc` on((`cr`.`contaDestino` = `lhc`.`idConta`))) where ((`cr`.`valorReal` is not null) and (`cr`.`status` = 'RECEBIDO') and (`cr`.`desativado` = FALSE)) order by `cr`.`dataRealizado`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`view_relatorio_vendedor` AS select `c`.`Loja` AS `Loja`,group_concat(distinct `c`.`Vendedor` separator ',') AS `Vendedor`,group_concat(distinct `c`.`idUsuario` separator ',') AS `idUsuario`,sum(`c`.`Faturamento`) AS `Faturamento`,sum(`c`.`Comissão`) AS `Comissão`,((sum(`c`.`Comissão`) / sum(`c`.`Faturamento`)) * 100) AS `%`,`c`.`Mês` AS `Mês` from (`develop`.`comissao` `c` left join `develop`.`usuario` `u` on((`c`.`Vendedor` = `u`.`nome`))) group by `u`.`nome`,`c`.`Loja`,`c`.`Mês` order by `c`.`Loja`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_validacao` AS select `v`.`idVenda` AS `idVenda`,((`v`.`subTotalLiq` + `v`.`frete`) - `v`.`descontoReais`) AS `soma`,`v`.`total` AS `total`,sum(`c`.`valor`) AS `Pag.`,(sum(`c`.`valor`) - `v`.`total`) AS `Dif` from (`develop`.`venda` `v` left join `develop`.`conta_a_receber_has_pagamento` `c` on((`v`.`idVenda` = `c`.`idVenda`))) where ((`c`.`tipo` <> 'Crédito') and (not((`v`.`idVenda` like '%D'))) and (`c`.`desativado` = FALSE)) group by `c`.`idVenda`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_venda` AS select `v`.`idLoja` AS `idLoja`,`v`.`idUsuario` AS `idUsuario`,`v`.`idUsuarioConsultor` AS `idUsuarioConsultor`,`v`.`statusFinanceiro` AS `statusFinanceiro`,`v`.`dataFinanceiro` AS `dataFinanceiro`,`v`.`status` AS `Status`,((to_days(`v`.`data`) + `v`.`prazoEntrega`) - to_days(now())) AS `Dias restantes`,`v`.`idVenda` AS `Código`,group_concat(distinct `pf`.`ordemRepresentacao` separator ',') AS `OC Rep`,`u`.`nome` AS `Vendedor`,`u2`.`nome` AS `Consultor`,`c`.`nome_razao` AS `Cliente`,`p`.`nome_razao` AS `Profissional`,`v`.`data` AS `Data`,`v`.`total` AS `Total R$`,group_concat(distinct `vhf`.`dataFollowup` separator ',') AS `Data Followup`,group_concat(distinct `vhf`.`observacao` separator ',') AS `Observação` from ((((((((`develop`.`venda` `v` left join `develop`.`pedido_fornecedor_has_produto` `pf` on((`v`.`idVenda` = `pf`.`idVenda`))) left join `develop`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`))) left join `develop`.`cliente_has_endereco` `e` on((`v`.`idEnderecoEntrega` = `e`.`idEndereco`))) left join `develop`.`profissional` `p` on((`v`.`idProfissional` = `p`.`idProfissional`))) left join `develop`.`cliente` `c` on((`v`.`idCliente` = `c`.`idCliente`))) left join `develop`.`usuario` `u2` on((`v`.`idUsuarioConsultor` = `u2`.`idUsuario`))) left join (select max(`tmp`.`idFollowup`) AS `idFollowup`,`tmp`.`idVendaBase` AS `idVendaBase` from `develop`.`venda_has_followup` `tmp` group by `tmp`.`idVendaBase`) `vhf2` on((`v`.`idVendaBase` = `vhf2`.`idVendaBase`))) left join `develop`.`venda_has_followup` `vhf` on((`vhf2`.`idFollowup` = `vhf`.`idFollowup`))) group by `v`.`idVenda`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `develop`.`view_venda_produto` AS select `vp2`.`status` AS `status`,`v`.`statusFinanceiro` AS `statusFinanceiro`,`v`.`dataFinanceiro` AS `dataFinanceiro`,`vp2`.`idVenda` AS `idVenda`,substring_index(`u`.`nome`,' ',1) AS `Vendedor`,`vp2`.`fornecedor` AS `fornecedor`,`vp2`.`produto` AS `produto`,`vp2`.`obs` AS `obs`,`vp2`.`caixas` AS `caixas`,`vp2`.`quant` AS `quant`,`vp2`.`un` AS `un`,`vp2`.`codComercial` AS `codComercial`,`vp2`.`formComercial` AS `formComercial`,cast((`v`.`data` + interval `v`.`prazoEntrega` day) as date) AS `prazoEntrega`,cast((`v`.`data` + interval `v`.`novoPrazoEntrega` day) as date) AS `novoPrazoEntrega` from ((`develop`.`venda` `v` left join `develop`.`venda_has_produto2` `vp2` on((`v`.`idVenda` = `vp2`.`idVenda`))) left join `develop`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `develop`.`xxxx` AS select `ehc`.`idVendaProduto2` AS `idVendaProduto2`,max(`bbb`.`percst`) AS `percST` from ((`develop`.`estoque_has_consumo` `ehc` left join `develop`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) left join `develop`.`bbb` on((`bbb`.`idNFe` = `e`.`idNFe`))) where ((`bbb`.`percst` is not null) and (`ehc`.`idVendaProduto2` is not null)) group by `ehc`.`idVendaProduto2`;
USE `mydb` ;
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_compras_financeiro` (`Status` INT, `Status Financeiro` INT, `OC` INT, `Compra` INT, `Fornecedor` INT, `Produtos` INT, `Total` INT, `Código` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_fluxo_caixa_pendente`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_fluxo_caixa_pendente` (`Compra/Venda` INT, `dataPagamento` INT, `dataRealizado` INT, `R$` INT, `Tipo` INT, `contaDestino` INT, `idConta` INT, `Status` INT, `ENT/SAI` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_fluxo_caixa_realizado`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_fluxo_caixa_realizado` (`Compra/Venda` INT, `dataRealizado` INT, `R$` INT, `Tipo` INT, `contaDestino` INT, `idConta` INT, `Status` INT, `ENT/SAI` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_fluxo_resumo_pendente`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_fluxo_resumo_pendente` (`dataPagamento` INT, `SAIDA` INT, `ENTRADA` INT, `R$` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_fluxo_resumo_realizado`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_fluxo_resumo_realizado` (`dataRealizado` INT, `contaDestino` INT, `idConta` INT, `SAIDA` INT, `ENTRADA` INT, `R$` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_grafico_loja`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_grafico_loja` (`idLoja` INT, `dia` INT, `mes12` INT, `mes11` INT, `mes10` INT, `mes9` INT, `mes8` INT, `mes7` INT, `mes6` INT, `mes5` INT, `mes4` INT, `mes3` INT, `mes2` INT, `mes1` INT, `mes0` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_grafico_lojas`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_grafico_lojas` (`dia` INT, `mes12` INT, `mes11` INT, `mes10` INT, `mes9` INT, `mes8` INT, `mes7` INT, `mes6` INT, `mes5` INT, `mes4` INT, `mes3` INT, `mes2` INT, `mes1` INT, `mes0` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_ordemcompra` (`idVenda` INT, `idVendaProduto` INT, `status` INT, `fornecedor` INT, `produto` INT, `codComercial` INT, `quant` INT, `un` INT, `caixas` INT, `formComercial` INT, `obs` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_ordemcompra_nfe`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_ordemcompra_nfe` (`idCompra` INT, `ordemCompra` INT, `numeroNFe` INT, `idNFe` INT);

-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`view_ordemcompra_resumo`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_ordemcompra_resumo` (`Venda` INT, `Status` INT, `OC` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_produto_estoque` (`idVendaProduto` INT, `idProduto` INT, `fornecedor` INT, `produto` INT, `descUnitario` INT, `caixas` INT, `quant` INT, `un` INT, `unCaixa` INT, `codComercial` INT, `formComercial` INT, `total` INT, `codBarras` INT, `ncm` INT, `cfop` INT, `cfopEntrada` INT, `unTrib` INT, `numeroPedido` INT, `itemPedido` INT, `tipoICMS` INT, `tipoICMSEntrada` INT, `orig` INT, `cstICMS` INT, `modBC` INT, `vBC` INT, `pICMS` INT, `vICMS` INT, `modBCST` INT, `pMVAST` INT, `vBCST` INT, `pICMSST` INT, `vICMSST` INT, `cEnq` INT, `cstIPI` INT, `cstPIS` INT, `vBCPIS` INT, `pPIS` INT, `vPIS` INT, `cstCOFINS` INT, `vBCCOFINS` INT, `pCOFINS` INT, `vCOFINS` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_relatorio` (`Loja` INT, `Vendedor` INT, `idUsuario` INT, `idVenda` INT, `Mês` INT, `Data` INT, `Faturamento` INT, `Comissão` INT, `%` INT);
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mydb`.`view_agendar_coleta` AS select cast(min((`v`.`data` + interval `v`.`prazoEntrega` day)) as date) AS `prazoEntrega`,group_concat(distinct `pf`.`dataRealFat` separator ',') AS `dataRealFat`,`e`.`idEstoque` AS `idEstoque`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco`,group_concat(distinct `pf`.`fornecedor` separator ',') AS `fornecedor`,if(((upper(`e`.`un`) = 'M²') or (upper(`e`.`un`) = 'M2') or (upper(`e`.`un`) = 'ML')),`p`.`m2cx`,`p`.`pccx`) AS `unCaixa`,`p`.`formComercial` AS `formComercial`,`p`.`idProduto` AS `idProduto`,`n`.`numeroNFe` AS `numeroNFe`,`n`.`idNFe` AS `idNFe`,group_concat(distinct `pf`.`idVenda` separator ',') AS `idVenda`,group_concat(distinct `pf`.`ordemCompra` separator ',') AS `ordemCompra`,`e`.`descricao` AS `produto`,`e`.`codComercial` AS `codComercial`,abs(`e`.`quant`) AS `quant`,`e`.`un` AS `un`,`e`.`caixas` AS `caixas`,group_concat(distinct `pf`.`kgcx` separator ',') AS `kgcx`,`n`.`transportadora` AS `Transportadora`,(`e`.`caixas` * max(`pf`.`kgcx`)) AS `Peso` from (((((`mydb`.`estoque` `e` left join `mydb`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `mydb`.`pedido_fornecedor_has_produto` `pf` on((`ehc`.`idPedido` = `pf`.`idPedido`))) left join `mydb`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `mydb`.`venda` `v` on((`v`.`idVenda` = `pf`.`idVenda`))) left join `mydb`.`produto` `p` on((`e`.`idProduto` = `p`.`idProduto`))) where ((`e`.`status` = 'EM COLETA') and isnull(`pf`.`dataPrevColeta`) and (`pf`.`idPedido` is not null)) group by `e`.`idEstoque` desc order by `prazoEntrega`;
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_compras_financeiro` AS select group_concat(distinct `pf`.`status` separator ', ') AS `Status`,group_concat(distinct `pf`.`statusFinanceiro` separator ', ') AS `Status Financeiro`,group_concat(distinct `pf`.`ordemCompra` separator ', ') AS `OC`,`pf`.`idCompra` AS `Compra`,group_concat(distinct `pf`.`fornecedor` separator ', ') AS `Fornecedor`,count(0) AS `Produtos`,sum(`pf`.`preco`) AS `Total`,group_concat(distinct `pf`.`idVenda` separator ', ') AS `Código` from `mydb`.`pedido_fornecedor_has_produto` `pf` where (`pf`.`idCompra` is not null) group by `pf`.`idCompra`;
...
-- -----------------------------------------------------
-- View `mydb`.`view_fluxo_caixa_pendente`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`view_fluxo_caixa_pendente`;
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_fluxo_caixa_pendente` AS select `cp`.`idCompra` AS `Compra/Venda`,`cp`.`dataPagamento` AS `dataPagamento`,`cp`.`dataRealizado` AS `dataRealizado`,(`cp`.`valor` * -(1)) AS `R$`,`cp`.`tipo` AS `Tipo`,(select concat(`mydb`.`loja_has_conta`.`banco`,' - ',`mydb`.`loja_has_conta`.`agencia`,' - ',`mydb`.`loja_has_conta`.`conta`) from `mydb`.`loja_has_conta` where (`mydb`.`loja_has_conta`.`idConta` = `cp`.`contaDestino`)) AS `contaDestino`,`cp`.`contaDestino` AS `idConta`,`cp`.`status` AS `Status`,'SAIDA' AS `ENT/SAI` from `mydb`.`conta_a_pagar_has_pagamento` `cp` where ((`cp`.`status` in ('PENDENTE','CONFERIDO')) and (`cp`.`desativado` = FALSE)) union all select `cr`.`idVenda` AS `idVenda`,`cr`.`dataPagamento` AS `dataPagamento`,`cr`.`dataRealizado` AS `dataRealizado`,`cr`.`valor` AS `valor`,`cr`.`tipo` AS `tipo`,(select concat(`mydb`.`loja_has_conta`.`banco`,' - ',`mydb`.`loja_has_conta`.`agencia`,' - ',`mydb`.`loja_has_conta`.`conta`) from `mydb`.`loja_has_conta` where (`mydb`.`loja_has_conta`.`idConta` = `cr`.`contaDestino`)) AS `Name_exp_15`,`cr`.`contaDestino` AS `contaDestino`,`cr`.`status` AS `status`,'ENTRADA' AS `ENTRADA` from `mydb`.`conta_a_receber_has_pagamento` `cr` where ((`cr`.`status` in ('PENDENTE','CONFERIDO')) and (`cr`.`desativado` = FALSE) and (`cr`.`representacao` = FALSE)) order by `dataPagamento`;

-- -----------------------------------------------------
-- View `mydb`.`view_fluxo_caixa_realizado`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`view_fluxo_caixa_realizado`;
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_fluxo_caixa_realizado` AS select `cp`.`idCompra` AS `Compra/Venda`,`cp`.`dataRealizado` AS `dataRealizado`,(`cp`.`valorReal` * -(1)) AS `R$`,`cp`.`tipo` AS `Tipo`,(select concat(`mydb`.`loja_has_conta`.`banco`,' - ',`mydb`.`loja_has_conta`.`agencia`,' - ',`mydb`.`loja_has_conta`.`conta`) from `mydb`.`loja_has_conta` where (`mydb`.`loja_has_conta`.`idConta` = `cp`.`contaDestino`)) AS `contaDestino`,`cp`.`contaDestino` AS `idConta`,`cp`.`status` AS `Status`,'SAIDA' AS `ENT/SAI` from `mydb`.`conta_a_pagar_has_pagamento` `cp` where ((`cp`.`status` = 'PAGO') and (`cp`.`desativado` = FALSE)) union all select `cr`.`idVenda` AS `idVenda`,`cr`.`dataRealizado` AS `dataRealizado`,`cr`.`valorReal` AS `valorReal`,`cr`.`tipo` AS `tipo`,(select concat(`mydb`.`loja_has_conta`.`banco`,' - ',`mydb`.`loja_has_conta`.`agencia`,' - ',`mydb`.`loja_has_conta`.`conta`) from `mydb`.`loja_has_conta` where (`mydb`.`loja_has_conta`.`idConta` = `cr`.`contaDestino`)) AS `Name_exp_15`,`cr`.`contaDestino` AS `contaDestino`,`cr`.`status` AS `status`,'ENTRADA' AS `ENTRADA` from `mydb`.`conta_a_receber_has_pagamento` `cr` where ((`cr`.`status` = 'RECEBIDO') and (`cr`.`desativado` = FALSE)) order by `dataRealizado`;

-- -----------------------------------------------------
-- View `mydb`.`view_fluxo_resumo_pendente`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`view_fluxo_resumo_pendente`;
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_fluxo_resumo_pendente` AS select `v`.`dataPagamento` AS `dataPagamento`,sum(if((`v`.`ENT/SAI` = 'SAIDA'),`v`.`R$`,0)) AS `SAIDA`,sum(if((`v`.`ENT/SAI` = 'ENTRADA'),`v`.`R$`,0)) AS `ENTRADA`,sum(`v`.`R$`) AS `R$` from `mydb`.`view_fluxo_caixa_pendente` `v` group by `v`.`dataPagamento` order by `v`.`dataPagamento`;

-- -----------------------------------------------------
-- View `mydb`.`view_fluxo_resumo_realizado`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`view_fluxo_resumo_realizado`;
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_fluxo_resumo_realizado` AS select `v`.`dataRealizado` AS `dataRealizado`,`v`.`contaDestino` AS `contaDestino`,`v`.`idConta` AS `idConta`,coalesce(sum(if((`v`.`ENT/SAI` = 'SAIDA'),`v`.`R$`,0)),0) AS `SAIDA`,coalesce(sum(if((`v`.`ENT/SAI` = 'ENTRADA'),`v`.`R$`,0)),0) AS `ENTRADA`,coalesce(sum(`v`.`R$`),0) AS `R$` from `mydb`.`view_fluxo_caixa_realizado` `v` where (`v`.`Status` in ('PAGO','RECEBIDO')) group by `v`.`dataRealizado`,`v`.`idConta`,`v`.`contaDestino` order by `v`.`dataRealizado`;
...
-- -----------------------------------------------------
-- View `mydb`.`view_grafico_loja`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`view_grafico_loja`;
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mydb`.`view_grafico_loja` AS select `v`.`idLoja` AS `idLoja`,dayofmonth(`v`.`data`) AS `dia`,sum((case when ((month(`v`.`data`) = month((now() - interval 12 month))) and (year(`v`.`data`) = year((now() - interval 12 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes12`,sum((case when ((month(`v`.`data`) = month((now() - interval 11 month))) and (year(`v`.`data`) = year((now() - interval 11 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes11`,sum((case when ((month(`v`.`data`) = month((now() - interval 10 month))) and (year(`v`.`data`) = year((now() - interval 10 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes10`,sum((case when ((month(`v`.`data`) = month((now() - interval 9 month))) and (year(`v`.`data`) = year((now() - interval 9 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes9`,sum((case when ((month(`v`.`data`) = month((now() - interval 8 month))) and (year(`v`.`data`) = year((now() - interval 8 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes8`,sum((case when ((month(`v`.`data`) = month((now() - interval 7 month))) and (year(`v`.`data`) = year((now() - interval 7 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes7`,sum((case when ((month(`v`.`data`) = month((now() - interval 6 month))) and (year(`v`.`data`) = year((now() - interval 6 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes6`,sum((case when ((month(`v`.`data`) = month((now() - interval 5 month))) and (year(`v`.`data`) = year((now() - interval 5 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes5`,sum((case when ((month(`v`.`data`) = month((now() - interval 4 month))) and (year(`v`.`data`) = year((now() - interval 4 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes4`,sum((case when ((month(`v`.`data`) = month((now() - interval 3 month))) and (year(`v`.`data`) = year((now() - interval 3 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes3`,sum((case when ((month(`v`.`data`) = month((now() - interval 2 month))) and (year(`v`.`data`) = year((now() - interval 2 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes2`,sum((case when ((month(`v`.`data`) = month((now() - interval 1 month))) and (year(`v`.`data`) = year((now() - interval 1 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes1`,sum((case when ((month(`v`.`data`) = month(now())) and (year(`v`.`data`) = year(now()))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes0` from `mydb`.`venda` `v` where (`v`.`status` <> 'CANCELADO') group by dayofmonth(`v`.`data`),`v`.`idLoja`;

-- -----------------------------------------------------
-- View `mydb`.`view_grafico_lojas`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`view_grafico_lojas`;
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mydb`.`view_grafico_lojas` AS select dayofmonth(`v`.`data`) AS `dia`,sum((case when ((month(`v`.`data`) = month((now() - interval 12 month))) and (year(`v`.`data`) = year((now() - interval 12 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes12`,sum((case when ((month(`v`.`data`) = month((now() - interval 11 month))) and (year(`v`.`data`) = year((now() - interval 11 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes11`,sum((case when ((month(`v`.`data`) = month((now() - interval 10 month))) and (year(`v`.`data`) = year((now() - interval 10 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes10`,sum((case when ((month(`v`.`data`) = month((now() - interval 9 month))) and (year(`v`.`data`) = year((now() - interval 9 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes9`,sum((case when ((month(`v`.`data`) = month((now() - interval 8 month))) and (year(`v`.`data`) = year((now() - interval 8 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes8`,sum((case when ((month(`v`.`data`) = month((now() - interval 7 month))) and (year(`v`.`data`) = year((now() - interval 7 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes7`,sum((case when ((month(`v`.`data`) = month((now() - interval 6 month))) and (year(`v`.`data`) = year((now() - interval 6 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes6`,sum((case when ((month(`v`.`data`) = month((now() - interval 5 month))) and (year(`v`.`data`) = year((now() - interval 5 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes5`,sum((case when ((month(`v`.`data`) = month((now() - interval 4 month))) and (year(`v`.`data`) = year((now() - interval 4 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes4`,sum((case when ((month(`v`.`data`) = month((now() - interval 3 month))) and (year(`v`.`data`) = year((now() - interval 3 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes3`,sum((case when ((month(`v`.`data`) = month((now() - interval 2 month))) and (year(`v`.`data`) = year((now() - interval 2 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes2`,sum((case when ((month(`v`.`data`) = month((now() - interval 1 month))) and (year(`v`.`data`) = year((now() - interval 1 month)))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes1`,sum((case when ((month(`v`.`data`) = month(now())) and (year(`v`.`data`) = year(now()))) then (`v`.`total` - `v`.`frete`) else 0 end)) AS `mes0` from `mydb`.`venda` `v` where (`v`.`status` <> 'CANCELADO') group by dayofmonth(`v`.`data`);
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mydb`.`view_ordemcompra` AS select `vp`.`idVenda` AS `idVenda`,`vp`.`idVendaProduto` AS `idVendaProduto`,`vp`.`status` AS `status`,`vp`.`fornecedor` AS `fornecedor`,`vp`.`produto` AS `produto`,`vp`.`codComercial` AS `codComercial`,`vp`.`quant` AS `quant`,`vp`.`un` AS `un`,`vp`.`caixas` AS `caixas`,`vp`.`formComercial` AS `formComercial`,`vp`.`obs` AS `obs` from `mydb`.`venda_has_produto` `vp`;
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_ordemcompra_nfe` AS select `pf`.`idCompra` AS `idCompra`,`pf`.`ordemCompra` AS `ordemCompra`,`n`.`numeroNFe` AS `numeroNFe`,`n`.`idNFe` AS `idNFe` from (((`mydb`.`pedido_fornecedor_has_produto` `pf` left join `mydb`.`estoque_has_compra` `ehc` on((`pf`.`idPedido` = `ehc`.`idPedido`))) left join `mydb`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) left join `mydb`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) where (`n`.`idNFe` is not null) group by `pf`.`ordemCompra`,`n`.`idNFe`;
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_ordemcompra_resumo` AS select `v`.`idVenda` AS `Venda`,`v`.`status` AS `Status`,`pf`.`ordemCompra` AS `OC` from (`mydb`.`venda` `v` left join `mydb`.`pedido_fornecedor_has_produto` `pf` on((`v`.`idVenda` = `pf`.`idVenda`))) where ((`v`.`devolucao` = FALSE) and (`v`.`representacao` = FALSE));
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_produto_estoque` AS select `ehc`.`idVendaProduto` AS `idVendaProduto`,group_concat(distinct `ehc`.`idProduto` separator ',') AS `idProduto`,`vp`.`fornecedor` AS `fornecedor`,`vp`.`produto` AS `produto`,(`vp`.`total` / `vp`.`quant`) AS `descUnitario`,sum(`ehc`.`caixas`) AS `caixas`,sum((`ehc`.`quant` * -(1))) AS `quant`,group_concat(distinct `ehc`.`un` separator ',') AS `un`,`vp`.`unCaixa` AS `unCaixa`,group_concat(distinct `ehc`.`codComercial` separator ',') AS `codComercial`,`vp`.`formComercial` AS `formComercial`,sum((((`vp`.`total` / `vp`.`quant`) * `ehc`.`quant`) * -(1))) AS `total`,group_concat(distinct `ehc`.`codBarras` separator ',') AS `codBarras`,group_concat(distinct `ehc`.`ncm` separator ',') AS `ncm`,group_concat(distinct `ehc`.`cfop` separator ',') AS `cfop`,group_concat(distinct `e`.`cfop` separator '/') AS `cfopEntrada`,group_concat(distinct `ehc`.`unTrib` separator ',') AS `unTrib`,group_concat(distinct `ehc`.`numeroPedido` separator ',') AS `numeroPedido`,group_concat(distinct `ehc`.`itemPedido` separator ',') AS `itemPedido`,group_concat(distinct `ehc`.`tipoICMS` separator ',') AS `tipoICMS`,group_concat(distinct `e`.`tipoICMS` separator '/') AS `tipoICMSEntrada`,group_concat(distinct `ehc`.`orig` separator ',') AS `orig`,group_concat(distinct `ehc`.`cstICMS` separator ',') AS `cstICMS`,group_concat(distinct `ehc`.`modBC` separator ',') AS `modBC`,sum(`ehc`.`vBC`) AS `vBC`,group_concat(distinct `ehc`.`pICMS` separator ',') AS `pICMS`,sum(`ehc`.`vICMS`) AS `vICMS`,group_concat(distinct `ehc`.`modBCST` separator ',') AS `modBCST`,group_concat(distinct `ehc`.`pMVAST` separator ',') AS `pMVAST`,sum(`ehc`.`vBCST`) AS `vBCST`,group_concat(distinct `ehc`.`pICMSST` separator ',') AS `pICMSST`,sum(`ehc`.`vICMSST`) AS `vICMSST`,group_concat(distinct `ehc`.`cEnq` separator ',') AS `cEnq`,group_concat(distinct `ehc`.`cstIPI` separator ',') AS `cstIPI`,group_concat(distinct `ehc`.`cstPIS` separator ',') AS `cstPIS`,sum(`ehc`.`vBCPIS`) AS `vBCPIS`,group_concat(distinct `ehc`.`pPIS` separator ',') AS `pPIS`,sum(`ehc`.`vPIS`) AS `vPIS`,group_concat(distinct `ehc`.`cstCOFINS` separator ',') AS `cstCOFINS`,sum(`ehc`.`vBCCOFINS`) AS `vBCCOFINS`,group_concat(distinct `ehc`.`pCOFINS` separator ',') AS `pCOFINS`,sum(`ehc`.`vCOFINS`) AS `vCOFINS` from ((`mydb`.`venda_has_produto` `vp` left join `mydb`.`estoque_has_consumo` `ehc` on((`vp`.`idVendaProduto` = `ehc`.`idVendaProduto`))) left join `mydb`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) where ((`ehc`.`idEstoque` is not null) and (`ehc`.`status` in ('PRÉ-CONSUMO','CONSUMO'))) group by `vp`.`idVendaProduto`;
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mydb`.`view_relatorio` AS select `c`.`Loja` AS `Loja`,`c`.`Vendedor` AS `Vendedor`,group_concat(distinct `c`.`idUsuario` separator ',') AS `idUsuario`,`c`.`idVenda` AS `idVenda`,group_concat(distinct `c`.`Mês` separator ',') AS `Mês`,`v`.`data` AS `Data`,sum(`c`.`Faturamento`) AS `Faturamento`,sum(`c`.`Comissão`) AS `Comissão`,`c`.`%` AS `%` from (`mydb`.`comissao` `c` left join `mydb`.`venda` `v` on((`c`.`idVenda` = `v`.`idVenda`))) group by `c`.`Loja`,`c`.`Vendedor`,`c`.`idVenda`,`c`.`%` order by `c`.`Loja`,`c`.`Vendedor`,`c`.`idVenda`;
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mydb`.`view_relatorio_vendedor` AS select `c`.`Loja` AS `Loja`,group_concat(distinct `c`.`Vendedor` separator ',') AS `Vendedor`,group_concat(distinct `c`.`idUsuario` separator ',') AS `idUsuario`,sum(`c`.`Faturamento`) AS `Faturamento`,sum(`c`.`Comissão`) AS `Comissão`,((sum(`c`.`Comissão`) / sum(`c`.`Faturamento`)) * 100) AS `%`,`c`.`Mês` AS `Mês` from (`mydb`.`comissao` `c` left join `mydb`.`usuario` `u` on((`c`.`Vendedor` = `u`.`nome`))) group by `u`.`nome`,`c`.`Loja`,`c`.`Mês` order by `c`.`Loja`;
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mydb`.`view_resumo_relatorio` AS select `l`.`descricao` AS `Loja`,`u`.`user` AS `Vendedor`,sum(if(((convert(date_format(`o`.`data`,'%Y-%m') using utf8mb4) < convert(`MYDATE`() using utf8mb4)) and ((`o`.`status` in ('ATIVO','EXPIRADO')) or ((`o`.`status` = 'FECHADO') and (convert(date_format(`v`.`data`,'%Y-%m') using utf8mb4) >= convert(`MYDATE`() using utf8mb4))) or ((`o`.`status` = 'PERDIDO') and (convert(date_format(`o`.`lastUpdated`,'%Y-%m') using utf8mb4) >= convert(`MYDATE`() using utf8mb4))))),`o`.`total`,0)) AS `Validos Anteriores`,sum(if(((convert(date_format(`o`.`data`,'%Y-%m') using utf8mb4) = convert(`MYDATE`() using utf8mb4)) and isnull(`o_1`.`data`)),`o`.`total`,0)) AS `Gerados Mes`,sum(if(((convert(date_format(`o`.`data`,'%Y-%m') using utf8mb4) = convert(`MYDATE`() using utf8mb4)) and (`o_1`.`data` is not null)),`o`.`total`,0)) AS `Revalidados Mes`,sum(if(((convert(date_format(`v`.`data`,'%Y-%m') using utf8mb4) = convert(`MYDATE`() using utf8mb4)) and (`v`.`devolucao` = FALSE) and (`o`.`status` = 'FECHADO')),`o`.`total`,0)) AS `Fechados Mes`,sum(if(((convert(date_format(`o`.`lastUpdated`,'%Y-%m') using utf8mb4) = convert(`MYDATE`() using utf8mb4)) and (`o`.`status` = 'PERDIDO')),`o`.`total`,0)) AS `Perdidos Mes`,sum(if(((convert(date_format(`o`.`data`,'%Y-%m') using utf8mb4) <= convert(`MYDATE`() using utf8mb4)) and ((`o`.`status` = 'ATIVO') or (`o`.`status` = 'EXPIRADO') or ((`o`.`status` = 'FECHADO') and (convert(date_format(`v`.`data`,'%Y-%m') using utf8mb4) > convert(`MYDATE`() using utf8mb4))) or ((`o`.`status` = 'PERDIDO') and (convert(date_format(`o`.`lastUpdated`,'%Y-%m') using utf8mb4) > convert(`MYDATE`() using utf8mb4))))),`o`.`total`,0)) AS `Validos Mes`,((sum(if(((convert(date_format(`v`.`data`,'%Y-%m') using utf8mb4) = convert(`MYDATE`() using utf8mb4)) and (`o`.`status` = 'FECHADO')),`o`.`total`,0)) / (sum(if(((convert(date_format(`o`.`data`,'%Y-%m') using utf8mb4) = convert(`MYDATE`() using utf8mb4)) and isnull(`o_1`.`data`)),`o`.`total`,0)) + sum(if(((convert(date_format(`o`.`data`,'%Y-%m') using utf8mb4) = convert(`MYDATE`() using utf8mb4)) and (`o_1`.`data` is not null)),`o`.`total`,0)))) * 100) AS `% Fechados / Gerados`,((sum(if(((convert(date_format(`v`.`data`,'%Y-%m') using utf8mb4) = convert(`MYDATE`() using utf8mb4)) and (`o`.`status` = 'FECHADO')),`o`.`total`,0)) / ((sum(if(((convert(date_format(`o`.`data`,'%Y-%m') using utf8mb4) < convert(`MYDATE`() using utf8mb4)) and ((`o`.`status` = 'ATIVO') or (`o`.`status` = 'EXPIRADO') or ((`o`.`status` = 'FECHADO') and (convert(date_format(`v`.`data`,'%Y-%m') using utf8mb4) >= convert(`MYDATE`() using utf8mb4))) or ((`o`.`status` = 'PERDIDO') and (convert(date_format(`o`.`lastUpdated`,'%Y-%m') using utf8mb4) >= convert(`MYDATE`() using utf8mb4))))),`o`.`total`,0)) + sum(if(((convert(date_format(`o`.`data`,'%Y-%m') using utf8mb4) = convert(`MYDATE`() using utf8mb4)) and isnull(`o_1`.`data`)),`o`.`total`,0))) + sum(if(((convert(date_format(`o`.`data`,'%Y-%m') using utf8mb4) = convert(`MYDATE`() using utf8mb4)) and (`o_1`.`data` is not null)),`o`.`total`,0)))) * 100) AS `% Fechados / Carteira` from ((((`mydb`.`orcamento` `o` left join `mydb`.`loja` `l` on((`o`.`idLoja` = `l`.`idLoja`))) left join `mydb`.`usuario` `u` on((`o`.`idUsuario` = `u`.`idUsuario`))) left join `mydb`.`orcamento` `o_1` on((`o`.`replicadoDe` = `o_1`.`idOrcamento`))) left join `mydb`.`venda` `v` on((`o`.`idOrcamento` = `v`.`idOrcamento`))) where (`o`.`status` not in ('REPLICADO','CANCELADO')) group by `l`.`descricao`,`u`.`user` order by `l`.`descricao`,`u`.`user`;
...
CREATE
DEFINER=`root`@`%`
TRIGGER `develop`.`pedido_fornecedor_has_produto_AFTER_INSERT`
AFTER INSERT ON `develop`.`pedido_fornecedor_has_produto`
FOR EACH ROW
BEGIN
	CALL copy_pedido_fornecedor_has_produto(NEW.idPedido1);
END$$

USE `staccato`$$
CREATE
DEFINER=`root`@`%`
TRIGGER `develop`.`estoque_has_consumo_AFTER_DELETE`
AFTER DELETE ON `develop`.`estoque_has_consumo`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`root`@`%`
TRIGGER `develop`.`estoque_has_consumo_AFTER_INSERT`
AFTER INSERT ON `develop`.`estoque_has_consumo`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`root`@`%`
TRIGGER `develop`.`estoque_has_consumo_AFTER_UPDATE`
AFTER UPDATE ON `develop`.`estoque_has_consumo`
FOR EACH ROW
BEGIN
	CALL update_produto_estoque_quant(NEW.idEstoque);
	CALL update_quant_estoque(NEW.idEstoque);
...
CREATE
DEFINER=`root`@`%`
TRIGGER `develop`.`venda_has_produto_AFTER_DELETE`
AFTER DELETE ON `develop`.`venda_has_produto`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`root`@`%`
TRIGGER `develop`.`venda_has_produto_AFTER_INSERT`
AFTER INSERT ON `develop`.`venda_has_produto`
FOR EACH ROW
BEGIN
...
CREATE
DEFINER=`root`@`%`
TRIGGER `develop`.`venda_has_produto_AFTER_UPDATE`
AFTER UPDATE ON `develop`.`venda_has_produto`
FOR EACH ROW
BEGIN
 	IF(NEW.status = 'CANCELADO') THEN DELETE FROM comissao WHERE idVendaProduto1 = NEW.idVendaProduto1; END IF;
END$$

USE `staccato`$$
CREATE
DEFINER=`root`@`%`
TRIGGER `develop`.`pedido_fornecedor_has_produto2_AFTER_UPDATE`
AFTER UPDATE ON `develop`.`pedido_fornecedor_has_produto2`
FOR EACH ROW
BEGIN
	CALL update_pedido_fornecedor_status(NEW.idPedidoFK);
END$$
...
CREATE
DEFINER=`root`@`%`
TRIGGER `mydb`.`venda_has_produto_AFTER_DELETE`
AFTER DELETE ON `mydb`.`venda_has_produto`
FOR EACH ROW
BEGIN
	DELETE FROM comissao WHERE idVendaProduto = OLD.idVendaProduto;
END$$

USE `mydb`$$
CREATE
DEFINER=`root`@`%`
TRIGGER `mydb`.`venda_has_produto_AFTER_INSERT`
AFTER INSERT ON `mydb`.`venda_has_produto`
FOR EACH ROW
BEGIN
	INSERT INTO comissao (idVendaProduto, `Loja`, `Vendedor`, `idUsuario`, `idVenda`, `Mês`, `Faturamento`, `Comissão`, `%`) SELECT 
    vp.idVendaProduto,
    `l`.`descricao` AS `Loja`,
    `u`.`nome` AS `Vendedor`,
    `u`.`idUsuario` AS `idUsuario`,
...
FROM
    ...
    `fornecedor` `f` ON (`f`.`idFornecedor` = `p`.`idFornecedor`)
WHERE
 vp.idVendaProduto = NEW.idVendaProduto;
END$$

USE `mydb`$$
CREATE
DEFINER=`root`@`%`
TRIGGER `mydb`.`venda_has_produto_AFTER_UPDATE`
AFTER UPDATE ON `mydb`.`venda_has_produto`
FOR EACH ROW
BEGIN
	IF(NEW.status = 'CANCELADO') THEN DELETE FROM comissao WHERE idVendaProduto = NEW.idVendaProduto; END IF;
    IF(NEW.total <> OLD.total) THEN UPDATE comissao SET `Faturamento` = NEW.total, `Comissão` = NEW.total * `%` / 100 WHERE idVendaProduto = NEW.idVendaProduto; END IF;
END$$
...
SET @restante := (SELECT e.quant + COALESCE(SUM(ehc.quant), 0) 
					FROM estoque e 
					LEFT JOIN estoque_has_consumo ehc ON e.idEstoque = ehc.idEstoque 
					WHERE e.idEstoque = OLD.idEstoque GROUP BY ehc.idEstoque);
...
SET @restante := (SELECT e.quant + COALESCE(SUM(ehc.quant), 0) 
					FROM estoque e 
					LEFT JOIN estoque_has_consumo ehc ON e.idEstoque = ehc.idEstoque 
					WHERE e.idEstoque = NEW.idEstoque GROUP BY ehc.idEstoque);
