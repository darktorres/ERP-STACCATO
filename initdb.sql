...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_produto_estoque` AS select `vp2`.`idVendaProduto2` AS `idVendaProduto2`,`vp2`.`idRelacionado` AS `idRelacionado`,`vp2`.`idProduto` AS `idProduto`,`vp2`.`fornecedor` AS `fornecedor`,`vp2`.`produto` AS `produto`,if((`vp2`.`total` < 0.0),0,`vp2`.`descUnitario`) AS `descUnitario`,abs(`vp2`.`caixas`) AS `caixas`,abs(`vp2`.`quant`) AS `quant`,`vp2`.`un` AS `un`,if((`vp2`.`total` < 0.0),0,`vp2`.`total`) AS `total`,`vp2`.`quantCaixa` AS `quantCaixa`,`vp2`.`codComercial` AS `codComercial`,`vp2`.`formComercial` AS `formComercial`,coalesce(`p`.`codBarras`,'') AS `codBarras`,abs((`p`.`kgcx` * `vp2`.`caixas`)) AS `peso`,`p`.`ncm` AS `ncm`,coalesce(`n`.`cest`,'') AS `cest`,NULL AS `cfop`,NULL AS `unTrib`,NULL AS `numeroPedido`,NULL AS `itemPedido`,NULL AS `tipoICMS`,NULL AS `orig`,NULL AS `cstICMS`,NULL AS `modBC`,NULL AS `vBC`,NULL AS `pICMS`,NULL AS `vICMS`,NULL AS `modBCST`,NULL AS `pMVAST`,NULL AS `vBCST`,NULL AS `pICMSST`,NULL AS `vICMSST`,NULL AS `cEnq`,NULL AS `cstIPI`,NULL AS `cstPIS`,NULL AS `vBCPIS`,NULL AS `pPIS`,NULL AS `vPIS`,NULL AS `cstCOFINS`,NULL AS `vBCCOFINS`,NULL AS `pCOFINS`,NULL AS `vCOFINS` from ((`staccato`.`venda_has_produto2` `vp2` left join `staccato`.`produto` `p` on((`vp2`.`idProduto` = `p`.`idProduto`))) left join `staccato`.`ncm` `n` on((`p`.`ncm` = `n`.`ncm`)));
