...
bool bandSortBandLessThen(const BandDesignIntf *c1, const BandDesignIntf *c2) { return c1->geometry().top() < c2->geometry().top(); }

PageDesignIntf::PageDesignIntf(QObject *parent)
    : QGraphicsScene(parent), m_pageItem(0), m_insertMode(false), m_itemInsertRect(0), m_itemMode(DesignMode), m_cutterBorder(0), m_currentCommand(-1), m_changeSizeMode(false), m_changePosMode(false),
      m_changePosOrSizeMode(false), m_executingCommand(false), m_hasChanges(false), m_isLoading(false), m_executingGroupCommand(false), m_settings(0), m_selectionRect(0),
      m_verticalGridStep(Const::DEFAULT_GRID_STEP), m_horizontalGridStep(Const::DEFAULT_GRID_STEP), m_updating(false), m_currentObjectIndex(1), m_multiSelectStarted(false), m_movedItem(0),
      m_joinItem(0), m_magneticMovement(false), m_reportSettings(0), m_currentPage(0) {
...
PageDesignIntf::~PageDesignIntf() {
  ...
  foreach (PageItemDesignIntf::Ptr pageItem, m_reportPages) { removeItem(pageItem.data()); }
...
void PageDesignIntf::updatePageRect() {
  ...
  this->setSceneRect(-Const::SCENE_MARGIN, -Const::SCENE_MARGIN, pageItem()->geometry().width() + Const::SCENE_MARGIN * 2, pageItem()->boundingRect().height() + Const::SCENE_MARGIN * 2);
  emit sceneRectChanged(sceneRect());
}

void PageDesignIntf::keyPressEvent(QKeyEvent *event) {
  if (event->modifiers() == Qt::NoModifier ||
      (event->key() != Qt::Key_Left && event->key() != Qt::Key_Right && event->key() != Qt::Key_Up && event->key() != Qt::Key_Down && !m_changePosMode && !m_changeSizeMode)) {
...
void PageDesignIntf::startInsertMode(const QString &ItemType) {
  ...
  if (page) m_itemInsertRect->setParentItem(page);
}
...
PageItemDesignIntf *PageDesignIntf::pageItem() { return m_currentPage ? m_currentPage : m_pageItem.data(); }
...
void PageDesignIntf::setPageItems(QList<PageItemDesignIntf::Ptr> pages) {
  m_currentPage = 0;
  if (!m_pageItem.isNull()) {
    if (m_pageItem->scene() == this) removeItem(m_pageItem.data());
    m_pageItem.clear();
  }
  int curHeight = 0;
  int curWidth = 0;
  ...
  if (m_reportPages.count() > 0) m_currentPage = m_reportPages.at(0).data();
}
...
void PageDesignIntf::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if ((m_insertMode) && (page && page->rect().contains(page->mapFromScene(event->scenePos())))) {
    ...
    if (magneticMovement()) { rectMoved(QRectF(m_itemInsertRect->pos().x(), m_itemInsertRect->pos().y(), m_itemInsertRect->boundingRect().width(), m_itemInsertRect->boundingRect().height())); }
  } else {
...
void PageDesignIntf::setSelectionRect(QRectF selectionRect) {
  ...
  foreach (QGraphicsItem *item, items()) {
    if (selectionRect.intersects(item->mapRectToScene(item->boundingRect())))
      if (dynamic_cast<ItemDesignIntf *>(item)) item->setSelected(true);
  }
}

BaseDesignIntf *PageDesignIntf::addBand(const QString &bandType) { return internalAddBand(bandType); }

BaseDesignIntf *PageDesignIntf::addBand(BandDesignIntf::BandsType bandType) { return internalAddBand(bandType); }
...
template <typename T> BaseDesignIntf *PageDesignIntf::internalAddBand(T bandType) {
  ...
  needParentBands << BandDesignIntf::SubDetailBand << BandDesignIntf::SubDetailHeader << BandDesignIntf::SubDetailFooter << BandDesignIntf::GroupHeader << BandDesignIntf::GroupFooter
                  << BandDesignIntf::DataHeader << BandDesignIntf::DataFooter;
...
QPointF PageDesignIntf::placePosOnGrid(QPointF point) {
  return QPointF(div(point.x(), horizontalGridStep()).quot * horizontalGridStep(), div(point.y(), verticalGridStep()).quot * verticalGridStep());
}

QSizeF PageDesignIntf::placeSizeOnGrid(QSizeF size) {
  return QSizeF(div(size.width(), horizontalGridStep()).quot * horizontalGridStep(), div(size.height(), verticalGridStep()).quot * verticalGridStep());
}
...
BaseDesignIntf *PageDesignIntf::addReportItem(const QString &itemType, QPointF pos, QSizeF size) {
  ...
  if (band) {
    ...
    if (page) {
      ...
      if (ii) ii->setItemLocation(ItemDesignIntf::Page);
      return reportItem;
    }
  }
...
CommandIf::Ptr createBandDeleteCommand(PageDesignIntf *page, BandDesignIntf *band) {
  ...
  if (band->hasChildren()) {
    ...
    foreach (BandDesignIntf *curband, band->childBands()) { command->addCommand(createBandDeleteCommand(page, curband), false); }
    return command;
  } else {
...
CommandIf::Ptr PageDesignIntf::removeReportItemCommand(BaseDesignIntf *item) {
  ...
  if (band) {
    ...
    if (layout && (layout->childrenCount() == 2)) {
      ...
      CommandIf::Ptr command = (dynamic_cast<LayoutDesignIntf *>(item)) ? DeleteLayoutCommand::create(this, dynamic_cast<LayoutDesignIntf *>(item)) : DeleteItemCommand::create(this, item);
      return command;
    }
  }
}
...
bool PageDesignIntf::saveCommand(CommandIf::Ptr command, bool runCommand) {
  ...
  if (m_currentCommand < (m_commandsList.count() - 1)) m_commandsList.remove(m_currentCommand + 1, m_commandsList.size() - (m_currentCommand + 1));
...
void PageDesignIntf::dropEvent(QGraphicsSceneDragDropEvent *event) {
  if (event->mimeData()->hasText() && ((event->mimeData()->text().indexOf("field:") == 0) || (event->mimeData()->text().indexOf("variable:") == 0))) {
...
void PageDesignIntf::checkSizeOrPosChanges() {
  ...
  if ((selectedItems().count() > 0) && (m_positionStamp.count() > 0)) {
    ...
    if (!bandFound && (m_positionStamp[0].pos != selectedItems().at(0)->pos())) { posCommand = createChangePosCommand(); }
    m_positionStamp.clear();
  }
...
void PageDesignIntf::reactivatePageItem(PageItemDesignIntf::Ptr pageItem) {
  pageItem->setItemMode(itemMode());
  if (pageItem.data()->scene() != this) addItem(pageItem.data());
}
...
void PageDesignIntf::saveChangeProppertyCommand(const QString &objectName, const QString &propertyName, const QVariant &oldPropertyValue, const QVariant &newPropertyValue) {
  if (!m_executingCommand) {
    ...
    if (propertyName.compare("ItemAlign", Qt::CaseInsensitive) == 0) {
      command = PropertyItemAlignChangedCommand::create(this, objectName, BaseDesignIntf::ItemAlign(oldPropertyValue.toInt()), BaseDesignIntf::ItemAlign(newPropertyValue.toInt()));
    } else {
...
void PageDesignIntf::setCurrentPage(PageItemDesignIntf *currentPage) {
  if (m_currentPage != currentPage) {
    ...
    if (m_itemMode == DesignMode) { m_currentPage->setItemMode(DesignMode); }
  }
}
...
void PageDesignIntf::setMagneticMovement(bool magneticMovement) { m_magneticMovement = magneticMovement; }

int PageDesignIntf::horizontalGridStep() const { return m_horizontalGridStep; }

void PageDesignIntf::setHorizontalGridStep(int horizontalGridStep) { m_horizontalGridStep = horizontalGridStep; }
...
void PageDesignIntf::selectAllChildren(BaseDesignIntf *item) {
  if (item) {
    foreach (BaseDesignIntf *child, item->childBaseItems()) { child->setSelected(true); }
  }
}
...
void PageDesignIntf::rectMoved(QRectF itemRect, BaseDesignIntf *container) {
  ...
  if (container) {
    m_projections.clear();
    foreach (BaseDesignIntf *bi, container->childBaseItems()) { m_projections.append(ItemProjections(bi)); }
  }
...
void PageDesignIntf::itemMoved(BaseDesignIntf *item) {
  if (m_movedItem != item) {
    ...
    while (curItem) {
      ...
      if (!m_movedItemContainer) m_movedItemContainer = dynamic_cast<PageItemDesignIntf *>(curItem);
    ...
    if (m_movedItemContainer) {
      ...
      foreach (BaseDesignIntf *bi, m_movedItemContainer->childBaseItems()) {
        if (bi != item) m_projections.append(ItemProjections(bi));
      }
    }
  }
...
void PageDesignIntf::setVerticalGridStep(int verticalGridStep) { m_verticalGridStep = verticalGridStep; }
...
BaseDesignIntf *PageDesignIntf::findDestObject(BaseDesignIntf *item) {
  ...
  while (curItem && !curItem->canAcceptPaste()) { curItem = dynamic_cast<BaseDesignIntf *>(curItem->parentItem()); }
  return curItem;
}
...
void PageDesignIntf::deleteSelected() {
  if (selectedItems().count() == 1) {
    if (!dynamic_cast<PageItemDesignIntf *>(selectedItems().at(0))) saveCommand(removeReportItemCommand(dynamic_cast<BaseDesignIntf *>(selectedItems().at(0))));
  } else {
    ...
    foreach (BandDesignIntf *band, bands) {
      foreach (QGraphicsItem *bandItem, band->childItems()) { itemsToDelete.removeOne(bandItem); }
    }

    if (!itemsToDelete.isEmpty()) {
      foreach (QGraphicsItem *item, itemsToDelete) {
        if (!dynamic_cast<PageItemDesignIntf *>(item)) commandGroup->addCommand(removeReportItemCommand(dynamic_cast<BaseDesignIntf *>(item)), false);
      }
    }

    if (!bands.isEmpty()) {
      foreach (BandDesignIntf *band, bands) { commandGroup->addCommand(removeReportItemCommand(band), false); }
    }
...
void PageDesignIntf::bringToFront() {
  foreach (QGraphicsItem *item, selectedItems()) {
    ...
    foreach (QGraphicsItem *colItem, collidingItems(item)) {
      if (zOrder <= colItem->zValue()) zOrder = colItem->zValue() + 0.1;
    }
    BaseDesignIntf *bdItem = dynamic_cast<BaseDesignIntf *>(item);
    ...
    if (bdItem) {
      ...
      if (!band && !page) bdItem->setZValueProperty(zOrder);
    } else {
    ...
    foreach (QGraphicsItem *item, selectedItems()) {
      ...
      if (bdItem && !bdItem->isGeometryLocked() && !bdItem->isBand()) {
        ...
        if (moveInBand && dynamic_cast<BandDesignIntf *>(bdItem->parent())) {
          bdItem->setPos(QPointF(dynamic_cast<BandDesignIntf *>(bdItem->parent())->geometry().width() - bdItem->width(), bdItem->pos().y()));
        } else {
  ...
  if ((selectedItems().count() > 0) && m_firstSelectedItem) {
    ...
    foreach (QGraphicsItem *item, selectedItems()) {
      ...
      if (bdItem && !bdItem->isGeometryLocked() && !bdItem->isBand()) {
        QRectF oldGeometry = bdItem->geometry();
        if (moveInBand && dynamic_cast<BandDesignIntf *>(bdItem->parent())) {
          bdItem->setPos(QPointF((dynamic_cast<BandDesignIntf *>(bdItem->parent())->geometry().width() / 2) - bdItem->width() / 2, bdItem->pos().y()));
        } else {
...
void PageDesignIntf::setTextAlign(const Qt::Alignment &alignment) { changeSelectedGroupProperty("alignment", QVariant(alignment)); }

void PageDesignIntf::setBorders(const BaseDesignIntf::BorderLines &border) { changeSelectedGroupProperty("borders", (int)border); }
...
CommandIf::Ptr DeleteItemCommand::create(PageDesignIntf *page, BaseDesignIntf *item) {
  ...
  LayoutDesignIntf *layout = dynamic_cast<LayoutDesignIntf *>(item->parent());
  if (layout) command->m_layoutName = layout->objectName();
...
CommandIf::Ptr DeleteLayoutCommand::create(PageDesignIntf *page, LayoutDesignIntf *item) {
  ...
  command->setPage(page);
  command->setItem(item);
  foreach (BaseDesignIntf *childItem, item->childBaseItems()) { command->m_childItems.append(childItem->objectName()); }
  return CommandIf::Ptr(command);
}
...
bool PasteCommand::doIt() {
  ...
  foreach (QString name, m_itemNames) { page()->reportItemByName(name)->setSelected(true); }
...
bool PasteCommand::insertItem(ItemsReaderIntf::Ptr reader) {
  ...
  if (parentItem) {
    ...
    if (item) {
      ...
      foreach (QObject *child, item->children()) { changeName(page(), child); };
...
CommandIf::Ptr PropertyChangedCommand::create(PageDesignIntf *page, const QString &objectName, const QString &propertyName, const QVariant &oldValue, const QVariant &newValue) {
...
CommandIf::Ptr InsertBandCommand::create(PageDesignIntf *page, const QString &bandName) {
  ...
  if (band->parentBand()) command->m_parentBandName = band->parentBandName();
  return CommandIf::Ptr(command);
}

bool InsertBandCommand::doIt() {
  if (!m_parentBandName.isEmpty() && page()->reportItemByName(m_parentBandName)) page()->reportItemByName(m_parentBandName)->setSelected(true);
...
void CommandGroup::undoIt() {
  foreach (CommandIf::Ptr command, m_commands) { command->undoIt(); }
}

void CommandGroup::addCommand(CommandIf::Ptr command, bool execute) {
  if (execute) {
    if (command->doIt()) m_commands.append(command);
  } else
    m_commands.append(command);
}

PrintRange::PrintRange(QAbstractPrintDialog::PrintRange rangeType, int fromPage, int toPage) : m_rangeType(rangeType), m_fromPage(fromPage), m_toPage(toPage) {}
...
CommandIf::Ptr InsertHLayoutCommand::create(PageDesignIntf *page) {
  ...
  for (it = si.begin(); it != si.end(); ++it) {
    ...
    if (bi) command->m_elements.insert(bi->objectName(), bi->pos());
  }
...
bool InsertHLayoutCommand::doIt() {
  foreach (QString itemName, m_elements.keys()) {
    ...
    if (bi) bi->setSelected(true);
  }
  LayoutDesignIntf *layout = page()->internalAddHLayout();
  if (layout) m_layoutName = layout->objectName();
  return layout != 0;
}
...
CommandIf::Ptr PropertyItemAlignChangedCommand::create(PageDesignIntf *page, const QString &objectName, BaseDesignIntf::ItemAlign oldValue, BaseDesignIntf::ItemAlign newValue) {
...
bool Projection::intersect(Projection projection) {
  if ((projection.start() >= start() && projection.start() <= end()) || (projection.end() >= start() && projection.end() <= end()) || (projection.start() <= start() && projection.end() >= end()))
    return true;
  return false;
}
...
bool ItemProjections::intersect(BaseDesignIntf *item) { return intersect(QRectF(item->pos().x(), item->pos().y(), item->width(), item->height())); }

qreal lineLength(qreal start, qreal end, Projection p) {
  qreal result = 0;
  if (start >= p.start() && end <= p.end()) result = end - start;
...
qreal ItemProjections::square(BaseDesignIntf *item) { return square(QRectF(item->pos().x(), item->pos().y(), item->width(), item->height())); }
...
void BandSwapCommand::undoIt() {
  BandDesignIntf *band = dynamic_cast<BandDesignIntf *>(page()->reportItemByName(bandName));
  BandDesignIntf *bandToSwap = dynamic_cast<BandDesignIntf *>(page()->reportItemByName(bandToSwapName));
  if (page() && band && bandToSwap) page()->pageItem()->swapBands(bandToSwap, band);
}
...
void BandMoveFromToCommand::undoIt() {
  if (page() && page()->pageItem()) page()->pageItem()->moveBandFromTo(reverceFrom, reverceTo);
}
