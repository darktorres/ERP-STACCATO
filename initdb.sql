...
CREATE TABLE IF NOT EXISTS `mydb`.`veiculo_has_produto` (
  ...
  INDEX `veiculo_idCompra_idx` (`idCompra` ASC),
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_compras` (`OC` INT, `Compra` INT, `Fornecedor` INT, `Data` INT, `Produtos` INT, `Total` INT, `dataPrevConf` INT, `Vendedor` INT, `Venda` INT);
...
CREATE 
    ...
    DEFINER = `root`@`%` 
    SQL SECURITY DEFINER
VIEW `mydb`.`ViewDespesasExcel` AS
    SELECT 
        DATE_FORMAT(`cp`.`dataEmissao`, '%Y/%m') AS `mes`,
        `cp`.`status` AS `status`,
        SUM(IF((`cp`.`status` = 'pago'),
            `cp`.`valorReal`,
            (`cp`.`valor` + 0E0))) AS `valor`,
        `mydb`.`loja`.`descricao` AS `centroCusto`,
        `cp`.`grupo` AS `grupo`
    FROM
        (`mydb`.`conta_a_pagar_has_pagamento` `cp`
        LEFT JOIN `mydb`.`loja` ON ((`mydb`.`loja`.`idLoja` = `cp`.`centroCusto`)))
    WHERE
        ((`cp`.`desativado` = 0)
            AND (`cp`.`dataEmissao` > '2016-12-31')
            AND (`cp`.`status` <> 'cancelado')
            AND (`cp`.`status` <> 'substituido'))
    GROUP BY DATE_FORMAT(`cp`.`dataEmissao`, '%Y/%m') , `cp`.`status` , `cp`.`centroCusto` , `cp`.`grupo`;
...
CREATE 
    ...
    DEFINER = `root`@`%` 
    SQL SECURITY DEFINER
VIEW `mydb`.`viewFreteExcel` AS
    SELECT 
        DATE_FORMAT(`v`.`data`, '%Y/%m') AS `mes`,
        `v`.`status` AS `status`,
        SUM(`v`.`frete`) AS `frete`,
        `mydb`.`loja`.`descricao` AS `Loja`
    FROM
        (`mydb`.`venda` `v`
        LEFT JOIN `mydb`.`loja` ON ((`mydb`.`loja`.`idLoja` = `v`.`idLoja`)))
    WHERE
        ((`v`.`status` <> 'cancelado')
            AND (`v`.`data` > '2016-12-31'))
    GROUP BY DATE_FORMAT(`v`.`data`, '%Y/%m') , `v`.`status` , `mydb`.`loja`.`descricao`;
...
CREATE 
    ...
    DEFINER = `root`@`%` 
    SQL SECURITY DEFINER
VIEW `mydb`.`viewRelatoriosGestao` AS
    SELECT 
        DATE_FORMAT(`mydb`.`venda`.`data`, '%Y/%m') AS `Mes`,
        `mydb`.`usuario`.`user` AS `Vendedor`,
        `mydb`.`profissional`.`nome_razao` AS `Profissional`,
        `mydb`.`loja`.`descricao` AS `Loja`,
        `mydb`.`venda_has_produto`.`fornecedor` AS `fornecedor`,
        (SUM(`mydb`.`venda_has_produto`.`total`) + 0) AS `Total`,
    ...
    GROUP BY DATE_FORMAT(`mydb`.`venda`.`data`, '%Y/%m') , `mydb`.`usuario`.`user` , `mydb`.`loja`.`descricao` , `mydb`.`profissional`.`nome_razao` , `mydb`.`venda_has_produto`.`fornecedor` , `mydb`.`venda_has_produto`.`status`
    HAVING (`mydb`.`venda_has_produto`.`status` <> 'cancelado');
...
CREATE 
    ...
    DEFINER = `root`@`%` 
    SQL SECURITY DEFINER
VIEW `mydb`.`viewSimuladorExcel` AS
    SELECT 
        `ohp`.`idOrcamentoProduto` AS `idOrcamentoProduto`,
        `ohp`.`idOrcamento` AS `idOrcamento`,
        `ohp`.`fornecedor` AS `fornecedor`,
        `ohp`.`produto` AS `produto`,
        `ohp`.`codComercial` AS `codComercial`,
        `ohp`.`prcUnitario` AS `prcUnitario`,
        `ohp`.`quant` AS `quant`,
        `ohp`.`desconto` AS `desconto`,
        `ohp`.`descGlobal` AS `descGlobal`,
        `ohp`.`total` AS `total`,
        `o`.`frete` AS `frete`,
        `o`.`total` AS `totalgeral`,
        `p`.`custo` AS `custo`,
        `p`.`st` AS `st`,
        `pr`.`nome_razao` AS `profissional`,
        `pr`.`comissao` AS `comissao`,
        `c`.`nome_razao` AS `cliente`
    FROM
        ((((`mydb`.`orcamento_has_produto` `ohp`
        LEFT JOIN `mydb`.`orcamento` `o` ON ((`ohp`.`idOrcamento` = `o`.`idOrcamento`)))
        LEFT JOIN `mydb`.`produto` `p` ON ((`p`.`idProduto` = `ohp`.`idProduto`)))
        LEFT JOIN `mydb`.`profissional` `pr` ON ((`pr`.`idProfissional` = `o`.`idProfissional`)))
        LEFT JOIN `mydb`.`cliente` `c` ON ((`c`.`idCliente` = `o`.`idCliente`)))
    WHERE
        ((`o`.`status` = 'ativo')
...
CREATE 
    ...
    DEFINER = `root`@`%` 
    SQL SECURITY DEFINER
VIEW `mydb`.`viewTaxaCartaoExcel` AS
    SELECT 
        DATE_FORMAT(`v`.`dataEmissao`, '%Y/%m') AS `mes`,
        SUM((IF((`v`.`status` = 'RECEBIDO'),
            `v`.`valorReal`,
            `v`.`valor`) * -(1))) AS `valor`,
        `mydb`.`loja`.`descricao` AS `Loja`
    FROM
        (`mydb`.`conta_a_receber_has_pagamento` `v`
        LEFT JOIN `mydb`.`loja` ON ((`mydb`.`loja`.`idLoja` = `v`.`centroCusto`)))
    WHERE
        ((`v`.`status` <> 'cancelado')
            AND (`v`.`status` <> 'cancelado')
            AND (`v`.`desativado` = 0)
            AND (`v`.`taxa` = 1)
            AND (`v`.`dataEmissao` > '2016-12-31'))
    GROUP BY DATE_FORMAT(`v`.`dataEmissao`, '%Y/%m') , `mydb`.`loja`.`descricao`;
...
VIEW `mydb`.`view_agendar_coleta` AS
    ...
    GROUP BY `e`.`idEstoque` DESC , `pf`.`dataRealFat` , `pf`.`fornecedor` , `n`.`numeroNFe` , `n`.`idNFe` , `pf`.`idVenda` , `pf`.`ordemCompra` , `pf`.`kgcx`
...
VIEW `mydb`.`view_calendario_carga` AS
    SELECT 
        ...
        ANY_VALUE((SELECT 
                    ...
                    FROM
                        `mydb`.`nfe`
                    WHERE
                        (`mydb`.`nfe`.`idNFe` = `vp`.`idNFeFutura`))) AS `NFe Futura`,
    ...
    GROUP BY `vhp`.`idVeiculo` , `vhp`.`data` , `n`.`idNFe` , `vhp`.`idVenda` , `vhp`.`idEvento` , `vhp`.`status` , `n`.`numeroNFe`;
...
VIEW `mydb`.`view_calendario_entrega` AS
    ...
    WHERE
        ((`vhp`.`status` = 'ENTREGA AGEND.')
            OR (`vhp`.`status` = 'EM ENTREGA'))
    GROUP BY CAST(`vhp`.`data` AS DATE) , `vhp`.`idVeiculo`
    ORDER BY CAST(`vhp`.`data` AS DATE);
...
VIEW `mydb`.`view_compras` AS
    SELECT 
        ...
        GROUP_CONCAT(DISTINCT SUBSTRING_INDEX(`u`.`nome`, ' ', 1)
    ...
    WHERE
        (`p`.`status` = 'EM COMPRA')
    GROUP BY `p`.`idCompra` , `p`.`ordemCompra` , `p`.`fornecedor` , `p`.`dataPrevConf`;
...
VIEW `mydb`.`view_produto_estoque` AS
    ...
    WHERE
        ((`ehc`.`idEstoque` IS NOT NULL)
            AND ((`ehc`.`status` = 'PRÉ-CONSUMO')
            OR (`ehc`.`status` = 'CONSUMO')))
    GROUP BY `vp`.`idVendaProduto`;
