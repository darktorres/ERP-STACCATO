...
void DataBrowser::addCSV(SQLEditResult result) {
  try {
    m_report->dataManager()->addCSV(result.datasourceName, result.csv, result.separator, result.firstRowIsHeader);
  } catch (ReportError &exception) { qDebug() << exception.what(); }
}
...
void DataBrowser::applyChanges(SQLEditResult result) {
  if (result.resultMode == currentDatasourceType(result.oldDatasourceName)) {
    ...
    case SQLEditResult::SubProxy: changeProxy(result); break;
    case SQLEditResult::CSVText: changeCSV(result); break;
    default: break;
    }
  } else {
...
void DataBrowser::addDatasource(SQLEditResult result) {
  ...
  case SQLEditResult::SubProxy: addProxy(result); break;
  case SQLEditResult::CSVText: addCSV(result); break;
  default: break;
  }
  activateItem(result.datasourceName, DataBrowserTree::Table);
}
...
void DataBrowser::activateItem(const QString &name, DataBrowserTree::NodeType type) {
  ...
  if (item) item->treeWidget()->setCurrentItem(item);
}
