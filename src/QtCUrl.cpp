...
CURLcode curlGlobalInit() { return curl_global_init(CURL_GLOBAL_ALL); }
...
void QtCUrl::setTextCodec(const char *codecName) { _textCodec = QTextCodec::codecForName(codecName); }

void QtCUrl::setTextCodec(QTextCodec *codec) { _textCodec = codec; }
...
QString QtCUrl::exec(Options &opt) {
  ...
  if (_textCodec) { return _textCodec->toUnicode(reply); }
...
void QtCUrl::setOptions(Options &opt) {
  ...
  while (i.hasNext()) {
    ...
    if (!opt.contains(i.key())) { opt[i.key()] = i.value(); }
  }
  ...
  while (i.hasNext()) {
    ...
    case QVariant::StringList: {
      ...
      foreach (const QString &tmp, value.toStringList()) { slist = curl_slist_append(slist, tmp.toUtf8().data()); }
      _slist.append(slist);
