...
bool XMLReader::extractFirstNode() {
  if (m_firstNode.isNull()) {
    ...
    if (!m_firstNode.hasAttributes()) m_firstNode = m_firstNode.firstChildElement();
  }
...
QVariant XMLReader::getValue(QDomElement *node) {
  ...
  } catch (ReportError &e) { qDebug() << node->attribute("Type") << e.what(); }
...
void XMLReader::readQObject(QObject *item, QDomElement *node) {
  ...
  if (childItem) readItemFromNode(childItem, node);
  EASY_END_BLOCK;
}
...
void XMLReader::readCollection(QObject *item, QDomElement *node) {
  ...
  if (collection) {
    ...
    for (int i = 0; i < node->childNodes().count(); ++i) {
      ...
      if (obj) readItemFromNode(obj, &currentNode);
    }
