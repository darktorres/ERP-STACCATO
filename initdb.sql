...
CREATE TABLE IF NOT EXISTS `staccato`.`galpao` (
  ...
  `tamanho` VARCHAR(45) NOT NULL DEFAULT NULL,
  PRIMARY KEY (`idBloco`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_compras` (`OC` INT, `Compra` INT, `Fornecedor` INT, `Data Venda` INT, `Produtos` INT, `Total` INT, `dataPrevConf` INT, `Vendedor` INT, `Venda` INT, `Data Followup` INT, `Observação` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_faturamento` (`OC` INT, `Fornecedor` INT, `Data Venda` INT, `Produtos` INT, `Total` INT, `idCompra` INT, `dataPrevFat` INT, `Vendedor` INT, `Código` INT, `representacao` INT, `Data Followup` INT, `Observação` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_galpao` (`idEstoque_idConsumo` INT, `descricao` INT, `status` INT, `caixas` INT, `quant` INT, `idBloco` INT, `idVendaProduto2` INT, `tipo` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_nfe_entrada` (`idNFe` INT, `chaveAcesso` INT, `CNPJ Dest` INT, `Fornecedor` INT, `NFe` INT, `Status` INT, `Recebido Por` INT, `Data Receb` INT, `GARE` INT, `GARE Pago Em` INT, `OC` INT, `Venda` INT, `nsu` INT, `utilizada` INT, `Importado em` INT);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_galpao` AS select `ehc`.`idConsumo` AS `idEstoque_idConsumo`,`ehc`.`descricao` AS `descricao`,`vp2`.`status` AS `status`,`ehc`.`caixas` AS `caixas`,(`ehc`.`quant` * -(1)) AS `quant`,`ehc`.`idBloco` AS `idBloco`,`ehc`.`idVendaProduto2` AS `idVendaProduto2`,'CLIENTE' AS `tipo` from ((`staccato`.`estoque_has_consumo` `ehc` left join `staccato`.`venda_has_produto2` `vp2` on((`ehc`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) left join `staccato`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) where (`vp2`.`status` in ('ESTOQUE','ENTREGA AGEND.','EM ENTREGA')) union select `staccato`.`vec`.`idEstoque` AS `idEstoque`,`staccato`.`vec`.`descricao` AS `descricao`,`staccato`.`vec`.`status` AS `status`,`staccato`.`vec`.`caixasContabil` AS `Caixas contabil`,`staccato`.`vec`.`contabil` AS `contabil`,`staccato`.`vec`.`idBloco` AS `idBloco`,NULL AS `idVendaProduto2`,'ESTOQUE' AS `ESTOQUE` from `staccato`.`view_estoque_contabil` `vec` where (`staccato`.`vec`.`contabil` > 0);
