...
QVariant GroupFunction::addition(QVariant value1, QVariant value2) { return value1.toDouble() + value2.toDouble(); }

QVariant GroupFunction::subtraction(QVariant value1, QVariant value2) { return value1.toDouble() - value2.toDouble(); }

QVariant GroupFunction::division(QVariant value1, QVariant value2) { return value1.toDouble() / value2.toDouble(); }

QVariant GroupFunction::multiplication(QVariant value1, QVariant value2) { return value1.toDouble() * value2.toDouble(); }
...
GroupFunction *GroupFunctionFactory::createGroupFunction(const QString &functionName, const QString &expression, const QString &dataBandName, DataSourceManager *dataManager) {
  if (m_creators.contains(functionName)) { return m_creators.value(functionName)->createFunction(expression, dataBandName, dataManager); }
  return 0;
}
...
QVariant SumGroupFunction::calculate(PageItemDesignIntf *page) {
  ...
  if (!page) {
    ...
    foreach (BandDesignIntf *band, page->bands()) { res = addition(res, m_valuesByBand.value(band)); }
  }
  return res;
}
...
QVariant AvgGroupFunction::calculate(PageItemDesignIntf *page) {
  ...
  if (!res.isNull() && (values().count() > 0)) { res = division(res, values().count()); }
  return res;
}
...
QVariant CountGroupFunction::calculate(PageItemDesignIntf *page) {
  if (!page) {
    ...
    int res = 0;
    foreach (BandDesignIntf *band, page->bands()) {
      if (!m_valuesByBand.value(band).isNull()) { res++; }
    }
    return res;
  }
}
