...
namespace {
LimeReport::ObjectPropItem *createColorPropItem(QObject *object, LimeReport::ObjectPropItem::ObjectsList *objects, const QString &name, const QString &displayName, const QVariant &data,
...
bool ColorPropItem::paint(QPainter *painter, const StyleOptionViewItem &option, const QModelIndex &index) {
  if (index.column() == 1) {
    ...
    QRect rect(option.rect.x() + border, option.rect.y() + border, style->pixelMetric(QStyle::PM_IndicatorWidth), style->pixelMetric(QStyle::PM_IndicatorWidth));
