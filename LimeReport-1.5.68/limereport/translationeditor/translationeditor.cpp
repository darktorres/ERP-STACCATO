...
TranslationEditor::TranslationEditor(QWidget *parent)
    : QWidget(parent), ui(new Ui::TranslationEditor), m_translationContainer(0), m_currentReportTranslation(0), m_currentPageTranslation(0), m_currentPropertyTranslation(0),
...
TranslationEditor::~TranslationEditor() { delete ui; }
...
QLocale::Language TranslationEditor::getLanguageByName(const QString &languageName) {
  foreach (QLocale::Language language, m_translationContainer->translations()->keys()) {
    if (QLocale::languageToString(language).compare(languageName) == 0) { return language; }
  }
  ...
  if (m_translationContainer) {
    ...
    if (translations) {
      foreach (QLocale::Language language, translations->keys()) {
        if (language != QLocale::AnyLanguage) ui->lvLanguages->addItem(QLocale::languageToString(language));
      }
  ...
  if (pageTranslation) {
    ...
    foreach (QString itemName, items) {
      ...
      foreach (PropertyTranslation *propertyTranslation, itemTranslation->propertyesTranslation) {
        ...
        if (propertyTranslation->sourceHasBeenChanged) { checkItem->setIcon(QIcon(":/translationeditor/question")); }
        if (propertyTranslation->checked) { checkItem->setIcon(QIcon(":/translationeditor/checked")); }
...
void TranslationEditor::on_tbStrings_itemSelectionChanged() {
  if (m_currentPageTranslation && !m_translationChanging) activateTranslation(ui->tbStrings->item(ui->tbStrings->currentRow(), 1)->text(), ui->tbStrings->item(ui->tbStrings->currentRow(), 2)->text());
}

void TranslationEditor::on_teTranslation_textChanged() {
  if (m_currentPropertyTranslation && !m_translationChanging) m_currentPropertyTranslation->value = ui->teTranslation->toPlainText();
}
...
void TranslationEditor::on_twPages_itemSelectionChanged() {
  if (!ui->twPages->selectedItems().isEmpty()) { activatePage(m_currentReportTranslation->findPageTranslation(ui->twPages->selectedItems().at(0)->text(0))); }
}
...
void TranslationEditor::on_tbAddLanguage_clicked() {
  ...
  if (dialog.exec()) {
    ...
    foreach (QListWidgetItem *item, ui->lvLanguages->findItems(QLocale::languageToString(dialog.getSelectedLanguage()), Qt::MatchExactly)) { item->setSelected(true); }
    m_translationChanging = false;
  }
}
...
void TranslationEditor::on_lvLanguages_itemSelectionChanged() {
  if (ui->lvLanguages->currentItem() && m_currentReportTranslation) { activateLanguage(getLanguageByName(ui->lvLanguages->currentItem()->text())); }
}
