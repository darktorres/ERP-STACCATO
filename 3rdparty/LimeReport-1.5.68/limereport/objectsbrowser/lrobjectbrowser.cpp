...
ObjectBrowser::ObjectBrowser(QWidget *parent) : QWidget(parent), m_designerWidget(NULL), m_mainWindow(NULL), m_changingItemSelection(false), m_movingItem(false) {
...
void ObjectBrowser::setReportEditor(ReportDesignWidget *designerWidget) {
  ...
  connect(m_designerWidget, SIGNAL(itemAdded(LimeReport::PageDesignIntf *, LimeReport::BaseDesignIntf *)), this, SLOT(slotItemAdded(LimeReport::PageDesignIntf *, LimeReport::BaseDesignIntf *)));
  connect(m_designerWidget, SIGNAL(itemDeleted(LimeReport::PageDesignIntf *, LimeReport::BaseDesignIntf *)), this, SLOT(slotItemDeleted(LimeReport::PageDesignIntf *, LimeReport::BaseDesignIntf *)));
  connect(m_designerWidget, SIGNAL(bandAdded(LimeReport::PageDesignIntf *, LimeReport::BandDesignIntf *)), this, SLOT(slotBandAdded(LimeReport::PageDesignIntf *, LimeReport::BandDesignIntf *)));
  connect(m_designerWidget, SIGNAL(bandDeleted(LimeReport::PageDesignIntf *, LimeReport::BandDesignIntf *)), this, SLOT(slotBandDeleted(LimeReport::PageDesignIntf *, LimeReport::BandDesignIntf *)));
  connect(m_treeView, SIGNAL(itemSelectionChanged()), this, SLOT(slotObjectTreeItemSelectionChanged()));
  connect(m_designerWidget, SIGNAL(itemSelected(LimeReport::BaseDesignIntf *)), this, SLOT(slotItemSelected(LimeReport::BaseDesignIntf *)));
  connect(m_designerWidget, SIGNAL(multiItemSelected()), this, SLOT(slotMultiItemSelected()));
  connect(m_designerWidget, SIGNAL(activePageUpdated(LimeReport::PageDesignIntf *)), this, SLOT(slotActivePageUpdated(LimeReport::PageDesignIntf *)));
...
void ObjectBrowser::fillNode(QTreeWidgetItem *parentNode, BaseDesignIntf *reportItem, BaseDesignIntf *ignoredItem) {
  ...
  foreach (BaseDesignIntf *item, reportItem->childBaseItems()) {
    if (item != ignoredItem) {
      ...
      connect(item, SIGNAL(propertyObjectNameChanged(QString, QString)), this, SLOT(slotPropertyObjectNameChanged(QString, QString)), Qt::UniqueConnection);
      ItemDesignIntf *i = dynamic_cast<ItemDesignIntf *>(item);
      if (i) { connect(i, SIGNAL(itemLocationChanged(BaseDesignIntf *, BaseDesignIntf *)), this, SLOT(slotItemParentChanged(BaseDesignIntf *, BaseDesignIntf *)), Qt::UniqueConnection); }
      m_itemsMap.insert(item, treeItem);

      BandDesignIntf *band = dynamic_cast<BandDesignIntf *>(item);

      QSet<BandDesignIntf::BandsType> subBands;
      subBands << BandDesignIntf::SubDetailBand << BandDesignIntf::SubDetailHeader << BandDesignIntf::SubDetailFooter;
      ...
      if (band && subBands.contains(band->bandType())) {
        ...
        if (parentBandNode) parentBandNode->addChild(treeItem);
      } else {
  ...
  foreach (QGraphicsItem *item, itemsList) {
    if (item != ignoredItem) {
      ...
      if (reportItem && reportItem->parentItem() == 0) {
        ...
        connect(reportItem, SIGNAL(propertyObjectNameChanged(QString, QString)), this, SLOT(slotPropertyObjectNameChanged(QString, QString)));
...
void ObjectBrowser::slotPropertyObjectNameChanged(const QString &oldName, const QString &newName) {
  Q_UNUSED(oldName)
  if (m_itemsMap.contains(sender())) { m_itemsMap.value(sender())->setText(0, newName); }
}
    ...
    foreach (QTreeWidgetItem *item, m_treeView->selectedItems()) {
      ...
      if (tn) {
        ...
        if (si) {
          ...
          if (si->parentItem()) p = si->parentItem()->mapToScene(si->pos());
...
void ObjectBrowser::slotItemDoubleClicked(QTreeWidgetItem *item, int) {
  ...
  if (node) {
    ...
    if (baseItem) { baseItem->showEditorDialog(); }
  }
}
...
void ObjectBrowser::slotItemParentChanged(BaseDesignIntf *item, BaseDesignIntf *parent) {
  if (!m_movingItem) {
    ...
    foreach (QObject *di, m_defferedItems) {
      ...
      if (b) moveItemNode(b, parent);
    }
    m_defferedItems.clear();
  } else {
    if (!m_defferedItems.contains(item)) m_defferedItems.append(item);
  }
}
