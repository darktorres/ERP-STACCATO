...
SET kg = 0;
 ...
 SELECT 
    ...
    FROM
        ...
        WHERE
        e.status = 'ESTOQUE' and DATE(e.created) <= dia;
RETURN kg;
END$$
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_galpao` AS select `ehc`.`idConsumo` AS `id`,`ehc`.`descricao` AS `descricao`,`vp2`.`status` AS `status`,`ehc`.`caixas` AS `caixas`,(`ehc`.`quant` * -(1)) AS `quant`,`ehc`.`bloco` AS `bloco`,`ehc`.`idVendaProduto2` AS `idVendaProduto2`,'CONSUMO' AS `tipo` from ((`staccato`.`estoque_has_consumo` `ehc` left join `staccato`.`venda_has_produto2` `vp2` on((`ehc`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) left join `staccato`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) where (`vp2`.`status` in ('ESTOQUE','ENTREGA AGEND.','EM ENTREGA')) union select `staccato`.`vec`.`idEstoque` AS `idEstoque`,`staccato`.`vec`.`descricao` AS `descricao`,`staccato`.`vec`.`status` AS `status`,`staccato`.`vec`.`caixasContabil` AS `Caixas contabil`,`staccato`.`vec`.`contabil` AS `contabil`,`staccato`.`vec`.`bloco` AS `bloco`,NULL AS `idVendaProduto2`,'ESTOQUE' AS `ESTOQUE` from `staccato`.`view_estoque_contabil` `vec` where (`staccato`.`vec`.`contabil` > 0);
