...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_nfe_baixada` (`idNFe` INT, `numeroNFe` INT, `status` INT, `chaveAcesso` INT, `infCpl` INT);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_nfe_baixada` AS select `staccato`.`nfe`.`idNFe` AS `idNFe`,`staccato`.`nfe`.`numeroNFe` AS `numeroNFe`,`staccato`.`nfe`.`status` AS `status`,`staccato`.`nfe`.`chaveAcesso` AS `chaveAcesso`,`staccato`.`nfe`.`infCpl` AS `infCpl` from `staccato`.`nfe` where ((`staccato`.`nfe`.`tipo` = 'ENTRADA') and (`staccato`.`nfe`.`nsu` is not null) and (`staccato`.`nfe`.`utilizada` = FALSE));
