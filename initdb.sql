...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_entrega_pendente` (`data` INT, `prazoEntrega` INT, `novoPrazoEntrega` INT, `dataRealReceb` INT, `statusFinanceiro` INT, `idVenda` INT, `Estoque` INT, `Entregue` INT, `Outros` INT, `Bairro` INT, `Logradouro` INT, `Número` INT, `Cidade` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_estoque_consumo` (`Venda` INT, `idEstoque` INT, `statusProduto` INT, `status` INT, `local` INT, `fornecedor` INT, `descricao` INT, `quant` INT, `quantUpd` INT, `un` INT, `caixas` INT, `codComercial` INT, `created` INT);
...
BEGIN
    ...
    SET p.descontinuado = FALSE, p.estoqueRestante = v.restante
    WHERE v.idEstoque IS NOT NULL AND p.estoque = TRUE;

    UPDATE produto p LEFT JOIN view_estoque2 v ON p.idEstoque = v.idEstoque
    SET p.descontinuado = TRUE, p.estoqueRestante = 0
    WHERE v.idEstoque IS NULL AND p.estoque = TRUE;
...
VIEW `mydb`.`view_calendario_entrega` AS
    ...
    ORDER BY DATE_FORMAT(`vhp`.`data`, '%d-%m-%Y');
...
VIEW `mydb`.`view_compras` AS
    SELECT 
        GROUP_CONCAT(DISTINCT `p`.`ordemCompra`
            SEPARATOR ',') AS `OC`,
...
VIEW `mydb`.`view_entrega_pendente` AS
    SELECT 
        ...
        GROUP_CONCAT(DISTINCT `vp`.`dataRealReceb`
...
VIEW `mydb`.`view_relatorio_loja` AS
    SELECT 
        ...
        `v`.`Mês` AS `Mês`,
        (SELECT 
                SUM((`vp`.`quant` * `p`.`custo`))
            FROM
                (((`mydb`.`venda` `v2`
                LEFT JOIN `mydb`.`loja` `l` ON ((`v2`.`idLoja` = `l`.`idLoja`)))
                LEFT JOIN `mydb`.`venda_has_produto` `vp` ON ((`v2`.`idVenda` = `vp`.`idVenda`)))
                LEFT JOIN `mydb`.`produto` `p` ON ((`vp`.`idProduto` = `p`.`idProduto`)))
            WHERE
                ((`vp`.`reposicao` = TRUE)
                    AND (DATE_FORMAT(`vp`.`created`, '%Y-%m') = `v`.`Mês`)
                    AND (`l`.`descricao` = `v`.`Loja`))) AS `Reposição`
    FROM
...
VIEW `mydb`.`view_venda` AS
    SELECT 
        ...
        `v`.`data` AS `Data`,
        `v`.`total` AS `Total R$`,
        GROUP_CONCAT(DISTINCT `fl1`.`dataFollowup`
            SEPARATOR ',') AS `Data Followup`,
        GROUP_CONCAT(DISTINCT `fl1`.`observacao`
    ...
    FROM
        ...
        LEFT JOIN `mydb`.`venda_has_followup` `vhf` ON (((`vhf`.`idVenda` = LEFT(`v`.`idVenda`, 13))
            AND (`vhf`.`idFollowup` = (SELECT 
                MAX(`vhf2`.`idFollowup`)
            FROM
                `mydb`.`venda_has_followup` `vhf2`
            WHERE
                (`vhf2`.`idVenda` = LEFT(`v`.`idVenda`, 13)))))))
    GROUP BY `v`.`idVenda`;
