...
bool ConnectionDialog::checkConnection() {
  ...
  if (!m_controller->checkConnectionDesc(con.data())) { throw LimeReport::ReportError(m_controller->lastError()); }
  return true;
}
...
ConnectionDesc *ConnectionDialog::uiToConnection(LimeReport::ConnectionDesc *conDesc) {
  ...
  if (!ui->lePort->text().isEmpty()) result->setPort(ui->lePort->text());
...
void ConnectionDialog::on_toolButton_clicked() { ui->leDataBase->setText(QFileDialog::getOpenFileName()); }
...
void ConnectionDialog::on_toolButton_2_toggled(bool checked) { ui->lePassword->setEchoMode(checked ? QLineEdit::Normal : QLineEdit::Password); }
