...
CREATE TABLE IF NOT EXISTS `staccato`.`nfe` (
  ...
  FULLTEXT INDEX `index9_nfe` (`numeroNFe`, `chaveAcesso`, `infCpl`),
  FULLTEXT INDEX `index10_nfe` (`numeroNFe`, `xml`, `chaveAcesso`),
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_nfe_entrada` (`idNFe` INT, `chaveAcesso` INT, `CNPJ Dest` INT, `Fornecedor` INT, `NFe` INT, `Status` INT, `Recebido Por` INT, `Data Receb` INT, `GARE` INT, `GARE Pago Em` INT, `OC` INT, `Venda` INT, `created` INT);
