...
LimeReport::ReportExporterInterface *createPDFExporter(LimeReport::ReportEnginePrivate *parent) { return new LimeReport::PDFExporter(parent); }
...
bool PDFExporter::exportPages(ReportPages pages, const QString &fileName, const QMap<QString, QVariant> &params) {
  ...
  if (!fileName.isEmpty()) {
    ...
    if (!pages.isEmpty()) { m_reportEngine->printPages(pages, &printer); }
