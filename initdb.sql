...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_relatorio_loja` AS select `v`.`Loja` AS `Loja`,sum(`v`.`Faturamento`) AS `Faturamento`,sum(`v`.`Comissão`) AS `Comissão`,((sum(`v`.`Comissão`) / sum(`v`.`Faturamento`)) * 100) AS `%`,`v`.`Mês` AS `Mês`,(group_concat(distinct `r`.`custoReposicao` separator ',') + 0.0) AS `Reposição` from (`staccato`.`view_relatorio` `v` left join `staccato`.`view_relatorio_reposicao` `r` on(((`r`.`loja` = `v`.`Loja`) and (`r`.`data` = `v`.`Mês`)))) group by `v`.`Loja`,`v`.`Mês`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_resumo_relatorio` AS select `l`.`descricao` AS `Loja`,`u`.`user` AS `Vendedor`,sum(if(((date_format(`o`.`data`,'%Y-%m') < `MYDATE`()) and ((`o`.`status` in ('ATIVO','EXPIRADO')) or ((`o`.`status` = 'FECHADO') and (date_format(`v`.`data`,'%Y-%m') >= `MYDATE`())) or ((`o`.`status` = 'PERDIDO') and (date_format(`o`.`lastUpdated`,'%Y-%m') >= `MYDATE`())))),`o`.`total`,0)) AS `Validos Anteriores`,sum(if(((date_format(`o`.`data`,'%Y-%m') = `MYDATE`()) and isnull(`o_1`.`data`)),`o`.`total`,0)) AS `Gerados Mes`,sum(if(((date_format(`o`.`data`,'%Y-%m') = `MYDATE`()) and (`o_1`.`data` is not null)),`o`.`total`,0)) AS `Revalidados Mes`,sum(if(((date_format(`v`.`data`,'%Y-%m') = `MYDATE`()) and (`o`.`status` = 'FECHADO')),`o`.`total`,0)) AS `Fechados Mes`,sum(if(((date_format(`o`.`lastUpdated`,'%Y-%m') = `MYDATE`()) and (`o`.`status` = 'PERDIDO')),`o`.`total`,0)) AS `Perdidos Mes`,sum(if(((date_format(`o`.`data`,'%Y-%m') <= `MYDATE`()) and ((`o`.`status` = 'ATIVO') or (`o`.`status` = 'EXPIRADO') or ((`o`.`status` = 'FECHADO') and (date_format(`v`.`data`,'%Y-%m') > `MYDATE`())) or ((`o`.`status` = 'PERDIDO') and (date_format(`o`.`lastUpdated`,'%Y-%m') > `MYDATE`())))),`o`.`total`,0)) AS `Validos Mes`,((sum(if(((date_format(`v`.`data`,'%Y-%m') = `MYDATE`()) and (`o`.`status` = 'FECHADO')),`o`.`total`,0)) / (sum(if(((date_format(`o`.`data`,'%Y-%m') = `MYDATE`()) and isnull(`o_1`.`data`)),`o`.`total`,0)) + sum(if(((date_format(`o`.`data`,'%Y-%m') = `MYDATE`()) and (`o_1`.`data` is not null)),`o`.`total`,0)))) * 100) AS `% Fechados / Gerados`,((sum(if(((date_format(`v`.`data`,'%Y-%m') = `MYDATE`()) and (`o`.`status` = 'FECHADO')),`o`.`total`,0)) / ((sum(if(((date_format(`o`.`data`,'%Y-%m') < `MYDATE`()) and ((`o`.`status` = 'ATIVO') or (`o`.`status` = 'EXPIRADO') or ((`o`.`status` = 'FECHADO') and (date_format(`v`.`data`,'%Y-%m') >= `MYDATE`())) or ((`o`.`status` = 'PERDIDO') and (date_format(`o`.`lastUpdated`,'%Y-%m') >= `MYDATE`())))),`o`.`total`,0)) + sum(if(((date_format(`o`.`data`,'%Y-%m') = `MYDATE`()) and isnull(`o_1`.`data`)),`o`.`total`,0))) + sum(if(((date_format(`o`.`data`,'%Y-%m') = `MYDATE`()) and (`o_1`.`data` is not null)),`o`.`total`,0)))) * 100) AS `% Fechados / Carteira` from ((((`staccato`.`orcamento` `o` left join `staccato`.`loja` `l` on((`o`.`idLoja` = `l`.`idLoja`))) left join `staccato`.`usuario` `u` on((`o`.`idUsuario` = `u`.`idUsuario`))) left join `staccato`.`orcamento` `o_1` on((`o`.`replicadoDe` = `o_1`.`idOrcamento`))) left join `staccato`.`venda` `v` on((`o`.`idOrcamento` = `v`.`idOrcamento`))) where (`o`.`status` not in ('REPLICADO','CANCELADO')) group by `l`.`descricao`,`u`.`user` order by `l`.`descricao`,`u`.`user`;
