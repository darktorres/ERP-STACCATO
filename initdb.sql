...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_fluxo_caixa_realizado` AS select `cp`.`contraParte` AS `ContraParte`,`cp2`.`idCompra` AS `Compra/Venda`,`cp`.`dataRealizado` AS `dataRealizado`,(`cp`.`valorReal` * -(1)) AS `R$`,`cp`.`tipo` AS `Tipo`,(select concat(`staccato`.`loja_has_conta`.`banco`,' - ',`staccato`.`loja_has_conta`.`agencia`,' - ',`staccato`.`loja_has_conta`.`conta`) from `staccato`.`loja_has_conta` where (`staccato`.`loja_has_conta`.`idConta` = `cp`.`idConta`)) AS `contaDestino`,`cp`.`idConta` AS `idConta`,`cp`.`status` AS `Status`,'SAIDA' AS `ENT/SAI` from (`staccato`.`conta_a_pagar_has_pagamento` `cp` left join `staccato`.`conta_a_pagar_has_idcompra` `cp2` on((`cp`.`idPagamento` = `cp2`.`idPagamento`))) where ((`cp`.`status` in ('PAGO','PAGO GARE')) and (`cp`.`dataRealizado` is not null) and (`cp`.`valorReal` is not null) and (`cp`.`desativado` = false)) union all select `cr`.`contraParte` AS `ContraParte`,`cr`.`idVenda` AS `idVenda`,`cr`.`dataRealizado` AS `dataRealizado`,`cr`.`valorReal` AS `valorReal`,`cr`.`tipo` AS `tipo`,(select concat(`staccato`.`loja_has_conta`.`banco`,' - ',`staccato`.`loja_has_conta`.`agencia`,' - ',`staccato`.`loja_has_conta`.`conta`) from `staccato`.`loja_has_conta` where (`staccato`.`loja_has_conta`.`idConta` = `cr`.`idConta`)) AS `contaDestino`,`cr`.`idConta` AS `idConta`,`cr`.`status` AS `status`,'ENTRADA' AS `ENTRADA` from `staccato`.`conta_a_receber_has_pagamento` `cr` where ((`cr`.`status` = 'RECEBIDO') and (`cr`.`dataRealizado` is not null) and (`cr`.`valorReal` is not null) and (`cr`.`desativado` = false)) order by `dataRealizado`;
