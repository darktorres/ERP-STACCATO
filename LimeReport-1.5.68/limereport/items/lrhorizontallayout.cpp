...
bool horizontalLessThen(BaseDesignIntf *c1, BaseDesignIntf *c2) { return c1->pos().x() < c2->pos().x(); }

HorizontalLayout::HorizontalLayout(QObject *owner, QGraphicsItem *parent) : AbstractLayout(xmlTag, owner, parent) {}
...
void HorizontalLayout::sortChildren() { std::sort(layoutsChildren().begin(), layoutsChildren().end(), horizontalLessThen); }
...
void HorizontalLayout::divideSpace() {
  ...
  if (itemMode() == DesignMode && !layoutsChildren().isEmpty()) {
    ...
    for (int i = 0; i < layoutsChildren().size(); ++i) {
      if (layoutsChildren()[i]->isVisible() || itemMode() == DesignMode) layoutsChildren()[i]->setWidth(layoutsChildren()[i]->width() + delta);
