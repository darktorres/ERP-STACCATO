...
QueryHolder::QueryHolder(QString queryText, QString connectionName, DataSourceManager *dataManager)
    : m_queryText(queryText), m_connectionName(connectionName), m_mode(IDataSource::RENDER_MODE), m_dataManager(dataManager), m_prepared(true) {
  extractParams();
}
...
void QueryHolder::fillParams(QSqlQuery *query) {
  foreach (QString param, m_aliasesToParam.keys()) {
    ...
    if (value.isValid() || m_mode == IDataSource::DESIGN_MODE) query->bindValue(':' + param, value);
  }
}
...
QString QueryHolder::replaceVariables(QString query) { return dataManager()->replaceVariables(query, m_aliasesToParam); }
...
ModelToDataSource::ModelToDataSource(QAbstractItemModel *model, bool owned) : QObject(), m_model(model), m_owned(owned), m_curRow(-1), m_lastError("") {
  Q_ASSERT(model);
  if (model) {
...
QVariant ModelToDataSource::dataByRowIndex(const QString &columnName, int rowIndex) {
  if (m_model->rowCount() > rowIndex) return m_model->data(m_model->index(rowIndex, columnIndexByName(columnName)));
  return QVariant();
}

QVariant ModelToDataSource::dataByKeyField(const QString &columnName, const QString &keyColumnName, QVariant keyData) {
  for (int i = 0; i < m_model->rowCount(); ++i) {
    if (m_model->data(m_model->index(i, columnIndexByName(keyColumnName))) == keyData) { return m_model->data(m_model->index(i, columnIndexByName(columnName))); }
  }
  return QVariant();
}
...
QString ModelToDataSource::columnNameByIndex(int columnIndex) {
  if (isInvalid()) return "";
  QString result = m_model->headerData(columnIndex, Qt::Horizontal, Qt::UserRole).isValid() ? m_model->headerData(columnIndex, Qt::Horizontal, Qt::UserRole).toString()
...
int ModelToDataSource::columnIndexByName(QString name) {
  ...
  for (int i = 0; i < m_model->columnCount(); i++) {
    QString columnName =
        m_model->headerData(i, Qt::Horizontal, Qt::UserRole).isValid() ? m_model->headerData(i, Qt::Horizontal, Qt::UserRole).toString() : m_model->headerData(i, Qt::Horizontal).toString();
    if (columnName.compare(name, Qt::CaseInsensitive) == 0) return i;
  }
  return -1;
}
...
ConnectionDesc::ConnectionDesc(QSqlDatabase db, QObject *parent)
    : QObject(parent), m_connectionName(db.connectionName()), m_connectionHost(db.hostName()), m_connectionDriver(db.driverName()), m_databaseName(db.databaseName()), m_user(db.userName()),
      m_password(db.password()), m_port(""), m_autoconnect(false), m_internal(false), m_keepDBCredentials(true) {}

ConnectionDesc::ConnectionDesc(QObject *parent)
    : QObject(parent), m_connectionName(""), m_connectionHost(""), m_connectionDriver(""), m_databaseName(""), m_user(""), m_password(""), m_port(""), m_autoconnect(false), m_internal(false),
      m_keepDBCredentials(true) {}

ConnectionDesc::Ptr ConnectionDesc::create(QSqlDatabase db, QObject *parent) { return Ptr(new ConnectionDesc(db, parent)); }
...
bool ConnectionDesc::isEqual(const QSqlDatabase &db) {
  return (db.databaseName() == m_databaseName) && (db.driverName() == m_connectionDriver) && (db.hostName() == m_connectionHost) && (db.connectionName() == m_connectionName) &&
         (db.userName() == m_user) && (db.password() == m_password);
}

QString ConnectionDesc::connectionNameForUser(const QString &connectionName) { return connectionName.compare(QSqlDatabase::defaultConnection) == 0 ? tr("defaultConnection") : connectionName; }

QString ConnectionDesc::connectionNameForReport(const QString &connectionName) { return connectionName.compare(tr("defaultConnection")) == 0 ? QSqlDatabase::defaultConnection : connectionName; }
...
void ConnectionDesc::setKeepDBCredentials(bool keepDBCredentals) { m_keepDBCredentials = keepDBCredentals; }

QueryDesc::QueryDesc(QString queryName, QString queryText, QString connection) : m_queryName(queryName), m_queryText(queryText), m_connectionName(connection) {}
...
QString SubQueryHolder::replaceFields(QString query) { return dataManager()->replaceFields(query, m_aliasesToParam); }
...
ProxyHolder::ProxyHolder(ProxyDesc *desc, DataSourceManager *dataManager) : m_model(0), m_desc(desc), m_lastError(""), m_mode(IDataSource::RENDER_MODE), m_invalid(false), m_dataManager(dataManager) {}
  ...
  if (!m_datasource) {
    ...
    if (dataManager()) {
      ...
      if (master && child) {
        ...
        if (!child) m_lastError += ((m_lastError.isEmpty()) ? QObject::tr("Child") : QObject::tr(" and child ")) + QObject::tr("datasouce \"%1\" not found!").arg(m_desc->child());
      }
    }
  } else {
    if (!isInvalid()) {
...
QVariant MasterDetailProxyModel::sourceData(QString fieldName, int row) const {
  ...
  if (fieldIndex != -1) {
    ...
    throw ReportError(tr("Field: \"%1\" not found in \"%2\" child datasource").arg(fieldName).arg(m_childName));
  }
}
...
QVariant MasterDetailProxyModel::masterData(QString fieldName) const {
  ...
  if (master) {
    ...
    if (columnIndex != -1) {
      ...
      throw ReportError(tr("Field: \"%1\" not found in \"%2\" master datasource").arg(fieldName).arg(m_masterName));
    }
  }
  return QVariant();
}
...
bool CallbackDatasource::next() {
  if (!m_eof) {
    ...
    m_currentRow++;
    bool result = true;
    if (!m_getDataFromCache) emit changePos(CallbackInfo::Next, result);
...
QVariant CallbackDatasource::dataByRowIndex(const QString &columnName, int rowIndex) {
  ...
  if (backupCurrentRow != -1) {
    for (int i = 0; i < backupCurrentRow; ++i) next();
  }
  return result;
}
  ...
  if (m_columnCount == -1) {
    ...
    if (m_columnCount != -1) {
      for (int i = 0; i < m_columnCount; ++i) {
        ...
        if (columnName.isValid()) m_headers.append(columnName.toString());
      }
    } else {
      int currIndex = 0;
      do {
...
int CallbackDatasource::columnIndexByName(QString name) {
  for (int i = 0; i < m_headers.size(); ++i) {
    if (m_headers[i].compare(name, Qt::CaseInsensitive) == 0) return i;
  }
  //    if (m_headers.size()==0){
  //        QVariant data;
  //        bool hasNextRow;
...
CSVHolder::CSVHolder(const CSVDesc &desc, DataSourceManager *dataManager)
    : m_csvText(desc.csvText()), m_separator(desc.separator()),
