...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_consistencia_compra` (`descricao` INT, `prcUnitario` INT, `preco` INT, `prcUnitario * quant` INT, `created` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_consistencia_vp1_vp2_quant` (`vp1_quant` INT, `vp2_quant` INT, `created` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_consistencia_vp1_vp2_total`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_consistencia_vp1_vp2_total` (`vp1_total` INT, `vp2_total` INT, `created` INT);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_consistencia_compra` AS select `pf`.`descricao` AS `descricao`,`pf`.`prcUnitario` AS `prcUnitario`,`pf`.`preco` AS `preco`,(`pf`.`prcUnitario` * `pf`.`quant`) AS `prcUnitario * quant`,`pf`.`created` AS `created` from `staccato`.`pedido_fornecedor_has_produto` `pf` where (((`pf`.`prcUnitario` * `pf`.`quant`) / `pf`.`preco`) > 1.01);
