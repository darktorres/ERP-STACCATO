...
typedef unsigned int LWORD; /* Should be 32-bit = 4 bytes        */
#else
...
LWORD P = 0xb7e15163, Q = 0x9e3779b9;
...
private:
  ...
  void RC5_ENCRYPT(LWORD *pt, LWORD *ct);
  void RC5_DECRYPT(LWORD *ct, LWORD *pt);
  LWORD S[26] = {0};
  bool m_prepared;
};

void ChipperPrivate::RC5_SETUP(const char *K) {
  LWORD i, j, k, u = w / 8, A, B, L[c];
...
void ChipperPrivate::RC5_ENCRYPT(LWORD *pt, LWORD *ct) {
...
void ChipperPrivate::RC5_DECRYPT(LWORD *ct, LWORD *pt) {
...
QByteArray Chipper::cryptString(QString value) {
  ...
  if (!d->isPrepared()) d->RC5_SETUP(passPhrase);
  ...
  while (it != buff.end()) {
    ...
    for (int i = 0; i < 8; i++) { result += ct.bytes[i]; }
  }
  return result;
}
