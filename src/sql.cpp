...
QString Sql::view_a_pagar_vencidos() {
    return " SELECT "
           ...
           " WHERE "
           "     `cr`.`dataPagamento` < CURDATE() "
           "     AND `cr`.`status` IN ('PENDENTE' , 'CONFERIDO', 'AGENDADO') "
           " GROUP BY "
           "     `cr`.`dataPagamento`";
}
...
QString Sql::view_gare_vencidos() {
    return " SELECT "
           "     `cr`.`dataPagamento` AS `Data`, "
...
QString Sql::view_estoque_contabil(const QString &match, const QString &data) {
  ...
  return " WITH ehc AS"
         ...
         "         SUM(IF(vp2.status != 'DEVOLVIDO ESTOQUE', vp2.quant, 0)) AS consumoVenda,"
         "         GROUP_CONCAT(DISTINCT vp2.idVenda) AS idVenda"
         "     FROM"
         "         estoque_has_consumo ehc"
         "     LEFT JOIN "
         "         venda_has_produto2 vp2 ON ehc.idVendaProduto2 = vp2.idVendaProduto2"
         "     WHERE"
         "         (vp2.dataRealEnt <= '" + data + "' OR vp2.dataRealEnt IS NULL) AND ehc.status != 'CANCELADO'"
...
QString Sql::contasPagar(const QString &filtros, const QString &busca) {
  ...
  return " SELECT * FROM "
         ...
         "      LEFT JOIN "
         "          `conta_a_pagar_has_idcompra` `cp2` ON `cp`.`idPagamento` = `cp2`.`idPagamento` "
         "      LEFT JOIN "
         "          `pedido_fornecedor_has_produto2` `pf2` ON `cp2`.`idCompra` = `pf2`.`idCompra` "
         "      LEFT JOIN "
...
QString Sql::view_followup_venda_misto(const QString &idVenda) {
  return " SELECT * FROM "
         "     (SELECT "
         "          CONCAT('Vendas: ', vhf.idVenda) AS `Identificador`, "
         "          u.nome AS `Usuário`, "
         "          vhf.tipoOperacao AS `Tipo`, "
         "          vhf.observacao AS `Observação`, "
         "          vhf.dataFollowup AS `Data` "
         "      FROM "
         "          venda_has_followup vhf "
         "      LEFT JOIN "
         "          usuario u ON vhf.idUsuario = u.idUsuario "
         "      WHERE "
         "          idVenda LIKE '" + idVenda + "%' "
         "    UNION "
         "      SELECT "
         "          CONCAT('Compras: ', phf.ordemCompra), "
         "          u.nome, "
         "          phf.tipoOperacao, "
         "          phf.observacao, "
         "          phf.dataFollowup "
         "      FROM "
         "          pedido_fornecedor_has_followup phf "
         "      LEFT JOIN "
         ...
         "          ehf.dataFollowup "
         "      FROM "
         "          estoque_has_followup ehf "
         "      LEFT JOIN "
         "          usuario u ON ehf.idUsuario = u.idUsuario "
         "      WHERE "
         "          idEstoque IN (SELECT idEstoque FROM estoque_has_consumo ehc WHERE idVendaProduto2 IN "
         "          (SELECT idVendaProduto2 FROM venda_has_produto2 WHERE idVenda LIKE '" + idVenda + "%')) "
         "    UNION "
         "      SELECT "
         "          CONCAT('NFe: ' , nhf.idNFe), "
         "          u.nome, "
         "          nhf.tipoOperacao, "
         "          nhf.observacao, "
         "          nhf.dataFollowup "
         "      FROM "
         "          nfe_has_followup nhf "
         "      LEFT JOIN "
         "          usuario u on nhf.idUsuario = u.idUsuario "
         "      WHERE "
         "          idNFe IN (SELECT idNFeSaida FROM venda_has_produto2 WHERE idVenda LIKE '" + idVenda + "%') "
         " ) x "
         " ORDER BY `Data` ";
}
