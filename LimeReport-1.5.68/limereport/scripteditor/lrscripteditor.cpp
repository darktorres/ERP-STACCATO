...
ScriptEditor::ScriptEditor(QWidget *parent) : QWidget(parent), ui(new Ui::ScriptEditor), m_reportEngine(0), m_page(0), m_tabIndention(4) {
  ui->setupUi(this);
...
void ScriptEditor::initEditor(DataSourceManager *dm) {
  ...
  if (dm) {
    if (dm->isNeedUpdateDatasourceModel()) dm->updateDatasourceModel();
    ui->twData->setModel(dm->datasourcesModel());
    ui->twScriptEngine->setModel(se.model());
  }

  if (ui->twScriptEngine->selectionModel()) {
    connect(ui->twScriptEngine->selectionModel(), SIGNAL(currentChanged(QModelIndex, QModelIndex)), this, SLOT(slotOnCurrentChanged(QModelIndex, QModelIndex)));
  }
}
...
void ScriptEditor::on_twData_doubleClicked(const QModelIndex &index) {
  ...
  if (node->type() == LimeReport::DataNode::Field) { ui->textEdit->insertPlainText(QString("$D{%1.%2}").arg(node->parent()->name()).arg(node->name())); }
  if (node->type() == LimeReport::DataNode::Variable) { ui->textEdit->insertPlainText(QString("$V{%1}").arg(node->name())); }
...
void ScriptEditor::on_twScriptEngine_doubleClicked(const QModelIndex &index) {
  ...
  if (node->type() == LimeReport::ScriptEngineNode::Function) { ui->textEdit->insertPlainText(node->name() + "()"); }
  ui->textEdit->setFocus();
}
...
QString ReportStructureCompleater::pathFromIndex(const QModelIndex &index) const {
  ...
  for (QModelIndex i = index; i.isValid(); i = i.parent()) { dataList.prepend(model()->data(i, Qt::DisplayRole).toString()); }
  return dataList.join(".");
}

QStringList ReportStructureCompleater::splitPath(const QString &path) const { return path.split("."); }
...
void ReportStructureCompleater::updateCompleaterModel(ReportEnginePrivateInterface *report) {
  if (report) {
    ...
    for (int i = 0; i < report->pageCount(); ++i) {
      ...
      foreach (BaseDesignIntf *item, page->pageItem()->childBaseItems()) { addChildItem(item, itemNode->text(), m_model.invisibleRootItem()); }
    }
...
QStringList ReportStructureCompleater::extractProperties(BaseDesignIntf *item) {
  ...
  QMetaObject const *mo = item->metaObject();
  while (mo) {
    for (int i = mo->propertyOffset(); i < mo->propertyCount(); ++i) { result.append(QString::fromLatin1(mo->property(i).name())); }
    mo = mo->superClass();
  }
  result.sort();
  return result;
...
void ReportStructureCompleater::addChildItem(BaseDesignIntf *item, const QString &pageName, QStandardItem *parent) {
  ...
  foreach (BaseDesignIntf *child, item->childBaseItems()) { addChildItem(child, pageName, parent); }
}
