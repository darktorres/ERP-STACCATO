...
static const char *const keywords[KEYWORDS_COUNT] = {
    "do",      "if",      "in",      "for",      "int",      "new",      "try",      "var",      "byte",      "case",      "char",      "else",       "enum",       "goto",        "long",
    "null",    "this",    "true",    "void",     "with",     "break",    "catch",    "class",    "const",     "false",     "final",     "float",      "short",      "super",       "throw",
    "while",   "delete",  "double",  "export",   "import",   "native",   "public",   "return",   "static",    "switch",    "throws",    "typeof",     "boolean",    "default",     "extends",
    "finally", "package", "private", "abstract", "continue", "debugger", "function", "volatile", "interface", "protected", "transient", "implements", "instanceof", "synchronized"};

enum LiteralsType { SpaceFound, AlpahabetFound, NumberFound, HashFound, SlashFound, AsterixFound, BracketFound, QuotationFound, ApostropheFound, SeparatorFound, BackSlashFound, LiteralsCount };
...
void ScriptHighlighter::highlightBlock(const QString &text) {
  ...
  int oldState = -1;
  int stateMaschine[StatesCount][LiteralsCount] = {{Separator, MayBeKeyWord, MayBeNumber, Separator, MayBeComment, Separator, Separator, String, String2, Separator, Separator},
  ...
  for (;;) {
    ...
    case ' ': literal = SpaceFound; break;
    case '/': literal = SlashFound; break;
    case '*': literal = AsterixFound; break;
    case '#': literal = HashFound; break;
    case '\'': literal = ApostropheFound; break;
    case '\\': literal = BackSlashFound; break;
    case '"': literal = QuotationFound; break;
    case '{':
    case '[':
    case '(':
    case '}':
    case ']':
    case ')': literal = BracketFound; break;
    case '1':
    case '2':
    case '3':
    case '4':
    ...
    case '8':
    case '9':
    case '0': literal = NumberFound; break;
    default:
    ...
    if (oldState != state) {
      switch (state) {
      case MayBeComment:
        if (oldState == MayBeNumber) { setFormat(i - (buffer.length() - 1), buffer.length() - 1, m_formats[NumberFormat]); }
        buffer.clear();
        buffer += currentChar;
        break;
      case String:
      ...
      case MayBeNumber:
        ...
        buffer += currentChar;
        break;
      case Comment2: setCurrentBlockState(Comment2); [[fallthrough]];
      case Separator:
        switch (oldState) {
        case MayBeComment2End:
          ...
          break;
        case MayBeKeyWord:
          if (isKeyWord(buffer.left(buffer.length() - 1))) { setFormat(i - (buffer.length() - 1), buffer.length() - 1, m_formats[KeywordFormat]); }
          buffer.clear();
          break;
        case MayBeNumber:
    ...
    if (state == Comment || state == Comment2) { setFormat(i - (buffer.length() - 1), buffer.length(), m_formats[CommentFormat]); }

    if (state == String || state == String2) { setFormat(i - (buffer.length() - 1), buffer.length(), m_formats[StringFormat]); }
...
TextBlockData::~TextBlockData() {
  foreach (ParenthesisInfo *info, m_parentheses) { delete info; }
}
...
void TextBlockData::insert(ParenthesisInfo *info) {
  int i = 0;
  while (i < m_parentheses.size() && info->position > m_parentheses.at(i)->position) ++i;
