...
namespace {
LimeReport::ObjectPropItem *createReqtItem(QObject *object, LimeReport::ObjectPropItem::ObjectsList *objects, const QString &name, const QString &displayName, const QVariant &data,
...
LimeReport::ObjectPropItem *createReqtUnitItem(QObject *object, LimeReport::ObjectPropItem::ObjectsList *objects, const QString &name, const QString &displayName, const QVariant &data,
                                               LimeReport::ObjectPropItem *parent, bool readonly) {
...
bool VARIABLE_IS_NOT_USED registredRectMMProp =
    LimeReport::ObjectPropFactory::instance().registerCreator(LimeReport::APropIdent("geometry", "LimeReport::BaseDesignIntf"), QObject::tr("geometry"), createReqtUnitItem);
} // namespace

namespace LimeReport {

template <class T> QString rectToString(T rect) { return QString("[%1,%2] %3x%4").arg(rect.x()).arg(rect.y()).arg(rect.width()).arg(rect.height()); }
...
QString LimeReport::RectUnitPropItem::displayValue() const {
  ...
  return QString("[%1,%2] %3x%4 %5").arg(rect.x(), 0, 'f', 2).arg(rect.y(), 0, 'f', 2).arg(rect.width(), 0, 'f', 2).arg(rect.height(), 0, 'f', 2).arg(unitShortName());
}

LimeReport::RectUnitValuePropItem::RectUnitValuePropItem(QObject *object, ObjectsList *objects, const QString &name, const QString &displayName, const QVariant &value, ObjectPropItem *parent,
                                                         bool readonly)
    : ObjectPropItem(object, objects, name, displayName, value, parent, readonly) {}
...
qreal LimeReport::RectUnitValuePropItem::valueInUnits(qreal value) const {
  ...
  case LimeReport::BaseDesignIntf::Millimeters: return value / item->unitFactor();
  case LimeReport::BaseDesignIntf::Inches: return value / (item->unitFactor() * 10);
  }
...
QString LimeReport::RectUnitValuePropItem::unitShortName() const {
  BaseDesignIntf *item = dynamic_cast<BaseDesignIntf *>(object());
  switch (item->unitType()) {
  case LimeReport::BaseDesignIntf::Millimeters: return QObject::tr("mm");
  case LimeReport::BaseDesignIntf::Inches: return QObject::tr("''");
  }

  return QString();
}

QString LimeReport::RectUnitValuePropItem::displayValue() const { return QString("%1 %2").arg(valueInUnits(propertyValue().toReal()), 0, 'f', 2).arg(unitShortName()); }

void LimeReport::RectUnitPropItem::itemPosChanged(QObject * /*object*/, QPointF newPos, QPointF oldPos) {
  if (not qFuzzyCompare(newPos.x(), oldPos.x())) { setValue("x", newPos.x()); }
  if (not qFuzzyCompare(newPos.y(), oldPos.y())) { setValue("y", newPos.y()); }
}

void LimeReport::RectUnitPropItem::itemGeometryChanged(QObject * /*object*/, QRectF newGeometry, QRectF oldGeometry) {
  if (not qFuzzyCompare(newGeometry.x(), oldGeometry.x())) { setValue("x", newGeometry.x()); }
  if (not qFuzzyCompare(newGeometry.y(), oldGeometry.y())) { setValue("y", newGeometry.y()); }
  if (not qFuzzyCompare(newGeometry.width(), oldGeometry.width())) { setValue("width", newGeometry.width()); }
  if (not qFuzzyCompare(newGeometry.height(), oldGeometry.height())) { setValue("height", newGeometry.height()); }
}
  ...
  if (name != "") {
    ...
    if (propItem) {
      ...
      if (itemModel) {
        ...
        if (propItem->modelIndex().isValid()) itemModel->itemDataChanged(propItem->modelIndex());
      }
    }
  }
}
...
QRectF LimeReport::RectUnitPropItem::rectInUnits(QRectF rect) const {
  BaseDesignIntf *item = dynamic_cast<BaseDesignIntf *>(object());
  switch (item->unitType()) {
  case LimeReport::BaseDesignIntf::Millimeters: return QRectF(rect.x() / item->unitFactor(), rect.y() / item->unitFactor(), rect.width() / item->unitFactor(), rect.height() / item->unitFactor());
  case LimeReport::BaseDesignIntf::Inches:
    return QRectF(rect.x() / (item->unitFactor() * 10), rect.y() / (item->unitFactor() * 10), rect.width() / (item->unitFactor() * 10), rect.height() / (item->unitFactor() * 10));
  }
