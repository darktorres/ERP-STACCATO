...
void ReportDesignWidget::createTabs() {
  for (int i = 0; i < m_report->pageCount(); ++i) {
    ...
    connect(m_report->pageAt(i)->pageItem(), SIGNAL(propertyObjectNameChanged(QString, QString)), this, SLOT(slotPagePropertyObjectNameChanged(QString, QString)));
  }
...
bool ReportDesignWidget::isNeedToSave() {
  if (m_report) return m_report->isNeedToSave();
  return false;
}
