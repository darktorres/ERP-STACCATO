...
template <class Clock> auto calcDelta(int magic = 200000) -> decltype(Clock::now().time_since_epoch().count()) {
...
template <class Clock> double calcDuration(int objects) {
  ...
  for (int i = 0; i < objects; i++) { summ += calcDelta<Clock>(); }
...
double calcDurationByRdtsc(int objects) {
  ...
  for (int i = 0; i < objects; i++) { summ += calcDeltaRdtsc(); }
...
double calcDurationBySyscall(int objects, int type) {
  ...
  for (int i = 0; i < objects; i++) { summ += calcDeltaSysCall(200000, type); }
...
double calcDurationByGetTime(int objects, int type) {
  ...
  for (int i = 0; i < objects; i++) { summ += calcDeltaSysGetTime(200000, type); }
...
double calcDurationByGetTimeOfDay(int objects) {
  ...
  for (int i = 0; i < objects; i++) { summ += calcDeltaSysGetTimeOfDay(); }
...
int main(int argc, char *argv[]) {
  if (argc > 1 && argv[1]) { OBJECTS = std::atoi(argv[1]); }
