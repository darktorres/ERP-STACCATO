...
AbstractLayout::AbstractLayout(QString xmlTag, QObject *owner, QGraphicsItem *parent)
    : LayoutDesignIntf(xmlTag, owner, parent), m_isRelocating(false), m_layoutType(Layout), m_hideEmptyItems(false), m_layoutSpacing(0) {
...
void AbstractLayout::paintChild(BaseDesignIntf *child, QPointF parentPos, QPainter *painter) {
  if (!child->childBaseItems().isEmpty()) {
    foreach (BaseDesignIntf *item, child->childBaseItems()) { paintChild(item, child->pos(), painter); }
  }
  painter->drawRect(QRectF(parentPos.x() + child->pos().x(), parentPos.y() + child->pos().y(), child->rect().bottomRight().rx(), child->rect().bottomRight().ry()));
}
...
void AbstractLayout::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget) {
  if (isSelected()) {
    ...
    foreach (BaseDesignIntf *item, m_children) { paintChild(item, QPointF(0, 0), painter); }
    painter->restore();
  }
...
QVariant AbstractLayout::itemChange(QGraphicsItem::GraphicsItemChange change, const QVariant &value) {
  if (change == QGraphicsItem::ItemSelectedHasChanged) {
    ...
    foreach (BaseDesignIntf *item, layoutsChildren()) { item->setVisible(!value.toBool()); }
...
void AbstractLayout::updateItemSize(DataSourceManager *dataManager, RenderPass pass, int maxHeight) {
  ...
  foreach (QGraphicsItem *child, childItems()) {
    ...
    if (item && item->isNeedUpdateSize(pass)) item->updateItemSize(dataManager, pass, maxHeight);
  }
  updateLayoutSize();
  relocateChildren();
  setIsRelocating(false);
...
void AbstractLayout::rebuildChildrenIfNeeded() {
  if (layoutsChildren().count() < childItems().size() - 1) {
    ...
    foreach (BaseDesignIntf *childItem, childBaseItems()) { layoutsChildren().append(childItem); }
    sortChildren();
  }
}
...
void AbstractLayout::slotOnChildSelectionHasChanged(BaseDesignIntf *item, bool value) { item->setZValue(value ? item->zValue() + 1 : item->zValue() - 1); }
