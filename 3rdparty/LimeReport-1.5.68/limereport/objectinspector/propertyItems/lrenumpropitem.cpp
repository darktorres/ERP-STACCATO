...
LimeReport::ObjectPropItem *createEnumPropItem(QObject *object, LimeReport::ObjectPropItem::ObjectsList *objects, const QString &name, const QString &displayName, const QVariant &data,
...
bool VARIABLE_IS_NOT_USED registred = LimeReport::ObjectPropFactory::instance().registerCreator(LimeReport::APropIdent("enum", ""), QObject::tr("enum"), createEnumPropItem);
...
QWidget *EnumPropItem::createProperyEditor(QWidget *parent) const {
  ...
  for (int i = 0; i < propEnum.keyCount(); i++) {
    if (m_acceptableValues.isEmpty())
      ...
      if (m_acceptableValues.contains(propEnum.value(i))) { enumValues.append(tr(propEnum.key(i))); }
    }
  }
...
void EnumPropItem::initTranslation() {
  QMetaEnum propEnum = object()->metaObject()->property(object()->metaObject()->indexOfProperty(propertyName().toLatin1())).enumerator();
  for (int i = 0; i < propEnum.keyCount(); i++) { m_translation.insert(QString(tr(propEnum.key(i))), QString(propEnum.key(i))); }
}
