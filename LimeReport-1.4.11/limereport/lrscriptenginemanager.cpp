...
QScriptValue line(QScriptContext *pcontext, QScriptEngine *pengine) {
  ...
  if (dm->variable(varName).isValid()) {
    ...
    res = pengine->newVariant(QString("Variable line for band %1 not found").arg(band));
  return res;
}
...
QScriptValue numberFormat(QScriptContext *pcontext, QScriptEngine *pengine) {
  ...
  QScriptValue res =
      (locale.isEmpty()) ? pengine->newVariant(QString::number(value.toDouble(), format, precision)) : pengine->newVariant(QLocale(locale).toString(value.toDouble(), format, precision));
  return res;
}
...
QScriptValue now(QScriptContext * /*pcontext*/, QScriptEngine *pengine) { return pengine->newVariant(QDateTime::currentDateTime()); }

QScriptValue date(QScriptContext * /*pcontext*/, QScriptEngine *pengine) { return pengine->newVariant(QDate::currentDate()); }
...
QScriptValue groupFunction(QScriptContext *pcontext, QScriptEngine *pengine) { return callGroupFunction(pcontext->callee().property("functionName").toString(), pcontext, pengine); }
...
ScriptEngineManager::ScriptEngineManager() : m_model(nullptr), m_dataManager(nullptr) {
  ...
  addFunction("numberFormat", numberFormat, "NUMBER", R"(numberFormat(")" + tr("Value") + R"(",")" + tr("Format") + R"(",")" + tr("Precision") + R"(",")" + tr("Locale") + R"("))");
  ...
  while (curPos < m_context.length()) {
    ...
    case OpenBracketFound:
      if (m_context[curPos] == '}') {
        ...
        if (m_context[curPos] == '{') extractBracket(curPos);
      }
      [[fallthrough]];
    case None:
      if (m_context[curPos] == '$') {
...
bool ScriptExtractor::isStartScriptLexem(int &curPos) { return isStartLexem(curPos, SCRIPT_SIGN); }

bool ScriptExtractor::isStartFieldLexem(int &curPos) { return isStartLexem(curPos, FIELD_SIGN); }

bool ScriptExtractor::isStartVariableLexem(int &curPos) { return isStartLexem(curPos, VARIABLE_SIGN); }
...
#ifdef HAVE_UI_LOADER
void ScriptEngineContext::addDialog(const QString &name, const QByteArray &description) { m_dialogs.push_back(DialogDescriber::create(name, description)); }
...
void ScriptEngineContext::setInitScript(const QString &initScript) { m_initScript = initScript; }
