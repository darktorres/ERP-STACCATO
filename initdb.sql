...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_venda` (`idLoja` INT, `idUsuario` INT, `idUsuarioConsultor` INT, `statusFinanceiro` INT, `dataFinanceiro` INT, `status` INT, `diasRestantes` INT, `idVenda` INT, `vendedor` INT, `consultor` INT, `cliente` INT, `profissional` INT, `data` INT, `data2` INT, `data3` INT, `total` INT, `idFollowup` INT, `dataFollowup` INT, `observacao` INT);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`ViewExcelEntregasPendentes` AS select date_format(`v`.`data`,'%Y-%m-%d') AS `data`,date_format((`v`.`data` + (`v`.`prazoEntrega` * 1000000)),'%Y-%m-%d') AS `przentrega`,date_format((`v`.`data` + (`v`.`novoPrazoEntrega` * 1000000)),'%Y-%m-%d') AS `nvprzentrega`,`vp`.`status` AS `status`,`vp`.`fornecedor` AS `fornecedor`,`vp`.`idVenda` AS `idvenda`,`vp`.`produto` AS `produto`,`vp`.`kg` AS `kg`,`vp`.`caixas` AS `caixas`,`vp`.`quant` AS `quant`,`vp`.`un` AS `un`,`vp`.`codComercial` AS `codComercial`,`vp`.`formComercial` AS `formComercial`,`vp`.`dataPrevEnt` AS `dataPrevEnt`,`v`.`representacao` AS `representacao` from (`staccato`.`venda_has_produto2` `vp` left join `staccato`.`venda` `v` on((`vp`.`idVenda` = `v`.`idVenda`))) where (`vp`.`status` not in ('entregue','cancelado','finalizado','devolvido','devolvido estoque','devolvido forn.','substituido','quebrado'));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_orcamento` AS select `o`.`idLoja` AS `idLoja`,`o`.`idUsuario` AS `idUsuario`,`o`.`idUsuarioConsultor` AS `idUsuarioConsultor`,`o`.`status` AS `status`,if((`o`.`status` not in ('FECHADO','PERDIDO','CANCELADO','REPLICADO')),(`o`.`validade` - (to_days(now()) - to_days(`o`.`data`))),'') AS `diasRestantes`,`o`.`idOrcamento` AS `idOrcamento`,`u`.`nome` AS `vendedor`,`u2`.`nome` AS `consultor`,`c`.`nome_razao` AS `cliente`,`p`.`nome_razao` AS `profissional`,`c`.`tel` AS `tel`,`c`.`telCel` AS `telCel`,if((`p`.`telCel` is null),if((`p`.`tel` is null),`p`.`telCom`,`p`.`tel`),`p`.`telCel`) AS `telProf`,`o`.`data` AS `data`,`o`.`data2` AS `data2`,`o`.`total` AS `total`,`ohf`.`idFollowup` AS `idFollowup`,`ohf`.`dataFollowup` AS `dataFollowup`,`ohf`.`dataProxFollowup` AS `dataProxFollowup`,`ohf`.`observacao` AS `observacao`,`ohf`.`semaforo` AS `semaforo` from (((((`staccato`.`orcamento` `o` left join `staccato`.`cliente` `c` on((`o`.`idCliente` = `c`.`idCliente`))) left join `staccato`.`usuario` `u` on((`o`.`idUsuario` = `u`.`idUsuario`))) left join `staccato`.`profissional` `p` on((`o`.`idProfissional` = `p`.`idProfissional`))) left join `staccato`.`usuario` `u2` on((`o`.`idUsuarioConsultor` = `u2`.`idUsuario`))) left join `staccato`.`orcamento_has_followup` `ohf` on((`o`.`idFollowup` = `ohf`.`idFollowup`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_venda` AS select `v`.`idLoja` AS `idLoja`,`v`.`idUsuario` AS `idUsuario`,`v`.`idUsuarioConsultor` AS `idUsuarioConsultor`,`v`.`statusFinanceiro` AS `statusFinanceiro`,`v`.`dataFinanceiro` AS `dataFinanceiro`,`v`.`status` AS `status`,if((`v`.`status` not in ('ENTREGUE','CANCELADO','DEVOLVIDO')),((to_days(`v`.`data`) + `v`.`prazoEntrega`) - to_days(now())),'') AS `diasRestantes`,`v`.`idVenda` AS `idVenda`,`u`.`nome` AS `vendedor`,`u2`.`nome` AS `consultor`,`c`.`nome_razao` AS `cliente`,`p`.`nome_razao` AS `profissional`,`v`.`data` AS `data`,`v`.`data2` AS `data2`,`v`.`data3` AS `data3`,`v`.`total` AS `total`,`vhf`.`idFollowup` AS `idFollowup`,`vhf`.`dataFollowup` AS `dataFollowup`,`vhf`.`observacao` AS `observacao` from ((((((`staccato`.`venda` `v` left join `staccato`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`))) left join `staccato`.`cliente_has_endereco` `e` on((`v`.`idEnderecoEntrega` = `e`.`idEndereco`))) left join `staccato`.`profissional` `p` on((`v`.`idProfissional` = `p`.`idProfissional`))) left join `staccato`.`cliente` `c` on((`v`.`idCliente` = `c`.`idCliente`))) left join `staccato`.`usuario` `u2` on((`v`.`idUsuarioConsultor` = `u2`.`idUsuario`))) left join `staccato`.`venda_has_followup` `vhf` on((`v`.`idFollowup` = `vhf`.`idFollowup`)));
