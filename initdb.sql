...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mydb`.`view_resumo_relatorio` AS select `l`.`descricao` AS `Loja`,`u`.`user` AS `Vendedor`,sum(if(((date_format(`o`.`data`,'%Y-%m') < convert(`MYDATE`() using utf8mb4)) and ((`o`.`status` in ('ATIVO','EXPIRADO')) or ((`o`.`status` = 'FECHADO') and (date_format(`v`.`data`,'%Y-%m') >= convert(`MYDATE`() using utf8mb4))) or ((`o`.`status` = 'PERDIDO') and (date_format(`o`.`lastUpdated`,'%Y-%m') >= convert(`MYDATE`() using utf8mb4))))),`o`.`total`,0)) AS `Validos Anteriores`,sum(if(((date_format(`o`.`data`,'%Y-%m') = convert(`MYDATE`() using utf8mb4)) and isnull(`o_1`.`data`)),`o`.`total`,0)) AS `Gerados Mes`,sum(if(((date_format(`o`.`data`,'%Y-%m') = convert(`MYDATE`() using utf8mb4)) and (`o_1`.`data` is not null)),`o`.`total`,0)) AS `Revalidados Mes`,sum(if(((date_format(`v`.`data`,'%Y-%m') = convert(`MYDATE`() using utf8mb4)) and (`o`.`status` = 'FECHADO')),`o`.`total`,0)) AS `Fechados Mes`,sum(if(((date_format(`o`.`lastUpdated`,'%Y-%m') = convert(`MYDATE`() using utf8mb4)) and (`o`.`status` = 'PERDIDO')),`o`.`total`,0)) AS `Perdidos Mes`,sum(if(((date_format(`o`.`data`,'%Y-%m') <= convert(`MYDATE`() using utf8mb4)) and ((`o`.`status` = 'ATIVO') or (`o`.`status` = 'EXPIRADO') or ((`o`.`status` = 'FECHADO') and (date_format(`v`.`data`,'%Y-%m') > convert(`MYDATE`() using utf8mb4))) or ((`o`.`status` = 'PERDIDO') and (date_format(`o`.`lastUpdated`,'%Y-%m') > convert(`MYDATE`() using utf8mb4))))),`o`.`total`,0)) AS `Validos Mes`,((sum(if(((date_format(`v`.`data`,'%Y-%m') = convert(`MYDATE`() using utf8mb4)) and (`o`.`status` = 'FECHADO')),`o`.`total`,0)) / (sum(if(((date_format(`o`.`data`,'%Y-%m') = convert(`MYDATE`() using utf8mb4)) and isnull(`o_1`.`data`)),`o`.`total`,0)) + sum(if(((date_format(`o`.`data`,'%Y-%m') = convert(`MYDATE`() using utf8mb4)) and (`o_1`.`data` is not null)),`o`.`total`,0)))) * 100) AS `% Fechados / Gerados`,((sum(if(((date_format(`v`.`data`,'%Y-%m') = convert(`MYDATE`() using utf8mb4)) and (`o`.`status` = 'FECHADO')),`o`.`total`,0)) / ((sum(if(((date_format(`o`.`data`,'%Y-%m') < convert(`MYDATE`() using utf8mb4)) and ((`o`.`status` = 'ATIVO') or (`o`.`status` = 'EXPIRADO') or ((`o`.`status` = 'FECHADO') and (date_format(`v`.`data`,'%Y-%m') >= convert(`MYDATE`() using utf8mb4))) or ((`o`.`status` = 'PERDIDO') and (date_format(`o`.`lastUpdated`,'%Y-%m') >= convert(`MYDATE`() using utf8mb4))))),`o`.`total`,0)) + sum(if(((date_format(`o`.`data`,'%Y-%m') = convert(`MYDATE`() using utf8mb4)) and isnull(`o_1`.`data`)),`o`.`total`,0))) + sum(if(((date_format(`o`.`data`,'%Y-%m') = convert(`MYDATE`() using utf8mb4)) and (`o_1`.`data` is not null)),`o`.`total`,0)))) * 100) AS `% Fechados / Carteira` from ((((`mydb`.`orcamento` `o` left join `mydb`.`loja` `l` on((`o`.`idLoja` = `l`.`idLoja`))) left join `mydb`.`usuario` `u` on((`o`.`idUsuario` = `u`.`idUsuario`))) left join `mydb`.`orcamento` `o_1` on((`o`.`replicadoDe` = `o_1`.`idOrcamento`))) left join `mydb`.`venda` `v` on((`o`.`idOrcamento` = `v`.`idOrcamento`))) where (`o`.`status` not in ('REPLICADO','CANCELADO')) group by `l`.`descricao`,`u`.`user` order by `l`.`descricao`,`u`.`user`;
