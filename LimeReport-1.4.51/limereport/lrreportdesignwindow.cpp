...
void ReportDesignWindow::slotItemSelected(LimeReport::BaseDesignIntf *item) {
  if (m_propertyModel->currentObject() != item) {
    ...
    if (m_propertyModel->subclassesAsLevel()) m_objectInspector->expandToDepth(0);
...
void ReportDesignWindow::slotPreviewReport() { m_reportDesignWidget->previewReport(); }
...
void ReportDesignWindow::renderPageFinished(int renderedPageCount) {
  if (m_progressDialog) m_progressDialog->setLabelText(QString::number(renderedPageCount) + tr(" page rendered"));
}
