...
namespace {
LimeReport::ObjectPropItem *createAlignItem(QObject *object, LimeReport::ObjectPropItem::ObjectsList *objects, const QString &name, const QString &displayName, const QVariant &data,
...
bool VARIABLE_IS_NOT_USED registred = LimeReport::ObjectPropFactory::instance().registerCreator(LimeReport::APropIdent("alignment", "LimeReport::TextItem"), QObject::tr("alignment"), createAlignItem);
} // namespace
...
QString AlignmentPropItem::displayValue() const { return associateValue(propertyValue().toInt(), &m_horizMap) + " | " + associateValue(propertyValue().toInt(), &m_vertMap); }
...
AlignmentItemEditor::AlignmentItemEditor(QObject *object, ObjectsList *objects, const QString &name, const QString &displayName, const QVariant &value, ObjectPropItem *parent, bool readonly,
...
int AlignmentItemEditor::clearAcceptableValues(int flags) {
  AlignMap::const_iterator it = m_acceptableValues.constBegin();
  for (; it != m_acceptableValues.constEnd(); ++it) {
    if (flags & it.value()) flags = flags ^ it.value();
  }
  return flags;
}
...
QString AlignmentItemEditor::displayValue() const { return m_acceptableValues.key(Qt::Alignment(propertyValue().toInt())); }

void AlignmentItemEditor::setPropertyValue(QVariant value) { ObjectPropItem::setPropertyValue(extractAcceptableValue(value.toInt())[0]); }
