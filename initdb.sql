...
-- Schema staccato
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `staccato` DEFAULT CHARACTER SET utf8mb4 ;
USE `staccato` ;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_relatorio_loja` AS select `v`.`Loja` AS `Loja`,sum(`v`.`Faturamento`) AS `Faturamento`,sum(`v`.`Comissão`) AS `Comissão`,((sum(`v`.`Comissão`) / sum(`v`.`Faturamento`)) * 100) AS `%`,`v`.`Mês` AS `Mês`,(group_concat(distinct `r`.`custoReposicao` separator ',') + 0.0) AS `Reposição` from (`staccato`.`view_relatorio` `v` left join `staccato`.`view_relatorio_reposicao` `r` on(((`r`.`loja` = `v`.`Loja`) and (convert(`r`.`data` using utf8mb4) = `v`.`Mês`)))) group by `v`.`Loja`,`v`.`Mês`;
