...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`view_venda_produto` (`status` INT, `statusFinanceiro` INT, `prazoEntrega` INT, `novoPrazoEntrega` INT, `idVenda` INT, `Vendedor` INT, `fornecedor` INT, `produto` INT, `obs` INT, `caixas` INT, `quant` INT, `un` INT, `codComercial` INT, `formComercial` INT);
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `mydb`.`view_venda_produto` AS select `vp`.`status` AS `status`,`v`.`statusFinanceiro` AS `statusFinanceiro`,cast((`v`.`data` + interval `v`.`prazoEntrega` day) as date) AS `prazoEntrega`,cast((`v`.`data` + interval `v`.`novoPrazoEntrega` day) as date) AS `novoPrazoEntrega`,`vp`.`idVenda` AS `idVenda`,substring_index(`u`.`nome`,' ',1) AS `Vendedor`,`vp`.`fornecedor` AS `fornecedor`,`vp`.`produto` AS `produto`,`vp`.`obs` AS `obs`,`vp`.`caixas` AS `caixas`,`vp`.`quant` AS `quant`,`vp`.`un` AS `un`,`vp`.`codComercial` AS `codComercial`,`vp`.`formComercial` AS `formComercial` from ((`mydb`.`venda` `v` left join `mydb`.`venda_has_produto` `vp` on((`v`.`idVenda` = `vp`.`idVenda`))) left join `mydb`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`)));
