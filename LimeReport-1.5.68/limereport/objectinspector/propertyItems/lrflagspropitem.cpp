...
LimeReport::ObjectPropItem *createFlagsPropItem(QObject *object, LimeReport::ObjectPropItem::ObjectsList *objects, const QString &name, const QString &displayName, const QVariant &data,
...
bool VARIABLE_IS_NOT_USED registred = LimeReport::ObjectPropFactory::instance().registerCreator(LimeReport::APropIdent("flags", ""), QObject::tr("flags"), createFlagsPropItem);
...
void FlagsPropItem::createChildren() {
  ...
  for (int i = 0; i < propEnum.keyCount(); i++) {
    if (propEnum.keyToValue(propEnum.key(i)) != 0) {
      this->appendItem(new LimeReport::FlagPropItem(object(), objects(), QString(propEnum.key(i)), tr(propEnum.key(i)),
                                                    bool((propertyValue().toInt() & propEnum.keyToValue(propEnum.key(i))) == propEnum.keyToValue(propEnum.key(i))), this, false));
    }
  }
}
...
void FlagsPropItem::updateChildren() {
  ...
  for (int i = 0; i < propEnum.keyCount(); i++) {
    ...
    if (property) property->setPropertyValue(bool((propertyValue().toInt() & propEnum.keyToValue(propEnum.key(i))) == propEnum.keyToValue(propEnum.key(i))));
  }
}
  ...
  for (int i = 0; i < propEnum.keyCount(); i++) {
    if ((propEnum.keyToValue(propEnum.key(i)) == 0) ? propertyValue().toInt() == 0 : (propertyValue().toInt() & propEnum.keyToValue(propEnum.key(i))) == propEnum.keyToValue(propEnum.key(i))) {
      if (result.isEmpty())
        ...
        result = result + " | " + (isTranslateProperty() ? tr(propEnum.key(i)) : propEnum.key(i));
    }
  }
  return result;
}
