...
ImageItemEditor::ImageItemEditor(LimeReport::IEditableImageItem *item, QWidget *parent) : QWidget(parent), ui(new Ui::ImageItemEditor), m_item(item) {
  ui->setupUi(this);
...
void ImageItemEditor::on_toolButton_clicked() {
  ...
  if (!m_resourcePathImage.isNull() && m_image.isNull()) ui->imageViewer->setPixmap(m_resourcePathImage);
}
