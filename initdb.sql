...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_produto_pendente` AS select `vp2`.`status` AS `status`,`vp2`.`idVendaProduto2` AS `idVendaProduto2`,`vp2`.`idVendaProdutoFK` AS `idVendaProdutoFK`,`vp2`.`fornecedor` AS `fornecedor`,`vp2`.`idVenda` AS `idVenda`,`vp2`.`idProduto` AS `idProduto`,`vp2`.`produto` AS `produto`,`vp2`.`obs` AS `obs`,`p`.`colecao` AS `colecao`,`vp2`.`formComercial` AS `formComercial`,`vp2`.`quant` AS `quant`,`vp2`.`un` AS `un`,`p`.`un2` AS `Un2`,`vp2`.`caixas` AS `Caixas`,`p`.`kgcx` AS `kgcx`,`vp2`.`codComercial` AS `codComercial`,`p`.`codBarras` AS `codBarras`,`p`.`custo` AS `custo`,`vp2`.`descUnitario` AS `custoVenda`,`vp2`.`idCompra` AS `idCompra`,`p`.`st` AS `st` from (`staccato`.`venda_has_produto2` `vp2` left join `staccato`.`produto` `p` on((`vp2`.`idProduto` = `p`.`idProduto`)));
