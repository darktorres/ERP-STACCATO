...
CREATE TABLE IF NOT EXISTS `staccato`.`loja` (
  ...
  `ultimaConsultaNSU` TIMESTAMP NULL DEFAULT NULL,
  `proximaConsultaPermitida` TIMESTAMP NULL DEFAULT NULL,
...
-- -----------------------------------------------------
-- Table `staccato`.`apartamento_cliente`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`apartamento_cliente` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `token` VARCHAR(12) NOT NULL,
  `nome` VARCHAR(255) NOT NULL,
  `email` VARCHAR(255) NULL DEFAULT NULL,
  `email_alternativo` VARCHAR(255) NULL DEFAULT NULL,
  `telefone` VARCHAR(50) NULL DEFAULT NULL,
  `ref` ENUM('1dorm', '2dorm') NOT NULL,
  `unidade` SMALLINT(6) NULL DEFAULT NULL,
  `area_privativa` DECIMAL(7,4) NULL DEFAULT NULL,
  `forma_pagamento` ENUM('direta', 'financiada') NULL DEFAULT NULL,
  `piso_incluso` TINYINT(1) NOT NULL DEFAULT '0',
  `contato_arquiteto` TINYINT(1) NOT NULL DEFAULT '0',
  `total` DECIMAL(10,2) NULL DEFAULT NULL,
  `saved_at` TIMESTAMP NULL DEFAULT NULL,
  `link_enviado_em` TIMESTAMP NULL DEFAULT NULL,
  `docusign_envelope_id` VARCHAR(64) NULL DEFAULT NULL,
  `docusign_status` VARCHAR(32) NULL DEFAULT NULL,
  `catalog_version` VARCHAR(40) NULL DEFAULT NULL,
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `last_updated` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uq_token` (`token` ASC) VISIBLE,
  UNIQUE INDEX `uq_nome_unidade` (`nome` ASC, `unidade` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `staccato`.`apartamento_selecao_item`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`apartamento_selecao_item` (
  `id` INT(11) NOT NULL AUTO_INCREMENT,
  `id_apartamento_cliente` INT(11) NOT NULL,
  ...
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `uq_client_group` (`id_apartamento_cliente` ASC, `group_id` ASC) VISIBLE,
  CONSTRAINT `fk_apartamento_selecao_cliente`
    FOREIGN KEY (`id_apartamento_cliente`)
    REFERENCES `staccato`.`apartamento_cliente` (`id`)
    ON DELETE CASCADE)
ENGINE = InnoDB
AUTO_INCREMENT = 16
DEFAULT CHARACTER SET = utf8mb4
...
-- Table `staccato`.`cfop`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`cfop` (
...
CREATE TABLE IF NOT EXISTS `staccato`.`cidade` (
  ...
  `uf` CHAR(2) NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `cidade_nome` (`nome` ASC) VISIBLE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci
ROW_FORMAT = DYNAMIC;
...
CREATE TABLE IF NOT EXISTS `staccato`.`profissional` (
  ...
  `cc` VARCHAR(45) NULL DEFAULT NULL,
  `poupanca` TINYINT(1) NULL DEFAULT NULL,
  `nomeBanco` VARCHAR(180) NULL DEFAULT NULL,
  `cpfBanco` VARCHAR(45) NULL DEFAULT NULL,
  `cnpjBanco` VARCHAR(45) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `staccato`.`cliente` (
  ...
  `exibirLogo` TINYINT(1) NOT NULL DEFAULT '1' COMMENT 'Whether to show company logo on quotes for this client',
  `desativado` TINYINT(1) NOT NULL DEFAULT '0',
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `lastUpdated` TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `staccato`.`comissao` (
  ...
  `Comissão_2` DECIMAL(15,4) NULL DEFAULT NULL,
  `dsr` DECIMAL(15,4) NULL DEFAULT NULL,
  `dsr_2` DECIMAL(15,4) NULL DEFAULT NULL,
  `ComissãoDsr` DECIMAL(15,4) NULL DEFAULT NULL,
  `ComissãoDsr_2` DECIMAL(15,4) NULL DEFAULT NULL,
...
-- -----------------------------------------------------
-- Table `staccato`.`compra_avulsa`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`compra_avulsa` (
  ...
  `statusFinanceiro` VARCHAR(45) NULL DEFAULT NULL,
  ...
  `idCompra` VARCHAR(45) NULL DEFAULT NULL,
  ...
  `quantUpd` INT(11) NULL DEFAULT '0',
  ...
  INDEX `pf1_status` (`status` ASC) VISIBLE)
ENGINE = InnoDB
AUTO_INCREMENT = 149002
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci
ROW_FORMAT = DYNAMIC;
...
CREATE TABLE IF NOT EXISTS `staccato`.`config` (
  ...
  `contaComissaoLoja` INT(11) NULL DEFAULT NULL,
  `contaComissaoProfissional` INT(11) NULL DEFAULT NULL,
  `contaComissaoVendedor` INT(11) NULL DEFAULT NULL,
...
-- -----------------------------------------------------
-- Table `staccato`.`conta_a_pagar_has_idcompra`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`conta_a_pagar_has_idcompra` (
...
CREATE TABLE IF NOT EXISTS `staccato`.`conta_a_pagar_has_pagamento` (
  ...
  `compraAvulsa` TINYINT(1) NULL DEFAULT NULL,
  `desativado` TINYINT(1) NOT NULL DEFAULT '0',
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `lastUpdated` TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
  ...
  INDEX `idx_conta_pagar_status_date_valor` (`status` ASC, `dataPagamento` ASC, `valor` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `staccato`.`orcamento` (
  ...
  `data2` VARCHAR(45) NULL DEFAULT NULL,
  ...
  `fornecedores` VARCHAR(200) NULL DEFAULT NULL,
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `lastUpdated` TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
  ...
  INDEX `index9` (`data2` ASC) VISIBLE,
  INDEX `fk_Orcamento_Followup_idx` (`idFollowup` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_Orcamento_EnderecoFat`
    ...
    REFERENCES `staccato`.`cliente_has_endereco` (`idEndereco`),
  CONSTRAINT `fk_Orcamento_Followup`
    FOREIGN KEY (`idFollowup`)
    REFERENCES `staccato`.`orcamento_has_followup` (`idFollowup`),
...
CREATE TABLE IF NOT EXISTS `staccato`.`venda` (
  ...
  `data` DATETIME NOT NULL,
  `data2` VARCHAR(45) NULL DEFAULT NULL,
  `data3` VARCHAR(45) NULL DEFAULT NULL,
  ...
  `fornecedores` VARCHAR(200) NULL DEFAULT NULL,
  `ordemRepresentacao` VARCHAR(90) NULL DEFAULT NULL,
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `lastUpdated` TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`idVenda`),
  ...
  INDEX `venda_data2` (`data2` ASC) VISIBLE,
  INDEX `fk_Venda_Followup_idx` (`idFollowup` ASC) VISIBLE,
  INDEX `venda_data3` (`data3` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_Venda_Endereco2`
    FOREIGN KEY (`idEnderecoFaturamento`)
    REFERENCES `staccato`.`cliente_has_endereco` (`idEndereco`),
  CONSTRAINT `fk_Venda_Followup`
    FOREIGN KEY (`idFollowup`)
    REFERENCES `staccato`.`venda_has_followup` (`idFollowup`),
...
CREATE TABLE IF NOT EXISTS `staccato`.`conta_a_receber_has_pagamento` (
  ...
  `idVenda` VARCHAR(30) NULL DEFAULT NULL,
  `idLoja` INT(10) UNSIGNED NOT NULL,
  `contraParte` VARCHAR(255) NULL DEFAULT NULL,
  `idNFe` INT(11) NULL DEFAULT NULL,
  `nfe` VARCHAR(44) NULL DEFAULT NULL,
  `valor` DECIMAL(15,4) NOT NULL,
  `tipo` VARCHAR(45) NOT NULL,
  ...
  INDEX `idx_conta_receber_status_date_rep` (`status` ASC, `dataPagamento` ASC, `representacao` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `staccato`.`estoque` (
  ...
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `lastUpdated` TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
  `cstIBS` VARCHAR(3) NULL DEFAULT NULL COMMENT 'CST IBS',
  `cClassTribIBS` VARCHAR(6) NULL DEFAULT NULL COMMENT 'Classificação Tributária IBS (6 dígitos)',
  `vBCIBS` DECIMAL(15,4) NULL DEFAULT NULL COMMENT 'Base cálculo comum IBS/CBS',
  `pIBSUF` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Alíquota IBS da UF',
  `vTribOpIBSUF` DECIMAL(13,2) NULL DEFAULT NULL COMMENT 'Valor bruto tributo UF',
  `pDifIBSUF` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Percentual diferimento IBS UF',
  `vDifIBSUF` DECIMAL(13,2) NULL DEFAULT NULL COMMENT 'Valor diferimento IBS UF',
  `pRedAliqIBSUF` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Percentual redução alíquota IBS UF',
  `pAliqEfetIBSUF` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Alíquota efetiva IBS UF',
  `pIBSMun` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Alíquota IBS Municipal',
  `vTribOpIBSMun` DECIMAL(13,2) NULL DEFAULT NULL COMMENT 'Valor bruto tributo Município',
  `pDifIBSMun` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Percentual diferimento IBS Mun',
  `vDifIBSMun` DECIMAL(13,2) NULL DEFAULT NULL COMMENT 'Valor diferimento IBS Mun',
  `pRedAliqIBSMun` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Percentual redução alíquota IBS Mun',
  `pAliqEfetIBSMun` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Alíquota efetiva IBS Mun',
  `cstCBS` VARCHAR(3) NULL DEFAULT NULL COMMENT 'CST CBS',
  `cClassTribCBS` VARCHAR(6) NULL DEFAULT NULL COMMENT 'Classificação Tributária CBS',
  `vBCCBS` DECIMAL(15,4) NULL DEFAULT NULL COMMENT 'Base cálculo CBS',
  `pCBS` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Alíquota CBS',
  `vCBS` DECIMAL(13,2) NULL DEFAULT NULL COMMENT 'Valor CBS',
  `vTribOpCBS` DECIMAL(13,2) NULL DEFAULT NULL COMMENT 'Valor bruto tributo CBS',
  `pDifCBS` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Percentual diferimento CBS',
  `vDifCBS` DECIMAL(13,2) NULL DEFAULT NULL COMMENT 'Valor diferimento CBS',
  `pRedAliqCBS` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Percentual redução alíquota CBS',
  `pAliqEfetCBS` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Alíquota efetiva CBS',
  `cstIS` VARCHAR(3) NULL DEFAULT NULL COMMENT 'CST Imposto Seletivo',
  `cClassTribIS` VARCHAR(6) NULL DEFAULT NULL COMMENT 'Classificação Tributária IS',
  `vBCIS` DECIMAL(15,4) NULL DEFAULT NULL COMMENT 'Base cálculo IS',
  `pIS` DECIMAL(5,4) NULL DEFAULT NULL COMMENT 'Alíquota IS',
  `vIS` DECIMAL(13,2) NULL DEFAULT NULL COMMENT 'Valor IS',
  `vTribOpIS` DECIMAL(13,2) NULL DEFAULT NULL COMMENT 'Valor bruto tributo IS',
  ...
  INDEX `idx_estoque_cClassTribIBS` (`cClassTribIBS` ASC) VISIBLE,
  INDEX `idx_estoque_cClassTribCBS` (`cClassTribCBS` ASC) VISIBLE,
  INDEX `idx_estoque_cClassTribIS` (`cClassTribIS` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `staccato`.`produto` (
  ...
  `minimo` DECIMAL(15,4) NULL DEFAULT NULL,
  `minimoUpd` INT(11) NOT NULL DEFAULT '0',
  `multiplo` DECIMAL(15,4) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `staccato`.`estoque_has_consumo` (
  ...
  INDEX `consumo_idVenda2` (`idVendaProduto2` ASC) VISIBLE,
  INDEX `idx_ehc_cClassTribIBS` (`cClassTribIBS` ASC) VISIBLE,
  INDEX `idx_ehc_cClassTribCBS` (`cClassTribCBS` ASC) VISIBLE,
  INDEX `idx_ehc_cClassTribIS` (`cClassTribIS` ASC) VISIBLE,
...
-- -----------------------------------------------------
-- Table `staccato`.`feriados`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`feriados` (
...
-- -----------------------------------------------------
-- Table `staccato`.`feriados_bancarios`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`feriados_bancarios` (
  `data` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`data`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb4
...
-- -----------------------------------------------------
-- Table `staccato`.`folha_pagamento`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`folha_pagamento` (
  `idPagamento` INT(11) NOT NULL AUTO_INCREMENT,
  `dataEmissao` DATE NULL DEFAULT NULL,
  ...
  INDEX `index7_pagar` (`nfe` ASC) VISIBLE,
  INDEX `index8_pagar` (`idNFe` ASC) VISIBLE,
  CONSTRAINT `folha_pagamento_idLoja`
    FOREIGN KEY (`idLoja`)
    REFERENCES `staccato`.`loja` (`idLoja`))
ENGINE = InnoDB
AUTO_INCREMENT = 264379
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci
ROW_FORMAT = DYNAMIC;
...
-- -----------------------------------------------------
-- Table `staccato`.`imposto_classificacao`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`imposto_classificacao` (
  `idClassTrib` INT(11) NOT NULL AUTO_INCREMENT,
  `codigo` VARCHAR(6) NOT NULL COMMENT 'Código 6 dígitos',
  `tipo` ENUM('IBS', 'CBS', 'IS') NOT NULL COMMENT 'Tipo de imposto',
  `descricao` VARCHAR(500) NULL DEFAULT NULL COMMENT 'Descrição da classificação',
  `dispositivo` VARCHAR(100) NULL DEFAULT NULL COMMENT 'Referência na PLP 68 (Lei de Reforma)',
  `creditoPresumido` TINYINT(1) NULL DEFAULT '0' COMMENT 'Habilita crédito presumido',
  `obrigatorios` JSON NULL DEFAULT NULL COMMENT 'Campos obrigatórios para esta classificação (JSON)',
  `created` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  `lastUpdated` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`idClassTrib`),
  UNIQUE INDEX `codigo_tipo` (`codigo` ASC, `tipo` ASC) VISIBLE,
  INDEX `idx_codigo` (`codigo` ASC) VISIBLE,
  INDEX `idx_tipo` (`tipo` ASC) VISIBLE,
  INDEX `idx_dispositivo` (`dispositivo` ASC) VISIBLE)
ENGINE = InnoDB
AUTO_INCREMENT = 77
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci;
...
-- -----------------------------------------------------
-- Table `staccato`.`month_stats`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`month_stats` (
  `yearMonth` VARCHAR(45) NOT NULL,
  `weekdays_count` INT(11) NULL DEFAULT NULL,
  `sundays_count` INT(11) NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `staccato`.`ncm` (
  ...
  `sujeitoIS` TINYINT(1) NOT NULL DEFAULT '0' COMMENT 'Indica se NCM está sujeito a Imposto Seletivo',
  `pIS` DECIMAL(7,4) NULL DEFAULT NULL COMMENT 'Alíquota IS ad valorem',
  `cClassTribIS` VARCHAR(6) NULL DEFAULT NULL COMMENT 'Código Classificação Tributária IS (6 dígitos)',
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `lastUpdated` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`idncm`),
...
-- -----------------------------------------------------
-- Table `staccato`.`orcamento_atelier`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`orcamento_atelier` (
  `idOrcamento` VARCHAR(30) NOT NULL COMMENT 'Format: SIGLA-YY + 4-digit sequence + OA (e.g., MRBL-260001OA); revisions: SIGLA-YY{XXXX}-REV{n} (e.g., MRBL-260001-REV1)',
  `idLoja` INT(10) UNSIGNED NOT NULL,
  `idUsuario` INT(10) UNSIGNED NOT NULL COMMENT 'User who created the quote',
  `idCliente` INT(10) UNSIGNED NULL DEFAULT NULL COMMENT 'FK to cliente table',
  `clienteAvulso` VARCHAR(200) NULL DEFAULT NULL COMMENT 'Manual client name when not selecting from system',
  `idProfissional` INT(10) UNSIGNED NULL DEFAULT NULL COMMENT 'FK to profissional table for professional/architect selection',
  `idEnderecoServico` INT(10) UNSIGNED NULL DEFAULT NULL COMMENT 'FK to cliente_has_endereco for service address',
  `titulo` VARCHAR(200) NOT NULL COMMENT 'Quote title (e.g., ATELIER MATRIZ - CLIENTE)',
  `data` DATE NOT NULL COMMENT 'Quote date',
  `validade` DATE NOT NULL COMMENT 'Quote expiration date',
  `enderecoServico` VARCHAR(300) NULL DEFAULT NULL COMMENT 'Full address string if not linked to cliente_has_endereco',
  `ambientes` JSON NOT NULL COMMENT 'Array of quote sections with room, material, product, details, notes',
  `outrosItens` JSON NULL DEFAULT NULL COMMENT 'Array of additional items like shipping, measurement fees',
  `condicoes` TEXT NULL DEFAULT NULL COMMENT 'Custom terms if different from default template',
  `observacao` TEXT NULL DEFAULT NULL,
  `subtotal` DECIMAL(15,4) NOT NULL DEFAULT '0.0000',
  `desconto` DECIMAL(15,4) NOT NULL DEFAULT '0.0000',
  `descontoPercentual` DECIMAL(15,4) NOT NULL DEFAULT '0.0000',
  `total` DECIMAL(15,4) NOT NULL DEFAULT '0.0000',
  `status` ENUM('ORCANDO', 'ATIVO', 'REVISAR', 'APROVADO', 'NAO_APROVADO') NOT NULL DEFAULT 'ORCANDO',
  `created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `lastUpdated` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`idOrcamento`),
  INDEX `idx_cliente` (`idCliente` ASC) VISIBLE,
  INDEX `idx_usuario` (`idUsuario` ASC) VISIBLE,
  INDEX `idx_loja` (`idLoja` ASC) VISIBLE,
  INDEX `idx_status` (`status` ASC) VISIBLE,
  INDEX `idx_data` (`data` ASC) VISIBLE,
  INDEX `idx_created` (`created` ASC) VISIBLE,
  INDEX `fk_atelier_endereco` (`idEnderecoServico` ASC) VISIBLE,
  INDEX `fk_atelier_profissional` (`idProfissional` ASC) VISIBLE,
  CONSTRAINT `fk_atelier_cliente`
    FOREIGN KEY (`idCliente`)
    REFERENCES `staccato`.`cliente` (`idCliente`)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  CONSTRAINT `fk_atelier_endereco`
    FOREIGN KEY (`idEnderecoServico`)
    REFERENCES `staccato`.`cliente_has_endereco` (`idEndereco`)
    ON DELETE SET NULL
    ON UPDATE CASCADE,
  CONSTRAINT `fk_atelier_loja`
    FOREIGN KEY (`idLoja`)
    REFERENCES `staccato`.`loja` (`idLoja`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_atelier_profissional`
    FOREIGN KEY (`idProfissional`)
    REFERENCES `staccato`.`profissional` (`idProfissional`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_atelier_usuario`
    FOREIGN KEY (`idUsuario`)
    REFERENCES `staccato`.`usuario` (`idUsuario`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE)
...
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_0900_ai_ci
COMMENT = 'Atelier-style marble/stone quotations with freeform sections';


-- -----------------------------------------------------
-- Table `staccato`.`orcamento_atelier_template`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`orcamento_atelier_template` (
  `idTemplate` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `idLoja` INT(10) UNSIGNED NOT NULL,
  `nome` VARCHAR(200) CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_unicode_ci' NOT NULL COMMENT 'Template name (e.g., \"Bancada Granito Preto\")',
  `ativo` TINYINT(1) NOT NULL DEFAULT '1',
  `descricao` VARCHAR(500) CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_unicode_ci' NULL DEFAULT NULL COMMENT 'Template description for library display',
  `categoria` VARCHAR(100) CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_unicode_ci' NULL DEFAULT NULL COMMENT 'Category for filtering (e.g., \"Bancadas\", \"Pisos\")',
  `tags` JSON NULL DEFAULT NULL COMMENT 'JSON array of tags for searching',
  `nomeAmbiente` VARCHAR(100) CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_unicode_ci' NOT NULL COMMENT 'Environment name (e.g., \"Cozinha\")',
  `tipoProduto` VARCHAR(100) CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_unicode_ci' NULL DEFAULT NULL COMMENT 'Product type (e.g., \"Bancada\", \"Piso\")',
  `material` VARCHAR(200) CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_unicode_ci' NULL DEFAULT NULL COMMENT 'Material description (e.g., \"Granito Preto São Gabriel\")',
  `metrosQuadrados` VARCHAR(20) CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_unicode_ci' NULL DEFAULT NULL COMMENT 'Optional area specification',
  `descricaoProduto` VARCHAR(1000) CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_unicode_ci' NOT NULL COMMENT 'Product description',
  `preco` DECIMAL(15,4) NOT NULL COMMENT 'Price in Brazilian Real (DECIMAL for precision)',
  `detalhes` JSON NULL DEFAULT NULL COMMENT 'JSON array of detail items',
  `notas` JSON NULL DEFAULT NULL COMMENT 'JSON array of special notes/conditions',
  `ultimoUso` TIMESTAMP NULL DEFAULT NULL,
  `contadorUso` INT(11) NULL DEFAULT '0',
  `created` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'When template was created',
  `lastUpdated` TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (`idTemplate`),
  INDEX `idx_idLoja` (`idLoja` ASC) COMMENT '\'Multi-tenant isolation\'' VISIBLE,
  INDEX `idx_categoria` (`categoria` ASC) COMMENT '\'Filter by category\'' VISIBLE,
  INDEX `idx_nome` (`nome` ASC) COMMENT '\'Search by name\'' VISIBLE,
  INDEX `idx_created` (`created` ASC) COMMENT '\'Sort by creation date\'' VISIBLE,
  INDEX `idx_lastUsed` (`ultimoUso` ASC) COMMENT '\'Find recently used templates\'' VISIBLE,
  INDEX `idx_usageCount` (`contadorUso` ASC) COMMENT '\'Find popular templates\'' VISIBLE,
  CONSTRAINT `fk_template_loja`
    FOREIGN KEY (`idLoja`)
    REFERENCES `staccato`.`loja` (`idLoja`)
    ON DELETE CASCADE)
ENGINE = InnoDB
...
DEFAULT CHARACTER SET = utf8mb4
COLLATE = utf8mb4_unicode_ci
COMMENT = 'Product templates for marble_quoter - reusable configurations';
...
CREATE TABLE IF NOT EXISTS `staccato`.`pedido_fornecedor_has_produto2` (
  ...
  INDEX `pf2_status` (`status` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `staccato`.`usuario_has_config` (
  ...
  `assinaturaEmail` VARCHAR(300) NULL DEFAULT NULL,
  `mensagemEmail` VARCHAR(3000) NULL DEFAULT NULL,
  PRIMARY KEY (`idConfig`),
...
-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`DEXCO_CLIENT`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`DEXCO_CLIENT` (`codi_cli` INT, `cnpj_cli` INT, `nome_cli` INT, `cepc_cli` INT, `muni_cli` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`DEXCO_DISTRIBUTOR`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`DEXCO_DISTRIBUTOR` (`codi_rev` INT, `raza_rev` INT, `cnpj_rev` INT, `muni_rev` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`DEXCO_INVOICE_INVOICEITEM`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`DEXCO_INVOICE_INVOICEITEM` (`idVendaProduto2` INT, `idConsumo` INT, `idNFe` INT, `xml` INT, `codi_ven` INT, `codi_rev` INT, `codi_fab` INT, `modl_ven` INT, `nume_ven` INT, `seri_ven` INT, `fina_ven` INT, `data_ven` INT, `oper_ven` INT, `cfop_ven` INT, `desc_opr` INT, `stat_ven` INT, `sina_opr` INT, `codi_cli` INT, `codi_pro` INT, `desc_pro` INT, `ncmp_pro` INT, `unid_pro` INT, `barr_pro` INT, `cind_pro` INT, `lote_ivn` INT, `pbru_ivn` INT, `pliq_ivn` INT, `qtde_ivn` INT, `codi_ved` INT, `nome_ved` INT, `cpf_pro` INT, `nome_pro` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`DEXCO_PRODUCTSTOCK`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`DEXCO_PRODUCTSTOCK` (`idEstoque` INT, `codi_rev` INT, `codi_fab` INT, `codi_pro` INT, `desc_pro` INT, `ncmp_pro` INT, `unid_pro` INT, `barr_pro` INT, `cind_pro` INT, `date_pro` INT, `lote_pro` INT, `dven_pro` INT, `qtde_pro` INT, `qtdi_pro` INT, `qtdt_pro` INT, `pest_pro` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`DEXCO_SOC`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`DEXCO_SOC` (`codi_rev` INT, `ano_ven` INT, `mes_ven` INT, `modl_ven` INT, `oper_ven` INT, `cfop_ven` INT, `desc_opr` INT, `ncmp_pro` INT, `sina_opr` INT, `valf_ven` INT, `volf_ven` INT, `volu_ven` INT, `volr_ven` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`EDU_view_ConsumoEstoque`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`EDU_view_ConsumoEstoque` (`idestoque` INT, `produto` INT, `quant` INT, `restante` INT, `un` INT, `caixas` INT, `codComercial` INT, `valorUnid` INT, `valor` INT, `valorgare` INT, `dataestoque` INT, `idConsumo` INT, `idVendaProduto2` INT, `status` INT, `qtdd` INT, `cxs` INT, `numeroPedido` INT, `dataconsumo` INT, `lastUpdated` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`EDU_view_Despesas`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`EDU_view_Despesas` (`mes` INT, `contraparte` INT, `status` INT, `valor` INT, `centroCusto` INT, `grupo` INT, `idvenda` INT, `obs` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`EDU_view_EstoqueCD`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`EDU_view_EstoqueCD` (`idEstoque` INT, `status` INT, `idProduto` INT, `fornecedor` INT, `descricao` INT, `formComercial` INT, `observacao` INT, `lote` INT, `local` INT, `idBloco` INT, `bloco` INT, `altura` INT, `quant` INT, `quantUpd` INT, `restante` INT, `ajuste` INT, `contabil` INT, `un` INT, `caixas` INT, `codBarras` INT, `codComercial` INT, `valorUnid` INT, `valor` INT, `created` INT, `lastUpdated` INT, `vendido` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`EDU_view_EstoquePortinari`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`EDU_view_EstoquePortinari` (`produto` INT, `codigo` INT, `lote` INT, `fornecedor` INT, `precovenda` INT, `descricao` INT, `formcomercial` INT, `status` INT, `quant` INT, `un` INT, `cx` INT, `cond` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`EDU_view_Orcamentos`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`EDU_view_Orcamentos` (`idOrcamento` INT, `loja` INT, `nome` INT, `idProfissional` INT, `data` INT, `mes` INT, `total` INT, `status` INT, `motivoCancelamento` INT, `observacaoCancelamento` INT, `fornecedores` INT, `idcliente` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`EDU_view_Receitas`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`EDU_view_Receitas` (`mes` INT, `status` INT, `valor` INT, `centroCusto` INT, `grupo` INT, `idvenda` INT, `obs` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`EDU_view_Vendas`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`EDU_view_Vendas` (`idVendaProduto2` INT, `mes` INT, `idvenda` INT, `loja` INT, `nome` INT, `fornecedor` INT, `produto` INT, `codComercial` INT, `ValorVenda` INT, `frete` INT, `servicos` INT, `fonte` INT, `custo` INT, `icms_iss` INT, `STnota` INT, `tipoicms` INT, `vICMSST` INT, `taxacartao` INT, `prz` INT, `quant` INT, `pfquant` INT, `st` INT, `custopedido` INT, `ecquant` INT, `custoestoque` INT, `custotabela` INT, `status` INT, `precotabela` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`EDU_view_Vendedores`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`EDU_view_Vendedores` (`nome` INT, `loja` INT, `mes` INT, `idProfissional` INT, `total` INT, `frete` INT, `subTotalBru` INT, `subTotalLiq` INT, `prazoEntrega` INT, `rt` INT, `representacao` INT, `status` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`ViewExcelDespesas`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`ViewExcelDespesas` (`dataEmissao` INT, `dataPagamento` INT, `contraParte` INT, `centroCusto` INT, `grupo` INT, `nfe` INT, `valor` INT, `status` INT, `observacao` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`ViewExcelEntregasPendentes`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`ViewExcelEntregasPendentes` (`data` INT, `przentrega` INT, `nvprzentrega` INT, `status` INT, `fornecedor` INT, `idvenda` INT, `produto` INT, `kg` INT, `caixas` INT, `quant` INT, `un` INT, `codComercial` INT, `formComercial` INT, `dataPrevEnt` INT, `representacao` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`ViewFLXCXExcel`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`ViewFLXCXExcel` (`origem` INT, `centroCusto` INT, `grupo` INT, `contraparte` INT, `data` INT, `datarealizado` INT, `valorreal` INT, `idConta` INT, `observacao` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`ViewRTCPExcel`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`ViewRTCPExcel` (`dataEmissao` INT, `idVenda` INT, `contraParte` INT, `valor` INT, `valorReal` INT, `dataPagamento` INT, `dataRealizado` INT, `status` INT, `quinzena` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`ViewRTVPExcel`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`ViewRTVPExcel` (`data` INT, `venda` INT, `fornecedor` INT, `total` INT, `rt` INT, `status_venda` INT, `status_financeiro` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`viewVendaxCMV` (`status` INT, `idVendaProduto2` INT, `mes` INT, `loja` INT, `SumOfvenda` INT, `CMV` INT, `margem` INT, `FonteCMV` INT, `ICMS` INT, `FonteICMS` INT, `vendadev` INT, `fornecedor` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_agendar_coleta` (`prazoEntrega` INT, `dataRealFat` INT, `idEstoque` INT, `lote` INT, `local` INT, `bloco` INT, `fornecedor` INT, `quantCaixa` INT, `formComercial` INT, `idProduto` INT, `idNFe` INT, `numeroNFe` INT, `idVenda` INT, `idCompra` INT, `ordemCompra` INT, `codFornecedor` INT, `produto` INT, `codComercial` INT, `quant` INT, `un` INT, `caixas` INT, `kgcx` INT, `Transportadora` INT, `Peso` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_coleta` (`prazoEntrega` INT, `dataPrevColeta` INT, `idEstoque` INT, `lote` INT, `local` INT, `bloco` INT, `fornecedor` INT, `idNFe` INT, `numeroNFe` INT, `idVenda` INT, `idCompra` INT, `ordemCompra` INT, `codFornecedor` INT, `produto` INT, `codComercial` INT, `quant` INT, `un` INT, `caixas` INT, `kgcx` INT, `Transportadora` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_conta_pagar_idcompra`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_conta_pagar_idcompra` (`idPagamento` INT, `dataEmissao` INT, `idCompra` INT, `idVenda` INT, `idLoja` INT, `contraParte` INT, `idCnab` INT, `idNFe` INT, `nfe` INT, `valor` INT, `tipo` INT, `parcela` INT, `dataPagamento` INT, `observacao` INT, `status` INT, `dataRealizado` INT, `valorReal` INT, `tipoReal` INT, `parcelaReal` INT, `idConta` INT, `tipoDet` INT, `centroCusto` INT, `grupo` INT, `subGrupo` INT, `compraAvulsa` INT, `desativado` INT, `created` INT, `lastUpdated` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_faturamento` (`ordemCompra` INT, `codFornecedor` INT, `fornecedor` INT, `data` INT, `produtos` INT, `total` INT, `idCompra` INT, `dataPrevFat` INT, `vendedor` INT, `idVenda` INT, `prazoEntrega` INT, `novoPrazoEntrega` INT, `representacao` INT, `dataFollowup` INT, `observacao` INT);
...
-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_mkt_creditos`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_mkt_creditos` (`dataEmissao` INT, `contraParte` INT, `nfe` INT, `valorreal` INT, `dataRealizado` INT, `status` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_mkt_pagamentos`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_mkt_pagamentos` (`dataEmissao` INT, `dataRealizado` INT, `centroCusto` INT, `contraParte` INT, `nfe` INT, `valor` INT, `valorReal` INT, `status` INT, `observacao` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_orcamento` (`idLoja` INT, `idUsuario` INT, `idUsuarioConsultor` INT, `status` INT, `diasRestantes` INT, `idOrcamento` INT, `vendedor` INT, `consultor` INT, `cliente` INT, `profissional` INT, `tel` INT, `telCel` INT, `telProf` INT, `data` INT, `data2` INT, `total` INT, `idFollowup` INT, `dataFollowup` INT, `dataProxFollowup` INT, `observacao` INT, `semaforo` INT, `fornecedores` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_recebimento` (`prazoEntrega` INT, `dataPrevReceb` INT, `idEstoque` INT, `idNFe` INT, `lote` INT, `local` INT, `bloco` INT, `fornecedor` INT, `numeroNFe` INT, `idVenda` INT, `idCompra` INT, `ordemCompra` INT, `codFornecedor` INT, `produto` INT, `codComercial` INT, `quant` INT, `un` INT, `caixas` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_relatorio`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_relatorio` (`Loja` INT, `Vendedor` INT, `idUsuario` INT, `idUsuarioConsultor` INT, `idVenda` INT, `Mês` INT, `Data` INT, `Faturamento` INT, `Comissão` INT, `DSR` INT, `Total` INT, `porcentagem` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_venda` (`idLoja` INT, `idUsuario` INT, `idUsuarioConsultor` INT, `statusFinanceiro` INT, `dataFinanceiro` INT, `status` INT, `diasRestantes` INT, `idVenda` INT, `ordemRepresentacao` INT, `vendedor` INT, `consultor` INT, `cliente` INT, `profissional` INT, `data` INT, `data2` INT, `data3` INT, `total` INT, `idFollowup` INT, `dataFollowup` INT, `observacao` INT, `fornecedores` INT);
...
-- -----------------------------------------------------
-- procedure atualizar_comissao
-- -----------------------------------------------------

DELIMITER $$
USE `staccato`$$
CREATE DEFINER=`root`@`%` PROCEDURE `atualizar_comissao`(newIdVendaProduto INT)
BEGIN  
    DECLARE first_day_of_month DATE;
    DECLARE weekdays_count INT;
    DECLARE sundays_count INT;

    -- Get the first day of the month based on the sale date
SELECT 
    DATE_FORMAT(data, '%Y-%m-01')
INTO first_day_of_month FROM
    venda v
        JOIN
...
LIMIT 1;
    ...
    -- Compute weekdays and Sundays count
SELECT  
    COUNT(CASE WHEN WEEKDAY(ds.dt) BETWEEN 0 AND 5 AND f.data IS NULL THEN 1 END) AS weekdays_count,  
    COUNT(CASE WHEN WEEKDAY(ds.dt) = 6 OR f.data IS NOT NULL THEN 1 END) AS sundays_count
INTO weekdays_count, sundays_count
FROM (
    WITH RECURSIVE date_series AS (  
        SELECT first_day_of_month AS dt  
        UNION ALL  
        SELECT dt + INTERVAL 1 DAY FROM date_series  
        WHERE dt < LAST_DAY(first_day_of_month)  
    )  
    SELECT dt FROM date_series
) AS ds
LEFT JOIN feriados f ON f.data = ds.dt;

    -- Update commission data using existing values from comissao
    UPDATE comissao c
    SET 
    -- sundays / weekdays = 4 / 25 = 0.16
    -- Comissao = Total / 1.16
    -- DSR = Comissao * 0.16
        c.Comissão_2 = c.ComissãoDsr / (1 + sundays_count / weekdays_count),
        c.dsr_2 = (c.ComissãoDsr / (1 + sundays_count / weekdays_count)) * sundays_count / weekdays_count
    WHERE c.idVendaProduto1 = newIdVendaProduto;
END$$

DELIMITER ;

-- -----------------------------------------------------
-- procedure call_inserir_comissao_for_all
-- -----------------------------------------------------

DELIMITER $$
USE `staccato`$$
CREATE DEFINER=`root`@`%` PROCEDURE `call_inserir_comissao_for_all`()
BEGIN
    DECLARE done INT DEFAULT 0;
    DECLARE vendaProdutoId INT;
    DECLARE cur CURSOR FOR SELECT idVendaProduto1 FROM comissao;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = 1;

    OPEN cur;

    read_loop: LOOP
        FETCH cur INTO vendaProdutoId;
        IF done THEN
            LEAVE read_loop;
        END IF;

        CALL atualizar_comissao(vendaProdutoId);
    END LOOP;
...
INSERT INTO comissao (
    idVendaProduto1, Loja, Vendedor, idUsuario, idUsuarioConsultor, idVenda, Mês,
    Faturamento, Comissão, dsr, ComissãoDsr, porcentagem, fornecedor, especialidade
)
SELECT 
    idVendaProduto1,
    Loja,
    ...
    Mês,
    Faturamento,
    ComissãoDsr - (ComissãoDsr * sundays_count / weekdays_count) AS comissao,
    ComissãoDsr * sundays_count / weekdays_count AS dsr,
    ComissãoDsr,
    porcentagem,
    fornecedor,
    especialidade
FROM (
    SELECT 
        ...
        IF(RIGHT(p.ui, 1) = 'L',
            ...
            (vp.total * f.comissao1) / 100) AS ComissãoDsr,
        IF(RIGHT(p.ui, 1) = 'L',
            f.comissao2,
    ...
    FROM
        venda v
        LEFT JOIN venda_has_produto vp ON v.idVenda = vp.idVenda
        LEFT JOIN loja l ON l.idLoja = v.idLoja
        LEFT JOIN usuario u ON u.idUsuario = v.idUsuario
        LEFT JOIN produto p ON p.idProduto = vp.idProduto
        LEFT JOIN fornecedor f ON f.idFornecedor = p.idFornecedor
        LEFT JOIN month_stats ms ON DATE_FORMAT(v.data, '%Y-%m') = ms.yearMonth
    WHERE
        vp.idVendaProduto1 = newIdVendaProduto
) AS subquery;
END$$
...
-- -----------------------------------------------------
-- procedure inserir_comissao2
-- -----------------------------------------------------

DELIMITER $$
USE `staccato`$$
CREATE DEFINER=`root`@`%` PROCEDURE `inserir_comissao2`(newIdVendaProduto INT)
BEGIN  
    DECLARE first_day_of_month DATE;
    DECLARE weekdays_count INT;
    DECLARE sundays_count INT;
    ...
    -- Get the first day of the month based on the sale date
    SELECT DATE_FORMAT(data, '%Y-%m-01') INTO first_day_of_month
    FROM venda v
    JOIN venda_has_produto vp ON v.idVenda = vp.idVenda
    WHERE vp.idVendaProduto1 = newIdVendaProduto
    LIMIT 1;

    -- Compute weekdays and Sundays count
    SELECT  
        COUNT(CASE WHEN WEEKDAY(dt) BETWEEN 0 AND 4 THEN 1 END),  
        COUNT(CASE WHEN WEEKDAY(dt) = 6 THEN 1 END)  
    INTO weekdays_count, sundays_count
    FROM (
        WITH RECURSIVE date_series AS (  
            SELECT first_day_of_month AS dt  
...
SET 
    status = 'EXPIRADO'
WHERE
    CURDATE() > DATE_ADD(data, INTERVAL validade DAY)
        AND status = 'ATIVO';
END$$
...
-- -----------------------------------------------------
-- procedure populate_month_stats
-- -----------------------------------------------------

DELIMITER $$
USE `staccato`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `populate_month_stats`()
BEGIN
    ...
    -- Loop through months from 1970 to 2070
    WHILE YEAR(`start_date`) <= 2070 DO
        SET `end_date` = LAST_DAY(`start_date`);

        -- Insert into month_stats with format yyyy-MM
        INSERT INTO `month_stats` (`yearMonth`, `weekdays_count`, `sundays_count`)
        SELECT
            DATE_FORMAT(`start_date`, '%Y-%m') AS `month_start`,
            (SELECT COUNT(*) FROM (
                SELECT DATE_ADD(`start_date`, INTERVAL @i:=@i+1 DAY) AS `day`
                FROM (SELECT @i:=-1) AS `init`,
                     (SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7) AS `days`
                WHERE DATE_ADD(`start_date`, INTERVAL @i DAY) <= `end_date`
            ) AS `days`
            WHERE DAYOFWEEK(`day`) BETWEEN 2 AND 6) AS `weekdays_count`,
            (SELECT COUNT(*) FROM (
                SELECT DATE_ADD(`start_date`, INTERVAL @i:=@i+1 DAY) AS `day`
                FROM (SELECT @i:=-1) AS `init`,
                     (SELECT 1 UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6 UNION SELECT 7) AS `days`
            ...
            WHERE DAYOFWEEK(`day`) = 1) AS `sundays_count`;
...
-- -----------------------------------------------------
-- procedure update_all_quant_estoque
-- -----------------------------------------------------

DELIMITER $$
USE `staccato`$$
CREATE DEFINER=`root`@`%` PROCEDURE `update_all_quant_estoque`()
BEGIN
	DECLARE done INT DEFAULT FALSE;
    DECLARE id INT;
    DECLARE cursor1 CURSOR FOR SELECT idEstoque FROM estoque;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET done = TRUE;
    ...
    read_loop: LOOP
        ...
        IF done THEN LEAVE read_loop;
        END IF;
        CALL update_quant_estoque(id);
        CALL update_produto_estoque_quant(id);
	END LOOP;
...
-- -----------------------------------------------------
-- procedure update_fornecedores_orcamento
-- -----------------------------------------------------

DELIMITER $$
USE `staccato`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_fornecedores_orcamento`(id varchar(30))
BEGIN
UPDATE orcamento set fornecedores = (
		SELECT 
            GROUP_CONCAT(DISTINCT `staccato`.`orcamento_has_produto`.`fornecedor`
                    SEPARATOR ',') AS `fornecedores`
        FROM
            `staccato`.`orcamento_has_produto` where `idOrcamento` = id
        GROUP BY `staccato`.`orcamento_has_produto`.`idOrcamento`) where `idOrcamento` = id;
END$$

DELIMITER ;

-- -----------------------------------------------------
-- procedure update_fornecedores_venda
-- -----------------------------------------------------

DELIMITER $$
USE `staccato`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_fornecedores_venda`(id varchar(30))
BEGIN
UPDATE venda set fornecedores = (
		SELECT 
            GROUP_CONCAT(DISTINCT `staccato`.`venda_has_produto`.`fornecedor`
                    SEPARATOR ',') AS `fornecedores`
        FROM
            `staccato`.`venda_has_produto` where `idVenda` = id
        GROUP BY `staccato`.`venda_has_produto`.`idVenda`) where `idVenda` = id;
END$$
...
-- -----------------------------------------------------
-- procedure update_ordemCompra_venda
-- -----------------------------------------------------

DELIMITER $$
USE `staccato`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_ordemCompra_venda`(id varchar(30))
BEGIN
UPDATE venda set ordemCompra = (
		SELECT 
            GROUP_CONCAT(DISTINCT `staccato`.`pedido_fornecedor_has_produto2`.`ordemRepresentacao`
                    SEPARATOR ',') AS `ordemRepresentacao`,
                `staccato`.`pedido_fornecedor_has_produto2`.`idVenda` AS `idVenda`
        FROM
            `staccato`.`pedido_fornecedor_has_produto2`
        WHERE
            ((`staccato`.`pedido_fornecedor_has_produto2`.`idVenda` IS NOT NULL)
                AND (`staccato`.`pedido_fornecedor_has_produto2`.`ordemRepresentacao` IS NOT NULL)) and idVenda = id
        GROUP BY `staccato`.`pedido_fornecedor_has_produto2`.`idVenda`
        ) 
where `idVenda` = id;
END$$
...
-- -----------------------------------------------------
-- procedure update_ordemRepresentacao_venda
-- -----------------------------------------------------

DELIMITER $$
USE `staccato`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_ordemRepresentacao_venda`(id varchar(30))
BEGIN
UPDATE venda set ordemRepresentacao = (
		SELECT 
            GROUP_CONCAT(DISTINCT `staccato`.`pedido_fornecedor_has_produto2`.`ordemRepresentacao`
                    SEPARATOR ',') AS `ordemRepresentacao`
        FROM
...
BEGIN
    UPDATE venda_has_produto vp1, venda_has_produto2 vp2 SET vp1.status = vp2.status WHERE vp1.idVendaProduto1 = vp2.idVendaProdutoFK AND currentId = vp2.idVendaProdutoFK AND vp2.status = 'DEVOLVIDO ESTOQUE' AND vp2.quant != 0;
    UPDATE venda_has_produto vp1, venda_has_produto2 vp2 SET vp1.status = vp2.status WHERE vp1.idVendaProduto1 = vp2.idVendaProdutoFK AND currentId = vp2.idVendaProdutoFK AND vp2.status = 'DEVOLVIDO FORN.' AND vp2.quant != 0;
    UPDATE venda_has_produto vp1, venda_has_produto2 vp2 SET vp1.status = vp2.status WHERE vp1.idVendaProduto1 = vp2.idVendaProdutoFK AND currentId = vp2.idVendaProdutoFK AND vp2.status = 'DEVOLVIDO' AND vp2.quant != 0;
    
    UPDATE venda_has_produto vp1, venda_has_produto2 vp2 SET vp1.status = vp2.status WHERE vp1.idVendaProduto1 = vp2.idVendaProdutoFK AND currentId = vp2.idVendaProdutoFK AND vp2.status = 'ENTREGUE' AND vp2.quant != 0;
    ...
    UPDATE venda_has_produto vp1, venda_has_produto2 vp2 SET vp1.status = vp2.status WHERE vp1.idVendaProduto1 = vp2.idVendaProdutoFK AND currentId = vp2.idVendaProdutoFK AND vp2.status = 'EM COLETA' AND vp2.quant != 0;
    UPDATE venda_has_produto vp1, venda_has_produto2 vp2 SET vp1.status = vp2.status WHERE vp1.idVendaProduto1 = vp2.idVendaProdutoFK AND currentId = vp2.idVendaProdutoFK AND vp2.status = 'EM FATURAMENTO' AND vp2.quant != 0;
    UPDATE venda_has_produto vp1, venda_has_produto2 vp2 SET vp1.status = vp2.status WHERE vp1.idVendaProduto1 = vp2.idVendaProdutoFK AND currentId = vp2.idVendaProdutoFK AND vp2.status = 'EM COMPRA' AND vp2.quant != 0;
    UPDATE venda_has_produto vp1, venda_has_produto2 vp2 SET vp1.status = vp2.status WHERE vp1.idVendaProduto1 = vp2.idVendaProdutoFK AND currentId = vp2.idVendaProdutoFK AND vp2.status = 'INICIADO' AND vp2.quant != 0;
    UPDATE venda_has_produto vp1, venda_has_produto2 vp2 SET vp1.status = vp2.status WHERE vp1.idVendaProduto1 = vp2.idVendaProdutoFK AND currentId = vp2.idVendaProdutoFK AND vp2.status = 'PENDENTE' AND vp2.quant != 0;
END$$
...
-- -----------------------------------------------------
-- View `staccato`.`DEXCO_CLIENT`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`DEXCO_CLIENT`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`DEXCO_CLIENT` AS select cast(`c`.`idCliente` as char(20) charset utf8mb4) AS `codi_cli`,cast('LGPD' as char(14) charset utf8mb4) AS `cnpj_cli`,cast('LGPD' as char(65) charset utf8mb4) AS `nome_cli`,cast('LGPD' as char(9) charset utf8mb4) AS `cepc_cli`,cast(`c2`.`codigo` as char(7) charset utf8mb4) AS `muni_cli` from (((((`staccato`.`venda_has_produto2` `vp2` left join `staccato`.`produto` `p` on((`vp2`.`idProduto` = `p`.`idProduto`))) left join `staccato`.`venda` `v` on((`vp2`.`idVenda` = `v`.`idVenda`))) left join `staccato`.`cliente` `c` on((`v`.`idCliente` = `c`.`idCliente`))) left join `staccato`.`cliente_has_endereco` `che` on((`v`.`idEnderecoEntrega` = `che`.`idEndereco`))) left join `staccato`.`cidade` `c2` on(((`che`.`cidade` = `c2`.`nome`) and (`che`.`uf` = `c2`.`uf`)))) where ((`p`.`idFornecedor` in (1,2,6,8,331,343,346)) and (`c2`.`codigo` is not null)) group by `c`.`idCliente`;

-- -----------------------------------------------------
-- View `staccato`.`DEXCO_DISTRIBUTOR`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`DEXCO_DISTRIBUTOR`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`DEXCO_DISTRIBUTOR` AS select cast(`l`.`sigla` as char(20) charset utf8mb4) AS `codi_rev`,cast(`l`.`razaoSocial` as char(60) charset utf8mb4) AS `raza_rev`,cast(regexp_replace(`l`.`cnpj`,'[^0-9]','') as char(14) charset utf8mb4) AS `cnpj_rev`,cast(`c`.`codigo` as char(7) charset utf8mb4) AS `muni_rev` from ((`staccato`.`loja` `l` left join `staccato`.`loja_has_endereco` `lhe` on((`l`.`idLoja` = `lhe`.`idLoja`))) left join `staccato`.`cidade` `c` on(((`lhe`.`cidade` = `c`.`nome`) and (`lhe`.`uf` = `c`.`uf`)))) where (`l`.`cnpj` is not null) group by `l`.`idLoja`;

-- -----------------------------------------------------
-- View `staccato`.`DEXCO_INVOICE_INVOICEITEM`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`DEXCO_INVOICE_INVOICEITEM`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`DEXCO_INVOICE_INVOICEITEM` AS select `vp2`.`idVendaProduto2` AS `idVendaProduto2`,`ehc`.`idConsumo` AS `idConsumo`,`n`.`idNFe` AS `idNFe`,`n`.`xml` AS `xml`,cast(`vp2`.`idVenda` as char(65535) charset utf8mb4) AS `codi_ven`,cast(`l`.`sigla` as char(20) charset utf8mb4) AS `codi_rev`,cast(`p`.`idFornecedor` as char(65535) charset utf8mb4) AS `codi_fab`,cast('55' as char(2) charset utf8mb4) AS `modl_ven`,cast(`n`.`numeroNFe` as char(20) charset utf8mb4) AS `nume_ven`,cast(substr(`n`.`chaveAcesso`,23,3) as char(20) charset utf8mb4) AS `seri_ven`,cast(extractvalue(`n`.`xml`,'//NFe//infNFe//ide//finNFe') as char(1) charset utf8mb4) AS `fina_ven`,cast(extractvalue(`n`.`xml`,'//NFe//infNFe//ide//dhEmi') as date) AS `data_ven`,cast(`ehc`.`cfop` as char(20) charset utf8mb4) AS `oper_ven`,cast(`ehc`.`cfop` as char(4) charset utf8mb4) AS `cfop_ven`,cast(`staccato`.`cfop`.`NAT` as char(60) charset utf8mb4) AS `desc_opr`,cast(if((`vp2`.`status` = 'CANCELADO'),'C','A') as char(1) charset utf8mb4) AS `stat_ven`,cast(if((`n`.`tipo` = 'ENTRADA'),'E','S') as char(1) charset utf8mb4) AS `sina_opr`,cast(`c`.`idCliente` as char(20) charset utf8mb4) AS `codi_cli`,cast(`p`.`idProduto` as char(65535) charset utf8mb4) AS `codi_pro`,cast(`p`.`descricao` as char(60) charset utf8mb4) AS `desc_pro`,cast(`p`.`ncm` as char(8) charset utf8mb4) AS `ncmp_pro`,cast(`p`.`un` as char(10) charset utf8mb4) AS `unid_pro`,cast(`p`.`codBarras` as char(14) charset utf8mb4) AS `barr_pro`,cast(`p`.`codComercial` as char(15) charset utf8mb4) AS `cind_pro`,cast(`e`.`lote` as char(20) charset utf8mb4) AS `lote_ivn`,cast(`ehc`.`valor` as decimal(18,4)) AS `pbru_ivn`,cast(`ehc`.`valor` as decimal(18,4)) AS `pliq_ivn`,cast(`ehc`.`quantTrib` as decimal(18,3)) AS `qtde_ivn`,cast(`u`.`idUsuario` as char(20) charset utf8mb4) AS `codi_ved`,cast(`u`.`nome` as char(60) charset utf8mb4) AS `nome_ved`,cast('LGPD' as char(11) charset utf8mb4) AS `cpf_pro`,cast('LGPD' as char(60) charset utf8mb4) AS `nome_pro` from (((((((((((`staccato`.`venda_has_produto2` `vp2` left join `staccato`.`produto` `p` on((`p`.`idProduto` = `vp2`.`idProduto`))) left join `staccato`.`loja` `l` on((`vp2`.`idLoja` = `l`.`idLoja`))) left join `staccato`.`nfe` `n` on((`vp2`.`idNFeSaida` = `n`.`idNFe`))) left join `staccato`.`estoque_has_consumo` `ehc` on((`vp2`.`idVendaProduto2` = `ehc`.`idVendaProduto2`))) left join `staccato`.`cfop` on((`ehc`.`cfop` in (`staccato`.`cfop`.`CFOP_DE`,`staccato`.`cfop`.`CFOP_FE`,`staccato`.`cfop`.`CFOP_IN`)))) left join `staccato`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) left join `staccato`.`venda` `v` on((`vp2`.`idVenda` = `v`.`idVenda`))) left join `staccato`.`cliente` `c` on((`v`.`idCliente` = `c`.`idCliente`))) left join `staccato`.`cliente_has_endereco` `che` on((`v`.`idEnderecoEntrega` = `che`.`idEndereco`))) left join `staccato`.`cidade` `c2` on((`che`.`cidade` = `c2`.`nome`))) left join `staccato`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`))) where ((`p`.`idFornecedor` in (1,2,6,8,331,343,346)) and (`v`.`representacao` = false) and (`v`.`devolucao` = false) and (`n`.`idNFe` is not null) and (`ehc`.`idConsumo` is not null) and (`ehc`.`cfop` is not null) and (`p`.`ncm` is not null) and (0 <> `ehc`.`quantTrib`) and (`c2`.`codigo` is not null) and (`l`.`cnpj` is not null)) group by `vp2`.`idVendaProduto2`;

-- -----------------------------------------------------
-- View `staccato`.`DEXCO_PRODUCTSTOCK`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`DEXCO_PRODUCTSTOCK`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`DEXCO_PRODUCTSTOCK` AS select `e`.`idEstoque` AS `idEstoque`,cast('CDCD' as char(20) charset utf8mb4) AS `codi_rev`,cast(`p`.`idFornecedor` as char(65535) charset utf8mb4) AS `codi_fab`,cast(`e`.`idProduto` as char(65535) charset utf8mb4) AS `codi_pro`,cast(`p`.`descricao` as char(60) charset utf8mb4) AS `desc_pro`,cast(`e`.`ncm` as char(8) charset utf8mb4) AS `ncmp_pro`,cast(`p`.`un` as char(10) charset utf8mb4) AS `unid_pro`,cast(`e`.`codBarras` as char(14) charset utf8mb4) AS `barr_pro`,cast(NULL as char(15) charset utf8mb4) AS `cind_pro`,cast(curdate() as date) AS `date_pro`,cast(`e`.`lote` as char(20) charset utf8mb4) AS `lote_pro`,cast(NULL as date) AS `dven_pro`,cast((`e`.`restante` + `e`.`ajuste`) as decimal(18,3)) AS `qtde_pro`,cast(((`e`.`quant` + `e`.`ajuste`) + coalesce(`ehc_sub`.`contabil`,0)) as decimal(18,3)) AS `qtdi_pro`,cast(0 as decimal(18,3)) AS `qtdt_pro`,cast(`p`.`precoVenda` as decimal(18,3)) AS `pest_pro` from ((`staccato`.`estoque` `e` left join (select `ehc`.`idEstoque` AS `idEstoque`,sum(`ehc`.`quant`) AS `contabil`,sum(if((`vp2`.`status` not in ('DEVOLVIDO ESTOQUE','QUEBRADO')),`vp2`.`quant`,0)) AS `consumoVenda`,group_concat(distinct `vp2`.`idVenda` separator ',') AS `idVenda` from (`staccato`.`estoque_has_consumo` `ehc` left join `staccato`.`venda_has_produto2` `vp2` on((`ehc`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) where (((`vp2`.`dataRealEnt` <= curdate()) or (`vp2`.`dataRealEnt` is null)) and (`ehc`.`status` <> 'CANCELADO')) group by `ehc`.`idEstoque`) `ehc_sub` on((`e`.`idEstoque` = `ehc_sub`.`idEstoque`))) left join `staccato`.`produto` `p` on((`e`.`idProduto` = `p`.`idProduto`))) where ((`p`.`idFornecedor` in (1,2,6,8,331,343,346)) and (cast((`e`.`restante` + `e`.`ajuste`) as decimal(18,3)) > 0) and (`e`.`ncm` is not null));

-- -----------------------------------------------------
-- View `staccato`.`DEXCO_SOC`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`DEXCO_SOC`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`DEXCO_SOC` AS select cast(`l`.`descricao` as char(20) charset utf8mb4) AS `codi_rev`,cast(year(`v`.`data`) as unsigned) AS `ano_ven`,cast(month(`v`.`data`) as unsigned) AS `mes_ven`,cast(55 as char(2) charset utf8mb4) AS `modl_ven`,cast(`ehc`.`cfop` as char(20) charset utf8mb4) AS `oper_ven`,cast(`ehc`.`cfop` as char(4) charset utf8mb4) AS `cfop_ven`,cast('venda' as char(60) charset utf8mb4) AS `desc_opr`,cast(`ehc`.`ncm` as char(8) charset utf8mb4) AS `ncmp_pro`,cast('S' as char(1) charset utf8mb4) AS `sina_opr`,cast(NULL as decimal(18,4)) AS `valf_ven`,cast(NULL as decimal(18,3)) AS `volf_ven`,cast(sum(`v`.`total`) as decimal(18,4)) AS `volu_ven`,cast(sum(`vp2`.`quant`) as decimal(18,3)) AS `volr_ven` from (((`staccato`.`venda_has_produto2` `vp2` left join `staccato`.`venda` `v` on((`vp2`.`idVenda` = `v`.`idVenda`))) left join `staccato`.`estoque_has_consumo` `ehc` on((`vp2`.`idVendaProduto2` = `ehc`.`idVendaProduto2`))) left join `staccato`.`loja` `l` on((`v`.`idLoja` = `l`.`idLoja`))) where (`ehc`.`idConsumo` is not null) group by `codi_rev`,`ano_ven`,`mes_ven`,`oper_ven`,`modl_ven`,`ncmp_pro`,`sina_opr` order by `ano_ven`,`mes_ven`;

-- -----------------------------------------------------
-- View `staccato`.`EDU_view_ConsumoEstoque`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`EDU_view_ConsumoEstoque`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`EDU_view_ConsumoEstoque` AS select `e`.`idEstoque` AS `idestoque`,`e`.`descricao` AS `produto`,`e`.`quant` AS `quant`,`e`.`restante` AS `restante`,`e`.`un` AS `un`,`e`.`caixas` AS `caixas`,`e`.`codComercial` AS `codComercial`,`e`.`valorUnid` AS `valorUnid`,`e`.`valor` AS `valor`,`e`.`valorGare` AS `valorgare`,`e`.`created` AS `dataestoque`,`ec`.`idConsumo` AS `idConsumo`,`ec`.`idVendaProduto2` AS `idVendaProduto2`,`ec`.`status` AS `status`,`ec`.`quant` AS `qtdd`,`ec`.`caixas` AS `cxs`,`ec`.`numeroPedido` AS `numeroPedido`,`ec`.`created` AS `dataconsumo`,`ec`.`lastUpdated` AS `lastUpdated` from (`staccato`.`estoque` `e` left join `staccato`.`estoque_has_consumo` `ec` on((`e`.`idEstoque` = `ec`.`idEstoque`))) where `e`.`idEstoque` in (select distinct `e`.`idEstoque` from (`staccato`.`estoque` `e` left join `staccato`.`estoque_has_consumo` `ec` on((`e`.`idEstoque` = `ec`.`idEstoque`))) where ((`e`.`status` <> 'cancelado') and ((`ec`.`status` is null) or (((`ec`.`created` - `e`.`created`) > 1) and (`e`.`created` > '2024-01-01') and (`ec`.`status` <> 'quebrado') and (`ec`.`status` <> 'cancelado'))) and (`e`.`fornecedor` like '%dexco%')));

-- -----------------------------------------------------
-- View `staccato`.`EDU_view_Despesas`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`EDU_view_Despesas`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`EDU_view_Despesas` AS select date_format(`cp`.`dataEmissao`,'%Y/%m') AS `mes`,`cp`.`contraParte` AS `contraparte`,`cp`.`status` AS `status`,if((`cp`.`status` = 'pago'),`cp`.`valorReal`,(`cp`.`valor` + 0E0)) AS `valor`,`staccato`.`loja`.`descricao` AS `centroCusto`,`cp`.`grupo` AS `grupo`,`cp`.`idVenda` AS `idvenda`,`cp`.`observacao` AS `obs` from (`staccato`.`conta_a_pagar_has_pagamento` `cp` left join `staccato`.`loja` on((`staccato`.`loja`.`idLoja` = `cp`.`centroCusto`))) where ((`cp`.`desativado` = 0) and (`cp`.`grupo` <> 'transferência') and (`cp`.`dataEmissao` > '2023-06-30') and (`cp`.`status` <> 'cancelado') and (`cp`.`status` <> 'cancelado gare') and (`cp`.`status` <> 'substituido'));

-- -----------------------------------------------------
-- View `staccato`.`EDU_view_EstoqueCD`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`EDU_view_EstoqueCD`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`EDU_view_EstoqueCD` AS select `e`.`idEstoque` AS `idEstoque`,`e`.`status` AS `status`,`e`.`idProduto` AS `idProduto`,`p`.`fornecedor` AS `fornecedor`,`e`.`descricao` AS `descricao`,`p`.`formComercial` AS `formComercial`,`e`.`observacao` AS `observacao`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`idBloco` AS `idBloco`,`e`.`bloco` AS `bloco`,`e`.`altura` AS `altura`,`e`.`quant` AS `quant`,`e`.`quantUpd` AS `quantUpd`,`e`.`restante` AS `restante`,`e`.`ajuste` AS `ajuste`,`e`.`contabil` AS `contabil`,`e`.`un` AS `un`,`e`.`caixas` AS `caixas`,`e`.`codBarras` AS `codBarras`,`e`.`codComercial` AS `codComercial`,`e`.`valorUnid` AS `valorUnid`,`e`.`valor` AS `valor`,`e`.`created` AS `created`,`e`.`lastUpdated` AS `lastUpdated`,`ec2`.`vendido` AS `vendido` from ((`staccato`.`estoque` `e` left join `staccato`.`produto` `p` on((`e`.`idProduto` = `p`.`idProduto`))) left join (select `ec`.`idEstoque` AS `idEstoque`,sum(`vp2`.`quant`) AS `vendido` from (`staccato`.`estoque_has_consumo` `ec` join `staccato`.`venda_has_produto2` `vp2` on((`ec`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) where ((`vp2`.`status` = 'estoque') and (`ec`.`status` <> 'devolvido')) group by `ec`.`idEstoque`) `ec2` on((`e`.`idEstoque` = `ec2`.`idEstoque`))) where (((`e`.`status` <> 'cancelado') and (`e`.`restante` <> 0)) or ((`ec2`.`vendido` is not null) and (`ec2`.`vendido` <> 0)));

-- -----------------------------------------------------
-- View `staccato`.`EDU_view_EstoquePortinari`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`EDU_view_EstoquePortinari`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`EDU_view_EstoquePortinari` AS select `p`.`descricao` AS `produto`,`e`.`codComercial` AS `codigo`,`e`.`lote` AS `lote`,`p`.`fornecedor` AS `fornecedor`,`p`.`precoVenda` AS `precovenda`,`p`.`descricao` AS `descricao`,`p`.`formComercial` AS `formcomercial`,`e`.`status` AS `status`,sum(`e`.`restante`) AS `quant`,`e`.`un` AS `un`,sum(((`e`.`caixas` / `e`.`quant`) * `e`.`restante`)) AS `cx`,if((`e`.`created` < '2024-01-01'),'estoque antigo','estoque recente') AS `cond` from (`staccato`.`estoque` `e` left join `staccato`.`produto` `p` on((`e`.`idProduto` = `p`.`idProduto`))) where ((`e`.`restante` > 0) and ((`p`.`fornecedor` = 'portinari') or (`p`.`fornecedor` = 'DECA') or (`p`.`fornecedor` = 'ceusa')) and (`e`.`status` <> 'cancelado')) group by `e`.`codComercial`,`e`.`lote`,`e`.`status`,if((`e`.`created` < '2024-01-01'),'estoque antigo','estoque recente');

-- -----------------------------------------------------
-- View `staccato`.`EDU_view_Orcamentos`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`EDU_view_Orcamentos`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`EDU_view_Orcamentos` AS select `o`.`idOrcamento` AS `idOrcamento`,left(`o`.`idOrcamento`,4) AS `loja`,`u`.`nome` AS `nome`,`o`.`idProfissional` AS `idProfissional`,`o`.`data` AS `data`,date_format(`o`.`data`,'%Y/%m') AS `mes`,`o`.`total` AS `total`,`o`.`status` AS `status`,`o`.`motivoCancelamento` AS `motivoCancelamento`,`o`.`observacaoCancelamento` AS `observacaoCancelamento`,`o`.`fornecedores` AS `fornecedores`,`o`.`idCliente` AS `idcliente` from (`staccato`.`orcamento` `o` left join `staccato`.`usuario` `u` on((`o`.`idUsuario` = `u`.`idUsuario`))) where ((`o`.`created` > '2024-01-01') and (`o`.`status` <> 'replicado'));

-- -----------------------------------------------------
-- View `staccato`.`EDU_view_Receitas`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`EDU_view_Receitas`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`EDU_view_Receitas` AS select date_format(`cr`.`dataEmissao`,'%Y/%m') AS `mes`,`cr`.`status` AS `status`,if((`cr`.`status` = 'recebido'),`cr`.`valorReal`,`cr`.`valor`) AS `valor`,`staccato`.`loja`.`descricao` AS `centroCusto`,`cr`.`grupo` AS `grupo`,`cr`.`idVenda` AS `idvenda`,`cr`.`observacao` AS `obs` from (`staccato`.`conta_a_receber_has_pagamento` `cr` left join `staccato`.`loja` on((`staccato`.`loja`.`idLoja` = `cr`.`centroCusto`))) where ((`cr`.`desativado` = 0) and (`cr`.`grupo` <> 'transferência') and (`cr`.`representacao` = 0) and (`cr`.`dataEmissao` > '2023-06-30') and (`cr`.`status` <> 'cancelado') and (`cr`.`status` <> 'substituido'));

-- -----------------------------------------------------
-- View `staccato`.`EDU_view_Vendas`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`EDU_view_Vendas`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`EDU_view_Vendas` AS select `vp`.`idVendaProduto2` AS `idVendaProduto2`,date_format(`v`.`data`,'%Y/%m') AS `mes`,`vp`.`idVenda` AS `idvenda`,`l`.`descricao` AS `loja`,`u`.`nome` AS `nome`,`vp`.`fornecedor` AS `fornecedor`,`vp`.`produto` AS `produto`,`vp`.`codComercial` AS `codComercial`,`vp`.`total` AS `ValorVenda`,if((`v`.`representacao` = 1),0,((`v`.`frete` / (`v`.`total` - `v`.`frete`)) * `vp`.`total`)) AS `frete`,if((`vp`.`fornecedor` in ('ATELIER STACCATO','STACCATO SERVIÇOS ESPECIAIS (SSE)','PEDRAS NATURAIS')),1,0) AS `servicos`,if((`v`.`representacao` = 1),'representação',if((avg(`ec`.`quant`) is null),if((avg(`pf`.`quant`) is null),if((avg(`p`.`custo`) is null),'erro','tabela'),if((avg(`pf`.`quant`) = `vp`.`quant`),'pedido','erro pf')),if((-(avg(`ec`.`quant`)) = `vp`.`quant`),'estoque','erro ec'))) AS `fonte`,if((`v`.`representacao` = 1),(`vp`.`total` * (1 - ((`f`.`comissaoLoja` - 2) / 100))),if((avg(`ec`.`quant`) is null),if((avg(`pf`.`quant`) is null),if((avg(`p`.`custo`) is null),(`vp`.`total` * 0.6),(avg(`p`.`custo`) * `vp`.`quant`)),if((avg(`pf`.`quant`) = `vp`.`quant`),avg(`pf`.`preco`),((avg(`pf`.`preco`) / `pf`.`quant`) * `vp`.`quant`))),if((-(avg(`ec`.`quant`)) = `vp`.`quant`),(if((`vp`.`quant` < 0),-(1),1) * avg(`ec`.`valor`)),((avg(`ec`.`valor`) / (`ec`.`quant` * -(1))) * `vp`.`quant`)))) AS `custo`,if((`v`.`representacao` = 1),((`vp`.`total` * ((`f`.`comissaoLoja` - 2) / 100)) * 0.05),if((avg(`ec`.`quant`) is null),if((avg(`pf`.`quant`) is null),if((avg(`p`.`custo`) is null),((`vp`.`total` * 0.4) * 0.12),if(((avg(`p`.`st`) = 0) or (avg(`p`.`st`) is null)),((`vp`.`total` * 0.4) * 0.12),((`vp`.`quant` * avg(`p`.`st`)) * avg(`p`.`custo`)))),if(((avg(`p`.`st`) = 0) or (avg(`p`.`st`) is null)),((`vp`.`total` - (if((`vp`.`quant` < 0),-(1),1) * avg(`pf`.`preco`))) * 0.12),((avg(`p`.`st`) * (avg(`pf`.`preco`) / `pf`.`quant`)) * `vp`.`quant`))),((if((avg(`e`.`vICMSST`) > avg(`e`.`valorGare`)),avg(`e`.`vICMSST`),avg(`e`.`valorGare`)) / `e`.`quant`) * `vp`.`quant`))) AS `icms_iss`,if((avg(`e`.`vICMSST`) > 0),((avg(`e`.`vICMSST`) / `e`.`quant`) * `vp`.`quant`),0) AS `STnota`,`ec`.`tipoICMS` AS `tipoicms`,`ec`.`vICMSST` AS `vICMSST`,((sum(if((`cr`.`tipo` like '%taxa cartão%'),-(`cr`.`valor`),0)) / (`v`.`total` - `v`.`frete`)) * `vp`.`total`) AS `taxacartao`,(sum(((to_days(`cr`.`dataPagamento`) - to_days(`cr`.`dataEmissao`)) * `cr`.`valor`)) / sum(`cr`.`valor`)) AS `prz`,`vp`.`quant` AS `quant`,avg(`pf`.`quant`) AS `pfquant`,`p`.`st` AS `st`,avg(`pf`.`preco`) AS `custopedido`,-(avg(`ec`.`quant`)) AS `ecquant`,(if((`vp`.`quant` < 0),-(1),1) * avg(`ec`.`valor`)) AS `custoestoque`,(avg(`p`.`custo`) * `vp`.`quant`) AS `custotabela`,`vp`.`status` AS `status`,(`p`.`precoVenda` * `vp`.`quant`) AS `precotabela` from (((((((((`staccato`.`venda_has_produto2` `vp` left join `staccato`.`venda` `v` on((`v`.`idVenda` = `vp`.`idVenda`))) left join `staccato`.`pedido_fornecedor_has_produto2` `pf` on((`vp`.`idVendaProduto2` = `pf`.`idVendaProduto2`))) left join `staccato`.`estoque_has_consumo` `ec` on((`ec`.`idVendaProduto2` = `vp`.`idVendaProduto2`))) left join `staccato`.`produto` `p` on((`vp`.`idProduto` = `p`.`idProduto`))) left join `staccato`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`))) left join `staccato`.`loja` `l` on((`vp`.`idLoja` = `l`.`idLoja`))) left join `staccato`.`fornecedor` `f` on((`vp`.`fornecedor` = `f`.`razaoSocial`))) left join `staccato`.`estoque` `e` on((`ec`.`idEstoque` = `e`.`idEstoque`))) left join `staccato`.`conta_a_receber_has_pagamento` `cr` on((`vp`.`idVenda` = `cr`.`idVenda`))) where ((`v`.`data` >= '2023-07-30') and (`vp`.`status` <> 'cancelado')) group by `vp`.`idVendaProduto2`;

-- -----------------------------------------------------
-- View `staccato`.`EDU_view_Vendedores`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`EDU_view_Vendedores`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`EDU_view_Vendedores` AS select `u`.`nome` AS `nome`,left(`v`.`idVenda`,4) AS `loja`,date_format(`v`.`data`,'%Y/%m') AS `mes`,`v`.`idProfissional` AS `idProfissional`,(`v`.`total` - `v`.`frete`) AS `total`,`v`.`frete` AS `frete`,`v`.`subTotalBru` AS `subTotalBru`,`v`.`subTotalLiq` AS `subTotalLiq`,`v`.`prazoEntrega` AS `prazoEntrega`,`v`.`rt` AS `rt`,`v`.`representacao` AS `representacao`,`v`.`status` AS `status` from (`staccato`.`venda` `v` left join `staccato`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`))) where ((`v`.`data` > '2024-01-01') and (`v`.`status` <> 'cancelado'));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`ViewDespesasServicos` AS select date_format(`c`.`dataEmissao`,'%Y/%m') AS `MES`,`c`.`grupo` AS `grupo`,`staccato`.`loja`.`descricao` AS `descricao`,`c`.`status` AS `status`,if((`c`.`status` = 'pago'),`c`.`valorReal`,`c`.`valor`) AS `valor`,`c`.`contraParte` AS `contraParte`,`c`.`observacao` AS `observacao` from (`staccato`.`conta_a_pagar_has_pagamento` `c` left join `staccato`.`loja` on((`c`.`idLoja` = `staccato`.`loja`.`idLoja`))) where (((`c`.`centroCusto` = '17') or (`c`.`centroCusto` = '18') or (`c`.`centroCusto` = '49')) and (`c`.`status` <> 'cancelado') and (`c`.`dataEmissao` > '2021-01-01') and (`c`.`desativado` = 0));

-- -----------------------------------------------------
-- View `staccato`.`ViewExcelDespesas`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`ViewExcelDespesas`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`ViewExcelDespesas` AS select `c`.`dataEmissao` AS `dataEmissao`,`c`.`dataPagamento` AS `dataPagamento`,`c`.`contraParte` AS `contraParte`,`l`.`descricao` AS `centroCusto`,`c`.`grupo` AS `grupo`,`c`.`nfe` AS `nfe`,if((`c`.`status` = 'pago'),`c`.`valorReal`,`c`.`valor`) AS `valor`,`c`.`status` AS `status`,`c`.`observacao` AS `observacao` from (`staccato`.`conta_a_pagar_has_pagamento` `c` left join `staccato`.`loja` `l` on((`l`.`idLoja` = `c`.`centroCusto`))) where ((`c`.`dataEmissao` >= '2022-01-01') and (`c`.`status` <> 'cancelado') and (`c`.`status` <> 'cancelado gare') and (`c`.`status` <> 'substituido') and (`c`.`desativado` = 0));

-- -----------------------------------------------------
-- View `staccato`.`ViewExcelEntregasPendentes`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`ViewExcelEntregasPendentes`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`ViewExcelEntregasPendentes` AS select date_format(`v`.`data`,'%Y-%m-%d') AS `data`,`v`.`prazoEntrega` AS `przentrega`,`v`.`novoPrazoEntrega` AS `nvprzentrega`,`vp`.`status` AS `status`,`vp`.`fornecedor` AS `fornecedor`,`vp`.`idVenda` AS `idvenda`,`vp`.`produto` AS `produto`,`vp`.`kg` AS `kg`,`vp`.`caixas` AS `caixas`,`vp`.`quant` AS `quant`,`vp`.`un` AS `un`,`vp`.`codComercial` AS `codComercial`,`vp`.`formComercial` AS `formComercial`,`vp`.`dataPrevEnt` AS `dataPrevEnt`,`v`.`representacao` AS `representacao` from (`staccato`.`venda_has_produto2` `vp` left join `staccato`.`venda` `v` on((`vp`.`idVenda` = `v`.`idVenda`))) where (`vp`.`status` not in ('entregue','cancelado','finalizado','devolvido','devolvido estoque','devolvido forn.','substituido','quebrado')) order by `vp`.`idVenda`,`vp`.`status`;

-- -----------------------------------------------------
-- View `staccato`.`ViewFLXCXExcel`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`ViewFLXCXExcel`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`ViewFLXCXExcel` AS select 'saida' AS `origem`,`staccato`.`conta_a_pagar_has_pagamento`.`centroCusto` AS `centroCusto`,`staccato`.`conta_a_pagar_has_pagamento`.`grupo` AS `grupo`,`staccato`.`conta_a_pagar_has_pagamento`.`contraParte` AS `contraparte`,`staccato`.`conta_a_pagar_has_pagamento`.`dataEmissao` AS `data`,`staccato`.`conta_a_pagar_has_pagamento`.`dataRealizado` AS `datarealizado`,-(`staccato`.`conta_a_pagar_has_pagamento`.`valorReal`) AS `valorreal`,`staccato`.`conta_a_pagar_has_pagamento`.`idConta` AS `idConta`,`staccato`.`conta_a_pagar_has_pagamento`.`observacao` AS `observacao` from `staccato`.`conta_a_pagar_has_pagamento` where ((`staccato`.`conta_a_pagar_has_pagamento`.`dataRealizado` > '2022-01-01') and (`staccato`.`conta_a_pagar_has_pagamento`.`status` like 'pago%')) union select 'entrada' AS `origem`,`staccato`.`conta_a_receber_has_pagamento`.`centroCusto` AS `centroCusto`,`staccato`.`conta_a_receber_has_pagamento`.`grupo` AS `grupo`,`staccato`.`conta_a_receber_has_pagamento`.`contraParte` AS `contraParte`,`staccato`.`conta_a_receber_has_pagamento`.`dataEmissao` AS `data`,`staccato`.`conta_a_receber_has_pagamento`.`dataRealizado` AS `datarealizado`,`staccato`.`conta_a_receber_has_pagamento`.`valorReal` AS `valorreal`,`staccato`.`conta_a_receber_has_pagamento`.`idConta` AS `idconta`,`staccato`.`conta_a_receber_has_pagamento`.`observacao` AS `observacao` from `staccato`.`conta_a_receber_has_pagamento` where ((`staccato`.`conta_a_receber_has_pagamento`.`dataEmissao` > '2022-01-01') and (`staccato`.`conta_a_receber_has_pagamento`.`status` like 'recebido%'));

-- -----------------------------------------------------
-- View `staccato`.`ViewRTCPExcel`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`ViewRTCPExcel`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`ViewRTCPExcel` AS select `staccato`.`conta_a_pagar_has_pagamento`.`dataEmissao` AS `dataEmissao`,`staccato`.`conta_a_pagar_has_pagamento`.`idVenda` AS `idVenda`,`staccato`.`conta_a_pagar_has_pagamento`.`contraParte` AS `contraParte`,`staccato`.`conta_a_pagar_has_pagamento`.`valor` AS `valor`,`staccato`.`conta_a_pagar_has_pagamento`.`valorReal` AS `valorReal`,`staccato`.`conta_a_pagar_has_pagamento`.`dataPagamento` AS `dataPagamento`,`staccato`.`conta_a_pagar_has_pagamento`.`dataRealizado` AS `dataRealizado`,`staccato`.`conta_a_pagar_has_pagamento`.`status` AS `status`,concat(year(`staccato`.`conta_a_pagar_has_pagamento`.`dataEmissao`),'/',if((month(`staccato`.`conta_a_pagar_has_pagamento`.`dataEmissao`) < 10),'0',''),month(`staccato`.`conta_a_pagar_has_pagamento`.`dataEmissao`),'/',truncate(((dayofmonth(`staccato`.`conta_a_pagar_has_pagamento`.`dataEmissao`) / 16) + 1),0)) AS `quinzena` from `staccato`.`conta_a_pagar_has_pagamento` where ((`staccato`.`conta_a_pagar_has_pagamento`.`grupo` = 'rt\'s') and ((`staccato`.`conta_a_pagar_has_pagamento`.`status` = 'pendente') or (`staccato`.`conta_a_pagar_has_pagamento`.`status` = 'conferido') or ((`staccato`.`conta_a_pagar_has_pagamento`.`status` = 'pago') and (`staccato`.`conta_a_pagar_has_pagamento`.`dataRealizado` > (curdate() + interval -(365) day))))) order by `staccato`.`conta_a_pagar_has_pagamento`.`idVenda`;

-- -----------------------------------------------------
-- View `staccato`.`ViewRTVPExcel`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`ViewRTVPExcel`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`ViewRTVPExcel` AS select `v`.`data` AS `data`,`vp`.`idVenda` AS `venda`,`vp`.`fornecedor` AS `fornecedor`,sum(`vp`.`total`) AS `total`,avg(`v`.`rt`) AS `rt`,`v`.`status` AS `status_venda`,`v`.`statusFinanceiro` AS `status_financeiro` from (`staccato`.`venda_has_produto` `vp` left join `staccato`.`venda` `v` on((`vp`.`idVenda` = `v`.`idVenda`))) where ((`v`.`rt` > 0) and (`v`.`data` > '2023-01-01')) group by `vp`.`fornecedor`,`venda`,`v`.`data`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`viewVendaxCMV` AS select distinct `viewVendaxCMV`.`status` AS `status`,`viewVendaxCMV`.`idVendaProduto2` AS `idVendaProduto2`,`viewVendaxCMV`.`mes` AS `mes`,`viewVendaxCMV`.`loja` AS `loja`,`viewVendaxCMV`.`SumOfvenda` AS `SumOfvenda`,if((`viewVendaxCMV`.`vendadev` = 'venda'),`viewVendaxCMV`.`CMV`,if((`viewVendaxCMV`.`FonteCMV` = 'erro'),`viewVendaxCMV`.`CMV`,-(`viewVendaxCMV`.`CMV`))) AS `CMV`,`viewVendaxCMV`.`margem` AS `margem`,`viewVendaxCMV`.`FonteCMV` AS `FonteCMV`,`viewVendaxCMV`.`ICMS` AS `ICMS`,`viewVendaxCMV`.`FonteICMS` AS `FonteICMS`,`viewVendaxCMV`.`vendadev` AS `vendadev`,`viewVendaxCMV`.`fornecedor` AS `fornecedor` from (select `vp2`.`status` AS `status`,`vp2`.`idVendaProduto2` AS `idVendaProduto2`,date_format(`v`.`data`,'%Y/%m') AS `mes`,`l`.`descricao` AS `loja`,(`vp2`.`total` + 0E0) AS `SumOfvenda`,if((-(`ehc`.`quant`) = `vp2`.`quant`),if((`pf2`.`quant` = `vp2`.`quant`),if(((`ehc`.`valor` - `pf2`.`preco`) between -(1) and 1),(`ehc`.`valor` + 0E0),(`pf2`.`preco` + 0E0)),(`ehc`.`valor` + 0E0)),if((`pf2`.`quant` = `vp2`.`quant`),(`pf2`.`preco` + 0E0),((`vp2`.`total` * 0.7) + 0E0))) AS `CMV`,if((`v`.`representacao` = 1),(`vp2`.`total` * (`f`.`comissaoLoja` / 100)),(`vp2`.`total` - if((-(`ehc`.`quant`) = `vp2`.`quant`),if((`pf2`.`quant` = `vp2`.`quant`),if(((`ehc`.`valor` - `pf2`.`preco`) between -(1) and 1),(`ehc`.`valor` + 0E0),(`pf2`.`preco` + 0E0)),(`ehc`.`valor` + 0E0)),if((`pf2`.`quant` = `vp2`.`quant`),(`pf2`.`preco` + 0E0),((`vp2`.`total` * 0.7) + 0E0))))) AS `margem`,if((`vp2`.`fornecedor` like '%staccato%'),'serviços',if((`v`.`representacao` = 1),'representacao',if((`vp2`.`status` = 'pendente'),'sem custo',if((-(`ehc`.`quant`) = `vp2`.`quant`),if((`pf2`.`quant` = `vp2`.`quant`),if(((`ehc`.`valor` - `pf2`.`preco`) between -(1) and 1),'ambos','pedido?'),'estoque'),if((`pf2`.`quant` = `vp2`.`quant`),'pedido','erro'))))) AS `FonteCMV`,if(((`e`.`tipoICMS` = 'icms60') or (`v`.`representacao` = 1)),0,if((if((`e`.`tipoICMS` = 'icms00'),`e`.`valorGare`,`e`.`vICMSST`) is null),(`pf2`.`preco` * 0.25),if((`e`.`tipoICMS` = 'icms00'),((`e`.`valorGare` / `e`.`quant`) * -(`ehc`.`quant`)),((`e`.`vICMSST` / `e`.`quant`) * -(`ehc`.`quant`))))) AS `ICMS`,if(((`e`.`tipoICMS` = 'icms60') or (`v`.`representacao` = 1)),'isento',if((if((`e`.`tipoICMS` = 'icms00'),`e`.`valorGare`,`e`.`vICMSST`) is null),'estimado',if((`e`.`valorGare` is null),'ST Fornecedor','ST Loja'))) AS `FonteICMS`,if((`v`.`devolucao` = 1),'devolução','venda') AS `vendadev`,`vp2`.`fornecedor` AS `fornecedor` from ((((((`staccato`.`venda_has_produto2` `vp2` left join `staccato`.`venda` `v` on((`vp2`.`idVenda` = `v`.`idVenda`))) left join `staccato`.`loja` `l` on((`vp2`.`idLoja` = `l`.`idLoja`))) left join (select avg(`staccato`.`pedido_fornecedor_has_produto2`.`quant`) AS `quant`,avg(`staccato`.`pedido_fornecedor_has_produto2`.`preco`) AS `preco`,`staccato`.`pedido_fornecedor_has_produto2`.`idVendaProduto2` AS `idvendaproduto2` from `staccato`.`pedido_fornecedor_has_produto2` group by `staccato`.`pedido_fornecedor_has_produto2`.`idVendaProduto2`) `pf2` on((`vp2`.`idVendaProduto2` = `pf2`.`idvendaproduto2`))) left join `staccato`.`estoque_has_consumo` `ehc` on((`vp2`.`idVendaProduto2` = `ehc`.`idVendaProduto2`))) left join `staccato`.`fornecedor` `f` on((`vp2`.`fornecedor` = `f`.`razaoSocial`))) left join `staccato`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) where ((`v`.`data` >= '2023-01-01') and (`vp2`.`status` not in ('CANCELADO','SUBSTITUIDO')))) `viewVendaxCMV`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_agendar_coleta` AS select cast(min((`v`.`data` + interval `v`.`prazoEntrega` day)) as date) AS `prazoEntrega`,group_concat(distinct `pf2`.`dataRealFat` separator ', ') AS `dataRealFat`,`e`.`idEstoque` AS `idEstoque`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco`,group_concat(distinct `pf2`.`fornecedor` separator ',') AS `fornecedor`,`p`.`quantCaixa` AS `quantCaixa`,`p`.`formComercial` AS `formComercial`,`p`.`idProduto` AS `idProduto`,`n`.`idNFe` AS `idNFe`,`n`.`numeroNFe` AS `numeroNFe`,group_concat(distinct `pf2`.`idVenda` separator ', ') AS `idVenda`,`pf2`.`idCompra` AS `idCompra`,group_concat(distinct `pf2`.`ordemCompra` separator ', ') AS `ordemCompra`,group_concat(distinct `pf2`.`codFornecedor` separator ', ') AS `codFornecedor`,`e`.`descricao` AS `produto`,`e`.`codComercial` AS `codComercial`,abs(`e`.`quant`) AS `quant`,`e`.`un` AS `un`,`e`.`caixas` AS `caixas`,group_concat(distinct `pf2`.`kgcx` separator ', ') AS `kgcx`,`n`.`transportadora` AS `Transportadora`,(`e`.`caixas` * max(`pf2`.`kgcx`)) AS `Peso` from (((((`staccato`.`estoque` `e` left join `staccato`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `staccato`.`pedido_fornecedor_has_produto2` `pf2` on((`ehc`.`idPedido2` = `pf2`.`idPedido2`))) left join `staccato`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `staccato`.`venda` `v` on((`v`.`idVenda` = `pf2`.`idVenda`))) left join `staccato`.`produto` `p` on((`e`.`idProduto` = `p`.`idProduto`))) where ((`e`.`status` = 'EM COLETA') and (`pf2`.`dataPrevColeta` is null) and (`pf2`.`idPedido2` is not null)) group by `e`.`idEstoque` order by `prazoEntrega`,`e`.`idEstoque`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_coleta` AS select cast(min((`v`.`data` + interval `v`.`prazoEntrega` day)) as date) AS `prazoEntrega`,group_concat(distinct `pf2`.`dataPrevColeta` separator ',') AS `dataPrevColeta`,`e`.`idEstoque` AS `idEstoque`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco`,group_concat(distinct `pf2`.`fornecedor` separator ',') AS `fornecedor`,`n`.`idNFe` AS `idNFe`,group_concat(distinct `n`.`numeroNFe` separator ',') AS `numeroNFe`,group_concat(distinct `pf2`.`idVenda` separator ', ') AS `idVenda`,`pf2`.`idCompra` AS `idCompra`,group_concat(distinct `pf2`.`ordemCompra` separator ', ') AS `ordemCompra`,group_concat(distinct `pf2`.`codFornecedor` separator ', ') AS `codFornecedor`,`e`.`descricao` AS `produto`,`e`.`codComercial` AS `codComercial`,abs(`e`.`quant`) AS `quant`,`e`.`un` AS `un`,`e`.`caixas` AS `caixas`,group_concat(distinct `pf2`.`kgcx` separator ',') AS `kgcx`,group_concat(distinct `n`.`transportadora` separator ',') AS `Transportadora` from ((((`staccato`.`estoque` `e` left join `staccato`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `staccato`.`pedido_fornecedor_has_produto2` `pf2` on((`ehc`.`idPedido2` = `pf2`.`idPedido2`))) left join `staccato`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `staccato`.`venda` `v` on((`v`.`idVenda` = `pf2`.`idVenda`))) where ((`e`.`status` = 'EM COLETA') and (`pf2`.`dataPrevColeta` is not null) and (`pf2`.`idPedido2` is not null)) group by `e`.`idEstoque` order by `prazoEntrega`,`e`.`idEstoque`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_compras` AS select any_value(`pf2`.`ordemCompra`) AS `OC`,`pf2`.`idCompra` AS `Compra`,any_value(`pf2`.`fornecedor`) AS `Fornecedor`,min(`v`.`data`) AS `Data Venda`,count(`pf2`.`idProduto`) AS `Produtos`,sum(`pf2`.`preco`) AS `Total`,any_value(`pf2`.`dataPrevConf`) AS `dataPrevConf`,group_concat(distinct substring_index(`u`.`nome`,' ',1) separator ', ') AS `Vendedor`,group_concat(distinct `pf2`.`idVenda` separator ', ') AS `Venda`,cast((`v`.`data` + interval `v`.`prazoEntrega` day) as date) AS `prazoEntrega`,cast((`v`.`data` + interval `v`.`novoPrazoEntrega` day) as date) AS `novoPrazoEntrega`,`pff`.`dataFollowup` AS `Data Followup`,`pff`.`observacao` AS `Observação` from ((((`staccato`.`pedido_fornecedor_has_produto2` `pf2` left join `staccato`.`pedido_fornecedor_has_produto` `pf1` on((`pf2`.`idPedidoFK` = `pf1`.`idPedido1`))) left join `staccato`.`pedido_fornecedor_has_followup` `pff` on((`pf1`.`idFollowup` = `pff`.`idFollowup`))) left join `staccato`.`venda` `v` on((`pf2`.`idVenda` = `v`.`idVenda`))) left join `staccato`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`))) where (`pf2`.`status` = 'EM COMPRA') group by `pf2`.`ordemCompra`,`pff`.`idFollowup`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_conta_pagar` AS select `cp`.`idPagamento` AS `idPagamento`,`cp`.`idLoja` AS `idLoja`,`cp`.`contraParte` AS `contraparte`,`cp`.`dataPagamento` AS `dataPagamento`,`cp`.`dataEmissao` AS `dataEmissao`,`cp`.`valor` AS `valor`,`cp`.`status` AS `status`,group_concat(distinct `pf2`.`ordemCompra` separator ',') AS `ordemCompra`,group_concat(distinct `pf2`.`idVenda` separator ', ') AS `idVenda`,group_concat(distinct `n`.`numeroNFe` separator ', ') AS `numeroNFe`,`cp`.`tipo` AS `tipo`,`cp`.`parcela` AS `parcela`,`cp`.`observacao` AS `observacao`,group_concat(distinct `pf2`.`statusFinanceiro` separator ',') AS `statusFinanceiro` from (((((`staccato`.`conta_a_pagar_has_pagamento` `cp` left join `staccato`.`conta_a_pagar_has_idcompra` `cp2` on((`cp`.`idPagamento` = `cp2`.`idPagamento`))) left join `staccato`.`pedido_fornecedor_has_produto2` `pf2` on((`cp2`.`idCompra` = `pf2`.`idCompra`))) left join `staccato`.`estoque_has_compra` `ehc` on((`ehc`.`idPedido2` = `pf2`.`idPedido2`))) left join `staccato`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) left join `staccato`.`nfe` `n` on((`n`.`idNFe` = `e`.`idNFe`))) where (`cp`.`desativado` = false) group by `cp`.`idPagamento` order by `cp`.`dataPagamento`;

-- -----------------------------------------------------
-- View `staccato`.`view_conta_pagar_idcompra`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`view_conta_pagar_idcompra`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_conta_pagar_idcompra` AS select `cp`.`idPagamento` AS `idPagamento`,`cp`.`dataEmissao` AS `dataEmissao`,`cp2`.`idCompra` AS `idCompra`,`cp`.`idVenda` AS `idVenda`,`cp`.`idLoja` AS `idLoja`,`cp`.`contraParte` AS `contraParte`,`cp`.`idCnab` AS `idCnab`,`cp`.`idNFe` AS `idNFe`,`cp`.`nfe` AS `nfe`,`cp`.`valor` AS `valor`,`cp`.`tipo` AS `tipo`,`cp`.`parcela` AS `parcela`,`cp`.`dataPagamento` AS `dataPagamento`,`cp`.`observacao` AS `observacao`,`cp`.`status` AS `status`,`cp`.`dataRealizado` AS `dataRealizado`,`cp`.`valorReal` AS `valorReal`,`cp`.`tipoReal` AS `tipoReal`,`cp`.`parcelaReal` AS `parcelaReal`,`cp`.`idConta` AS `idConta`,`cp`.`tipoDet` AS `tipoDet`,`cp`.`centroCusto` AS `centroCusto`,`cp`.`grupo` AS `grupo`,`cp`.`subGrupo` AS `subGrupo`,`cp`.`compraAvulsa` AS `compraAvulsa`,`cp`.`desativado` AS `desativado`,`cp`.`created` AS `created`,`cp`.`lastUpdated` AS `lastUpdated` from (`staccato`.`conta_a_pagar_has_pagamento` `cp` left join `staccato`.`conta_a_pagar_has_idcompra` `cp2` on((`cp`.`idPagamento` = `cp2`.`idPagamento`))) group by `cp`.`idPagamento`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_entrega` AS select cast(min((`v`.`data` + interval `v`.`prazoEntrega` day)) as date) AS `prazoEntrega`,`vp2`.`idVenda` AS `idVenda`,group_concat(distinct `vp2`.`recebeu` separator ',') AS `Recebeu`,group_concat(distinct `vp2`.`entregou` separator ',') AS `Entregou`,sum((`vp2`.`status` = 'ESTOQUE')) AS `Estoque`,sum((`vp2`.`status` = 'ENTREGUE')) AS `Entregue`,sum(((`vp2`.`status` <> 'ESTOQUE') and (`vp2`.`status` <> 'ENTREGUE'))) AS `Outros`,`che`.`bairro` AS `Bairro`,`che`.`logradouro` AS `Logradouro`,`che`.`numero` AS `Número`,`che`.`cidade` AS `Cidade` from ((`staccato`.`venda_has_produto2` `vp2` left join `staccato`.`venda` `v` on((`vp2`.`idVenda` = `v`.`idVenda`))) left join `staccato`.`cliente_has_endereco` `che` on((`v`.`idEnderecoEntrega` = `che`.`idEndereco`))) where (not((`vp2`.`idVenda` like '%D'))) group by `vp2`.`idVenda`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_faturamento` AS select any_value(`pf2`.`ordemCompra`) AS `ordemCompra`,group_concat(distinct `pf2`.`codFornecedor` separator ', ') AS `codFornecedor`,any_value(`pf2`.`fornecedor`) AS `fornecedor`,min(`v`.`data`) AS `data`,count(`pf2`.`idProduto`) AS `produtos`,sum(`pf2`.`preco`) AS `total`,`pf2`.`idCompra` AS `idCompra`,any_value(`pf2`.`dataPrevFat`) AS `dataPrevFat`,group_concat(distinct substring_index(`u`.`nome`,' ',1) separator ', ') AS `vendedor`,group_concat(distinct `pf2`.`idVenda` separator ', ') AS `idVenda`,cast((`v`.`data` + interval `v`.`prazoEntrega` day) as date) AS `prazoEntrega`,cast((`v`.`data` + interval `v`.`novoPrazoEntrega` day) as date) AS `novoPrazoEntrega`,any_value(`f`.`representacao`) AS `representacao`,`pff`.`dataFollowup` AS `dataFollowup`,`pff`.`observacao` AS `observacao` from ((((((`staccato`.`pedido_fornecedor_has_produto2` `pf2` left join `staccato`.`pedido_fornecedor_has_produto` `pf1` on((`pf2`.`idPedidoFK` = `pf1`.`idPedido1`))) left join `staccato`.`pedido_fornecedor_has_followup` `pff` on((`pf1`.`idFollowup` = `pff`.`idFollowup`))) left join `staccato`.`produto` `p` on((`pf2`.`idProduto` = `p`.`idProduto`))) left join `staccato`.`fornecedor` `f` on((`p`.`idFornecedor` = `f`.`idFornecedor`))) left join `staccato`.`venda` `v` on((`v`.`idVenda` = `pf2`.`idVenda`))) left join `staccato`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`))) where (`pf2`.`status` = 'EM FATURAMENTO') group by `pf2`.`idCompra`,`pff`.`idFollowup`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_fluxo_caixa_realizado` AS select `cp`.`contraParte` AS `ContraParte`,group_concat(distinct `pf`.`ordemCompra` separator ',') AS `Compra/Venda`,`cp`.`dataRealizado` AS `dataRealizado`,(`cp`.`valorReal` * -(1)) AS `R$`,`cp`.`tipo` AS `Tipo`,(select concat(`staccato`.`loja_has_conta`.`banco`,' - ',`staccato`.`loja_has_conta`.`agencia`,' - ',`staccato`.`loja_has_conta`.`conta`) from `staccato`.`loja_has_conta` where (`staccato`.`loja_has_conta`.`idConta` = `cp`.`idConta`)) AS `contaDestino`,`cp`.`idConta` AS `idConta`,`cp`.`status` AS `Status`,'SAIDA' AS `ENT/SAI` from ((`staccato`.`conta_a_pagar_has_pagamento` `cp` left join `staccato`.`conta_a_pagar_has_idcompra` `cp2` on((`cp`.`idPagamento` = `cp2`.`idPagamento`))) left join `staccato`.`pedido_fornecedor_has_produto` `pf` on((`cp2`.`idCompra` = `pf`.`idCompra`))) where ((`cp`.`status` in ('PAGO','PAGO GARE')) and (`cp`.`dataRealizado` is not null) and (`cp`.`valorReal` is not null) and (`cp`.`desativado` = false)) group by `cp`.`idPagamento` union all select `cr`.`contraParte` AS `ContraParte`,`cr`.`idVenda` AS `Compra/Venda`,`cr`.`dataRealizado` AS `dataRealizado`,`cr`.`valorReal` AS `valorReal`,`cr`.`tipo` AS `tipo`,(select concat(`staccato`.`loja_has_conta`.`banco`,' - ',`staccato`.`loja_has_conta`.`agencia`,' - ',`staccato`.`loja_has_conta`.`conta`) from `staccato`.`loja_has_conta` where (`staccato`.`loja_has_conta`.`idConta` = `cr`.`idConta`)) AS `contaDestino`,`cr`.`idConta` AS `idConta`,`cr`.`status` AS `status`,'ENTRADA' AS `ENTRADA` from `staccato`.`conta_a_receber_has_pagamento` `cr` where ((`cr`.`status` = 'RECEBIDO') and (`cr`.`dataRealizado` is not null) and (`cr`.`valorReal` is not null) and (`cr`.`desativado` = false)) order by `dataRealizado`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_fluxo_caixa_realizado_grupo` AS select group_concat(distinct `pf`.`ordemCompra` separator ',') AS `Compra/Venda`,`cp`.`dataRealizado` AS `dataRealizado`,(`cp`.`valorReal` * -(1)) AS `R$`,`cp`.`tipo` AS `Tipo`,`cp`.`grupo` AS `grupo`,`cp`.`observacao` AS `obs`,`cp`.`centroCusto` AS `ccusto`,(select concat(`staccato`.`loja_has_conta`.`banco`,' - ',`staccato`.`loja_has_conta`.`agencia`,' - ',`staccato`.`loja_has_conta`.`conta`) from `staccato`.`loja_has_conta` where (`staccato`.`loja_has_conta`.`idConta` = `cp`.`idConta`)) AS `contaDestino`,`cp`.`idConta` AS `idConta`,`cp`.`status` AS `Status`,'SAIDA' AS `ENT/SAI` from ((`staccato`.`conta_a_pagar_has_pagamento` `cp` left join `staccato`.`conta_a_pagar_has_idcompra` `cp2` on((`cp`.`idPagamento` = `cp2`.`idPagamento`))) left join `staccato`.`pedido_fornecedor_has_produto` `pf` on((`cp2`.`idCompra` = `pf`.`idCompra`))) where ((`cp`.`status` in ('PAGO','PAGO GARE')) and (`cp`.`dataRealizado` is not null) and (`cp`.`valorReal` is not null) and (`cp`.`desativado` = false)) group by `cp`.`idPagamento` union all select `cr`.`idVenda` AS `idVenda`,`cr`.`dataRealizado` AS `dataRealizado`,`cr`.`valorReal` AS `valorReal`,`cr`.`tipo` AS `tipo`,`cr`.`grupo` AS `grupo`,`cr`.`observacao` AS `obs`,`cr`.`centroCusto` AS `ccusto`,(select concat(`staccato`.`loja_has_conta`.`banco`,' - ',`staccato`.`loja_has_conta`.`agencia`,' - ',`staccato`.`loja_has_conta`.`conta`) from `staccato`.`loja_has_conta` where (`staccato`.`loja_has_conta`.`idConta` = `cr`.`idConta`)) AS `contaDestino`,`cr`.`idConta` AS `idConta`,`cr`.`status` AS `status`,'ENTRADA' AS `ENTRADA` from `staccato`.`conta_a_receber_has_pagamento` `cr` where ((`cr`.`status` = 'RECEBIDO') and (`cr`.`dataRealizado` is not null) and (`cr`.`valorReal` is not null) and (`cr`.`desativado` = false)) order by `dataRealizado`;
...
-- -----------------------------------------------------
-- View `staccato`.`view_mkt_creditos`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`view_mkt_creditos`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_mkt_creditos` AS select `staccato`.`conta_a_pagar_has_pagamento`.`dataEmissao` AS `dataEmissao`,`staccato`.`conta_a_pagar_has_pagamento`.`contraParte` AS `contraParte`,`staccato`.`conta_a_pagar_has_pagamento`.`nfe` AS `nfe`,`staccato`.`conta_a_pagar_has_pagamento`.`valorReal` AS `valorreal`,`staccato`.`conta_a_pagar_has_pagamento`.`dataRealizado` AS `dataRealizado`,`staccato`.`conta_a_pagar_has_pagamento`.`observacao` AS `status` from `staccato`.`conta_a_pagar_has_pagamento` where ((`staccato`.`conta_a_pagar_has_pagamento`.`idConta` = 38) and (`staccato`.`conta_a_pagar_has_pagamento`.`dataEmissao` > '2025-01-01') and (`staccato`.`conta_a_pagar_has_pagamento`.`status` <> 'cancelado'));

-- -----------------------------------------------------
-- View `staccato`.`view_mkt_pagamentos`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`view_mkt_pagamentos`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_mkt_pagamentos` AS select `staccato`.`conta_a_pagar_has_pagamento`.`dataEmissao` AS `dataEmissao`,`staccato`.`conta_a_pagar_has_pagamento`.`dataRealizado` AS `dataRealizado`,`staccato`.`conta_a_pagar_has_pagamento`.`centroCusto` AS `centroCusto`,`staccato`.`conta_a_pagar_has_pagamento`.`contraParte` AS `contraParte`,`staccato`.`conta_a_pagar_has_pagamento`.`nfe` AS `nfe`,`staccato`.`conta_a_pagar_has_pagamento`.`valor` AS `valor`,`staccato`.`conta_a_pagar_has_pagamento`.`valorReal` AS `valorReal`,`staccato`.`conta_a_pagar_has_pagamento`.`status` AS `status`,`staccato`.`conta_a_pagar_has_pagamento`.`observacao` AS `observacao` from `staccato`.`conta_a_pagar_has_pagamento` where ((`staccato`.`conta_a_pagar_has_pagamento`.`grupo` = 'marketing - reembolsável') and (`staccato`.`conta_a_pagar_has_pagamento`.`dataEmissao` > '2025-01-01') and (`staccato`.`conta_a_pagar_has_pagamento`.`status` <> 'cancelado'));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_orcamento` AS select `o`.`idLoja` AS `idLoja`,`o`.`idUsuario` AS `idUsuario`,`o`.`idUsuarioConsultor` AS `idUsuarioConsultor`,`o`.`status` AS `status`,if((`o`.`status` not in ('FECHADO','PERDIDO','CANCELADO','REPLICADO')),(`o`.`validade` - (to_days(now()) - to_days(`o`.`data`))),'') AS `diasRestantes`,`o`.`idOrcamento` AS `idOrcamento`,`u`.`nome` AS `vendedor`,`u2`.`nome` AS `consultor`,`c`.`nome_razao` AS `cliente`,`p`.`nome_razao` AS `profissional`,`c`.`tel` AS `tel`,`c`.`telCel` AS `telCel`,if((`p`.`telCel` is null),if((`p`.`tel` is null),`p`.`telCom`,`p`.`tel`),`p`.`telCel`) AS `telProf`,`o`.`data` AS `data`,`o`.`data2` AS `data2`,`o`.`total` AS `total`,`ohf`.`idFollowup` AS `idFollowup`,`ohf`.`dataFollowup` AS `dataFollowup`,`ohf`.`dataProxFollowup` AS `dataProxFollowup`,`ohf`.`observacao` AS `observacao`,`ohf`.`semaforo` AS `semaforo`,`o`.`fornecedores` AS `fornecedores` from (((((`staccato`.`orcamento` `o` left join `staccato`.`cliente` `c` on((`o`.`idCliente` = `c`.`idCliente`))) left join `staccato`.`usuario` `u` on((`o`.`idUsuario` = `u`.`idUsuario`))) left join `staccato`.`profissional` `p` on((`o`.`idProfissional` = `p`.`idProfissional`))) left join `staccato`.`usuario` `u2` on((`o`.`idUsuarioConsultor` = `u2`.`idUsuario`))) left join `staccato`.`orcamento_has_followup` `ohf` on((`o`.`idFollowup` = `ohf`.`idFollowup`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_recebimento` AS select cast(min((`v`.`data` + interval `v`.`prazoEntrega` day)) as date) AS `prazoEntrega`,group_concat(distinct `pf2`.`dataPrevReceb` separator ',') AS `dataPrevReceb`,`e`.`idEstoque` AS `idEstoque`,`e`.`idNFe` AS `idNFe`,`e`.`lote` AS `lote`,`e`.`local` AS `local`,`e`.`bloco` AS `bloco`,group_concat(distinct `pf2`.`fornecedor` separator ',') AS `fornecedor`,group_concat(distinct `n`.`numeroNFe` separator ',') AS `numeroNFe`,group_concat(distinct `pf2`.`idVenda` separator ', ') AS `idVenda`,`pf2`.`idCompra` AS `idCompra`,group_concat(distinct `pf2`.`ordemCompra` separator ', ') AS `ordemCompra`,group_concat(distinct `pf2`.`codFornecedor` separator ', ') AS `codFornecedor`,`e`.`descricao` AS `produto`,`e`.`codComercial` AS `codComercial`,abs(`e`.`quant`) AS `quant`,`e`.`un` AS `un`,`e`.`caixas` AS `caixas` from ((((`staccato`.`estoque` `e` left join `staccato`.`estoque_has_compra` `ehc` on((`e`.`idEstoque` = `ehc`.`idEstoque`))) left join `staccato`.`pedido_fornecedor_has_produto2` `pf2` on((`ehc`.`idPedido2` = `pf2`.`idPedido2`))) left join `staccato`.`nfe` `n` on((`e`.`idNFe` = `n`.`idNFe`))) left join `staccato`.`venda` `v` on((`v`.`idVenda` = `pf2`.`idVenda`))) where ((`e`.`status` = 'EM RECEBIMENTO') and (`pf2`.`dataPrevReceb` is not null) and (`pf2`.`idPedido2` is not null)) group by `e`.`idEstoque` order by `prazoEntrega`,`e`.`idEstoque`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_venda` AS select `v`.`idLoja` AS `idLoja`,`v`.`idUsuario` AS `idUsuario`,`v`.`idUsuarioConsultor` AS `idUsuarioConsultor`,`v`.`statusFinanceiro` AS `statusFinanceiro`,`v`.`dataFinanceiro` AS `dataFinanceiro`,`v`.`status` AS `status`,if((`v`.`status` not in ('ENTREGUE','CANCELADO','DEVOLVIDO')),((to_days(`v`.`data`) + `v`.`prazoEntrega`) - to_days(now())),'') AS `diasRestantes`,`v`.`idVenda` AS `idVenda`,`v`.`ordemRepresentacao` AS `ordemRepresentacao`,`u`.`nome` AS `vendedor`,`u2`.`nome` AS `consultor`,`c`.`nome_razao` AS `cliente`,`p`.`nome_razao` AS `profissional`,`v`.`data` AS `data`,`v`.`data2` AS `data2`,`v`.`data3` AS `data3`,`v`.`total` AS `total`,`vhf`.`idFollowup` AS `idFollowup`,`vhf`.`dataFollowup` AS `dataFollowup`,`vhf`.`observacao` AS `observacao`,`v`.`fornecedores` AS `fornecedores` from ((((((`staccato`.`venda` `v` left join `staccato`.`usuario` `u` on((`v`.`idUsuario` = `u`.`idUsuario`))) left join `staccato`.`cliente_has_endereco` `e` on((`v`.`idEnderecoEntrega` = `e`.`idEndereco`))) left join `staccato`.`profissional` `p` on((`v`.`idProfissional` = `p`.`idProfissional`))) left join `staccato`.`cliente` `c` on((`v`.`idCliente` = `c`.`idCliente`))) left join `staccato`.`usuario` `u2` on((`v`.`idUsuarioConsultor` = `u2`.`idUsuario`))) left join `staccato`.`venda_has_followup` `vhf` on((`v`.`idFollowup` = `vhf`.`idFollowup`)));
...
TRIGGER `staccato`.`nfe_has_followup_AFTER_DELETE`
AFTER DELETE ON `staccato`.`nfe_has_followup`
FOR EACH ROW
BEGIN
	UPDATE nfe n SET n.idFollowup = NULL WHERE n.idNFe = OLD.idNFe;
END$$

USE `staccato`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `staccato`.`nfe_has_followup_AFTER_INSERT`
AFTER INSERT ON `staccato`.`nfe_has_followup`
FOR EACH ROW
BEGIN
	UPDATE nfe n SET n.idFollowup = NEW.idFollowup WHERE n.idNFe = NEW.idNFe;
END$$

USE `staccato`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `staccato`.`pedido_fornecedor_has_followup_AFTER_DELETE`
AFTER DELETE ON `staccato`.`pedido_fornecedor_has_followup`
FOR EACH ROW
BEGIN
	UPDATE pedido_fornecedor_has_produto pf1 SET pf1.idFollowup = NULL WHERE pf1.ordemCompra = OLD.ordemCompra;
END$$

USE `staccato`$$
CREATE
DEFINER=`root`@`localhost`
TRIGGER `staccato`.`pedido_fornecedor_has_followup_AFTER_INSERT`
AFTER INSERT ON `staccato`.`pedido_fornecedor_has_followup`
FOR EACH ROW
BEGIN
	UPDATE pedido_fornecedor_has_produto pf1 SET pf1.idFollowup = NEW.idFollowup WHERE pf1.ordemCompra = NEW.ordemCompra;
END$$
