...
QString TextItem::replaceBR(QString text) { return text.replace("<br/>", "\n"); }
...
TextItem::TextPtr TextItem::textDocument() const {
  ...
  if (m_lineSpacing != 1 or not qFuzzyIsNull(m_textIndent)) {

    for (QTextBlock block = text->begin(); block.isValid(); block = block.next()) {
      QTextCursor tc = QTextCursor(block);
      QTextBlockFormat fmt = block.blockFormat();
      fmt.setTextIndent(m_textIndent);
      if (not qFuzzyCompare(fmt.lineHeight(), m_lineSpacing)) { fmt.setLineHeight(m_lineSpacing, QTextBlockFormat::LineDistanceHeight); }
...
void TextItem::setTextIndent(const qreal &textIndent) {
  if (not qFuzzyCompare(m_textIndent, textIndent)) {
