...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_galpao` (`idEstoque_idConsumo` INT, `descricao` INT, `status` INT, `idVenda` INT, `codComercial` INT, `numeroNFe` INT, `idNFe` INT, `caixas` INT, `quant` INT, `idBloco` INT, `idVendaProduto2` INT, `lote` INT, `tipo` INT);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_galpao` AS select `staccato`.`vec`.`idEstoque` AS `idEstoque_idConsumo`,`staccato`.`vec`.`descricao` AS `descricao`,`staccato`.`vec`.`status` AS `status`,group_concat(distinct `vp2`.`idVenda` separator ', ') AS `idVenda`,`staccato`.`vec`.`codComercial` AS `codComercial`,`staccato`.`vec`.`nfe` AS `numeroNFe`,`staccato`.`vec`.`idNFe` AS `idNFe`,`staccato`.`vec`.`caixasContabil` AS `caixas`,`staccato`.`vec`.`contabil` AS `quant`,`staccato`.`vec`.`idBloco` AS `idBloco`,NULL AS `idVendaProduto2`,`staccato`.`vec`.`lote` AS `lote`,'EST. LOJA' AS `tipo` from ((`staccato`.`view_estoque_contabil` `vec` left join `staccato`.`estoque_has_consumo` `ehc` on((`staccato`.`vec`.`idEstoque` = `ehc`.`idEstoque`))) left join `staccato`.`venda_has_produto2` `vp2` on((`ehc`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) where ((`staccato`.`vec`.`contabil` > 0) and (`vp2`.`status` in ('ESTOQUE','ENTREGA AGEND.','EM ENTREGA'))) group by `staccato`.`vec`.`idEstoque`;
