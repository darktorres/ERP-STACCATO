...
LimeReport::PropertyDelegate::PropertyDelegate(QObject *parent) : QStyledItemDelegate(parent), m_objectInspector(NULL), m_editingItem(0), m_isEditing(false) {}
...
void LimeReport::PropertyDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const {
  ...
  if (node) {
    if (!node->isHaveValue()) {
      ...
      if ((node->isValueReadonly()) && (!node->isHaveChildren())) { so.palette.setColor(QPalette::Text, so.palette.color(QPalette::Dark)); }

      QColor backgroundColor = (so.features & StyleOptionViewItem::Alternate) ? so.palette.alternateBase().color() : so.palette.base().color();

      qreal luma = 0.2126 * backgroundColor.red() + 0.7152 * backgroundColor.green() + 0.0722 * backgroundColor.blue();
...
QWidget *LimeReport::PropertyDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const {
  ...
  if (editor) {
    ...
    if (editor->metaObject()->indexOfSignal("editingFinished()") != -1) connect(editor, SIGNAL(editingFinished()), this, SLOT(commitAndCloseEditor()));
...
void LimeReport::PropertyDelegate::setObjectInspector(ObjectInspectorTreeView *objectInspector) { m_objectInspector = objectInspector; }
