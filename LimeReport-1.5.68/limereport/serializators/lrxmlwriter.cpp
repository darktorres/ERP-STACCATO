...
void XMLWriter::init() {
  ...
  QDomNode xmlNode = m_doc->createProcessingInstruction("xml", "version=\"1.0\" encoding=\"UTF8\"");
...
bool XMLWriter::setContent(QString fileName) {
  ...
  if (xmlFile.open(QFile::ReadOnly)) { return m_doc->setContent(&xmlFile); }
  return false;
}
...
void XMLWriter::saveProperty(QString name, QObject *item, QDomElement *node) {
  ...
  if (enumOrFlag(name, item))
    ...
    } catch (LimeReport::ReportError &exception) { qDebug() << "class name =" << item->metaObject()->className() << "property name=" << name << " property type=" << typeName << exception.what(); }
  ...
  if (creator) {
    ...
    if (cs) { cs->setPassPhrase(m_passPhrase); }
    serializator->save(item->property(name.toLatin1()), name);
  }
}

void XMLWriter::saveProperties(QObject *item, QDomElement *node) {
  for (int i = 0; i < item->metaObject()->propertyCount(); i++) { saveProperty(item->metaObject()->property(i).name(), item, node); }
}
...
void XMLWriter::saveCollection(QString propertyName, QObject *item, QDomElement *node) {
  ...
  for (int i = 0; i < collection->elementsCount(propertyName); i++) { putCollectionItem(collection->elementAt(propertyName, i), &collectionNode); }
    ...
    foreach (QLocale::Language language, translations->keys()) {
      ...
      foreach (PageTranslation *page, curTranslation->pagesTranslation()) {
        ...
        foreach (ItemTranslation *item, page->itemsTranslation) {
          ...
          if (!itemNode.childNodes().isEmpty()) pageNode.appendChild(itemNode);
        }
      }
    }
...
void XMLWriter::putCollectionItem(QObject *item, QDomElement *parentNode) { putChildQObjectItem("item", item, parentNode); }

void XMLWriter::putQObjectProperty(QString propertyName, QObject *item, QDomElement *parentNode) { putChildQObjectItem(propertyName, item, parentNode); }
