...
FontEditorWidget::FontEditorWidget(const QString &title, QWidget *parent) : ItemEditorWidget(title, parent), m_ignoreSlots(false) { initEditor(); }
...
void FontEditorWidgetForPage::slotFontChanged(const QFont &font) {
  if (!ignoreSlots()) m_page->setFont(font);
}
