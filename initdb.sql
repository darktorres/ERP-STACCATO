...
BEGIN
    ...
    UPDATE produto, view_estoque2
    SET produto.estoqueRestante = view_estoque2.restante
    WHERE produto.idEstoque = view_estoque2.idEstoque;
    
    UPDATE produto p SET p.descontinuado = TRUE WHERE p.idEstoque NOT IN (SELECT idEstoque FROM view_estoque2);
    
    UPDATE produto
    SET produto.descontinuado = TRUE
    WHERE produto.estoque = TRUE AND estoqueRestante = 0;
    
    UPDATE produto
    SET produto.descontinuado = FALSE
    WHERE produto.estoque = TRUE AND estoqueRestante > 0;
    ...
    FROM
        ...
        LEFT JOIN (SELECT 
            SUM(`ec`.`quant`) AS `consumo`,
                ...
                `ec`.`idVendaProduto` AS `idVendaProduto`
        FROM
