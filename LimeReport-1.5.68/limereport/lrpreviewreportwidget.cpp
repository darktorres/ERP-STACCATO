...
QRectF PreviewReportWidgetPrivate::calcPageShift() {
  ...
  qreal viewHeight = view->mapToScene(0, view->viewport()->height()).y() - view->mapToScene(0, 0).y();
  viewHeight = (pageHeight < viewHeight) ? pageHeight : viewHeight;
  QRectF pageStartPos = m_reportPages.at(m_currentPage - 1)->mapRectToScene(m_reportPages.at(m_currentPage - 1)->rect());
...
QList<QString> PreviewReportWidgetPrivate::aviableExporters() { return ExportersFactory::instance().map().keys(); }

void PreviewReportWidgetPrivate::startInsertTextItem() { m_previewPage->startInsertMode("TextItem"); }
...
PreviewReportWidget::PreviewReportWidget(ReportEngine *report, QWidget *parent)
    : QWidget(parent), ui(new Ui::PreviewReportWidget), d_ptr(new PreviewReportWidgetPrivate(this)), m_scaleType(FitWidth), m_scalePercent(0), m_previewPageBackgroundColor(Qt::white),
      m_defaultPrinter(0) {
  ui->setupUi(this);
...
bool PreviewReportWidget::exportReport(QString exporterName, const QMap<QString, QVariant> &params) {
  if (ExportersFactory::instance().map().contains(exporterName)) {
    ...
    if (!fileName.isEmpty()) {
      ...
      if (fi.suffix().isEmpty()) fileName += QString(".%1").arg(e->exporterFileExt());
...
void PreviewReportWidget::initPreview() {
  ...
  if (page) connect(page, SIGNAL(itemInserted(LimeReport::PageDesignIntf *, QPointF, QString)), this, SIGNAL(itemInserted(LimeReport::PageDesignIntf *, QPointF, QString)));
}

void PreviewReportWidget::setErrorsMesagesVisible(bool visible) { ui->errorsView->setVisible(visible); }
...
void PreviewReportWidget::zoomOut() {
  if (d_ptr->m_scalePercent > 0) d_ptr->m_scalePercent = (d_ptr->m_scalePercent / 10) * 10 - 10;
  setScalePercent(d_ptr->m_scalePercent);
}
...
void PreviewReportWidget::printPages(QPrinter *printer) {
  if (!d_ptr->m_reportPages.isEmpty()) d_ptr->m_report->printPages(d_ptr->m_reportPages, printer);
...
void PreviewReportWidget::print() {
  ...
  if (dialog.exec() == QDialog::Accepted) { printPages(printer); }
}
...
void PreviewReportWidget::setDefaultPrinter(QPrinter *defaultPrinter) { m_defaultPrinter = defaultPrinter; }
...
void PreviewReportWidget::setPreviewPageBackgroundColor(QColor color) { m_previewPageBackgroundColor = color; }
...
void PreviewReportWidget::activateCurrentPage() {
  PageDesignIntf *page = dynamic_cast<PageDesignIntf *>(ui->graphicsView->scene());
  if (page) page->setCurrentPage(d_ptr->currentPage().data());
}
...
void PreviewReportWidget::reportEngineDestroyed(QObject *object) {
  if (object == d_ptr->m_report) { d_ptr->m_report = 0; }
}
...
void PreviewReportWidget::resizeDone() {
  ...
  case OneToOne: setScalePercent(100); break;
  case Percents: setScalePercent(m_scalePercent); break;
  }
}
