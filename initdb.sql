...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_galpao` (`idEstoque_idConsumo` INT, `descricao` INT, `status` INT, `idVenda` INT, `codComercial` INT, `numeroNFe` INT, `caixas` INT, `quant` INT, `idBloco` INT, `idVendaProduto2` INT, `lote` INT, `tipo` INT);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_galpao` AS select `ehc`.`idConsumo` AS `idEstoque_idConsumo`,`ehc`.`descricao` AS `descricao`,`vp2`.`status` AS `status`,`vp2`.`idVenda` AS `idVenda`,`vp2`.`codComercial` AS `codComercial`,`n`.`numeroNFe` AS `numeroNFe`,`ehc`.`caixas` AS `caixas`,(`ehc`.`quant` * -(1)) AS `quant`,`ehc`.`idBloco` AS `idBloco`,`ehc`.`idVendaProduto2` AS `idVendaProduto2`,`e`.`lote` AS `lote`,'CLIENTE' AS `tipo` from (((`staccato`.`estoque_has_consumo` `ehc` left join `staccato`.`venda_has_produto2` `vp2` on((`ehc`.`idVendaProduto2` = `vp2`.`idVendaProduto2`))) left join `staccato`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) left join `staccato`.`nfe` `n` on((`n`.`idNFe` = `e`.`idNFe`))) where (`vp2`.`status` in ('ESTOQUE','ENTREGA AGEND.','EM ENTREGA')) union select `staccato`.`vec`.`idEstoque` AS `idEstoque`,`staccato`.`vec`.`descricao` AS `descricao`,`staccato`.`vec`.`status` AS `status`,'' AS `Name_exp_15`,`staccato`.`vec`.`codComercial` AS `codComercial`,`staccato`.`vec`.`nfe` AS `nfe`,`staccato`.`vec`.`caixasContabil` AS `Caixas contabil`,`staccato`.`vec`.`contabil` AS `contabil`,`staccato`.`vec`.`idBloco` AS `idBloco`,NULL AS `idVendaProduto2`,`staccato`.`vec`.`lote` AS `lote`,'EST. LOJA' AS `tipo` from `staccato`.`view_estoque_contabil` `vec` where (`staccato`.`vec`.`contabil` > 0);
