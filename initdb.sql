...
CREATE TABLE IF NOT EXISTS `staccato`.`cnab` (
  ...
  `conteudo` BLOB NULL DEFAULT NULL,
  PRIMARY KEY (`idCnab`))
ENGINE = InnoDB
AUTO_INCREMENT = 10
DEFAULT CHARACTER SET = utf8;
...
CREATE TABLE IF NOT EXISTS `staccato`.`forma_pagamento` (
  ...
  `idConta` INT(10) NULL DEFAULT NULL,
  `prazoRecebe` INT(10) NULL DEFAULT NULL,
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_gares` (`idPagamento` INT, `idNFe` INT, `referencia` INT, `status` INT, `valor` INT, `numeroNFe` INT, `cnpjOrig` INT, `dataPagamento` INT, `dataRealizado` INT, `banco` INT);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_gares` AS select `cp`.`idPagamento` AS `idPagamento`,`n`.`idNFe` AS `idNFe`,extractvalue(`n`.`xml`,'nfeProc//NFe//infNFe//ide//dhEmi') AS `referencia`,`cp`.`status` AS `status`,`cp`.`valor` AS `valor`,`n`.`numeroNFe` AS `numeroNFe`,`n`.`cnpjOrig` AS `cnpjOrig`,`cp`.`dataPagamento` AS `dataPagamento`,`cp`.`dataRealizado` AS `dataRealizado`,`c`.`banco` AS `banco` from ((`staccato`.`conta_a_pagar_has_pagamento` `cp` left join `staccato`.`nfe` `n` on((`cp`.`idNFe` = `n`.`idNFe`))) left join `staccato`.`cnab` `c` on((`cp`.`idCnab` = `c`.`idCnab`))) where (`cp`.`status` in ('PENDENTE GARE','LIBERADO GARE','GERADO GARE','PAGO GARE'));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_pagamento_loja` AS select `lfp`.`idLoja` AS `idLoja`,`lfp`.`idPagamento` AS `idPagamento`,`fp`.`pagamento` AS `pagamento` from (`staccato`.`forma_pagamento` `fp` left join `staccato`.`loja_has_forma_pagamento` `lfp` on((`fp`.`idPagamento` = `lfp`.`idPagamento`)));
