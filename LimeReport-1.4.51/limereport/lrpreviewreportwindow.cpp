...
PreviewReportWindow::PreviewReportWindow(ReportEnginePrivate *report, QWidget *parent, QSettings *settings, Qt::WindowFlags flags)
    : QMainWindow(parent, flags), ui(new Ui::PreviewReportWindow), m_settings(settings), m_ownedSettings(false) {
  ui->setupUi(this);
...
void PreviewReportWindow::setHideResultEditButton(bool value) { ui->actionEdit_Mode->setVisible(value); }
...
void PreviewReportWindow::scaleComboboxChanged(QString text) { m_previewReportWidget->setScalePercent(text.remove(text.count() - 1, 1).toInt()); }

void PreviewReportWindow::slotScalePercentChanged(int percent) { m_scalePercent->setEditText(QString("%1%").arg(percent)); }
