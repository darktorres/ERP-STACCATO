...
ItemDesignIntf::ItemDesignIntf(const QString &xmlTypeName, QObject *owner, QGraphicsItem *parent) : BaseDesignIntf(xmlTypeName, owner, parent), m_itemLocation(Band), m_stretchToMaxHeight(false) {
  initFlags();
}
...
QGraphicsItem *ItemDesignIntf::bandByPos() {
  foreach (QGraphicsItem *item, scene()->items()) {
    if (dynamic_cast<BandDesignIntf *>(item) && item->collidesWithItem(this)) { return item; }
  }
  return 0;
}
...
void ContentItemDesignIntf::setContentBackedUp(bool contentBackedUp) { m_contentBackedUp = contentBackedUp; }
