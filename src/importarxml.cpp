...
void ImportarXML::cadastrarProdutoEstoque(const QVector<ProdutoEstoque> &tuples) {
  ...
  query.prepare(
      ...
      "p.cfop, p.cfopUpd, p.icms, p.icmsUpd, p.cst, p.cstUpd, p.qtdPallet, p.qtdPalletUpd, :custo, p.custoUpd, p.ipi, p.ipiUpd, p.st, p.stUpd, p.sticms, p.sticmsUpd, p.mva, "
      ...
      "p.ui, p.uiUpd, NULL, p.validadeUpd, :descontinuado, p.descontinuadoUpd, p.atualizarTabelaPreco, p.representacao, 0, 1, 0, p.idProduto, 0, NOW(), NULL FROM produto p WHERE p.idProduto = "
      ":idProduto");
