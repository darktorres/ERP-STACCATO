...
CREATE TABLE IF NOT EXISTS `staccato`.`venda_has_produto2` (
  ...
  INDEX `fk_vendaproduto2_nfe_idx` (`idNFeFutura` ASC),
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_compras_financeiro` AS select group_concat(distinct `pf`.`status` separator ', ') AS `Status`,group_concat(distinct `pf`.`statusFinanceiro` separator ', ') AS `Status Financeiro`,group_concat(distinct `pf`.`ordemCompra` separator ', ') AS `OC`,`pf`.`idCompra` AS `Compra`,group_concat(distinct `pf`.`fornecedor` separator ', ') AS `Fornecedor`,count(0) AS `Produtos`,sum(`pf`.`preco`) AS `Total`,group_concat(distinct `pf`.`idVenda` separator ', ') AS `Código` from `staccato`.`pedido_fornecedor_has_produto` `pf` where (`pf`.`idCompra` is not null) group by `pf`.`idCompra`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_consistencia_compra` AS select `staccato`.`pedido_fornecedor_has_produto`.`descricao` AS `descricao`,`staccato`.`pedido_fornecedor_has_produto`.`prcUnitario` AS `prcUnitario`,`staccato`.`pedido_fornecedor_has_produto`.`preco` AS `preco`,(`staccato`.`pedido_fornecedor_has_produto`.`prcUnitario` * `staccato`.`pedido_fornecedor_has_produto`.`quant`) AS `prcUnitario * quant` from `staccato`.`pedido_fornecedor_has_produto` where (((`staccato`.`pedido_fornecedor_has_produto`.`prcUnitario` * `staccato`.`pedido_fornecedor_has_produto`.`quant`) / `staccato`.`pedido_fornecedor_has_produto`.`preco`) > 1.01);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_conta_pagar` AS select `cp`.`idPagamento` AS `idPagamento`,`cp`.`idLoja` AS `idLoja`,`cp`.`contraParte` AS `contraparte`,`cp`.`dataPagamento` AS `dataPagamento`,`cp`.`dataEmissao` AS `dataEmissao`,`cp`.`valor` AS `valor`,`cp`.`status` AS `status`,group_concat(distinct `pf`.`ordemCompra` separator ',') AS `ordemCompra`,group_concat(distinct `pf`.`idVenda` separator ', ') AS `idVenda`,group_concat(distinct `n`.`numeroNFe` separator ', ') AS `numeroNFe`,`cp`.`tipo` AS `tipo`,`cp`.`parcela` AS `parcela`,`cp`.`observacao` AS `observacao`,group_concat(distinct `pf`.`statusFinanceiro` separator ',') AS `statusFinanceiro` from ((((`staccato`.`conta_a_pagar_has_pagamento` `cp` left join `staccato`.`pedido_fornecedor_has_produto2` `pf` on((`cp`.`idCompra` = `pf`.`idCompra`))) left join `staccato`.`estoque_has_compra` `ehc` on((`ehc`.`idPedido2` = `pf`.`idPedido2`))) left join `staccato`.`estoque` `e` on((`ehc`.`idEstoque` = `e`.`idEstoque`))) left join `staccato`.`nfe` `n` on((`n`.`idNFe` = `e`.`idNFe`))) where (`cp`.`desativado` = FALSE) group by `cp`.`idPagamento` order by `cp`.`dataPagamento`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_entrega_produtos` AS select `vp2`.`idVendaProduto2` AS `idVendaProduto2`,`vp2`.`idProduto` AS `idProduto`,`vp2`.`dataPrevEnt` AS `dataPrevEnt`,`vp2`.`dataRealEnt` AS `dataRealEnt`,`vp2`.`status` AS `status`,`vp2`.`fornecedor` AS `fornecedor`,`vp2`.`idVenda` AS `idVenda`,`vp2`.`produto` AS `produto`,`vp2`.`caixas` AS `caixas`,`vp2`.`quant` AS `quant`,`vp2`.`un` AS `un`,`vp2`.`unCaixa` AS `unCaixa`,`vp2`.`codComercial` AS `codComercial`,`vp2`.`formComercial` AS `formComercial`,group_concat(distinct `ehc`.`idConsumo` separator ',') AS `idConsumo` from (`staccato`.`venda_has_produto2` `vp2` left join `staccato`.`estoque_has_consumo` `ehc` on((`vp2`.`idVendaProduto2` = `ehc`.`idVendaProduto2`))) group by `vp2`.`idVendaProduto2`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_fluxo_caixa_pendente` AS select `cp`.`idCompra` AS `Compra/Venda`,`cp`.`dataPagamento` AS `dataPagamento`,`cp`.`dataRealizado` AS `dataRealizado`,(`cp`.`valor` * -(1)) AS `R$`,`cp`.`tipo` AS `Tipo`,(select concat(`staccato`.`loja_has_conta`.`banco`,' - ',`staccato`.`loja_has_conta`.`agencia`,' - ',`staccato`.`loja_has_conta`.`conta`) from `staccato`.`loja_has_conta` where (`staccato`.`loja_has_conta`.`idConta` = `cp`.`contaDestino`)) AS `contaDestino`,`cp`.`contaDestino` AS `idConta`,`cp`.`status` AS `Status`,'SAIDA' AS `ENT/SAI` from `staccato`.`conta_a_pagar_has_pagamento` `cp` where ((`cp`.`status` in ('PENDENTE','CONFERIDO')) and (`cp`.`desativado` = FALSE)) union all select `cr`.`idVenda` AS `idVenda`,`cr`.`dataPagamento` AS `dataPagamento`,`cr`.`dataRealizado` AS `dataRealizado`,`cr`.`valor` AS `valor`,`cr`.`tipo` AS `tipo`,(select concat(`staccato`.`loja_has_conta`.`banco`,' - ',`staccato`.`loja_has_conta`.`agencia`,' - ',`staccato`.`loja_has_conta`.`conta`) from `staccato`.`loja_has_conta` where (`staccato`.`loja_has_conta`.`idConta` = `cr`.`contaDestino`)) AS `Name_exp_15`,`cr`.`contaDestino` AS `contaDestino`,`cr`.`status` AS `status`,'ENTRADA' AS `ENTRADA` from `staccato`.`conta_a_receber_has_pagamento` `cr` where ((`cr`.`status` in ('PENDENTE','CONFERIDO')) and (`cr`.`desativado` = FALSE) and (`cr`.`representacao` = FALSE)) order by `dataPagamento`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_fluxo_caixa_realizado` AS select `cp`.`idCompra` AS `Compra/Venda`,`cp`.`dataRealizado` AS `dataRealizado`,(`cp`.`valorReal` * -(1)) AS `R$`,`cp`.`tipo` AS `Tipo`,(select concat(`staccato`.`loja_has_conta`.`banco`,' - ',`staccato`.`loja_has_conta`.`agencia`,' - ',`staccato`.`loja_has_conta`.`conta`) from `staccato`.`loja_has_conta` where (`staccato`.`loja_has_conta`.`idConta` = `cp`.`contaDestino`)) AS `contaDestino`,`cp`.`contaDestino` AS `idConta`,`cp`.`status` AS `Status`,'SAIDA' AS `ENT/SAI` from `staccato`.`conta_a_pagar_has_pagamento` `cp` where ((`cp`.`status` = 'PAGO') and (`cp`.`desativado` = FALSE)) union all select `cr`.`idVenda` AS `idVenda`,`cr`.`dataRealizado` AS `dataRealizado`,`cr`.`valorReal` AS `valorReal`,`cr`.`tipo` AS `tipo`,(select concat(`staccato`.`loja_has_conta`.`banco`,' - ',`staccato`.`loja_has_conta`.`agencia`,' - ',`staccato`.`loja_has_conta`.`conta`) from `staccato`.`loja_has_conta` where (`staccato`.`loja_has_conta`.`idConta` = `cr`.`contaDestino`)) AS `Name_exp_15`,`cr`.`contaDestino` AS `contaDestino`,`cr`.`status` AS `status`,'ENTRADA' AS `ENTRADA` from `staccato`.`conta_a_receber_has_pagamento` `cr` where ((`cr`.`status` = 'RECEBIDO') and (`cr`.`desativado` = FALSE)) order by `dataRealizado`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_ordemcompra` AS select `vp`.`idVenda` AS `idVenda`,`vp`.`idVendaProduto1` AS `idVendaProduto1`,`vp`.`status` AS `status`,`vp`.`fornecedor` AS `fornecedor`,`vp`.`produto` AS `produto`,`vp`.`codComercial` AS `codComercial`,`vp`.`quant` AS `quant`,`vp`.`un` AS `un`,`vp`.`caixas` AS `caixas`,`vp`.`formComercial` AS `formComercial`,`vp`.`obs` AS `obs` from `staccato`.`venda_has_produto` `vp`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_pagamento_loja` AS select `fpl`.`idLoja` AS `idLoja`,`fpl`.`idPagamento` AS `idPagamento`,`fp`.`pagamento` AS `pagamento` from (`staccato`.`forma_pagamento` `fp` left join `staccato`.`forma_pagamento_has_loja` `fpl` on((`fp`.`idPagamento` = `fpl`.`idPagamento`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_produto_pendente` AS select `vp2`.`status` AS `status`,`vp2`.`idVendaProduto2` AS `idVendaProduto2`,`vp2`.`idVendaProdutoFK` AS `idVendaProdutoFK`,`vp2`.`fornecedor` AS `fornecedor`,`vp2`.`idVenda` AS `idVenda`,`vp2`.`idProduto` AS `idProduto`,`vp2`.`produto` AS `produto`,`vp2`.`obs` AS `obs`,`p`.`colecao` AS `colecao`,`vp2`.`formComercial` AS `formComercial`,`vp2`.`quant` AS `quant`,`vp2`.`un` AS `un`,`p`.`un2` AS `Un2`,`vp2`.`caixas` AS `Caixas`,`p`.`kgcx` AS `kgcx`,`vp2`.`codComercial` AS `codComercial`,`p`.`codBarras` AS `codBarras`,`p`.`custo` AS `custo`,`vp2`.`idCompra` AS `idCompra` from (`staccato`.`venda_has_produto2` `vp2` left join `staccato`.`produto` `p` on((`vp2`.`idProduto` = `p`.`idProduto`)));
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_relatorio_loja` AS select `v`.`Loja` AS `Loja`,sum(`v`.`Faturamento`) AS `Faturamento`,sum(`v`.`Comissão`) AS `Comissão`,((sum(`v`.`Comissão`) / sum(`v`.`Faturamento`)) * 100) AS `%`,`v`.`Mês` AS `Mês`,(group_concat(distinct `x`.`custo` separator ',') + 0.0) AS `Reposição` from ((((`staccato`.`view_relatorio` `v` left join `staccato`.`usuario` `u` on((`v`.`Vendedor` = `u`.`nome`))) left join `staccato`.`venda` `v2` on((`v`.`idVenda` = `v2`.`idVenda`))) left join `staccato`.`loja` `l` on((`v2`.`idLoja` = `l`.`idLoja`))) left join (select `l`.`descricao` AS `loja`,date_format(`v`.`data`,'%Y-%m') AS `data`,sum((`vp`.`quant` * `p`.`custo`)) AS `custo` from (((`staccato`.`venda_has_produto` `vp` left join `staccato`.`produto` `p` on((`vp`.`idProduto` = `p`.`idProduto`))) left join `staccato`.`venda` `v` on((`vp`.`idVenda` = `v`.`idVenda`))) left join `staccato`.`loja` `l` on((`v`.`idLoja` = `l`.`idLoja`))) where (`vp`.`reposicaoEntrega` = TRUE) group by `l`.`descricao`,date_format(`v`.`data`,'%Y-%m') order by `l`.`descricao`,date_format(`v`.`data`,'%Y-%m')) `x` on(((`x`.`loja` = `v`.`Loja`) and (convert(`x`.`data` using utf8) = `v`.`Mês`)))) group by `v`.`Loja`,`v`.`Mês`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_relatorio_pagar` AS select `cp`.`dataEmissao` AS `dataEmissao`,`cp`.`dataRealizado` AS `dataRealizado`,`cp`.`valorReal` AS `valorReal`,concat(`lhc`.`banco`,' - ',`lhc`.`agencia`,' - ',`lhc`.`conta`) AS `Conta`,`cp`.`observacao` AS `observacao`,`cp`.`contraParte` AS `contraParte`,`cp`.`grupo` AS `grupo`,`cp`.`subGrupo` AS `subGrupo` from ((`staccato`.`conta_a_pagar_has_pagamento` `cp` left join `staccato`.`nfe` `n` on((`cp`.`nfe` = `n`.`idNFe`))) left join `staccato`.`loja_has_conta` `lhc` on((`cp`.`contaDestino` = `lhc`.`idConta`))) where ((`cp`.`dataRealizado` is not null) and (`cp`.`valorReal` is not null) and (`cp`.`status` = 'PAGO') and (`cp`.`desativado` = FALSE)) order by `cp`.`dataRealizado`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_relatorio_receber` AS select `cr`.`dataEmissao` AS `dataEmissao`,`cr`.`dataRealizado` AS `dataRealizado`,`cr`.`valorReal` AS `valorReal`,concat(`lhc`.`banco`,' - ',`lhc`.`agencia`,' - ',`lhc`.`conta`) AS `Conta`,`cr`.`observacao` AS `observacao`,`cr`.`contraParte` AS `contraParte`,`cr`.`grupo` AS `grupo`,`cr`.`subGrupo` AS `subGrupo` from (`staccato`.`conta_a_receber_has_pagamento` `cr` left join `staccato`.`loja_has_conta` `lhc` on((`cr`.`contaDestino` = `lhc`.`idConta`))) where ((`cr`.`valorReal` is not null) and (`cr`.`status` = 'RECEBIDO') and (`cr`.`desativado` = FALSE)) order by `cr`.`dataRealizado`;
