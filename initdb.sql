...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `staccato`.`view_produto_estoque` AS select `ehc`.`idVendaProduto2` AS `idVendaProduto2`,`ehc`.`idProduto` AS `idProduto`,`vp2`.`fornecedor` AS `fornecedor`,`vp2`.`produto` AS `produto`,(`vp2`.`total` / `vp2`.`quant`) AS `descUnitario`,sum(`ehc`.`caixas`) AS `caixas`,sum((`ehc`.`quant` * -(1))) AS `quant`,coalesce(`ehc`.`un`,'') AS `un`,sum((((`vp2`.`total` / `vp2`.`quant`) * `ehc`.`quant`) * -(1))) AS `total`,`vp2`.`quantCaixa` AS `quantCaixa`,`ehc`.`codComercial` AS `codComercial`,`vp2`.`formComercial` AS `formComercial`,coalesce(`ehc`.`codBarras`,'') AS `codBarras`,`ehc`.`ncm` AS `ncm`,`ehc`.`cfop` AS `cfop`,`n`.`cest` AS `cest`,coalesce(`ehc`.`unTrib`,'') AS `unTrib`,`ehc`.`numeroPedido` AS `numeroPedido`,`ehc`.`itemPedido` AS `itemPedido`,`ehc`.`tipoICMS` AS `tipoICMS`,`ehc`.`orig` AS `orig`,`ehc`.`cstICMS` AS `cstICMS`,`ehc`.`modBC` AS `modBC`,sum(`ehc`.`vBC`) AS `vBC`,`ehc`.`pICMS` AS `pICMS`,sum(`ehc`.`vICMS`) AS `vICMS`,`ehc`.`modBCST` AS `modBCST`,`ehc`.`pMVAST` AS `pMVAST`,sum(`ehc`.`vBCST`) AS `vBCST`,`ehc`.`pICMSST` AS `pICMSST`,sum(`ehc`.`vICMSST`) AS `vICMSST`,`ehc`.`cEnq` AS `cEnq`,`ehc`.`cstIPI` AS `cstIPI`,`ehc`.`cstPIS` AS `cstPIS`,sum(`ehc`.`vBCPIS`) AS `vBCPIS`,`ehc`.`pPIS` AS `pPIS`,sum(`ehc`.`vPIS`) AS `vPIS`,`ehc`.`cstCOFINS` AS `cstCOFINS`,sum(`ehc`.`vBCCOFINS`) AS `vBCCOFINS`,`ehc`.`pCOFINS` AS `pCOFINS`,sum(`ehc`.`vCOFINS`) AS `vCOFINS` from ((`staccato`.`venda_has_produto2` `vp2` left join `staccato`.`estoque_has_consumo` `ehc` on((`vp2`.`idVendaProduto2` = `ehc`.`idVendaProduto2`))) left join `staccato`.`ncm` `n` on((`ehc`.`ncm` = `n`.`ncm`))) where ((`ehc`.`idEstoque` is not null) and (`ehc`.`status` in ('PRÉ-CONSUMO','CONSUMO'))) group by `vp2`.`idVendaProduto2`;
