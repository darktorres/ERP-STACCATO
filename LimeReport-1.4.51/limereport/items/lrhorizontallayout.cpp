...
LimeReport::BaseDesignIntf *createHLayout(QObject *owner, LimeReport::BaseDesignIntf *parent) { return new LimeReport::HorizontalLayout(owner, parent); }
bool VARIABLE_IS_NOT_USED registred = LimeReport::DesignElementsFactory::instance().registerCreator(xmlTag, LimeReport::ItemAttribs(QObject::tr("HLayout"), LimeReport::Const::bandTAG), createHLayout);
} // namespace

namespace LimeReport {

bool lessThen(BaseDesignIntf *c1, BaseDesignIntf *c2) { return c1->pos().x() < c2->pos().x(); }

HorizontalLayout::HorizontalLayout(QObject *owner, QGraphicsItem *parent) : LayoutDesignIntf(xmlTag, owner, parent), m_isRelocating(false), m_layoutType(Layout) {
...
BaseDesignIntf *HorizontalLayout::createSameTypeItem(QObject *owner, QGraphicsItem *parent) { return new LimeReport::HorizontalLayout(owner, parent); }
...
int HorizontalLayout::childrenCount() { return m_children.size(); }
...
BaseDesignIntf *HorizontalLayout::cloneBottomPart(int height, QObject *owner, QGraphicsItem *parent) {
  ...
  for (BaseDesignIntf *item : childBaseItems()) {
    if ((item->geometry().top() < height) && (item->geometry().bottom() > height)) {
      ...
      if (maxHeight < tmpItem->height()) maxHeight = tmpItem->height();
    }
  }
...
void HorizontalLayout::setItemAlign(const BaseDesignIntf::ItemAlign &itemAlign) {
  if (itemAlign == ParentWidthItemAlign) setLayoutType(Table);
...
void HorizontalLayout::restoreChild(BaseDesignIntf *item) {
  ...
  connect(item, SIGNAL(itemAlignChanged(BaseDesignIntf *, ItemAlign, ItemAlign)), this, SLOT(slotOnChildItemAlignChanged(BaseDesignIntf *, ItemAlign, ItemAlign)));
...
void HorizontalLayout::addChild(BaseDesignIntf *item, bool updateSize) {
  ...
  connect(item, SIGNAL(itemSelectedHasBeenChanged(BaseDesignIntf *, bool)), this, SLOT(slotOnChildSelectionHasChanged(BaseDesignIntf *, bool)));
...
void HorizontalLayout::childAddedEvent(BaseDesignIntf *child) { addChild(child, false); }
...
void HorizontalLayout::divideSpace() {
  ...
  for (int i = 0; i < m_children.size(); ++i) {
    if (m_children[i]->isVisible() || itemMode() == DesignMode) m_children[i]->setWidth(m_children[i]->width() + delta);
    if ((i + 1) < m_children.size())
      if (m_children[i + 1]->isVisible() || itemMode() == DesignMode) m_children[i + 1]->setPos(m_children[i + 1]->pos().x() + delta * (i + 1), m_children[i + 1]->pos().y());
  }
  m_isRelocating = false;
}
...
HorizontalLayout::LayoutType HorizontalLayout::layoutType() const { return m_layoutType; }
...
LayoutMarker::LayoutMarker(HorizontalLayout *layout, QGraphicsItem *parent) : QGraphicsItem(parent), m_rect(0, 0, 30, 30), m_color(Qt::red), m_layout(layout) { setFlag(QGraphicsItem::ItemIsMovable); }
...
void LayoutMarker::mousePressEvent(QGraphicsSceneMouseEvent *event) {
  if (event->button() == Qt::LeftButton) {
    if (!(event->modifiers() & Qt::ControlModifier)) m_layout->scene()->clearSelection();
