...
void WidgetFinanceiroContas::updateTables() {
  if (not isSet) {
    ...
    ui->dateEditAte->setDate(qApp->serverDateTime().date());
    ui->dateEditDe->setDate(qApp->serverDateTime().date());
...
void WidgetFinanceiroContas::on_pushButtonReverterPagamento_clicked() {
  ...
  if (queryPagamento.value("dataPagamento").toDate().daysTo(qApp->serverDateTime().date()) > 5) { return qApp->enqueueError("No máximo 5 dias para reverter!", this); }
