...
ComboBoxEditor::ComboBoxEditor(QWidget *parent, bool clearable) : QWidget(parent), m_comboBox(new InternalComboBox(this)), m_buttonClear(0), m_settingValues(false) {
  ...
  if (clearable) layout->addWidget(m_buttonClear);
...
bool ComboBoxEditor::eventFilter(QObject *target, QEvent *event) {
  ...
  if (target == m_comboBox) {
    if (event->type() == QEvent::FocusOut) {
      if (!m_comboBox->isPopup() || (m_buttonClear && m_buttonClear->hasFocus())) emit editingFinished();
    }
  }
...
void ComboBoxEditor::resizeEvent(QResizeEvent *e) {
  if (m_buttonClear) m_buttonClear->setMinimumHeight(e->size().height() - 4);
}
