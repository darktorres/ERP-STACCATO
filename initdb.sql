...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`viewVendaxCMV` (`id` INT, `mes` INT, `loja` INT, `venda` INT, `qtddvenda` INT, `qtddcompra` INT, `qtddestq` INT, `custocompra` INT, `custoestq` INT, `comissrepr` INT, `repr` INT, `status` INT, `tipoICMS` INT, `vICMS` INT);
...
CREATE 
    ...
    DEFINER = `root`@`%` 
    SQL SECURITY DEFINER
VIEW `mydb`.`viewVendaxCMV` AS
    SELECT 
        `mydb`.`venda_has_produto`.`idVendaProduto` AS `id`,
        DATE_FORMAT(`mydb`.`venda`.`data`, '%Y/%m') AS `mes`,
        `mydb`.`loja`.`descricao` AS `loja`,
        `mydb`.`venda_has_produto`.`total` AS `venda`,
        `mydb`.`venda_has_produto`.`quant` AS `qtddvenda`,
        SUM(`mydb`.`pedido_fornecedor_has_produto`.`quant`) AS `qtddcompra`,
        SUM(`mydb`.`estoque_has_consumo`.`quant`) AS `qtddestq`,
        SUM(`mydb`.`pedido_fornecedor_has_produto`.`preco`) AS `custocompra`,
        SUM(`mydb`.`estoque_has_consumo`.`valor`) AS `custoestq`,
        `mydb`.`fornecedor`.`comissaoLoja` AS `comissrepr`,
        `mydb`.`venda`.`representacao` AS `repr`,
        `mydb`.`venda_has_produto`.`status` AS `status`,
        `mydb`.`estoque_has_consumo`.`tipoICMS` AS `tipoICMS`,
        `mydb`.`estoque_has_consumo`.`vICMSST` AS `vICMS`
    FROM
        ...
        LEFT JOIN `mydb`.`pedido_fornecedor_has_produto` ON ((`mydb`.`venda_has_produto`.`idVendaProduto` = `mydb`.`pedido_fornecedor_has_produto`.`idVendaProduto`)))
        LEFT JOIN `mydb`.`estoque_has_consumo` ON ((`mydb`.`venda_has_produto`.`idVendaProduto` = `mydb`.`estoque_has_consumo`.`idVendaProduto`)))
        LEFT JOIN `mydb`.`fornecedor` ON ((`mydb`.`venda_has_produto`.`fornecedor` = `mydb`.`fornecedor`.`razaoSocial`)))
    WHERE
        (`mydb`.`venda`.`data` >= '2017-01-01')
    GROUP BY `mydb`.`venda_has_produto`.`idVendaProduto` , DATE_FORMAT(`mydb`.`venda`.`data`, '%Y/%m') , `mydb`.`loja`.`descricao` , `mydb`.`venda_has_produto`.`total` , `mydb`.`venda_has_produto`.`quant` , `mydb`.`fornecedor`.`comissaoLoja` , `mydb`.`venda`.`representacao` , `mydb`.`venda_has_produto`.`status` , `mydb`.`estoque_has_consumo`.`tipoICMS` , `mydb`.`estoque_has_consumo`.`vICMS`
    HAVING ((`mydb`.`venda_has_produto`.`status` <> 'cancelado')
        AND (`mydb`.`venda_has_produto`.`status` <> 'substituido'));
