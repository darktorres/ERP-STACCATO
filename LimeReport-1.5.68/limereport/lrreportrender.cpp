...
ReportRender::ReportRender(QObject *parent)
    : QObject(parent), m_renderPageItem(0), m_pageCount(0), m_lastRenderedHeader(0), m_lastDataBand(0), m_lastRenderedFooter(0), m_lastRenderedBand(0), m_currentColumn(0), m_newPageStarted(false),
...
void ReportRender::analizeItem(ContentItemDesignIntf *contentItem, BandDesignIntf *band) {
  ...
  if (contentItem) {
    ...
    foreach (const QString &functionName, m_datasources->groupFunctionNames()) {
      ...
      rx.setMinimal(true);
      if (rx.indexIn(content) >= 0) { functions.append(functionName); }
    }
    if (functions.size() > 0) m_groupfunctionItems.insert(contentItem->patternName(), functions);
  }
}
...
void ReportRender::analizePage(PageItemDesignIntf *patternPage) {
  ...
  foreach (BandDesignIntf *band, patternPage->bands()) {
    if (band->isFooter() || band->isHeader()) { analizeContainer(band, band); }
  }
}
...
void ReportRender::renderPage(PageItemDesignIntf *patternPage, bool isTOC, bool /*isFirst*/, bool /*resetPageNumbers*/) {
  ...
  if (m_patternPageItem->resetPageNumber() && m_pageCount > 0 && !isTOC) { resetPageNumber(PageReset); }

  if (m_patternPageItem->resetPageNumber() && !isTOC && m_pageCount == 0) { m_pagesRanges.startNewRange(); }
  ...
  if (reportFooter) renderBand(reportFooter, 0, StartNewPageAsNeeded);
  ...
  if (tearOffBand) renderBand(tearOffBand, 0, StartNewPageAsNeeded);
  ...
  foreach (BaseDesignIntf *item, band->childBaseItems()) {
    ...
    if (contentItem) {
      ...
      foreach (QString functionName, m_datasources->groupFunctionNames()) {
        QRegExp rx(QString(Const::GROUP_FUNCTION_RX).arg(functionName));
        if (rx.indexIn(content) >= 0) { return true; }
      }
    }
  }
  return false;
          ...
          if (captures.size() >= 3) {
            ...
            if (dataBand) {
              ...
              if (gf) {
                ...
                connect(dataBand, SIGNAL(bandReRendered(BandDesignIntf *, BandDesignIntf *)), gf, SLOT(slotBandReRendered(BandDesignIntf *, BandDesignIntf *)));
              }
            } else {
          ...
          while ((pos = rx.indexIn(content, pos)) != -1) {
            ...
            if (captures.size() >= 3) {
              ...
              if (captures.size() < 5) {
                ...
                content.replace(captures.at(0), QString("%1(%2,%3,%4)").arg(functionName).arg('"' + expressionIndex + '"').arg('"' + band->objectName() + '"').arg(captures.at(4)));
              }
            }
            pos += rx.matchedLength();
          }
...
BandDesignIntf *ReportRender::renderBand(BandDesignIntf *patternBand, BandDesignIntf *bandData, ReportRender::DataRenderMode mode, bool isLast) {
  ...
  if (patternBand) {
    ...
    if (patternBand->isHeader()) m_lastRenderedHeader = patternBand;
    ...
    if (!bandClone->isEmpty() || patternBand->printIfEmpty()) {
      if (!registerBand(bandClone)) {
        if (patternBand && patternBand->isHeader() && patternBand->reprintOnEachPage()) m_reprintableBands.removeOne(patternBand);
          ...
          if (bandClone->maxScalePercent() >= percent) {
            ...
            if (mode == StartNewPageAsNeeded) {
              if (bandClone->columnsCount() > 1 && (bandClone->columnsFillDirection() == BandDesignIntf::Vertical || bandClone->columnsFillDirection() == BandDesignIntf::VerticalUniform)) {
                startNewColumn();
                if (patternBand->bandHeader() && patternBand->bandHeader()->columnsCount() > 1 && !m_lostHeadersMoved && patternBand->bandNestingLevel() == 0) {
...
void ReportRender::renderDataBand(BandDesignIntf *dataBand) {
  ...
  if (bandDatasource && !bandDatasource->eof() && !m_renderCanceled) {
    ...
    if (header && header->reprintOnEachPage()) m_reprintableBands.append(dataBand->bandHeader());
    ...
    while (!bandDatasource->eof() && !m_renderCanceled) {
      ...
      if (!rawData->isEmpty() || dataBand->printIfEmpty()) {
        ...
        if (header && !firstTime && header->repeatOnEachRow()) renderBand(header, 0, StartNewPageAsNeeded);
    ...
    if (!dataBand->keepFooterTogether()) m_reprintableBands.removeOne(header);
    ...
    if (footer && !footer->printAlways()) {
      ...
      if (dataBand->keepFooterTogether()) m_reprintableBands.removeOne(dataBand);
    }
...
void ReportRender::renderPageHeader(PageItemDesignIntf *patternPage) {
  ...
  if (band) {
    if (m_datasources->variable("#PAGE").toInt() != 1 || band->property("printOnFirstPage").toBool()) renderBand(band, 0);
  }
}
...
void ReportRender::renderReportHeader(PageItemDesignIntf *patternPage, PageRenderStage stage) {
  ...
  if (band) {
    if (band->property("printBeforePageHeader").toBool() && stage == BeforePageHeader) renderBand(band, 0, StartNewPageAsNeeded);
    if (!band->property("printBeforePageHeader").toBool() && stage == AfterPageHeader) renderBand(band, 0, StartNewPageAsNeeded);
  }
}
...
void ReportRender::renderChildFooter(BandDesignIntf *parent, BandPrintMode printMode) {
  foreach (BandDesignIntf *band, parent->childrenByType(BandDesignIntf::SubDetailFooter)) {
    ...
    if ((band != m_lastRenderedFooter) && (printAlways == (printMode == PrintAlwaysPrintable))) renderBand(band, 0, StartNewPageAsNeeded);
  }
}
...
void ReportRender::renderDataHeader(BandDesignIntf *header) {
  ...
  if (containsGroupFunctions(header)) m_recalcBands.append(renderedHeader);
}
...
void ReportRender::renderGroupHeader(BandDesignIntf *parentBand, IDataSource *dataSource, bool firstTime) {
  foreach (BandDesignIntf *band, parentBand->childrenByType(BandDesignIntf::GroupHeader)) {
    ...
    if (gb && !gb->isStarted()) {
      ...
      if (containsGroupFunctions(band)) m_recalcBands.append(renderedHeader);
    }
...
void ReportRender::renderGroupFooterByHeader(BandDesignIntf *groupHeader) {
  ...
  foreach (BandDesignIntf *header, groupHeader->childrenByType(BandDesignIntf::GroupHeader)) { renderGroupFooterByHeader(header); }
  foreach (BandDesignIntf *footer, groupHeader->childrenByType(BandDesignIntf::GroupFooter)) { renderBand(footer, 0, StartNewPageAsNeeded); }
...
void ReportRender::renderGroupFooter(BandDesignIntf *parentBand) {
  foreach (BandDesignIntf *band, parentBand->childrenByType(BandDesignIntf::GroupHeader)) {
    IGroupBand *gb = dynamic_cast<IGroupBand *>(band);
    if (gb && gb->isStarted()) { renderGroupFooterByHeader(band); }
  }
}
...
bool isMultiColumnHeader(BandDesignIntf *band) {
  return ((band->columnsCount() > 1) && (band->isHeader() && ((band->bandNestingLevel() == 0) || (band->columnsFillDirection() == BandDesignIntf::Horizontal))));
}
...
bool ReportRender::registerBand(BandDesignIntf *band, bool registerInChildren) {
  ...
  if ((band->columnsCount() > 1) && (!band->isHeader() || (band->bandNestingLevel() > 0 && band->columnsFillDirection() != BandDesignIntf::Horizontal))) {
    ...
    if (band->columnsFillDirection() == BandDesignIntf::Horizontal) {
      ...
      if ((m_currentColumn != 0) && (m_maxHeightByColumn[0] == m_maxHeightByColumn[m_currentColumn]) && (m_maxHeightByColumn[0] >= band->height())) { m_currentColumn = 0; }
    }
  }

  m_currentColumn = m_currentColumn == -1 ? 0 : m_currentColumn;

  if ((band->height() <= m_maxHeightByColumn[m_currentColumn]) || m_patternPageItem->endlessHeight() || (isMultiColumnHeader(band) && (band->height() <= m_maxHeightByColumn[0]))) {
    ...
    if (isMultiColumnHeader(band)) {
      ...
      if (band->bandType() != BandDesignIntf::PageFooter) { placeBandOnPage(band, m_currentColumn); }
    ...
    if (band->bandType() != BandDesignIntf::PageFooter) m_lastRenderedBand = band;
    return true;
  } else
    return false;
}
...
void ReportRender::updateTOC(BaseDesignIntf *item, int pageNumber) {
  ...
  if (bookmarkContainer) {
    ...
    foreach (QString key, bookmarkContainer->bookmarks()) { toc->setItem(key, bookmarkContainer->getBookMark(key).toString(), pageNumber); }
  }
}
...
void ReportRender::secondRenderPass(ReportPages renderedPages) {
  ...
  for (int i = 0; i < renderedPages.count(); ++i) {
    ...
    foreach (BaseDesignIntf *item, page->childBaseItems()) {
      if (item->isNeedUpdateSize(SecondPass)) item->updateItemSize(m_datasources, SecondPass);
    }
  }
}

void ReportRender::createTOCMarker(bool startNewRange) { m_pagesRanges.addTOCMarker(startNewRange); }
...
BandDesignIntf *ReportRender::saveUppperPartReturnBottom(BandDesignIntf *band, int height, BandDesignIntf *patternBand) {
  ...
  if (!bottomBandPart->isEmpty()) {
    ...
    if (upperBandPart->isEmpty()) bottomBandPart->copyBookmarks(band);
  }
...
BandDesignIntf *ReportRender::renderData(BandDesignIntf *patternBand) {
  ...
  if (patternBand->isHeader()) { replaceGroupsFunction(bandClone); }
...
void ReportRender::startNewPage(bool isFirst) {
  ...
  foreach (BandDesignIntf *band, m_reprintableBands) { renderBand(band, 0); }
...
bool bandLessThen(BandDesignIntf *b1, BandDesignIntf *b2) { return b1->bandIndex() < b2->bandIndex(); }
...
void ReportRender::checkLostHeadersOnPrevPage() {
  ...
  if (lostHeaders.size() > 0) {
    ...
    foreach (BandDesignIntf *header, lostHeaders) { registerBand(header); }
  } else {
    m_lostHeadersMoved = false;
  }
}
...
void ReportRender::savePage(bool isLast) {
  ...
  if (m_renderPageItem->pageFooter()) {
    ...
    if (m_renderPageItem->pageFooter()->property("removeGap").toBool()) { m_renderPageItem->pageFooter()->setPos(m_lastRenderedBand->geometry().bottomLeft()); }
  }
  ...
  if (isLast && m_patternPageItem->endlessHeight()) {
    qreal pageHeight = 0;
    foreach (BandDesignIntf *band, m_renderPageItem->bands()) { pageHeight += band->height(); }
...
QString ReportRender::toString() {
  ...
  foreach (PageItemDesignIntf::Ptr page, m_renderedPages) { writer->putItem(page.data()); }
...
int PagesRanges::findLastPageNumber(int index) {
  ...
  foreach (PagesRange range, m_ranges) {
    if (range.firstPage <= (index) && range.lastPage >= (index)) return (range.lastPage - (range.firstPage)) + 1;
  }
  return 0;
}
...
int PagesRanges::findPageNumber(int index) {
  index++;
  foreach (PagesRange range, m_ranges) {
    if (range.firstPage <= (index) && range.lastPage >= (index)) return (index - range.firstPage) + 1;
  }
  return 0;
}
...
void PagesRanges::addTOCPage() {
  ...
  if (m_TOCRangeIndex != -1) {
    ...
    if (tocRange.firstPage == 0) {
      ...
      if (tocRange.firstPage == 1 && tocRange.lastPage == 1) shiftRangesNextToTOC();
    } else {
      tocRange.lastPage++;
