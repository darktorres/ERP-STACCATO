...
-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_conferencia_sublinhas`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_conferencia_sublinhas` (`idVenda` INT, `idVendaProduto1` INT, `quant` INT, `SUM(v2.quant)` INT, `diff` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_consistencia_vp1_v_total` (`idVenda` INT, `totalVenda` INT, `totalProdutos` INT, `created` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_consistencia_vp1_vp2_quant`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_consistencia_vp1_vp2_quant` (`quant` INT, `SUM(vp2.quant)` INT, `created` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_consistencia_vp1_vp2_total`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_consistencia_vp1_vp2_total` (`total` INT, `SUM(vp2.total)` INT, `created` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_consistencia_vp2_ehc_quant`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_consistencia_vp2_ehc_quant` (`idVenda` INT, `status` INT, `quantVenda` INT, `quantCompra` INT, `created` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_consistencia_vp2_pf2_quant`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_consistencia_vp2_pf2_quant` (`idVenda` INT, `status` INT, `quantVenda` INT, `quantCompra` INT, `created` INT);

-- -----------------------------------------------------
-- Placeholder table for view `staccato`.`view_consistencia_vp_op_quant`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_consistencia_vp_op_quant` (`idVenda` INT, `codComercial` INT, `quantVenda` INT, `quantOrc` INT, `created` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `staccato`.`view_relatorio` (`Loja` INT, `Vendedor` INT, `idUsuario` INT, `idVenda` INT, `Mês` INT, `Data` INT, `Faturamento` INT, `Comissão` INT, `%` INT);
...
INSERT INTO pedido_fornecedor_has_produto2 (idPedidoFK, selecionado, aliquotaSt, st, status, statusFinanceiro, ordemCompra, ordemRepresentacao, idVenda, idVendaProduto2, 
                         ...
                         idProduto, descricao, obs, colecao, codComercial, quant, quantUpd, un, un2, caixas, prcUnitario, preco, kgcx, formComercial, codBarras, dataPrevCompra, 
...
-- -----------------------------------------------------
-- View `staccato`.`view_conferencia_sublinhas`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `staccato`.`view_conferencia_sublinhas`;
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_conferencia_sublinhas` AS select `v1`.`idVenda` AS `idVenda`,`v1`.`idVendaProduto1` AS `idVendaProduto1`,`v1`.`quant` AS `quant`,sum(`v2`.`quant`) AS `SUM(v2.quant)`,(`v1`.`quant` - sum(`v2`.`quant`)) AS `diff` from (`staccato`.`venda_has_produto` `v1` left join `staccato`.`venda_has_produto2` `v2` on((`v1`.`idVendaProduto1` = `v2`.`idVendaProdutoFK`))) group by `v1`.`idVendaProduto1`;
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_consistencia_vp1_v_total` AS select `vp`.`idVenda` AS `idVenda`,(`v`.`total` - `v`.`frete`) AS `totalVenda`,sum(`vp`.`total`) AS `totalProdutos`,`vp`.`created` AS `created` from (`staccato`.`venda_has_produto` `vp` left join `staccato`.`venda` `v` on((`vp`.`idVenda` = `v`.`idVenda`))) group by `v`.`idVenda` having (abs((`totalVenda` - `totalProdutos`)) > 1);
...
USE `staccato`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `staccato`.`view_consistencia_vp_op_quant` AS select `vp`.`idVenda` AS `idVenda`,`vp`.`codComercial` AS `codComercial`,sum(`vp`.`quant`) AS `quantVenda`,sum(`op`.`quant`) AS `quantOrc`,`vp`.`created` AS `created` from ((`staccato`.`venda_has_produto` `vp` left join `staccato`.`venda` `v` on((`vp`.`idVenda` = `v`.`idVenda`))) left join `staccato`.`orcamento_has_produto` `op` on(((`v`.`idOrcamento` = `op`.`idOrcamento`) and (`vp`.`codComercial` = `op`.`codComercial`)))) where (`v`.`devolucao` = FALSE) group by `vp`.`idVenda`,`vp`.`codComercial` having (`quantVenda` <> `quantOrc`);
...
CREATE 
    ...
    DEFINER = `root`@`localhost` 
    SQL SECURITY DEFINER
VIEW `staccato`.`view_venda` AS
    SELECT 
        `v`.`idLoja` AS `idLoja`,
        `v`.`idUsuario` AS `idUsuario`,
        `v`.`idUsuarioConsultor` AS `idUsuarioConsultor`,
        ...
        `v`.`idVenda` AS `Código`,
        GROUP_CONCAT(DISTINCT `pf2`.`ordemRepresentacao`
            SEPARATOR ',') AS `OC Rep`,
        `u`.`nome` AS `Vendedor`,
        `u2`.`nome` AS `Consultor`,
        `c`.`nome_razao` AS `Cliente`,
        `p`.`nome_razao` AS `Profissional`,
        `v`.`data` AS `Data`,
        `v`.`total` AS `Total R$`,
        GROUP_CONCAT(DISTINCT `vhf`.`dataFollowup`
            SEPARATOR ',') AS `Data Followup`,
        GROUP_CONCAT(DISTINCT `vhf`.`observacao`
            SEPARATOR ',') AS `Observação`
    FROM
        ((((((((`staccato`.`venda` `v`
        LEFT JOIN `staccato`.`pedido_fornecedor_has_produto2` `pf2` ON ((`v`.`idVenda` = `pf2`.`idVenda`)))
        LEFT JOIN `staccato`.`usuario` `u` ON ((`v`.`idUsuario` = `u`.`idUsuario`)))
        LEFT JOIN `staccato`.`cliente_has_endereco` `e` ON ((`v`.`idEnderecoEntrega` = `e`.`idEndereco`)))
        LEFT JOIN `staccato`.`profissional` `p` ON ((`v`.`idProfissional` = `p`.`idProfissional`)))
        LEFT JOIN `staccato`.`cliente` `c` ON ((`v`.`idCliente` = `c`.`idCliente`)))
        LEFT JOIN `staccato`.`usuario` `u2` ON ((`v`.`idUsuarioConsultor` = `u2`.`idUsuario`)))
        LEFT JOIN (SELECT 
            MAX(`tmp`.`idFollowup`) AS `idFollowup`,
                `tmp`.`idVendaBase` AS `idVendaBase`
        FROM
            `staccato`.`venda_has_followup` `tmp`
        GROUP BY `tmp`.`idVendaBase`) `vhf2` ON ((`v`.`idVendaBase` = `vhf2`.`idVendaBase`)))
        LEFT JOIN `staccato`.`venda_has_followup` `vhf` ON ((`vhf2`.`idFollowup` = `vhf`.`idFollowup`)))
    GROUP BY `v`.`idVenda`;
