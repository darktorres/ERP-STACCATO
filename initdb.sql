...
-- -----------------------------------------------------
-- Placeholder table for view `mydb`.`viewVendaxCMV`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`viewVendaxCMV` (`status` INT, `idVendaProduto` INT, `mes` INT, `loja` INT, `SumOfvenda` INT, `CMV` INT, `FonteCMV` INT, `MargemBruta` INT, `ICMS` INT, `%ICMS` INT, `FonteICMS` INT, `VendaDev` INT);
...
-- -----------------------------------------------------
-- View `mydb`.`viewVendaxCMV`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `mydb`.`viewVendaxCMV`;
DROP VIEW IF EXISTS `mydb`.`viewVendaxCMV` ;
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mydb`.`viewVendaxCMV` AS select `VendasCMV`.`status` AS `status`,`VendasCMV`.`id` AS `idVendaProduto`,`VendasCMV`.`mes` AS `mes`,`VendasCMV`.`loja` AS `loja`,(sum(`VendasCMV`.`venda`) + 0E0) AS `SumOfvenda`,(sum(if((`VendasCMV`.`repr` = 1),`VendasCMV`.`custocompra`,if((`VendasCMV`.`qtddvenda` = `VendasCMV`.`qtddcompra`),`VendasCMV`.`custocompra`,if((`VendasCMV`.`qtddvenda` = -(`VendasCMV`.`qtddestq`)),`VendasCMV`.`custoestq`,0)))) + 0E0) AS `CMV`,if((`VendasCMV`.`repr` = 1),'representacao',if((`VendasCMV`.`qtddvenda` = `VendasCMV`.`qtddcompra`),'pedido',if(((`VendasCMV`.`qtddvenda` = -(`VendasCMV`.`qtddestq`)) and (`VendasCMV`.`venda` <= (`VendasCMV`.`custoestq` * 4))),'estoque',if((`VendasCMV`.`status` = 'PENDENTE'),'sem custo','erro')))) AS `FonteCMV`,(sum(if((`VendasCMV`.`repr` = 1),((`VendasCMV`.`venda` * `VendasCMV`.`comissrepr`) / 100),(`VendasCMV`.`venda` - if((`VendasCMV`.`qtddvenda` = `VendasCMV`.`qtddcompra`),`VendasCMV`.`custocompra`,if((`VendasCMV`.`qtddvenda` = -(`VendasCMV`.`qtddestq`)),`VendasCMV`.`custoestq`,0))))) + 0E0) AS `MargemBruta`,(sum(if((`VendasCMV`.`vICMS` = 0),if((`VendasCMV`.`tipoICMS` = 'ICMS60'),0,if(isnull(`VendasCMV`.`percST`),(`VendasCMV`.`custocompra` * 0.0696),(`VendasCMV`.`custoestq` * `VendasCMV`.`percST`))),`VendasCMV`.`vICMS`)) + 0E0) AS `ICMS`,(((sum(if((`VendasCMV`.`vICMS` = 0),if((`VendasCMV`.`tipoICMS` = 'ICMS60'),0,if(isnull(`VendasCMV`.`percST`),(`VendasCMV`.`custocompra` * 0.0696),(`VendasCMV`.`custoestq` * `VendasCMV`.`percST`))),`VendasCMV`.`vICMS`)) / sum(`VendasCMV`.`custocompra`)) * 100) + 0E0) AS `%ICMS`,if((`VendasCMV`.`vICMS` = 0),if((`VendasCMV`.`tipoICMS` = 'ICMS60'),'isento',if(isnull(`VendasCMV`.`percST`),'estimado','ST Loja')),'ST Fornecedor') AS `FonteICMS`,if(((sum(`VendasCMV`.`venda`) + 0E0) < 0),'Devolução','Venda') AS `VendaDev` from `mydb`.`VendasCMV` group by `VendasCMV`.`id`,`VendasCMV`.`mes`,`VendasCMV`.`loja`,if((`VendasCMV`.`repr` = 1),'representacao',if((`VendasCMV`.`qtddvenda` = `VendasCMV`.`qtddcompra`),'pedido',if(((`VendasCMV`.`qtddvenda` = -(`VendasCMV`.`qtddestq`)) and (`VendasCMV`.`venda` <= (`VendasCMV`.`custoestq` * 4))),'estoque',if((`VendasCMV`.`status` = 'PENDENTE'),'sem custo','erro')))),if((`VendasCMV`.`vICMS` = 0),if((`VendasCMV`.`tipoICMS` = 'ICMS60'),'isento',if(isnull(`VendasCMV`.`percST`),'estimado','ST Loja')),'ST Fornecedor');
...
USE `mydb`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`%` SQL SECURITY DEFINER VIEW `mydb`.`view_relatorio_temp2` AS select dayofmonth(`v`.`data`) AS `dia`,sum((case when ((month(`v`.`data`) = month(now())) and (year(`v`.`data`) = (year(now()) - 1))) then `v`.`total` else 0 end)) AS `mes12`,sum((case when ((month(`v`.`data`) = (month(now()) - 11)) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes11`,sum((case when ((month(`v`.`data`) = (month(now()) - 10)) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes10`,sum((case when ((month(`v`.`data`) = (month(now()) - 9)) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes9`,sum((case when ((month(`v`.`data`) = (month(now()) - 8)) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes8`,sum((case when ((month(`v`.`data`) = (month(now()) - 7)) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes7`,sum((case when ((month(`v`.`data`) = (month(now()) - 6)) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes6`,sum((case when ((month(`v`.`data`) = (month(now()) - 5)) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes5`,sum((case when ((month(`v`.`data`) = (month(now()) - 4)) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes4`,sum((case when ((month(`v`.`data`) = (month(now()) - 3)) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes3`,sum((case when ((month(`v`.`data`) = (month(now()) - 2)) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes2`,sum((case when ((month(`v`.`data`) = (month(now()) - 1)) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes1`,sum((case when ((month(`v`.`data`) = month(now())) and (year(`v`.`data`) = year(now()))) then `v`.`total` else 0 end)) AS `mes0` from `mydb`.`venda` `v` where (`v`.`status` <> 'CANCELADO') group by dayofmonth(`v`.`data`);
